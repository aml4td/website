...
imagefiles <-
  list.files(path = "MP6843_img_full/", pattern = "F04_1056", full.names = TRUE)
...
nuclear_feat =
  ...
  clean_names() %>%
  select(
    nucleus_y = nucleus_nucleus_m_cy,
    nucleus_x = nucleus_nucleus_m_cx,
    nucleus_eccentricity = nucleus_nucleus_m_eccentricity,
    nucleus_area = nucleus_0_s_area,
    nucleus_mean = nucleus_nucleus_b_mean
  )
...
cell_tubulin =
  ...
  clean_names() %>%
  select(
    cell_eccentricity = cell_0_m_eccentricity,
    cell_area = cell_0_s_area,
    cell_mean = cell_cell_tubulin_b_mean
  ) %>%
  mutate(
