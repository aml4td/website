...
server <- function(input, output) {
  ...
  percip_grid <- tibble(
    annual_precipitation = seq(percip_rng[1], percip_rng[2], length.out = 100)
  )
  ...
  spline_fit <-
    logistic_reg() %>%
    fit(
      class ~ naturalSpline(annual_precipitation, df = num_spline),
      data = percip
    )
    ...
    renderPlot(
      {
        if (input$yaxis == "rate") {
          p <-
            ...
            ggplot(aes(percip)) +
            labs(
              x = "Log10 Annual Percipitation",
              y = "Probability of Forestation"
            ) +
            lims(y = 0:1)
        } else {
          p <-
