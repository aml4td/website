...
convert_tensor <- function(x) {
  y <- torch_tensor(
    ifelse(x$class == levels(x$class)[1], 1, 0),
    dtype = torch_float()
  )
  y <- y$unsqueeze(2)
...
att_format <- function(model, x, tibble = FALSE) {
  ...
  colnames(attn_matrix) <- rownames(attn_matrix) <- recipes::names0(
    ncol(attn_matrix),
    "hidden unit "
  )
  if (tibble) {
...
location_df <- as_tibble(three_locations) |> mutate(label = paste(1:3))
new_tens <- torch_tensor(three_locations, dtype = torch_float())
...
att_format(col_att_model$model, three_locations[1, , drop = FALSE]) |>
  round(2) |>
  knitr::kable()

att_format(col_att_model$model, three_locations[2, , drop = FALSE]) |>
  round(2) |>
  knitr::kable()

att_format(col_att_model$model, three_locations[3, , drop = FALSE]) |>
  round(2) |>
  knitr::kable()
...
# bind_rows(
#   att_format(col_att_model$model, three_locations[1,, drop = FALSE], tibble = TRUE) |> mutate(Sample = "Location 1"),
#   att_format(col_att_model$model, three_locations[2,, drop = FALSE], tibble = TRUE) |> mutate(Sample = "Location 2"),
#   att_format(col_att_model$model, three_locations[3,, drop = FALSE], tibble = TRUE) |> mutate(Sample = "Location 3")
# ) |>
