...
compute_metrics_icl <- function(i, seed) {
 ...
 split <- initial_split(forested_train, prop = i / n, strata = class)
  ...
  icl_pred <-
   try(
    icl_fit |>
     predict(forested_test, type = "prob") |>
  ...
  if (!inherits(icl_pred, "try-error")) {
   res <- mtr_set(icl_pred, class, .pred_Yes) |>
    mutate(samples = i, seed = seed, model = "TabICL")
  } else {
   res <-
    tibble::tribble(
...
icl_icl_bench <- map2(sample_grid$sizes, sample_grid$reps, compute_metrics_icl)
icl_icl_metrics <- map_dfr(icl_icl_bench, ~ .x$stats)
icl_icl_predictions <- map_dfr(icl_icl_bench, ~ .x$pred)
...
save(icl_icl_metrics, icl_icl_predictions, file = "icl_icl_bench.RData")
