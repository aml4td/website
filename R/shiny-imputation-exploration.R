	...
	output$plot <- renderPlot(
		{
			...
			f1 <- ggplot(
				v$for_figure,
				aes(x1, x2, pch = Obs_Type, color = Obs_Type)
			) +
				facet_grid(cols = vars(Type)) +
				geom_point(size = 3) +
				scale_color_manual(
					values = c("#56B4E9", "#D55E00", "#56B4E9", "#D55E00")
				) +
...
getData <- function(
	...
	rows_missing <- sample(
		1:nrow(current_data),
		size = PCTMISSING * nrow(current_data) / 100,
		replace = FALSE
	)
	x1_missing <- rows_missing[1:(length(rows_missing) / 2)]
	x2_missing <- rows_missing[
		((length(rows_missing) / 2) + 1):length(rows_missing)
	]
	...
	current_recipe <- recipe(Observed ~ x1_new + x2_new, data = current_data)
	...
	if (IMPUTETYPE == "BAGIMPUTE") {
		...
		if (DATA == "LINEAR") {
			...
			current_recipe <- current_recipe %>%
				...
				step_impute_linear(
					x2_new,
					impute_with = imp_vars(x2_new, x1_new_sq)
				) %>%
				step_rm(x1_new_sq)
		}
	}
	...
	for_figure <- bind_rows(orig_data, impute_data) %>%
		mutate(
			Obs_Type = factor(paste0(Observed, ": ", Type), levels = groups),
