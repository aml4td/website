...
The consequence of this is that we cannot make inferences about the model parameters (since they are considered to be single points). Most of our inference is about the MLEs. This leads to the circuitous explanation of confidence intervals and hypothesis tests. 
...
Bayesian optimization (BO)[@gramacy2020surrogates;@garnett2023] is a search procedure that uses an overarching statistical model to predict the next set of candidate values, given past performance. It is currently the most used iterative search routine for optimizing models. 
