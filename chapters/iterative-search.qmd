...
where $i$ is the iteration number. To compare 6.1% versus 7.0%, the acceptance probability is `r signif(exp(-(7-6.10)), 3)`. To make the determination, a random uniform number^[Between zero and one] $U$ is generated and, if $U le Pr[accept]$,  we accept $\theta}_{i}$ and it is used to make $\theta}_{i+1}$. Note that the probability "cools" over time; if this difference were to occur at a later iteration, say $i = 5$, the probability would drop to `r signif(exp(-5*(7-6.10)), 3)`.
