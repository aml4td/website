...
To formalize the concept of optimization, we need an objective function that defines what we are trying to optimize.  This function, often called a loss function (specifically to be minimized), is denoted by $\psi()$.  The parameters that modify $\psi()$ are represented by $\boldsymbol{\theta}$, a $p \time 1$ vector of real numbers.  For simplicity, we will assume that $\psi()$ is smooth and generally differentiable.  Additionally, without loss of generality, we will assume that _smaller_ values of $\psi()$ are better.
...
where $i$ is the iteration number. To compare 6.1% versus 7.0%, the acceptance probability is `r signif(exp(-(7-6.10)), 3)`. To make the determination, a random uniform number $U$ is generated and, if $U \le Pr[accept]$,  we accept $\boldsymbol{\theta}_{i}$ and it is used to make $\boldsymbol{\theta}_{i+1}$. Note that the probability "cools" over time; if this difference were to occur at a later iteration, say $i = 5$, the probability would drop to `r signif(exp(-5*(7-6.10)), 3)`.
...
It is often tied to a particular Bayesian predictive model: the Gaussian process (GP) regression model [@binois2022survey]. However, before diving into the details of that model, we should discuss how any Bayesian method can be used for optimization.

Similar to the racing model from @eq-perf-mod-racing, the Bayesian model's outcome will be our estimated performance metric (i.e., $\hat{Q}$). The predictors for this model will be the tuning parameter values:
...
The second candidate has a mean RMSE that is predicted to have about a 2-fold improvement but also has considerably large uncertainty. On average, this is a better choice but there is more area to the right of the vertical line. The probability of candidate two having worse results than the current best is `r two_cand_probs$prob[two_cand_probs$group == "# 2"]`%. 

Which choice is better? The first is a safer bet and the second offers higher risk with a higher reward. Which candidate to pick is often driven by out state of mind. However, we should definitely consider _why_ the variance is so large for the second candidate. 
...
Let's see how this approach works in a single dimension. @fig-bo-example shows how expected improvement can be used to find the global minimum of $\psi(\theta) = \theta\: cos(\theta/2)$. Th light grey line in the lower panel represents the true value of $\psi(\theta)$ across the parameter space. 
...
The example shown here did not show that the performance of iterative methods was overwhelmingly better than that of the grid search. While this may not be the best example to demonstrate when it is superior, these results are in line with what we have experienced with other data sets. 
