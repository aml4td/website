...
A visualization of model performance when only the SVM cost and scaling factor parameters are optimized. It shows the combination with the smallest RMSE and a ridge of candidate values with nearly equivalent performance.
...
sa_tbl <- 
  ...
  dplyr::filter(.iter == 0) %>% 
  arrange(mean) %>% 
  dplyr::select(RMSE, log_cost, log_scale) %>% 
  gt() %>% 
  fmt_number(
    columns = c(RMSE, log_cost, log_scale),
    decimals = 1
  ) %>% 
  cols_label(
    RMSE = "RMSE (%)", 
    log_cost = "Cost (log-2)", 
    log_scale = "Scale (log-10)"
  ) %>%
  tab_options(
    table.width = pct(75)  
  )
...
beta_cols <- c("#CC6677FF", "#F9D662FF", "#77AADDFF")
beta_p <-
  crossing(
  ...
  scale_colour_manual(values = beta_cols)
...
two_cand_p +
  annotate("text", x = 6.5, y = 1.25, label = "Current\nbest", col = dark_data) + 
  geom_vline(mapping = aes(xintercept = 6), lty = 2, col = dark_data) +
  thm_dk
...
An example of a choice between two candidates where performance is gauged by RMSE. One candidate has slightly better predicted mean performance and another where the mean RMSE is predicted to be much lower but with high uncertainty.
...
smoothed_p_dk <-
  smoothed_p +
  geom_point(data = raw_plot_data, cex = 1/2, col = dark_data) +
  thm_dk
...
diff_p_dk <- 
  diff_p +
  geom_hline(yintercept = 0, lty = 2, alpha = 1 / 2) +
  geom_point(data = raw_diff_plot_data, cex = 1/2, col = dark_data) +
  thm_dk

smoothed_p_lt / diff_p_lt

smoothed_p_dk / diff_p_dk
```

:::

Demonstrations of the Savitzky-Golay preprocessing method for a small region for one specific sample previously shown in @fig-barley-data.
...
initial_tbl <- 
  ...
  cols_label(
    `RMSE (%)` = "RMSE (%)",
    `window size` = "Window",
    differentiation_order = "Diff. Order",
    degree = "Degree",
    hidden_units = "Units",
    `penalty (log-10)` = "Penalty (log-10)",
    activation = "Activation",
    `learn rate (log-10)` = "Learn Rate (log-10)",
...
initial_tbl|> thm_tbl_lt()

initial_tbl|> thm_tbl_dk()
```
...
::: {#fig-nnet-sg-iterations}

::: {.figure-content}

```{r}
#| label: nnet-sg-iterations
#| renderings: [light, dark]
#| echo: false
#| fig-width: 6
#| fig-height: 5
...
mlp_sfd_p+
  scale_color_manual(values = c(dark_data, "#FF7F00")) +
  geom_vline(data = restarts, aes(xintercept = .iter), col = "red", lty = 3) + 
  thm_dk
```

:::

Validation set RMSE results for the optimization. The validation set RMSE is shown on the y-axis with 90% confidence intervals. Orange points indicate iterations where a new optimal value was found and vertical lines indicate where the algorithm restarted from the previous best candidate.
...
best_tbl <-
  ...
  dplyr::select(-.metric, -.estimator, -.config, -penalty, -learn_rate,
  ...
  mutate(Parameter = factor(Parameter, levels = iter_rows)) %>%
  arrange(Parameter) %>%
  gt()
