...
Non-greedy search methods are not constrained to always go in the absolute best direction. One such method is _simulated annealing_ (SA). It is a controlled random search that moves in random directions but with some amount of control over the path. It can also incorporate restarts if the search moves into clearly poor regions.
...
First, consider candidates that are real numbers. One option is to keep them as-is (i.e., floating point values). To use them to reproduce, we can make two children from a pair of good-performance candidates via a linear combination. For candidate vectors $\theta_j$, we can use:
...
To illustrate in two dimensions (similar to @fig-sa-example), a population size of 10 was used and 5 generations were created. These are not good default values but were instead chosen to be at parity with the previous SA search and the optimization method used in the next section. The values of the two tuning parameters were kept as floating point values. The parental selection was conducted using sampling weights proportional to the RMSE values (smaller being better), a mutation rate of 10%, and a single candidate was retained at each generation via elitism.  The computations within a generation where computed in parallel. @fig-ga-example illustrates the results of the search.   
...
The genetic algorithm can parallelize over the populations in each generation. In parallel, this optimization took `r prettyunits::pretty_sec(ga_time[3] / nrow(ga_history))` per candidate (on average). 
