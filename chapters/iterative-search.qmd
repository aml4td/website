...
Let's start by framing the problem in terms of linear regression. 

Many models make probabilistic assumptions about their data or parameters. For example, a linear regression model using ordinary least squares estimation^[This implies that the objective function is the sum of squared error.] can be constructed by making assumptions regarding the model errors. If we assume that the residuals are independent of one another and follow a Gaussian distribution with zero mean and a constant standard deviation. From there, we can determine that the regression parameters ($\beta$ coefficients) also follow Gaussian distributions. If we minimize the sum of the squared errors, ordinary least squares also maximizes the Gaussian log-likelihood.   We treat the data as fixed (i.e., not random variables) and find values for the parameters that maximize log-likelihood. In other words, MLE finds the parameters' most plausible (i.e., "likely") values for the observed data. Let's denote the likelihood as $\ell(z|\theta)$. 

The consequence of this probabilistic specification is that the model parameters follow a singular, specific probability distribution.

A Bayesian approach uses a different philosophy regarding our probability assumptions. It does not assume that the model parameters come from a single distribution. It assumes that the unknown parameters are drawn from a prior distribution that reflects what we know about them. The "prior" adjective is important; the modeler should specify it before seeing the observed data. 

For example, recall our model for the time to deliver food. A reasonable prior for the regression parameter associated with distance would have a distribution that has zero probability for values less than zero. We would never think that delivery times decrease with distance. If we worked at the restaurant, we might be able to be more specific based on what we've seen. There might be a basic belief that, all other things are equal, and we might think that every mile further away from the restaurant increases time two-fold. From there, we could find a probability distribution that would fit those assumptions. We'll denote our prior distribution as $\pi(\theta)$. 
...
$$
\pi(\theta|x) = \frac{\pi(\theta) \ell(z|\theta)} {\pi(x)}
$$ {#eq-bayes-rule-distr}
...
Instead of treating the unknown parameter as a single value that we estimate, Bayesian methods propose that the parameter comes from a _distribution_ of possible values. This distribution reflects our prior understanding or belief about the parameter based on what we know of the situation. For this reason, it is called a _prior_ distribution. If we think that the probability that a location in Washington state is forested, we would choose a distribution with higher probabilities near 1.0. 

For a binomial model, an example of a prior distribution for the probability of an event is the Beta distribution. Beta values range between zero and one. The Beta distribution is very flexible. It is indexed by two parameters: $\alpha$ and $\beta$. @fig-beta-dist shows different versions of the Beta for different values.  If we think that larger probabilities of forestation are more likely, we might choose a Beta prior that has values of $\alpha$  that are larger than $\beta$. This is an example of an informative prior, albeit a weak one. As our prior distribution is more peaked, it reflects that before seeing any data, we have strong beliefs. 
...
What if we have no idea of what the prior should be? In this case, a flat (or nearly flat) prior would equally weight every possible point. In @fig-beta-dist we can see that the combination of $\alpha = \beta = 1$ is a uniform distribution. 

We’ll see how the prior can affect some calculations later in @sec-naive-bayes. In the meantime, let’s see how it comes into play in our simple binomial example. If the training set has $n_{tr}$ points and $n_+$ of them are known to be forested, the ordinary sample proportion estimate is $\hat{p} = n_+ / n_{tr}$. In a Bayesian analysis, the final estimate is a function of both the prior distribution and our observed data. For a Beta prior, the Bayesian estimate is 

$$
\hat{p}_{BB} = \frac{n_+ +\alpha}{n_{tr} + \alpha + \beta}
$$ {#eq-beta-bin-mean}

If, before seeing the data, we had chosen $\alpha = 5$ and $\beta = 1$, we estimate that $\hat{p}_{BB} = `r round(beta_bin_estimate(n_y, n_tr, 5, 1), 3)`$%. This is pretty close to our simple estimate because there is so much data in the training set ($n_{tr} = `r n_tr`$) that the influence of the prior is severely diminished. As a counter-example, if our data were $n_y = 1$ and $n_{tr} = 2$, the prior would pull the estimate from 50% to 75%. 

Once we have specified our prior and collected data, the results of a Bayesian analysis is a _posterior_ distribution. In our binomial example, we are lucky that the Beta prior has a special relationship with the binomial distribution; when used together, the posterior _also_ has a beta distribution. @eq-beta-bin-mean is the mean of that distribution. In general, it is unusual for the posterior to be so cleanly described. In most cases, it has an unknown form that must be estimated using Monte Carlo methods. We often summarize the prior using statistics like the mean, mode, standard deviation, etc. One strength of Bayesian methods is that we can easily derive complex statistics from them, including the posterior of individual predictions (called posterior predicted distribution). 
