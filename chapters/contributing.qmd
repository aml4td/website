...
When you open the `website.Rproj` file, the renv package should be automatically installed/updated (if needed). For example: 
...
If you try to compile the book, you probably get an error:
...
Since we have package versions recorded in the lockfile, we can install them using `renv::restore()`. Here is an example of that output: 
...
use_tidy_contrib <- function(repo_spec = NULL, from = NULL, to = NULL) {
  ...
  contributors <- contributors[
    !(contributors %in% c("topepo", "kjell-stattenacity"))
  ]
