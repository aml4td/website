...
rates_by_var <- function(.data, variable, group, conf.level = 0.95, as_percent = TRUE, reorder = TRUE) {
  ...
  res <- 
    ...
    dplyr::mutate(
      obj = purrr::map2(events, total, ~ binom.test(.x, .y, conf.level = conf.level)),
      obj = purrr::map(obj, broom::tidy)
    ) |> 
    tidyr::unnest(obj) |> 
    dplyr::select(-statistic, -parameter, -method, -alternative) |> 
    dplyr::rename(rate = estimate, p_value = p.value, lower = conf.low, upper = conf.high)
...
p_complaint <- 
  ...
  ggplot(aes(x = rate, y = complaint_category, col = complaint_priority, pch = complaint_priority)) +
  geom_point() + 
  geom_errorbar(aes(xmin = lower, xmax = upper), width = 3 / 4, alpha = 2 / 3) +
