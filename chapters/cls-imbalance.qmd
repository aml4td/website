...
myopia_party <- 
  myopia_train |> 
  mutate(father_myopic = factor(ifelse(father_myopic == 1, "myopic", "normal"))) |> 
  rename(
    `paternal genetic` = father_myopic,
    `spherical equivalent refraction` = spherical_equivalent_refraction
  )
tree_fit <- ctree(class ~ ., data = myopia_party)
tree_depth <- depth(tree_fit)
tree_term_nodes <- 
  tree_fit |> 
  predict(, type = "node") |> 
  vctrs::vec_unique_count()
tree_terms <- names(varimp(tree_fit))
tree_terms[tree_terms == "father_myopic"] <- "paternal genetics"
tree_terms <- gsub("_", " ", tree_terms)
```

```{r}
#| label: fig-myopia-tree
#| echo: false
#| out-width: 100%
#| fig-width: 10
#| fig-height: 7
#| fig-cap: "A conditional inference tree fir to the myopia training set. Most of the data from the training set are routed through a single rule since thet are the majority class."
