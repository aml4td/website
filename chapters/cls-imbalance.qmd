...
glmn_plain_grid <-
  crossing(mixture = c(0.05, (1:5) / 5), penalty = path)
...
glmn_plain_best_mtr <- collect_metrics(glmn_plain_res) |>
  inner_join(glmn_plain_best, by = ".config")
...
glmn_plain_roc_curve <- 
  ...
  roc_curve(class, .pred_no)

glmn_pr_roc_curve <- 
  glmn_plain_best_pred |>
  pr_curve(class, .pred_no)
...
lgb_plain_best_mtr <- collect_metrics(lgb_plain_res) |>
  inner_join(lgb_plain_best, by = ".config")
...
rf_plain_best_mtr <- collect_metrics(rf_plain_res) |>
  inner_join(rf_plain_best, by = ".config")
...
rf_default <- closest_point(rf_roc_curve)
rf_sensitive <- closest_point(rf_roc_curve, "sensitivity", 0.95)
```
...
xrf_plain_best_mtr <- collect_metrics(xrf_plain_res) |>
  inner_join(xrf_plain_best, by = ".config")
