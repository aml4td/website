...
post_prob <- function(prior, estimate) {
  (estimate * prior) / ((estimate * prior) + ((1 - estimate) * (1 - prior)))
}

best_prob <- function(x, ...) {
  abs(post_prob(estimate = x, ...) - 0.5)
}
```
...
```{r}
#| label: fig-myopia-cart
#| echo: false
#| fig-width: 10
#| fig-height: 5
#| fig-cap: A CART fit to the myopia training set. Most of the training set data are covered by the initial split, which focuses on the majority class. 
#| out-width: 90%
rpart(class ~ ., data = myopia_formatted) |> 
  as.party() |> 
  plot()
```
...
Third, the event rate significantly affects our class probability estimates, even in non-Bayesian models. Recall from @sec-nonlinear-da that one of the prior distributions corresponds to the probability distribution of our outcome ($Pr[Y = k]$ in @eq-discrim). If our supervised model is represented by $Pr[\boldsymbol{x}|Y = k]$, its predicted probabilities are shown by the event rate to produce the posterior probability. In other words, if the minority class event rate is 5% and our model predicts that a sample estimates the probability of being an event as 80%, the overall (posterior) probability of being an event is about `r round(post_prob(0.05, 0.8) * 100, 0)`%. This raises the bar for the supervised model, requiring exquisite performance to overcome the prevalence. With this event rate, the model would have to produce a class probability estimate of `r round(optimize(best_prob, c(0.8, 1), prior = 0.05)$minimum * 100, 0)`% or more to produce a posterior probability of at least 50%. We'll discuss this idea in the next section as well as in @sec-prevalence-again.
...
mirai::daemons(3)
tab_icl_spec <- tabular_icl(mode = "classification")
set.seed(820)
tab_icl_res <-
  tab_icl_spec |>
  fit_resamples(
...
To choose the threshold using a custom cost metric (the first option in the list above), we would have to determine an appropriate cost structure for false positives and false negatives, usually based on the consequences of either error. For our myopia example, this is a somewhat subjective choice that depends on some context-specific rationale. In other cases, the costs might be analytically calculated as a function of monetary costs [@zadrozny2001learning]. @fig-class-cost shows examples of cost curves if the false positive cost is fixed at 1.0 and the cost of a false negative varies in severity. The y-axis is the mean of the cost values averaged across the `r nrow(myopia_rs)` assessment sets. 
...
This approach is straightforward: every class with more data than the minority class is randomly sampled, ensuring that all classes have the same number of training set points. The minority class stays the same. For our myopia data, the majority class is randomly reduced from `r sum(myopia_train$class == "no")` to `r sum(myopia_down_formatted$class == "no")` patients. @fig-myopia-subsampling show examples for the two predictor subset including how the FDA fit changes with the reduced data set. The same class boundary is shown within each of the facets. We can see that the boundary is slightly more simplistic than @fig-myopia-fda but has shifted to the right, capturing more events with the default 50% cutoff (at the expense of increased false positives). 
...
sampling_glmn_rec_list <-
  list(
    Downsample = down_glmn_rec,
    SMOTE = smote_glmn_rec,
    ROSE = rose_glmn_rec,
    `Near Miss` = nm_glmn_rec
  )
...
sampling_set <-
  bind_rows(
    workflow_set(
      preproc = sampling_glmn_rec_list,
...
```{r}
#| label: fig-smote-rose
#| echo: false
#| out-width: 80%
#| fig-width: 7
#| fig-height: 3.5
#| warning: false
#| fig-cap: "Demonstrations of how SMOTE and ROSE synthesize new samples. The chosen point is the large red circle, and SMOTE's selected nearest neighbor is the large red square. The ellipses in panel (b) represent Gaussian probability contours based on the kernel density estimate. Twenty points are synthesized for each panel."
...
train_minority_nn <-
  train_minority |>
  slice(graph$nn.idx[selected_ind, ])

train_minority_nn_sel <-
  train_minority |>
  slice(graph$nn.idx[selected_ind, selected_neighbor])

train_minority_example <-
  train_minority |>
  slice(selected_ind)
...
n <- sum(myopia_smol_formatted$class == "yes")
yes_ind <- which(myopia_smol_formatted$class == "yes")
yes_pred <- myopia_smol_formatted |> filter(class == "yes") |> select(-class)
ids.generation <- rep(selected_ind, num_synth)

set.seed(282)
cons.kernel <- (4 / ((num_pred + 2) * n))^(1 / (num_pred + 4))
H <- hmult * cons.kernel * diag(apply(yes_pred, 2, sd), num_pred)
rose_synth <- matrix(rnorm(num_synth * num_pred), num_synth, num_pred) %*% H
rose_synth <- as_tibble(rose_synth + yes_pred[ids.generation, ])
point_center <- as.numeric(yes_pred[ids.generation[1], 1:2])
...
xrng <- extendrange(tmp$`spherical equivalent refraction`)
yrng <- extendrange(tmp$`axial length`)
# make lower right range wider
xrng[2] <- 0.5
yrng[1] <- 22
...
rose_grid <- crossing(
  `spherical equivalent refraction` = seq(xrng[1], xrng[2], length.out = 100),
  `axial length` = seq(yrng[1], yrng[2], length.out = 100)
)

# Assumed independent in H so we can use the product of two univariates
rose_grid$probability <- 
  dnorm(rose_grid$`spherical equivalent refraction`, mean = point_center[1], sd = sqrt(H[1,1])) *
  dnorm(rose_grid$`axial length`, mean = point_center[2], sd = sqrt(H[2,2]))

p_rose <-
  myopia_smol_formatted |>
  ggplot(aes(`spherical equivalent refraction`, y = `axial length`)) +
  geom_raster(data = rose_grid, aes(fill = probability), alpha = 0.2) +
  geom_contour(
    data = rose_grid,
    aes(z = probability),
    col = "#DE2D26",
...
smote_synth <-
  tibble(
    `spherical equivalent refraction` = runif(
      num_synth,
      min = train_minority_example[[1]],
      max = train_minority_nn_sel[[1]]
    ),
    `axial length` = runif(
      num_synth,
      min = train_minority_nn_sel[[2]],
      max = train_minority_example[[2]]
    )
  )
...
p_smote <-
  myopia_smol_formatted |>
  ggplot(aes(`spherical equivalent refraction`, y = `axial length`)) +
  geom_point(aes(col = class), alpha = 1 / 2, show.legend = FALSE) +
  lims(x = xrng, y = yrng) +
  geom_point(
    data = myopia_smote_neighbor,
    aes(x = `spherical equivalent refraction`, y = `axial length`, col = class),
    cex = 5,
    show.legend = FALSE
  ) +
...
```{r}
#| label: near-miss-data
#| include: false

num_sim_min <- 4
num_sim_maj <- 6
min_labs <- (num_sim_maj + 1):(num_sim_min + num_sim_maj)
min_labs <- paste(min_labs)
...
#| fig-height: 5
#| warning: false
#| fig-cap: "Data to demonstrate Near Miss and Tomek Link methods."

min_maj |>
  ggplot(aes(V1, V2, col = class)) +
  geom_point(cex = 9, pch = 1) +
  geom_text(aes(label = id), cex = 6, show.legend = FALSE) +
  lims(
    x = extendrange(min_maj$V1),
    y = extendrange(min_maj$V2)
  ) + 
...
```{r}
#| label: near-miss-calcs
#| include: false

min_sim <- min_maj |> filter(class == "minority")
maj_sim <- min_maj |> filter(class == "majority")

distances <-
  proxyC::dist(
    x = min_sim |> select(-class, -id) |> as.matrix(),
    y = maj_sim |> select(-class, -id) |> as.matrix()
  )
# minority rows x majority rows
dist_mat <- as.matrix(distances, 1)
colnames(dist_mat) <- 1:nrow(maj_sim)
rownames(dist_mat) <- (1:nrow(min_sim)) + nrow(maj_sim)

dist_df <-
  dist_mat |>
  as_tibble(rownames = "Minority")

dist_and_ind <- function(x, k = 5, ...) {
  ordering <- order(x, ...)
  inds <- seq_along(x)[order(x, ...)]
  closest <- inds[1:k]
  mn <- mean(x[closest])
  list(index = closest, mean = mn, labels = min_labs[closest])
}

# ------------------------------------------------------------------------------


nm1_info <- apply(distances, 2, dist_and_ind, k = 2)
nm1_dists <- map_dbl(nm1_info, ~ .x$mean)
names(nm1_dists) <- colnames(dist_mat)
nm1_sel <- names(sort(nm1_dists))
nm1_sel <- sort(nm1_sel[1:min(num_sim_min, length(nm1_sel))])
nm1_sel <- cli::format_inline("{paste0('#', nm1_sel)}")

nm1_df <-
  bind_rows(
    dist_df,
    tibble::as_tibble_row(nm1_dists) |>
      mutate(Minority = "2NN Mean")
  )
...
for (i in seq_along(nm1_info)) {
  nm1_gt <-
    nm1_gt |>
    tab_style(
      style = list(
        cell_fill(color = "#B0E0E6")
      ),
      locations = cells_body(
        columns = i + 1,
        rows = nm1_info[[i]]$index
      )
    )
}
...
nm2_info <- apply(distances, 2, dist_and_ind, k = 2, decreasing = TRUE)
nm2_dists <- map_dbl(nm2_info, ~ .x$mean)
names(nm2_dists) <- colnames(dist_mat)
nm2_sel <- names(sort(nm2_dists))
nm2_sel <- sort(nm2_sel[1:min(num_sim_min, length(nm2_sel))])
nm2_sel <- cli::format_inline("{paste0('#', nm2_sel)}")

nm2_df <-
  bind_rows(
    dist_df,
    tibble::as_tibble_row(nm2_dists) |>
      mutate(Minority = "2FN Mean")
  )
...
reverse_info <- apply(distances, 1, dist_and_ind, k = 2)
nm3_keep <- map(reverse_info, ~ .x$index) |> unlist() |> sort() |> unique()
nm3_distances <- distances[, nm3_keep]

nm3_info <- apply(nm3_distances, 2, dist_and_ind, k = 2)
nm3_dists <- map_dbl(nm3_info, ~ .x$mean)
names(nm3_dists) <- colnames(dist_mat)[nm3_keep]
nm3_sel <- names(sort(nm3_dists, decreasing = TRUE))
nm3_sel <- sort(nm3_sel[1:min(num_sim_min, length(nm3_sel))])
nm3_sel <- cli::format_inline("{paste0('#', nm3_sel)}")

nm3_df <-
  bind_rows(
    dist_df |> select(Minority, all_of(paste(nm3_keep))),
    tibble::as_tibble_row(nm3_dists) |>
      mutate(Minority = "2NN Mean")
  )
...
nm1_gt |> 
  ...
  tab_spanner(label = "Majority Point Labels", columns = c(-Minority)) |> 
  tab_style(
    style = cell_borders(
      sides = "bottom",
      col = "#D3D3D3",
    ...
    locations = cells_body(rows = nrow(nm1_df) - 1)
  ) |> 
...
For the first majority-class observation, its closest minority-class samples are #7 and #8 (each column's two nearest neighbors are highlighted with a background accent color). The average distance between its two neighbors, shown in the bottom row of the table, is `r round(nm1_dists[1], 2)`. To balance the data, NearMiss-1 would retain four samples: `r nm1_sel`.
...
Note that the neighbors are the opposite of those in the previous table. In this instance, we would retain samples `r nm2_sel`. 
...
Based on these distances, NearMiss-3 selects `r nm3_sel` to include in the majority class. 

```{r}
#| label: tomek-calcs
#| include: false
nn_mat_min <- nn_mat_maj <- dist_mat * 0
tom_col_info <- apply(distances, 2, dist_and_ind, k = 1)
tom_col_nn <- map_int(tom_col_info, ~ .x$ind)
tom_row_info <- apply(distances, 1, dist_and_ind, k = 1)
tom_row_nn <- map_int(tom_row_info, ~ .x$ind)

min_ind <- 7:10
for (i in seq_along(tom_col_nn)) {
  tmp_row <- tom_col_nn[i]
  nn_mat_maj[tmp_row, i] <- 1
}

for (j in seq_along(tom_row_info)) {
  tmp_col <- tom_row_nn[j]
  nn_mat_min[j, tmp_col] <- 1
}

# nn_mat_min
keep_min <- apply(nn_mat_min, 2, function(x) all(x == 0)) |> which() |> names()

# nn_mat_maj
keep_maj <- apply(nn_mat_maj, 1, function(x) all(x == 0)) |> which() |> names()

tomek_sel <- sort(c(keep_maj, keep_min))
tomek_sel <- cli::format_inline("{paste0('#', tomek_sel)}")

tomek_myopia <- imbalanced_sampled |> filter(Data == "Tomek") |> count(class)
```
...
When applied to the myopia data, the number of myopic samples is reduced from `r sum(myopia_train$class == "yes")` to `r tomek_myopia$n[tomek_myopia$class == "yes"]` and the number of non-myopic rows in the training set drops from `r sum(myopia_train$class == "no")` to `r tomek_myopia$n[tomek_myopia$class == "no"]`. While the reduction in events might compromise the model's effectiveness, @fig-myopia-subsampling shows that the FDA class boundaries produced with the entire data set and the reduced version are extremely similar. 
...
#| fig-height: 7
#| out-width: 80%
#| fig-cap: "Cross-validated statistics for the subsampling and synthesis methods. The top panel shows an ROC-like visualization with sensitivity and 1 - specificity. The bottom panel relates sensitivity ot the Brier Score."
...
A flexible discriminant fit to the myopia training set using cost-sensitive learning. The myopic subjects had a 15-fold higher cost in the objective function and `r fda_cost_capture` of the `r sum(myopia_smol_formatted$class == "yes")` events (in red) are identified using the default 50% threshold. This figure is comparable with @fig-myopia-fda and @fig-myopia-subsampling.
...
#| fig-height: 7
#| out-width: 80%
#| fig-cap: "Cross-validated statistics for the cost-sensitive models. The top panel shows an ROC-like visualization with sensitivity and 1 - specificity. The bottom panel relates sensitivity ot the Brier Score."
