...
The effects of single paternal and material genetics are in between, with rates `r rates_by_parent$rate[2]`% and `r rates_by_parent$rate[3]`%, respectively, with similar confidence intervals.    
...
fda_plain_fit <- discrim_flexible(prod_degree = 2) |> 
  fit(class ~ ., data = myopia_smol_party)
fda_plain_pred <- augment(fda_plain_fit, new_data = smol_grid)
fda_plain_capture <- 
  augment(fda_plain_fit, new_data = myopia_smol_party) |> 
...
A flexible discriminant fit to the myopia training set. Using the default probability cutoff of 50%, shown as the black contour, `r fda_plain_capture` of the `r sum(myopia_smol_party$class == "yes")` events are correctly predicted as events. 
::::
...
If we are satisfied with the model's ability to discriminate between classes (as evidenced by the ROC curve) and that the probabilities are sufficiently calibrated, we can address the sensitivity problem by selecting an appropriate probability threshold. As with most optimization problems, we require an objective function to make the best decision regarding this tuning parameter. There are a few ways we can approach the problem: 
 ...
 2. Seek a constrained optimization. For example, we can try to maximize the sensitivity under the constraint that the specificity is at an acceptable value. The ROC curve provides the substrate to make these calculations since it includes sensitivity and specificity for many threshold in the data. 
...
This seeming disconnect can become worse as the prevalence becomes smaller. For very rare events (i.e., < 5%), the cutoff associated with the sensitivity-specificity trade off can be very small, perhaps less than 1%. This is the price that is paid for having probability estimates that are consistent with the rate the event occurs in the wild. 
 
 
## Strategy 2: Corrections for the Imblanace
...
set.seed(616)
myopia_down_smol_party <- 
  recipe(class ~ ., data = myopia_smol_party) |> 
  step_downsample(class) |>
  prep() |> 
  bake(new_data = NULL)

fda_down_fit <- discrim_flexible(prod_degree = 2) |> 
  fit(class ~ ., data = myopia_down_smol_party)
fda_down_pred <- augment(fda_down_fit, new_data = smol_grid)
fda_down_capture <- 
  augment(fda_down_fit, new_data = myopia_smol_party) |> 
  filter(.pred_yes >= 0.5 & class == "yes") |> 
  nrow()
```
...
A flexible discriminant fit to the downsampled myopia training set. The subsampling process increased the number of events found using the 50% threshold (`r fda_down_capture` out of `r sum(myopia_smol_party$class == "yes")`). This figure is comparable with @fig-myopia-fda.
...
CART book, resamping weights versus case weights


```{r}
#| label: myopia-cost-fda
#| include: false
myopia_smol_wts_party <- 
  myopia_smol_party |> 
  mutate(
...
fda_cost_fit <- fit(fda_wts_wflow, data = myopia_smol_wts_party)
fda_cost_pred <- augment(fda_cost_fit, new_data = smol_grid)
fda_cost_capture <- 
  augment(fda_cost_fit, new_data = myopia_smol_party) |> 
  filter(.pred_yes >= 0.5 & class == "yes") |> 
  nrow()
```
...
A flexible discriminant fit to the myopia training set using cost-sensitive learning. The myopic subjects had a 15-fold higher cost in the objective function and `r fda_cost_capture` of the `r sum(myopia_smol_party$class == "yes")` events (in red) are identified using the default 50% threshold. This figure is comparable with @fig-myopia-fda and @fig-myopia-down-fda.
...
cost_sens_brier <- 
  cost_mtr |>
  ggplot(aes(sensitivity, brier_class, col = class_weight)) +
