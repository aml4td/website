...
We will begin by describing transformations that are applied to one predictor at a time that yield a revised for of the predictor (one in, one out).  Then we will explore transformations that can be applied to a group of predictors and yield an informative summary of the group (many in, some out).  Later, in @sec-interactions-nonlinear, we will examine techniques for expanding a single predictor to many predictors (one in, many out).  Let's begin by understanding some general data characteristics that need to be addressed via feature engineering and when transformations should be applied. 
