...
lot_area_raw_light <- 
  ...
  geom_histogram(bins = 20, col = light_bg, fill = "#8E195C", alpha = 1 / 2) +
  thm_lt
...
lot_area_raw_dark <- 
  ...
  geom_rug(alpha = 1 / 2, length = unit(0.04, "npc"), linewidth = 1.2, 
           col = dark_data) + 
  geom_histogram(bins = 20, col = dark_bg, fill = "#BD5630FF", alpha = 3 / 4) +
  thm_dk
...
(lot_area_raw_light  +  lot_area_yj_light) / (lot_area_pctl_light + lot_area_norm_light)

(lot_area_raw_dark  +  lot_area_yj_dark) / (lot_area_pctl_dark + lot_area_norm_dark)
```

:::

Lot area for houses in Ames, IA. The raw data (a) are shown along with transformed versions using the Yeo-Johnson (b), percentile (c), and ordered quantile normalization (d) transformations.
...
gross_area_norm_light <- 
  gross_area_norm + 
  geom_rug(alpha = 1 / 2, length = unit(0.04, "npc"), linewidth = 1.2) + 
  geom_histogram(bins = 20, col = light_bg, fill = "#8E195C", alpha = 1 / 2) +
  thm_lt

gross_area_norm_dark <- 
  gross_area_norm + 
  geom_rug(alpha = 1 / 2, length = unit(0.04, "npc"), linewidth = 1.2, 
           col = dark_data) + 
  geom_histogram(bins = 20, col = dark_bg, fill = "#BD5630FF", alpha = 3 / 4) +
...
gross_area_range_light <- 
  gross_area_range + 
  geom_rug(alpha = 1 / 2, length = unit(0.04, "npc"), linewidth = 1.2) + 
  geom_histogram(bins = 20, col = light_bg, fill = "#8E195C", alpha = 1 / 2) +
  thm_lt

gross_area_range_dark <- 
  gross_area_range + 
  geom_rug(alpha = 1 / 2, length = unit(0.04, "npc"), linewidth = 1.2, 
           col = dark_data) + 
  geom_histogram(bins = 20, col = dark_bg, fill = "#BD5630FF", alpha = 3 / 4) +
...
gross_area_raw_light + gross_area_norm_light + gross_area_range_light

gross_area_raw_dark + gross_area_norm_dark + gross_area_range_dark
```

:::

The original gross living area data and two standardized versions.
...
data_cols_light <- c(rgb(0.27, 0.59, 0.15), rgb(0, 0, 0, 1/5))
data_cols_dark <- c("#459626", dark_data)
...
two_areas_raw_light <- 
  two_areas_raw +
  scale_color_manual(values = data_cols_light) + 
  thm_lt

two_areas_raw_dark <- 
  two_areas_raw +
  scale_color_manual(values = data_cols_dark) + 
  dk_gif_thm
...
::: {#fig-ames-lot-living-area}

::: {.figure-content}

```{r}
#| label: ames-lot-living-area
#| renderings: [light, dark]
#| fig-width: 8
#| fig-height: 3
#| out-width: "100%"

((two_areas_raw_light + two_areas_norm_light + two_areas_ss_light) +
    plot_layout(guides = "collect"))

((two_areas_raw_dark + two_areas_norm_dark + two_areas_ss_dark) +
    plot_layout(guides = "collect"))
```

:::

Lot area (x) versus gross living area (y) in raw format as well as with order-norm and spatial sign transformations.
