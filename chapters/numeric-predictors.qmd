...
#| fig-alt: "Four histograms of lot area arranged in a two-by-two grid. Panel (a) shows the original data, which is heavily right-skewed with a long tail extending toward large lot sizes. Panel (b) applies the Yeo-Johnson transformation, producing a more symmetric but still slightly skewed distribution. Panel (c) uses the percentile transformation, resulting in a roughly uniform distribution. Panel (d) applies ordered quantile normalization, yielding a symmetric bell-shaped distribution."
...
|----------------|:--------------------------------------------------------------:|:----------------------:|
| Modulus        | $$x^* = \begin{cases} sign(x)\lambda^{-1}\left[(\vert x\vert+1)^\lambda-1\right] & \text{if $\lambda \neq 0$,}\\[3pt] sign(x) \log{(\vert x\vert+1)} &\text{if $\lambda = 0$} \end{cases}$$ | @john1980alternative |
| Bickel-Doksum | $$x^* = \lambda^{-1}\left[sign(x)\vert x\vert^\lambda - 1\right]\quad\text{if $\lambda \neq 0$}$$ | @bickel1981analysis |
| Glog / Gpower  | $$x^* = \begin{cases} \lambda^{-1}\left[({x+ \sqrt{x^2+1}})^\lambda-1\right]  & \text{if $\lambda \neq 0$,}\\[3pt] \log({x+ \sqrt{x^2+1}}) &\text{if $\lambda = 0$} \end{cases}$$ | @durbin2002variance, @kelmansky2013new |
...
Skewness can also be resolved using techniques related to distributional percentiles. A percentile is a value with a specific proportion of data below it. For example, for the original lot area data, the 10th percentile is `r format(quantile(ames_train$Lot_Area, prob = .1), big.mark = ",")` square feet, which means that 10{{< pct >}} of the training set has lot areas less than `r format(quantile(ames_train$Lot_Area, prob = .1), big.mark = ",")` square feet. The minimum, median, and maximum are the 0th, 50th and 100th percentiles, respectively.
...
#| fig-height: 3
#| out-width: "92%"
#| fig-alt: "Three histograms of gross living area arranged side by side. All three have the same right-skewed shape. Panel (a) shows the original values ranging roughly from 500 to 5000 square feet. Panel (b) is centered and scaled, shifting the x-axis to center on zero with a standard deviation of one. Panel (c) is range scaled, compressing values to the zero-to-one interval. The distribution shape is identical across all three panels; only the x-axis scale changes."
gross_area_raw <-
  ames_train |>
...
$$
x^*_{ij}=\frac{x_{ij}}{\sqrt{\sum\limits^{p}_{j=1} x_{ij}^2}}
$$  {#eq-spatial-sign}
...
two_areas_rec <-
  ...
  step_mutate(
    location = ifelse(
      Lot_Area > 30000 | Gr_Liv_Area > 3500,
      "'outlying'",
      "mainstream"
    )
  ) |>
...
two_areas_raw <-
  ...
  geom_point(
    aes(col = location, pch = location, size = location),
    alpha = 1 / 2
  ) +
...
#| fig-height: 3
#| out-width: "100%"
#| fig-alt: "Three scatter plots of lot area versus gross living area arranged side by side. The left panel shows the raw data with a cluster of points in the lower-left and a handful of outlying points highlighted with a distinct shape far to the right or top. The middle panel applies an order-norm transformation, spreading the data more evenly and making the outlying points less extreme. The right panel applies the spatial sign transformation, projecting all points onto a circle centered at the origin so that the previously outlying points are no longer distributionally abnormal."
