  ...
  output$plot <-
    renderPlot({
      ...
      if (identical(show_cls, 1:2)) {
        p <- p +
          geom_point(
            data = new_df |> filter(class %in% 1),
            col = "black",
            cex = 2
          ) +
          geom_point(
            data = new_df |> filter(class %in% 2),
            col = "black",
            cex = 2,
            pch = 17
          ) +
...
We'll need to tune the model over cost as well as any kernel parameters. It is difficult to know _a priori_ ranges for many of the kernel parameters; they are very dataset-specific. Additionally, there is often an interaction effect between the tuning parameters. For example, @fig-svm-grid shows this. The polynomial kernel was fixed with $\theta_3 = 2$ (i.e., quadratic), and the optimization was over $\mathcal{C}$ and $\theta_1$. The figure shows the optimal parameter region, with a line representing a ridge of optimal parameters. The key takeaway is that, for a fixed cost parameter, the optimal scaling parameter changes (and vice versa). 
...
The polynomial kernel was tuned over $\theta_1$ and $\theta_3$ (fixing $\theta_2 = 1$). We tuned the RBF kernel over $\sigma$.  The search spaces were:

- Cost: 2<sup>-10</sup> to 2<sup>5</sup>.
- Polynomial degree ($\theta_3$): 1, 2, or 3.
- Scale factor ($\theta_1$): 10<sup>-10</sup> to 10<sup>-1</sup>.
