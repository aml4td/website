...
Recall that vectors have a magnitude and direction. Magnitude is usually measured by its length, which we'll denote by $||\boldsymbol{a}||$ for some vector $\boldsymbol{a}$ with $p$ elements.

Direction is concerned with the angle of the vector to something, say another vector $\boldsymbol{y}$ with the same dimension ($p$).  
...
Dot products measure both the magnitude of each vector as well as the angle between them (denoted as $\vartheta$). We can rewrite @eq-dot-prod-sum as
