...
AdamW was used to train the models, and a maximum of 25 epochs (with early stopping at five bad epochs) was allowed. 
...
@fig-forest-mlp-ranks shows the results. The candidates were ranked by their resampling estimate of the Brier scores, and the figure presents these ranked from best to worst while breaking out the combinations of preprocessing scenarios. While each panel contains models with sufficiently low Brier scores, there are a few trends. The "Plain" panel represents models using dummy variable indicators with no symmetry transformation, and this did not do well (with or without PCA feature extraction). 
...
mlp_ranks|> 
  mutate(
    ...
    encoding = factor(encoding, levels = c("orderNorm", "plain", "encoded")),
    ...
    upper = mean + qnorm(0.025) * std_err
  ) |> 
...
The two cases that did the best were the "orderNorm" and "encoded" panels. While both used the orderNorm transformation, the former panel used indicators, while the latter used the supervised effect encoding, where a single column was used to represent the county effect. This suggests that eliminating distributional skew in the predictors may buy us a small amount of predictive performance. 
...
While the Brier scores are low, the areas under the ROC curves are all at least 0.94, indicating that the predictions do a good job separating the classes and that the probability estimates are accurate. 
