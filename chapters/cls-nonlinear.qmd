...
In this chapter, several recurring motifs are present across different types of models. First, this involves the use of an operation that utilizes the positive part of a function, known as "hinge" or "rectifier" functions. We'll see these in @sec-fda and @sec-nnet-activation. Another element that will be encountered across different models is the kernel function, previously seen in @sec-gp. These functions are used to quantify the distance or similarity between data points (or, more specifically, vectors). Both the K-nearest neighbor and the support vector machine (SVM) models rely on these functions. Finally, the dot product between vectors plays a critical role in the important _attention_ mechanism in neural networks (@sec-cls-attention) as well as in SVM models (@sec-cls-svm). 

To get started, the first section summarizes an old, traditional model for classification, linear discriminant analysis (LDA), and shows how it can be effectively modified to produce nonlinear class boundaries.   
...
forest_nb_res |>
  ...
  distinct() |>
  inner_join(
    select_best(forest_nb_res, metric = "brier_class"),
...
The predictions for the _best_ candidate are those shown in @fig-nb-probs. As noted there, the Brier score was mediocre while the ROC curve results indicate that the model can do a good job at qualitatively separating the classes. 
...
Being a contrast matrix, the column means are zero and the covariances between columns are zero.
...
An example of how MARS partitions the predictor space into two distinct regions, each with its own regression line that connects at the split point. For the bottom panel, we assume an intercept of $\beta_0 = 0$.
...
#| fig-width: 8
#| fig-height: 4
#| fig-cap: FDA-MARS tuning results for the forestation data.
...
Once the FDA-MARS model was fit to the training set, the result was `r nrow(fda_coefs)` total coefficients. The implementation of MARS checks several stopping conditions for the growing phase based on the coefficient of determination (a.k.a. R<sup>2</sup>), including: 
...
#| fig-width: 8
#| fig-height: 5
#| fig-cap: Profile plots showing how each predictor relates to the probability of forestation when an additive FDA-MARS is used. The y-axis has no numbers since the probabilities are relative.
...
- The probability of forestation has a notable jump after the year 2020.
...
$$
d_j(u_j, x_{j})={\frac {|u_{j}-x_{j}|}{R_{j}}}
$$
...
For categorical predictors, the component distance is a simple mismatch indicator, $d_j(u_j, x_{j}) = I(u_j \ne x_j)$.  This binary metric assigns zero distance for matching categories and unit distance otherwise.
...
How well does this technique work for the forestation data? To start, we used preprocessing that included the same  random forest importance filter as @sec-naive-bayes (and tuned the proportion of predictors retained). After that, an effect encoding was used for the county column, and then an orderNorm transformation was applied to all predictors to standardize them to the same units and potentially remove skewness. The model was tuned over the proportion of features retained (5% to 100%), the number of neighbors (two through fifty), the Minkowski degree parameter (between 0.1 and 2.0), and the kernel functions listed in @fig-weight-kernels. A total of fifty configurations were evaluated using resampling, and the Brier scores are shown in @fig-knn-cls-tune. 
...
Poor results occurred when there were fewer than twenty neighbors, and subsequently, the Brier scores remained consistently low. The distance parameter appeared to favor small values (around 0.5). There are no strong trends in terms of which kernel functions work best; Gaussian kernels appear to do well overall, but there are configurations that do well using any kernel. Numerically, the best results retained `r round(knn_best_res$prop_terms[1] * 100, 1)`% of the  predictors, used `r knn_best_res$neighbors[1]` neighbors, a Minkowski degree of `r signif(knn_best_res$dist_power[1], digits = 3)`, and a `r knn_best_res$weight_func[1] ` kernel function. In the figure above, we can see that there is a small increase in error when the entire predictor set is used. Otherwise, smaller feature sets are underfitting the data. The corresponding Brier score was `r signif(knn_best_res$mean[1], digits = 3)`, which is the smallest Brier score seen so far. The area under the ROC curve was also good, with a resampling estimate of `r forest_knn_best_roc`.

`r r_comp("cls-nonlinear.html#sec-cls-knn")`

## Sidebar: Dot Products {#sec-dot-prod}
...
Note that the dot product does not satisfy the properties of a distance metric since the dot product of a vector with itself could never be zero unless all of its elements are zero.
...
What is the dot product of $\boldsymbol{x}$ with itself? That value is $\boldsymbol{x}'\boldsymbol{x} =$ `r ref_prod`. The dashed black diagonal line shows the vectors whose dot product is `r ref_prod`. These are points that point in the same direction (i.e., $\vartheta = 0^{\circ}$) and magnitude as our example vector. Note that this line is parallel to the region of zero values, reemphasizing that the zero region is orthogonal to vectors like $\boldsymbol{x}$. 

The vector associated with the largest dot product would move in the same direction as $\boldsymbol{x}$. Since  $||\boldsymbol{x}||$ is fixed and $\vartheta = 0^{\circ}$, the magnitude of the vector would be as large as possible. Conversely, the vector with the smallest dot product will move in the opposite direction and as far as possible. For our $\boldsymbol{x}$, the dot product is maximized in the upper right corner of @fig-dot-products and minimized in the lower left corner. 

We'll see the dot product again in @sec-cls-attention where it is intended to measure the similarity of two embedding vectors within a neural network, and again in @sec-cls-svm where it is the computational backbone of support vector machines. 

## Nonlinear Support Vector Machines {#sec-cls-svm-nonlin}

In @sec-cls-svm-lin, we described the loss function for support vector machines and, in @eqn-svm-pred, we showed the prediction function: 

$$
\hat{\eta} = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \tilde{y}_i\hat{\alpha}_i \boldsymbol{x}_i'\boldsymbol{u}
$$ 

where the $\alpha_i$ values ($i = 1, \ldots, n_{tr}$) that are non-negative model coefficients, $\boldsymbol{x}$ and $\boldsymbol{u}$ are numeric vectors of predictors, and $\tilde{y}_i \in \pm 1$. Recall that the $\alpha_i$ are zero for some training set rows. 

For each support vector ($\alpha_i \ne 0$), the dot product is weighted by the corresponding model parameter, multiplied by the sign of the class indicator. The sum of these values defines what side of the default class boundary ($\eta = 0$) the new data point lies. 

Without feature engineering, the prediction boundary is linear in the predictors. This is due to the dot product $\boldsymbol{x}_i'\boldsymbol{u}$ being a linear operation. If there were $p=2$ predictors, the dot produce is simply $x_{i1}u_{i1} + x_{i2}u_{i2}$. REFERENCE DOT PROB section. 

Previously, we used feature engineering (natural splines) to add terms to make the linear predictor $\eta$ nonlinear in the parameters. These basis expansions created new column in the data. We'll call this an _explicit_ basis expansion since we generated additional columns. 

If we were to use a quadratic basis expansion (as in @sec-polynomials), we could define a function  $\phi(\boldsymbol{x})$ where, for $p = 2$ predictors, would be: 

$$
\phi(\boldsymbol{x}) = \left(1, \sqrt{2}x_1,  \sqrt{2}x_2,  \sqrt{2}x_1x_2, x_1^2, x_2^2\right)
$$

so that $\phi(\boldsymbol{x}_i)'\phi(\boldsymbol{u})$ would generate a dot product containing a full quadratic basis expansion. 

However, support vector machines can also naturally induce nonlinear class boundaries by using _implicit_ nonlinear functions like $\phi()$ that are computed within the model (i.e., without materializing actual columns in the data). In this case, the prediction equation is expanded to be: 

$$
\hat{\eta} = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \tilde{y}_i\hat{\alpha}_i \phi(\boldsymbol{x}_i)'\phi(\boldsymbol{u})= \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \tilde{y}_i\hat{\alpha}_i K(\boldsymbol{x}_i, \boldsymbol{u})
$$ {#eq-svm-nonlinear-model}

where $K()$ is a kernel function^[We've previously seen nonlinear SVMs in action in @sec-barley-svm and, in that same chapter, the radial basis function kernel and its utility as a covariance function in Gaussian process models.]. For example, an equivalent polynomial kernel for the vector form shown above is $K(\boldsymbol{x}, \boldsymbol{u}) = (1 + \boldsymbol{x}'\boldsymbol{u})^2$. A more general form for the polynomial kernel is: 

$$
K(\boldsymbol{x}, \boldsymbol{u}) = \left(\theta_1 \boldsymbol{x}'\boldsymbol{u} + \theta_2\right)^{\theta_3}
$$  {#eq-kernel-polynomial}

$\theta_3$ is an integer for the polynomial degree, $\theta_2$ is an offset that can be used to trade off between high- and lower-order polynomial effects, and $\theta_1$  is a scale parameter. There are also spline-based kernels [@gunn1997support;@YuanYubo2007Sfss]. 

The most popular kernel function for SVMs is the radial basis function (RBF) kernel: 

$$
K(\boldsymbol{x}, \boldsymbol{u}) = \exp\left(-\sigma\; ||\boldsymbol{x} - \boldsymbol{u}||^2\right)
$$  {#eq-kernel-rbf}

See @scholkopf1997comparing. There are several variants of the kernel such as the Laplacian kernel, that us an L<sub>1</sub> norm in the exponent, as well as radial basis kernel create using an ANOVA decomposition [@RBFANOVA]. 

There are also sigmoidal kernels [@sellathurai1999separability;@LinLin2003Sigmoid] that use the tanh activation function that will be seen in @sec-nnet-activation: 

$$
K(\boldsymbol{x}, \boldsymbol{u}) = tanh\left(\theta_1 \boldsymbol{x}' \boldsymbol{u} + \theta_2\right)
$$ 

Different kernel functions result in different patterns of nonlinear class boundaries. @fig-kernels shows an interactive visualization to demonstrate how the polynomial and RBF kernel functions produce class boundaries. In this example, there are eight support vectors for two classes. The figure shows the values of $\eta$ across a grid where all $\alpha_i = 1$. The default view shows the full summation of eight terms, but choosing one class or the other shows the accumulation of the four class-specific values on $abs(\eta)$. For the RBF kernel, $\eta$ shows peaks near the individual data points. Decreasing $\sigma$ expands the influence of each support vector, leading to fewer peaks and a more diffuse Gaussian-like pattern. Increasing $\sigma$ concentrates the influence until the individual effect of each support vector is limited to the space immediately surrounding it. The trend for the polynomial kernel are more predictable. The first-degree polynomial produces linear class boundaries, the second-degree polynomial produces a parabolic pattern, and the cubic polynomials generate boundaries that can have an "S" shape.  There is also a distinct difference in the shape of the class boundaries between the two kernel functions. 
  ...
  fluidRow(
    column(
      ...
      radioButtons(
        inputId = "kernel",
        label = "Kernel",
        choices = list("Radial Basis Function", "Polynomial")
      )
    ), # min distance
    column(
      width = 4,
    ...
    fluidRow(
      column(
        ...
        radioButtons(
          inputId = "data",
          label = "Show Data",
          choices = list("Both", "Class 1", "Class 2")
        )
      ),
      column(
        width = 6,
...
server <- function(input, output) {
  ...
  x_grid <- expand.grid(A = x_seq, B = x_seq)
  x_mat <- as.matrix(x_grid)
  ...
  new_mat <- matrix(new_vec, ncol = 2, byrow = TRUE)
  new_df <- tibble(
    A = new_mat[, 1],
    B = new_mat[, 2],
    A_start = 0,
    B_start = 0,
    class = rep(1:2, each = 4)
  )
  
  y_coefs <- rep(c(1, -1), each = 4)
  y_cls <- rep(1:2, each = 4)
  
  y_mat <- cbind(
    matrix(1, ncol = 4, nrow = nrow(x_mat)), 
    matrix(-1, ncol = 4, nrow = nrow(x_mat))
  )
  
  output$plot <-
    renderPlot({
      if (input$data == "Class 1") {
        show_cls <- 1
      } else if (input$data == "Class 2") {
        show_cls <- 2
      } else {
        show_cls <- 1:2
      }
      inc_col <- y_cls %in% show_cls

      if (input$kernel == "Radial Basis Function") {
        kern <- kernlab::rbfdot(sigma = 2^input$log_sigma)
      } else {
        kern <- kernlab::polydot(degree = input$degree)
      }
      k_grid <-  kernlab::kernelMatrix(kern, x_mat, new_mat)
      mat_prod <- k_grid * y_mat
      k_sum <- apply(mat_prod[, inc_col, drop = FALSE], 1, sum)
      if(length(show_cls) == 1) {
        k_sum <- abs(k_sum)
      }
      x_grid$dot_prod <- k_sum
      
      p <- x_grid |>
        ggplot(aes(A, B)) +
        geom_tile(aes(fill = dot_prod), alpha = 3 / 4) +
        geom_contour(aes(z = dot_prod), col = "black", linewidth = 1 / 10) +
        coord_fixed(ratio = 1) +
        theme(legend.position = "top") +
      ...
      if (identical(show_cls, 1:2)) {
        p <- p +
          geom_point(data = new_df |> filter(class %in% 1), col = "black", cex = 2) +
          geom_point(data = new_df |> filter(class %in% 2), col = "black", cex = 2, pch = 17) +
          scale_fill_gradient2(low = "red", mid = "white", high = "blue") 
      } else if (identical(show_cls, 1)) {
        p <- p +
          geom_point(data = new_df |> filter(class %in% 1), col = "black", cex = 2) +
...
A small SVM example with two classes (circles and squares) and eight support vectors. We can see how two different kernels and their parameters can effect the shape and complexity of the class boundary. The polynomial kernel set $\theta_1 = \theta_2 = 1$. 

:::

Recall that the SVM optimization problem uses an extra _cost_ parameter ($\mathcal{C}$) that requires specification before training. We usually tune this parameter in log<sub>2</sub> units. Unlike linear kernels, the cost parameter strongly affects how well the model performs. With small cost values, we tell the optimizer that prediction errors are acceptable. With a large cost value, the optimizer is told not to tolerate errors and to contort in every possible way to accurately predict all training-set samples. Increasing costs increases model complexity in support vector machines. The problem is that we want the fit to be flexible enough to effectively model the data but avoid overfitting. A visual example of this trade-off appears in @fig-two-class-overfit. We constructed these panels with an SVM using an RBF kernel. The low-complexity fit used $\mathcal{C} = 2^{-6.6}$ and the high-complexity model used $\mathcal{C} = 2^{20}$. The medium-complexity fit used a value of 1.0, which was close to the value estimated using grid search. 

We'll need to tune the model over cost as well as any kernel parameters. It is difficult to know _a priori_ ranges for many of the kernel parameters; they are very dataset-specific. Additionally, there is often an interaction effect between the tuning parameters. For example, @fig-svm-grid shows this. The polynomial kernel was fixed with $\theta_3 = 2$ (i.e., quadratic), and the optimization was over $\mathcal{C}$ and $\theta_2$. The figure shows the optimal parameter region, with a line representing a ridge of optimal parameters. The key takeaway is that, for a fixed cost parameter, the optimal scaling parameter changes (and vice versa). 

When tuning an SVM model, it can be effective to create a fairly large grid and use racing methods to evaluate candidates efficiently. As we saw in @fig-svm-grid, large regions of the parameter space perform poorly. Racing can quickly cull these regions. Alternatively, iterative search can also quickly learn the performance profile and adaptively find quality candidates. We'll take that approach for this section. 
...
extract_num_sv <- function(x) {
  require(tidymodels)
  require(kernlab)
  fit <- extract_fit_engine(x)
  tibble::tibble(num_sv = length(fit@SVindex))
}
...
svm_rbf_spec <-
  svm_rbf(cost = tune(), rbf_sigma = tune(), mode = "classification")
...
set.seed(872)
forest_svm_rbf_bo_res <-
  svm_rbf_spec |>
  tune_bayes(
    encode_rec,
    resamples = forested_rs,
    iter = 25,
    initial = forest_svm_rbf_res,
    metrics = cls_mtr,
    control = ctrl_bo
  )
...
set.seed(872)
forest_svm_poly_bo_res <-
  svm_poly_spec |>
  tune_bayes(
    encode_rec,
    resamples = forested_rs,
    iter = 25,
    initial = forest_svm_poly_res,
    metrics = cls_mtr,
    control = ctrl_bo
  )
...
best_svm <- 
  svm_path |> 
  slice_min(mean, n = 1, by = Kernel) |> 
  mutate(
    mean = round(mean, 3),
    cost = signif(log2(cost), 3),
    scale_factor = signif(log10(scale_factor), 3),
    rbf_sigma = signif(log10(rbf_sigma), 3)
  )

opt_deg <- 
  case_when(
    best_svm$degree[best_svm$Kernel == "Polynomial"] == 3 ~ "cubic",
    best_svm$degree[best_svm$Kernel == "Polynomial"] == 2 ~ "quadractic",
    TRUE ~ "linear"
  )

forest_svm_rbf_bo_res <- butcher::butcher(forest_svm_rbf_bo_res)
forest_svm_poly_bo_res <- butcher::butcher(forest_svm_poly_bo_res)
```

For preprocessing, the main operations are converting categorical predictors to a numeric format (e.g., dummy indicators, effect encoding, etc.) and normalizing predictors. For the forestation data, I used effect encoding for the location's county and ORD normalization to place the predictors on identical scales. I used an initial space-filling grid of 10 candidates, followed by 10 to 25 iterations of Bayesian optimization. The process stopped early if the Brier score did not improve for 10 consecutive iterations. 

The polynomial kernel was tuned over $\theta_2$ and $\theta_3$ (fixing $\theta_1 = 1$). We tuned the RBF kernel over $\sigma$.  The search spaces were:

- Cost: 2<sup>-10</sup> to 2<sup>5</sup>.
- Polynomial degree ($\theta_3$): 1, 2, or 3.
- Scale factor ($\theta_2$): 10<sup>-10</sup> to 10<sup>-1</sup>.
- RBF scale ($\sigma$): 10<sup>-10</sup> to 10<sup>0</sup>.

@fig-forest-svm-kernels shows the search results over time. The initial grid points had varying results, with some candidates achieving competitive performance while others had the worst possible Brier scores. The optimizer was able to find candidates that were slightly better than the initial set with the best result occurring at iteration `r best_svm$.iter[best_svm$Kernel == "Polynomial"]` for the polynomial kernel and `r best_svm$.iter[best_svm$Kernel == "RBF"]` for the radial basis function. 

```{r}
#| label: fig-forest-svm-kernels
#| echo: false
#| out-width: 70%
#| fig-width: 6
#| fig-height: 4.25
#| fig-cap: Bayesian optimization tuning results for SVM models with two different kernel functions. 
svm_path |> 
  ggplot(aes(.iter, mean, col = Kernel)) + 
  geom_point(position = position_dodge(width = 0.6), cex = 1 / 2) +
  geom_errorbar(
    data = svm_path |> filter(.iter > 0), 
    aes(ymin = lower, ymax = upper),
    position = position_dodge(width = 0.6),
    width = 1 / 3
  ) +
  scale_color_manual(values = c("#2CB11BFF", "#0076C0FF")) + 
  theme(legend.position = "top") +
  labs(x = "Iteration", y = "Brier Score")
```

The polynomial kernel's best candidate used a `r opt_deg` polynomial, a scale value of 10<sup>`r best_svm$scale_factor[best_svm$Kernel == "Polynomial"]`</sup>, and a cost value of 2<sup>`r best_svm$cost[best_svm$Kernel == "Polynomial"]`</sup>. The resulting Brier score estimate was `r best_svm$mean[best_svm$Kernel == "Polynomial"]`. The RBF kernel has similar performance, with a Brier score of `r best_svm$mean[best_svm$Kernel == "RBF"]`. The corresponding tuning values were a $\sigma$ value of of 10<sup>`r best_svm$rbf_sigma[best_svm$Kernel == "RBF"]`</sup>, and a cost value of 2<sup>`r best_svm$cost[best_svm$Kernel == "RBF"]`</sup>. These results are not too far off from the linear SVM that used feature engineering. The main difference between the two approaches is that the nonlinear kernels affect _all_ predictors, including the effect-encoded county variable. 

`r r_comp("cls-nonlinear.html#sec-cls-svm")`

## Comparisons to Previous Models {#sec-cls-nonlinear-compare}
...
forest_knn_set_res <- as_workflow_set(
  KNN = forest_knn_res |> butcher::butcher()
)
forest_nb_set_res <- as_workflow_set(
  "naive Bayes" = forest_nb_res |> butcher::butcher()
)
forest_fda_set_res <- as_workflow_set(
  "FDA" = forest_fda_res |> butcher::butcher()
)

forest_svm_set_res <- as_workflow_set(
  `SVM (poly)` = forest_svm_poly_bo_res,
  `SVM (RBF)` = forest_svm_rbf_bo_res
)
...
save_if_missing(forest_knn_set_res)
save_if_missing(forest_nb_set_res)
save_if_missing(forest_fda_set_res)
save_if_missing(forest_svm_poly_bo_res)
save_if_missing(forest_svm_rbf_bo_res)
```