...
$\theta_3$ is an integer for the polynomial degree, $\theta_2$ is an offset that can be used to trade off between high- and lower-order polynomial effects, and $\theta_1$  is a scale parameter. There are also spline-based kernels [Gunn, Yuan]. 
...
See REFERENCES. There are several variants of the kernel such as the Laplacian kernel, that us an L<sub>1</sub> norm in the exponent, as well as radial basis kernel create using an ANOVA decomposition [stinson]. 

There are also sigmoidal kernels [lin] that use the tanh activation function that will be seen in @sec-nnet-activation: 
...
The polynomial kernel was tuned over $\theta_2$ and $\theta_3$ (fixing $\theta_1 = 1$). We tuned the RBF kernel over $\sigma$.  Cost values from XX and XX were possible. 
