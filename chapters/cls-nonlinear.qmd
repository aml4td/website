...
As previously stated, ReLU is not a smooth function, so this demonstration will somewhat differ from the other activation functions; they generally don't completely isolate regions of the predictor space. However, the overall flow of the network remains the same. 
...
batch normalization and other encoded steps, like feature engineering

non-MLP

Attention in neural networks, as introduced by @bahdanau2016, is a mechanism that enables the model to selectively focus on the most relevant parts of the input space, assigning greater weight to important information during training. Originally motivated by machine translation, it can vastly improve performance by overcoming the limitations of older models, such as recurrent neural networks, which struggled with long-range dependencies in sequence data. The key mechanism, often referred to as self-attention, calculates the relationship between each element in a sequence and all other elements, enabling the model to determine the importance of each element within its current context.
