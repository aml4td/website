...
@fig-global-polynomial shows data from @bralower1997mid collected to understand the relationship between the age of a fossil and the ratio of two radioactive isotopes. The data set contains `r nrow(fossil)` data points for these two variables. The relationship is highly nonlinear, with several regions where the data have significant convex or concave patterns. 
...
A different method for representing spline basis expansions is the *B-spline*. The terms in the model are parameterized in a completely different manner. Instead of the regions using increasingly more model parameters to define the regional fit, B-spline terms have more localized features such that only a few are “active” (i.e., noon-zero) at a particular point $x_0$. This can be seen in @fig-bases where the basis function values for the truncated power series and B-spines
