...
Previously, we described **feature engineering** as techniques that make it easier for the model to understand and predict the outcome for a given data set. Unlike preprocessing methods (e.g., standardization via centering and scaling) that are _required by the model_, feature engineering is focused on the needs of the data set. 

This chapter will discuss feature engineering techniques to help with nonlinear relationships between the predictors and the outcome. Nonlinearity can be encoded into the features for a single predictor or as a combination of predictors. For single predictors, we will describe ways to enable nonlinearity smoothly (via splines) and via step functions (via discretization). 

The next section will focus on finding and encoding interaction terms for combinations of predictors. These combine two or more features. Before looking into interactions, let's take a look at a simple example that uses two predictors. 
...
In the predictors' original form (panels (b) and (c)), there is a significant overlap between the two classes of samples; neither is predictive on their own. 

However, when the ratio of the predictors is used, the newly derived predictor better discriminates between classes (shown in @fig-two-class-corr(d)). While not a general rule, the three data characteristics above suggest that the modeler might attempts to form ratios from such predictors ^[Alternatively, PCA after skewness-correcting transformations may be another good option.].
...
#| fig-width: 7
#| fig-height: 6
#| fig-cap: "Panel (a): two highly correlated, right-skewed predictors with two classes. Panel (b) and (c) show boxplots for each predictors, Panel (d): sepration of classes using $log(A/B)$."
...
biv_scat <- 
  ...
  labs(title = "(a)", x = "Predictor A", y = "Predictor B") +
  theme(legend.position = "top")

biv_a <-
  bivariate_train %>% 
  ggplot(aes(Class, A, fill = Class)) + 
  geom_boxplot(alpha = 1 / 2, show.legend = FALSE)+ 
  scale_fill_manual(values = biv_cols) +
  labs(title = "(b)", y = "Predictor A")

biv_b <-
  bivariate_train %>% 
  ggplot(aes(Class, B, fill = Class)) + 
  geom_boxplot(alpha = 1 / 2, show.legend = FALSE)+ 
  scale_fill_manual(values = biv_cols) +
  labs(title = "(c)", y = "Predictor B")
...
(plot_spacer() + biv_scat +  plot_spacer()) / (biv_a +  biv_b + biv_ratio) 
```

It is essential to understand that these _data_ require transformation. We can put the original predictors into a model as-is but we won’t have good predictive performance. We can induce a separation of the classes when a joint feature is used. In this case, the ratio is a nonlinear function of predictor B.

Let's focus on interaction terms first. 
...
The $\beta$ coefficients represent the overall average response ($\beta_0$), the average rate of change for each individual predictor ($\beta_1$ and $\beta_2$), and the incremental rate of change due to the combined effect of $x_1$ and $x_2$ ($\beta_3$) that goes beyond what $x_1$ and $x_2$ can explain alone.  The parameters for this equation can be estimated using a technique such as ordinary least squares (REF).  The sign and magnitude of $\beta_3$ indicate how and the extent to which the two predictors interact:   
 ...
 - Alternatively, when $\beta_3$ is negative, the interaction is _antagonistic_ since the response decreases as each increases.  
