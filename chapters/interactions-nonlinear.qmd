...
It turns out that we can make sure that the different fitted curves can connect at the ends and that they do that smoothly. This is done via a technique called splines which yeild better results and are more mathematically elegant.

### Spline Functions

Let’s assume we use piecewise cubic polynomials in the regional curve fits. To make the curves connect, we can add additional features that isolate local areas of $x$ via a new feature column such as $h((x - \eta))^3$ where
...
This zeros out parts of the predictor’s range that are greater than $\eta$. A smooth, continuous model would include a global polynomial expansion (i.e., $x$, $x^2$, and $x^3$) and these partial features for each value that defines the regions (these breakpoints are called _knots_ ^[Actually, they are the _interior knots_. The full set of knots includes the minimum and maximum values of $x$ in the training set.]). For example, if a model has three regions,  the basis expansion has 3 + 2 = 5 feature columns. In doing so, the first region (on the left-hand side) is only affected by three global polynomials. The middle region (second from the left) is has a model fit using three glbal and one localized model term. The region on the right uses all the slope parameters. For the fossil data, a three region model with knots at 107 and 114 would have model terms

$$
y_i = \beta_0 + \beta_1x_i + \beta_2x^2 + \beta_3x^3 + \beta_4h(x_i - 107)^3 + \beta_5h(x_i - 114)^3 + \epsilon_i
$$ {#eq-simple-spline}

The model fit, with the default interior knots, looks acceptable. As with global polynomials, there is a significant increase in the confidence interval width as the model begins to extrapolate slightly beyond the data used to fit the model.  
...
This is a simple example of a spline function (or just "a spline") that use polynomial trends in specific regions defined by a set of **knots**. They are designed so that the transitions between regions are continuous and smooth^[This is accomplished by constrains on the basis functions that allow the functions, and some of their derivatives, to be continuous at the knots. See @ruppert2003semiparametric and @arnold2019computational.].  
...
Splines basis functions can be generally written as a function of the knots and predictor values, with basis 

$$B_j^d(x) = \frac{h(x - \zeta_j)^d}{c_j}$$
where the function $h(\cdot)$ returns the positive part of the argument: 



and $c_j$ is a function of the knots. Spline terms restrict themseleves to "local" areas since $h(x - \zeta)$ will be zero for areas below some kn ot $\zeta$. For most splines $d \le 3$ since added local polynomials of higher order don't appear to have much benefit. 
...
To create a simulated data set, `r nrow(make_data())` samples were simulated. Of these, `r nrow(make_data()) - 1` were used for the training set and one were reserved for the demonstrating the variance and bias. The test set values were roughly equally spaced across the range of the predictor. One simulated data set is shown in @fig-nonlinear-sim, along with the true underlying pattern.  
...
#| fig-height: 4.25
#| warning: false
#| fig-cap: "Simulation results showing variance, squared bias, and mean squared error. The line is a spline-based smoother."
...
sim_res %>%
  ...
  labs(x = "Polynomial Degree", y = NULL)
```

The mean squared error shows the combined effect of variance and bias. It shows a steep decline as we add more terms, then plateaus between seven and fifteenth-degree models. After that, the rapidly escalating variance dominates the MSE as it rapidly increases.  
...
fossil_df %>% 
  ...
  geom_rug(data = super_breaks, aes(x = value, col = method), 
           linewidth = 3, alpha = 1 / 2) +
  geom_rug(data = unsuper_breaks, aes(x = value, col = method), 
           sides = "t", linewidth = 3, alpha = 1 / 2) +
...
Again, we want to emphasize that arbitrary or subjective discretization is almost always suboptimal. In the immortal words^[[`https://livefreeordichotomize.com/`](https://livefreeordichotomize.com/)] of Lucy D’Agostino McGowan and Nick Strayer: "Live free or dichotomize."
