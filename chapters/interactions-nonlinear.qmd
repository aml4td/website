...
the expansion would add additional terms with values of predictor expoentiated to the $p^{th}$ power^[In actuality, a more computationally preferred method is to use _orthogonal polynomials_ which rescale the variables before we exponentiate them. We’ll denote these modified versions of the predictor in equation @eq-poly-linear-reg as just $x$ to reduce mathematical clutter.]. TODO recall the ordered factor discussion. For a cubic polynomial model:
...
A global polynomial approach to modeling data from @bralower1997mid along with a representation of the features (above the data plot). The black dashed line is the true function and the shaded region is the 95% confidence intervals around the mean. 

:::

The issue here is that a global polynomial pattern is often insufficient because of the nonlinear relationships is different in different data sections. There may be steep increases in one area and gradual increases in others. Rather than using a global polynomial, what if we used different basis expansions in regions with more manageable or simplistic trends rather than a global polynomial? 
...
ui <- page_fillable(
  ...
  layout_columns(
    ...
    sliderInput(
      "piecewise_deg",
      label = "Polynomial Degree",
      min = 0L, max = 6L, step = 1L, value = 2L
    ), # sliderInput
    sliderInput(
      "cuts",
      label = "Cutpoints",
      min = 93L, max = 122L, step = 1, value = c( 99, 114)
    ) # sliderInput
  ), # layout_columns
  layout_columns(
    fill = FALSE,
...
A piecewise polynomial approach to the data from @fig-global-polynomial.
...
Specific types of spline functions; we’ll focus on one but describe a few others at the end of this section.
:::

There are many types of splines but one, the "natural cubic spline," is shown in Panel (c) of @fig-basis-expansions. The function demonstrates an excellent fit to the data when at least six terms are used and there are smooth transitions between regions of the data. 
...
A (natural) spline fit to the data previously shown in @fig-piecewise-polynomials and @fig-global-polynomial. Good fits occur when TODO degrees of freedom are used. 
