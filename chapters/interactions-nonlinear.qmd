...
Using these partial features, the first region (on the left-hand side) is only affected by three global polynomials. The middle region (second from the left) is has a model fit using three global and one localized model term. The region on the right uses all the slope parameters. This type of spline is called a _truncated power series_ and is mostly used to demonstrate how spline functions can fleixably model local trends.

This approach a simple example of a spline function (or just "a spline") that use polynomial trends in specific regions defined by a set of **knots**. They are designed so that the transitions between regions are continuous and smooth^[This is accomplished by constrains on the basis functions that allow the functions, and some of their derivatives, to be continuous at the knots. See @ruppert2003semiparametric and @arnold2019computational.].  
...
We'll consider a few types of spline functions but will favor one in particular. Before this, let's look at one practical aspect of splines: choosing the knots. 
...
$$B_j^d(x) = \frac{h(x - \_j)^d}{c_j}$$
where the function $h(\cdot)$ returns the positive part of the argument: 



and $c_j$ is a function of the knots. Spline terms restrict themseleves to "local" areas since $h(x - \xi)$ will be zero for areas below some kn ot $\xi$. For most splines $d \le 3$ since added local polynomials of higher order don't appear to have much benefit. 
