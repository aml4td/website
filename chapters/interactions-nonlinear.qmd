...
biv_cols_lt <- c(One = "#4DAF4A", Two = "#984EA3")
biv_cols_dk <- c(One = "#4DAF4A", Two = "#984EA3")
...
biv_scat_lt <- 
  biv_scat + 
  scale_color_manual(values = biv_cols_lt) +
  thm_lt

biv_scat_dk <- 
  biv_scat + 
  scale_color_manual(values = biv_cols_dk) +
  thm_dk

ratio_features <-
  bivariate_train %>% 
  mutate(`Log-Ratio` = log(A / B)) 

biv_ratio <-
  ratio_features %>% 
  ggplot(aes(Class, `Log-Ratio`, fill = Class)) + 
  geom_boxplot(alpha = 1 / 2, show.legend = FALSE)+ 
  # scale_fill_manual(values = biv_cols_lt) +
  ggtitle("(b)")
...
biv_ratio_dk <-
  biv_ratio + 
  scale_fill_manual(values = biv_cols_dk) +
  thm_dk

biv_scat_lt + biv_ratio_lt

biv_scat_dk + biv_ratio_dk
```

:::

Panel (a): two highly correlated, right-skewed predictors with two classes. Panel (b): separation of classes using $log(A/B)$.
...
::: {#fig-delivery-no-interact}

::: {.figure-content}

```{r}
#| label: delivery-no-interact
#| renderings: [light, dark]
#| fig-width: 8
#| fig-height: 4
#| out-width: "85%"
...
day_cols_lt <-  c("#000000FF", "#24FF24FF", "#009292FF",  "#B66DFFFF", 
                  "#6DB6FFFF",  "#920000FF",  "#FFB6DBFF")
day_cols_dk <-  c("white", "#24FF24FF", "#009292FF",  "#B66DFFFF", 
                  "#6DB6FFFF",  "#920000FF",  "#FFB6DBFF")
...
with_interactions <- 
  ...
  labs(
    ...
    x = "Order Time (decimal hours)", title = "(a)"
  ) +
  geom_smooth(se = FALSE) +
  ylim(time_lims) +
  guides(color = guide_legend(nrow = 1, title = NULL))

with_interactions_lt <- 
  with_interactions +
  scale_color_manual(values = day_cols_lt) +
  thm_lt

with_interactions_dk <- 
  with_interactions +
  scale_color_manual(values = day_cols_dk) +
  thm_dk
...
((with_interactions_lt + no_interactions_lt) +
    plot_layout(guides = 'collect')  ) &
  theme(legend.position = "bottom")

((with_interactions_dk + no_interactions_dk) +
    plot_layout(guides = 'collect')  ) &
  theme(legend.position = "bottom")
```
...
Delivery times versus the time of the order, colored by the day of the week. (a) A duplicate of a panel from @fig-delivery-predictors. (b) The same data with trendlines where the underlying model did _not_ allow an interaction between the delivery day and hour.
...
Interaction examples with two categorical predictors. Items 1 and 9 (left panel) do not interaction, while items 1 and 10 appear to have a strong  interaction (right panel).
...
anova_tbl|> thm_tbl_lt()

anova_tbl|> thm_tbl_dk()
```
...
lin_reg_cal_dk <- 
  lin_reg_cal +
  geom_point(alpha = 0.4, cex = 1, col = dark_data) +
  geom_smooth(se = FALSE, col = "red") +
  thm_dk
...
Validation set predicted vs. observed delivery times for linear regression models with and without additional interactions.
...
p_hour_ind_dk <-
  p_hour_ind + 
  geom_line(alpha = 1 / 50, col = dark_data) +
  thm_dk
...
p_hour_dk <-
  p_hour_p +
  geom_line(linewidth = 1, col = dark_data) +
  thm_dk
...
(((p_hour_ind_lt + p_hour_lt) / (p_day_lt + p_hour_day_lt)) +
    plot_layout(guides = "collect")) &
  theme(legend.position = "bottom")

(((p_hour_ind_dk + p_hour_dk) / (p_day_dk + p_hour_day_dk)) +
    plot_layout(guides = "collect")) &
  theme(legend.position = "bottom")
```
...
Partial dependence profiles for the food delivery hour and day, derived for the tree-based ensemble. Panel (a) shows the individual grid predictions from 1,000 randomly sampled points in the training set. The other panels show the average trends.
...
::: {#fig-hstats-interaction}

::: {.figure-content}

```{r}
#| label: hstats-interaction
#| renderings: [light, dark]
#| echo: false 
#| out-width: 60%
#| fig-width: 6
...
orf_two_way_int_p +
  geom_bar(stat = "identity", fill = dark_data) +
  thm_dk
```

:::

$H^2$ statistics for the top ten pairwise interaction terms.
...
::: {#fig-flat-interaction}

::: {.figure-content}

```{r}
#| label: flat-interaction
#| renderings: [light, dark]
#| echo: false
#| out-width: 60%
#| fig-width: 6
...
Importance statistics for the top ten pairwise interaction terms.
...
::: {#fig-bart-interaction}

::: {.figure-content}

```{r}
#| label: bart-interaction
#| renderings: [light, dark]
#| echo: false
#| out-width: 60%
#| fig-width: 6
...
BART model statistics identifying interactions by how often they occur in the tree rules.
...
Two representations of a cubic spline for the fossil data with knots at 107 and 114. The top panel illustrates the model terms defined by @eq-simple-spline. The bottom panel shows the B-spline representation. All lines have been scaled to the same y-axis range for easy visualization.
...
sim_p + 
  geom_line(data = sim_grid, lty = 3, col = dark_data) +
  thm_dk
```

:::

A simulated data set with a nonlinear function. The test set point is the location where our simulation estimates the bias and variance.
...
p_mse <- 
  ...
  scale_shape_manual(values = c(16, 4, 16), drop = FALSE) +
...
Simulation results showing variances, squared biases, and mean squared errors.
...
bias_p + 
  geom_vline(xintercept = 0, lty = 3, col = light_bg) +
  thm_dk
```

:::

A simple example of the variance-bias tradeoff.
...
pie_p + 
  theme(
    ...
    axis.title.x = ggplot2::element_text(colour = dark_line),
    axis.title.y = ggplot2::element_text(colour = dark_line),
...
An unfortunate visualization of two predictors and the outcome in the food delivery data. The colors of the pie chart reflect the binned delivery times at cut points of 20 and 30 minutes with lighter blue indicating earlier times.
...
bin_p + 
  geom_point(aes(age, ratio), alpha = 3 / 4, pch = 1, cex = 3, col = dark_data) +
  thm_dk
```

:::

The estimated relationship between fossil age and the isotope ratio previously shown in @fig-piecewise-polynomials, now using discretization methods.
