...
#| fig-alt: "Two side-by-side line plots of delivery time versus order hour, with separate trend lines for each day of the week. In panel (a), the trend lines cross and diverge, indicating an interaction between hour and day. In panel (b), the trend lines are forced to be parallel, showing a main-effects-only model that does not capture the interaction."
main_effects_mod <- lm(
  time_to_delivery ~ day + splines2::naturalSpline(hour, df = 10),
  delivery_train
)
...
#| fig-height: 4
#| out-width: "70%"
#| fig-cap: "Interaction examples with two categorical predictors. Items 1 and 9 (left panel) do not interact, while items 1 and 10 appear to have a strong  interaction (right panel)."
#| fig-alt: "Two interaction plots showing mean delivery time on the y-axis versus whether item 1 was ordered on the x-axis. In the left panel for item 9, the connecting lines are roughly parallel, indicating no interaction. In the right panel for item 10, the lines diverge sharply when both items are present, indicating a strong interaction. Error bars show 90 percent confidence intervals."
resampled_ratios |>
...
Within the context of statistical experimental design, @hamada1992analysis discuss some probabilistic aspects of predictor importance. The effect sparsity principle is that there are often few predictors that are relevant for predicting the outcome. Similarly, the effect hierarchy principle states that "main effects" (i.e. a feature involving only one predictor) are more likely to occur than interactions. Also, as more predictors are involved in the interaction, the less likely they become. Finally, the heredity principle conjectures that if an interaction is important, it is very likely that the corresponding main effects are important too. @chipman1996bayesian further expands this principle. 
...
#| fig-alt: "Three scatter plots of predicted versus observed delivery times on the validation set, one per linear regression model. The left panel shows the baseline model, the middle panel adds one interaction term with slightly fewer large residuals, and the right panel includes all item interactions with no clear improvement. Each panel includes a diagonal reference line and a smoothed trend line."
lvls <- c(
  "Previous Results",
...
So far, we have used visualizations to unearth potential interactions. This can be an effective strategy when the number of potential interactions is small, but the visual nature of this process is subjective. There are some specialized quantitative tools for identifying interactions. For example, @lim2015learning used regularized generalized linear models [@sec-logistic-penalized] to estimate all possible two-way interactions and use a penalization method to determine which should be retained. @miller1984selection and @fes ([Section 7.4.3](https://bookdown.org/max/FES/approaches-when-complete-enumeration-is-practically-impossible.html#the-feasible-solution-algorithm)) describe the feasible solution algorithm, an iterative search method for interaction discovery. Another, which we will now describe in more detail, is Friedman's H-statistic [@friedman2008predictive]. 
...
set.seed(1)
orf_fit <-
  orsf(time_to_delivery ~ ., data = delivery_train, mtry = p, n_tree = 50)
...
set.seed(1805)
pdp_int <- model_profile(orf_expl, N = 1000, variables = "hour", groups = "day")
...
#| fig-alt: "Four-panel partial dependence plot. Panel (a) shows a bundle of individual prediction profiles for delivery time across order hour, revealing a nonlinear trend. Panel (b) shows the averaged marginal effect of order hour as a single curve increasing then leveling off. Panel (c) shows marginal effects for each day of the week as individual points. Panel (d) shows joint profiles of hour grouped by day, with distinct non-parallel curves indicating an interaction."
#| eval: true
day_cols <- c(
  "#000000FF",
  "#24FF24FF",
...
$$
H^2_{jj'}=\frac{\sum\limits_{i=1}^B\left[\hat{f}(x_{ij},x_{ij'})-\hat{f}(x_{ij})-\hat{f}(x_{ij'})\right]}{\sum\limits_{i=1}^B\hat{f}^2(x_{ij}, x_{ij'})}
$$
...
#| fig-alt: "Horizontal bar chart of the top ten pairwise interaction terms ranked by their H-squared statistic, which measures the proportion of joint effect variability. The hour-by-day interaction has the largest value, followed by item 1-by-item 10, with the remaining interactions having progressively smaller values."
#| eval: true
orf_two_way_int |>
...
Alternatively, @greenwell2018simple measures the potential for variables to interact by assessing the "flatness" over regions of the partial dependence profile. The importance of a predictor, or a pair of predictors, is determined by computing the standard deviation of the flatness scores over regions. For example, @fig-delivery-items shows a flat profile for the item 1 $\times$ item 9 interaction while the item 1 $\times$ item 10 interaction has different ranges of means across values of item 1. @fig-flat-interaction shows ten interactions with the largest flatness importance statistics. As with the $H^2$ results, the hour-by-day interaction has the largest value. The two top ten lists have `r length(hstat_flat_common) - 1`  other interactions in common. 
...
#| fig-alt: "Horizontal bar chart of the top ten pairwise interaction terms ranked by the flatness importance statistic. The hour-by-day interaction again has the largest value, consistent with the H-squared results, with several item-by-item and item-by-hour interactions following."
#| eval: true
orsf_int_imp |>
  slice_max(score, n = 10) |>
...
#| fig-alt: "Horizontal bar chart of the top ten pairwise interactions from a BART model, ranked by the average count of consecutive splits involving both variables. The hour-by-day interaction dominates, confirming findings from the H-squared and flatness methods. Other interactions involve combinations of hour, distance, day, and item predictors."
#| eval: true
...
We might improve the fit by increasing the model complexity, i.e., adding additional polynomial terms. An eighth-degree polynomial seems to fit better, especially in the middle of the data where the pattern is most dynamic. However, there are two issues. 
...
A different method for representing spline basis expansions is the *B-spline* [@deboor2003practical]. The terms in the model are parameterized in a completely different manner. Instead of the regions using increasingly more model parameters to define the regional fit, B-spline terms have more localized features such that only a few are "active" (i.e., non-zero) at a particular point $x_0$. This can be seen in @fig-bases which contains the basis function values for the truncated power series and B-splines.
...
#| fig-alt: "Two-panel line plot comparing spline basis function representations for the fossil data. The top panel shows truncated power series terms where each basis function spans a wide range and the later terms activate only beyond their respective knots. The bottom panel shows B-spline terms that are localized, each active over a narrow region of the age axis. Both use five basis functions with knots at 107 and 114."
h <- function(x) {
  ifelse(x > 0, x, 0)
}
...
It turns out that the MSE is a combination of model variance and (squared) bias ($(\theta - \hat{\theta})^2$):
...
To create a simulated data set, `r nrow(make_data())` samples were generated. Of these, `r nrow(make_data()) - 1` were used for the training set and one was reserved for estimating the variance and bias. The training set values were roughly equally spaced across the range of the predictor. One simulated data set is shown in @fig-nonlinear-sim, along with the true underlying pattern.  
...
#| fig-alt: "Scatter plot of one simulated data set with a nonlinear true function shown as a dotted curve. Training points are scattered around the curve, and a single test set point is marked with a distinct shape, indicating the location where bias and variance will be estimated across simulation replicates."
sim_grid <-
...
#| fig-alt: "Two-panel scatter plot with smoothed trend lines. Panel (a) shows squared bias and variance versus polynomial degree. Squared bias drops steeply as degree increases and plateaus near zero, while variance starts low and grows sharply at high degrees. Panel (b) shows MSE versus polynomial degree, which decreases initially, plateaus around degrees seven through fifteen, then increases rapidly as variance dominates."
plot_data <-
  sim_res |>
  ...
  mutate(
    statistic = factor(statistic, levels = c("Squared Bias", "Variance", "MSE"))
  )
...
The mean squared error in panel (b) shows the combined effect of variance and bias. It shows a steep decline as we add more terms, then plateaus between seventh and fifteenth-degree models. After that, the rapidly escalating variance dominates the MSE as it rapidly increases.  
...
#| fig-alt: "Density plot comparing two estimators. The unbiased estimator is centered at the true value of zero but has a wide spread. The biased estimator is shifted slightly below zero but has a much narrower spread, illustrating that a small amount of bias can substantially reduce variance."
unbiased <- tibble(
  value = rnorm(50000, sd = 3),
  case = "unbiased, high variance"
)
biased <- tibble(
  value = rnorm(50000, mean = -1 / 2, sd = 1),
...
#| fig-alt: "A grid of small pie charts arranged by binned order hour along the columns and binned distance along the rows. Each pie shows the proportion of delivery times in three bins. Lighter slices represent shorter delivery times. As hour and distance increase, the pies shift toward darker slices, but the visualization is difficult to interpret due to the large number of small charts."
dist_pctl <- quantile(
  delivery_train$distance,
...
pie_by_group <-
  pie_n |>
  summarize(n_group = sum(n), .by = c(hour, distance))
...
- The number of features is increased, thus exacerbating the challenge of feature selection.
...
Let's again use the fossil data from @bralower1997mid  illustrated in the previous section on basis functions. We can fit a linear regression with qualitative terms for the age derived using: 
...
#| fig-alt: "Scatter plot of fossil age versus isotope ratio with two overlaid step functions representing unsupervised and supervised discretization methods. Both step functions approximate the nonlinear trend in the data with a staircase pattern. The two methods produce very similar fits, capturing the general shape but with blocky transitions between bins."
fossil_df |>
  ggplot() +
...
The results are remarkably similar to one another. The blocky nature of the fitted trend reflects that, within each bin, a simple mean is used to estimate the isotope ratio. 
