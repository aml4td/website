...
@fig-global-polynomial(a) shows data from @bralower1997mid collected to understand the relationship between the age of a fossil and the ratio of two radioactive isotopes. The data set contains `r nrow(fossil)` data points for these two variables. The relationship is highly nonlinear, with several regions where the data have significant convex or concave patterns. 
...
First, we can see from the 90% confidence bands around the line that the variance can be very large, especially at the ends. The fitted line and confidence bands go slightly beyond the range of the data (i.e., extrapolation). Polynomial functions become erratic when they are not close to observed data points. TODO MORE. This becomes increasingly pathological as the polynomial degree increases. For example, in @fig-global-polynomial, choosing degrees greater than 14 will show extreme uncertainty in the variance of the mean fit (the solid line). This is a good example of the variance-bias tradeoff discussed in Section TODO below.
...
A global polynomial is often insufficient because of the nonlinear relationships is different in different data sections. There may be steep increases in one area and gradual increases in others. Rather than using a global polynomial, what if we used different basis expansions in regions with more consistent trends rather than a global polynomial?
...
We can resolve these issues with a more mathematically elegant modeling approach that guarantees that the curves touch (and even touch smoothly).
...
We’ll define a “spline” or “spline function” as a collection of features that represent piece-wise polynomial features of a single-number predictor (with some specific properties).

“Regression splines” usually describe the use of splines of one or more predictors in a model fit by ordinary least squares. 

The term “smoothing splines” refers to a procedure that encapsulates both a spline function as well as an estimation method to fit it to the data (usually via regularization). There is typically a knot for each unique data point in $x$.

Also, there are many types of spline functions; we’ll focus on one but describe a few others at the end of this section.
:::

A model fit using a type of spline function called a _natural cubic spline_^[Also called a _restricted cubic spline_ in @harre1988regression.] is shown in @fig-natural-spline.  The default setting with three terms is fairly effective but values around six or so are probably better. Note that, with four or more terms, the basis functions shown above the plot do not affect the entire range of the data; “local” trends are only affected by relatively few model terms. Also, the natural spline basis functions do not “explode” at the extremes of the data distribution, even when many basis functions are used. The 90% confidence bands show an increase in uncertainty when extrapolating, but the increase is orders of magnitude smaller than a global polynomial with the same number of model terms. 
...
ui <- page_fillable(
  ...
  layout_columns(
    ...
    sliderInput(
      "spline_df",
      label = "# Spline Terms",
      min = 3L, max = 20L, step = 1L, value = 3L
    ), # sliderInput
  ), # layout_columns
  layout_columns(
    fill = FALSE,
...
A (natural) spline fit to the data previously shown in @fig-piecewise-polynomials and @fig-global-polynomial. Good fits occur when apprximately siz degrees of freedom are used. 

:::


more on the math here. describe regions. 
...
Many, but not all, splines specify how complex the fit should be in the area between the knots. However, cubic splines are a common choice because they allow for greater flexibility than linear or quadratic fits, but are not overly flexible, which could lead to over-fitting. Increasing the polynomial degree beyond three has more disadvantages than advantages. Recall that, in @fig-global-polynomial(a), there was an explosive variance at the ends of the predictor distribution when the polynomial degree is very large. 

### Natural Splines

One way to address the variability at the ends of the predictor space is to use a _natural cubic spline_. It employs a cubic polynomial but only uses a linear fit in the two most extreme regions of the predictor space. 
...
colnames(natural_6_res) <- names0(ncol(natural_6_res), "natural cubic spline ")
natural_6_res <- as_tibble(natural_6_res)
natural_6_res$Latitude <- ames_train$Latitude

feature_plot <- 
  natural_6_res %>%
  pivot_longer(starts_with("natural cubic spline"), names_to = "feature", values_to = "value") %>%
  filter(value > 0) %>%
...
colnames(natural_20_res) <- names0(ncol(natural_20_res), "natural cubic spline ")
...
#| fig-width: 7
#| fig-height: 4
#| fig-cap: Predictor coverage of six natural cubic spline terms based on latitude. 

feature_plot
```

Let’s use a set of 20 natural cubic spline features to model these data using ordinary linear regression. @fig-ames-latitude-fit shows the resulting fit on the training set. The red line shows the fitted curve resulting from the 21 parameter estimates. The different curves on the top of the plot area indicate how much each feature represents each portion of the predictor space.  This illustration demonstrates that as the number of features increases, the model fit becomes more flexible.
...
ui <- page_fillable(
  ...
  layout_columns(
        ...
        min = 2L, max = 50L, value = 20L, step = 1L
        )
  ),
  layout_columns(
    fill = FALSE,
    col_widths = breakpoints(sm = c(-1, 10, -1)),
    as_fill_carrier(plotOutput('latitude'))
  )
...
server <- function(input, output) {
  ...
  output$latitude <-
    renderPlot({
      ...
      colnames(natural_res) <- names0(ncol(natural_res), "natural cubic spline ")
...
The price of houses in Ames versus latitude with a 20 degree of freedom natural cubic spline. The curves above the plot illustrate how much each of the 20 features represents a different portion of the predictor space.
...
We can think of bias in terms of the question: “How close are we _aiming_ at a target?” and variance as: “How much do our results vary when shooting at a target?”

The concepts of variance and bias are paired because they are often at odds with one another. For models that predict, we’d like a _low variance_, _low bias_ model. In many cases, that can be exceedingly difficult. We often lower bias by adding model complexity. However, increased complexity usually comes at the expense of stability (i.e., high variance). We will often be in the position of trying to find an acceptable compromise between the two. This was discussed with global polynomials; linear models were bad for our data, but adding too many polynomial terms ends up adding unnecessary complexity and an explosion of variance (reflected in the confidence bands). 

This leads us to the mean squared error. We’ve seen the root mean squared error already where we used it as a measure of accuracy for regression models. More generally, we can write it in terms of some unknown parameter $\theta$ and some estimate $\hat{\theta}$ based on statistical estimation from data: 

$$MSE = E[(\theta - \hat{\theta})^2]$$
It turns out that the MSE is a combination of variance and (squared) bias: 

$$MSE = E[(\theta - \hat{\theta})^2] = Var[\hat{\theta}] + (\theta - \hat{\theta})^2 + \sigma^2$$
where $\sigma^2$ represents some unknown amount of "irreducible noise."

Because of this, MSE offers a statistic to minimize that accounts for both properties. In this, it can offer a compromise between the two.  


To illustrate the this, we simulated a simple nonlinear model: 

$$y_i = x_i^3 + 2\exp[-6(x_i - 0.3)^2] + \epsilon_i$$

where the error terms are $\epsilon_i \sim N(0, 0.1)$ and the predictor values were nearly uniform across [-1.2, 1.2]. Since we know the true values, we can compute the model bias. 

To create a simulated data set, 50 samples were simulated. Of these, 22 were used for the training set and eight were reserved for the test set. The test set values were roughly equally spaced across the range of the predictor. One simulated data set is shown in @fig-nonlinear-sim, along with the true underlying pattern. 

```{r}
#| label: variance-bias-functions
#| echo: false

make_data <- function(num_points = 30, error_sd =  0.1) {
  te_n <- ceiling(num_points * .25)
  holdout <- floor(seq(2, num_points - 1, length.out = te_n))
  tr_n <- num_points - te_n
  dat <- 
    tibble(x = seq(from = -1.2, to = 1.2, length.out = num_points)) %>% 
    mutate(
      x = x + rnorm(num_points, sd = 0.02),
      truth = (x^3) + 2 * exp(-6 * (x - 0.3)^2),
      y = truth + rnorm(num_points, sd = error_sd)
    )
  dat$data <- "train"
...
get_stats <- function(dat, deg) {
  tr <- dat[dat$data == "train", ]
  te <- dat[dat$data == "test", ]
  mod <- lm(y ~ poly(x, deg), data = tr)
  te$.pred <- predict(mod, newdata = te)
  tibble(
    rmse = rmse_vec(te$y, te$.pred),
    bias = mean(te$truth - te$.pred),
    var = var(te$.pred),
    degree = deg
  )
}

sim <- function(deg, n = 50) {
...
#| fig-width: 6
#| fig-height: 4.25
#| fig-cap: "A simulated data set with a nonlinear function."
...
set.seed(12)
sim_stats <- 
  map_dfr(rep(1:20, each = 50000), sim) %>% 
  summarize(
    rmse = mean(rmse),
    bias = mean(bias),
    var = mean(var),
...
A linear model was estimated using ordinary least squared and a polynomial expansion for each simulated data set. The test set values were predicted, and the bias, variance, and root mean squared error statistics were computed. This was repeated 50,000 times for polynomial degrees ranging from one to twenty.  

@fig-sim-results shows the average statistic values across model complexity (i.e., polynomial degree). A linear model does very poorly for the bias due to underfitting (as expected). Adding more nonlinearlity results in a substantial decrease in bias because the model fit is closer to the true equation. This improvement plateaus at a fourth-degree polynomial and stays low (nearly zero). The variance begins low; even though the linear model is ineffective, it is stable. Once additional terms are added, the variance of the model increases and explodes around an 18<sup>th</sup> degree polynomial^[In this particualr case, the variance becomes very large since the number of parameters is nearly the same as the number of training set points (22). This makes the underlying mathematical operation (matrix inversion) numerically unstable. Even so, it is the result of excessive model complexity.]. This shows the tradeoff; as the model becomes more complex, it fits the data better but eventually becomes unstable. 
...
#| fig-width: 9
#| fig-height: 3
#| fig-cap: "Simulation results showing variance, bias, and mean squared error."

p_bias <- 
  sim_stats %>% 
  ggplot(aes(degree, bias^2)) + 
  geom_point(cex = 1) +
  labs(x = "Polynomial Degree", title = "Squared Bias", y = NULL)

p_var <- 
  sim_stats %>% 
  ggplot(aes(degree, var)) + 
  geom_point(cex = 1) +
  labs(x = "Polynomial Degree", title = "Variance", y = NULL)

p_rmse <- 
  sim_stats %>% 
  ggplot(aes(degree, rmse^2)) + 
  geom_point(cex = 1) +
  labs(x = "Polynomial Degree", title = "MSE", y = NULL)

p_bias + p_var + p_rmse
```

The mean squared error shows the combined effect of variance and bias. It shows a steep decline as we add a few more terms, then plateaus between seven and thirteen-degree models. After that, the explosive variance dominates the MSE as it rapidly increases.  
...
@fig-tradeoff shows a simple example. Suppose we have some alternative method for estimatimg the mean of a group that adds some bias while reducing variance. That tradeoff might be worthwhile dpending on how much bias will greatly reduce the variance. In our figure, suppose the true population mean is zero. The green curve represents the sample mean for a fixed sample size. It is centered at zero but has signficant uncertainty. The other curve might be an alternative estimator that produces a slightly pessimistic estimate (its mean is slightly smaller than zero) but has 3-fold smaller variation. This may be a good idea depending on how the estimate will be used ^[This specific example will be referenced again when discussing resampling in @sec-resampling.]. 
...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: "A simnple example of the variance-bias tradeoff."
...
The categorical encoding approach shown in @eq-effect-posterior from @sec-effect-encodings is another example. Recall that $\bar{y}_j$ was the average daily rate for agent $j$. That is an unbiased estimator but has high variance, especially for agents with few bookings. The more complex estimator $\hat{j}_j$ in @eq-effect-posterior is better because it is more reliable. That reliability is bought by biasing the estimator towards the overall between-agent mean. 

As previously mentioned, we’ll come back to this topic several time in upcoming sections.
