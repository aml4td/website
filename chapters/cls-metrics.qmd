...
For this chapter, a training set of `r format(nrow(shroom_train), big.mark = ",")` data points, validation set of `r format(nrow(shroom_val), big.mark = ",")` data points, and a test set of `r format(nrow(shroom_val), big.mark = ",")` data points are used to illustrate various methods.
...
For the naive Bayes model, the accuracy computed on the validation set was `r round(nb_mtr$mean[nb_mtr$.metric == "accuracy"] * 100, 1)`{{< pct >}}.
