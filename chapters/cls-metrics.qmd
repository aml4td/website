...
original_int <-
  ...
  mutate(
    # arrange here for reproducibility below
    dat = map(splits, ~ analysis(.x) |> arrange(.row)),
...
#| fig-alt: "Faceted bar chart comparing two prior distributions for the number of Wordle guesses. The left panel shows the author's subjective belief, which peaks sharply at three guesses with about 40 percent probability. The right panel shows empirical data from the Bluesky social media platform, which has a more symmetric bell shape peaking at four guesses, and suspiciously high probabilities for one and two guesses."
tries <- c(paste(1:6), "X")

belief <- tibble(
  tries = tries,
  prob = c(0.1, 4, 40, 35, 10, 5.9, 5),
  source = "Author's Belief"
)
bluesky <- tibble(
  tries = tries,
  prob = prior_empirical * 100,
  source = "Bluesky Data"
)
...
::: {.important-box}
It is very important to carefully choose which metric (or metrics) to use to optimize a model. The metric defines what your model should consider important, and it will almost always believe that your chosen metric is the only important criterion. This can often lead to unintended consequences. 
:::

The next section discusses important characteristics of performance metrics. After this, two sections discuss specific metrics for both types of predictions. Another section discusses the special case where the class levels are ordered (such as with the Wordle scores). There is also a section specifically on model calibration and another describing the use of multiple metrics. 
...
non_cal_acc <- accuracy_vec(
  non_cal_example$class,
  non_cal_example$.pred_class
) *
  100
non_cal_brier <- brier_class_vec(
  non_cal_example$class,
  non_cal_example$.prob_event
)
```
...
#| fig-alt: "Density plot showing the distribution of predicted event probabilities for two classes. The two class distributions are well separated, indicating good discrimination, but both are compressed into a narrow range of probabilities rather than spanning the full zero-to-one range, indicating poor calibration."
p_ex_dens <-
  non_cal_example |>
...
#| fig-alt: "Faceted histogram of predicted probability of being poisonous, split by true class. The edible panel shows a distribution concentrated near zero with a long tail. The poisonous panel shows a distribution concentrated near one. A dotted vertical line at 0.5 marks the default classification threshold. The model rarely produces confidently incorrect predictions."
nb_val_pred |>
...
`r r_comp("cls-metrics.html#sec-mushrooms")`
...
acc_int <- binom.test(
  sum(diag(nb_conf_mat$table)),
  sum(nb_conf_mat$table),
  conf.level = 0.9
)
```
...
`r r_comp("cls-metrics.html#sec-cls-hard-metrics")`
...
There are particular metrics associated with the accuracy of each class. Sensitivity is the probability of correctly predicting the event of interest: 
...
Sensitivity and specificity are bedrock metrics for two classes since they represent the two possible ways models can go wrong. We'll examine their relationship below when receiver operating characteristic (ROC) curves are discussed.
...
The primary two metrics used in information retrieval are precision and recall. Precision is the rate that identified documents are actually relevant:

$$
Precision = \frac{\text{relevant and retrieved}}{\text{total retrieved}}=\frac{TP}{TP + FP}
$$ {#eq-precision}

For example, in the validation set, our naive Bayes model predicted/identified `r format(sum(nb_conf_mat$table["poisonous",]), big.mark = ",")` mushrooms to be poisonous using a 50{{< pct >}} cutoff. Of these, `r format( nb_conf_mat$table["poisonous","poisonous"], big.mark = ",")` were truly poisonous. The ratio of these values produced a precision value of `r round(nb_mtr$mean[nb_mtr$.metric == "precision"] * 100, 1)`{{< pct >}}.

It is sometimes said that precision is the same as the positive predictive value. However, this is only the case when the prevalence is 50{{< pct >}} (i.e., balanced classes). For information retrieval, it is doubtful that the event rate would be so large (otherwise, documents would be very easily found).  

The recall is the proportion of relevant documents in the data set that have been identified:
...
$$
F_{\beta}=(1+\beta^2)\left(\frac{precision \times recall}{(\beta^2 \times precision) + recall}\right)
$$ {#eq-f-score}
...
`r r_comp("cls-metrics.html#sec-cls-two-classes")`
...
A naive average of sensitivities is appropriate when the classes are balanced. When the classes are unbalanced, an unweighted average will bias this metric towards the sensitivity of the largest class. To account for class imbalance, *weighted macro averaging* uses the class proportions as weights

$$
S_{macro, wts} = \sum_{k=1}^C \frac{n_k}{n}S_k
$$ {#eq-sens-macro-wts}

Finally, *micro averaging* is more complex because it aggregates the ingredients that go into the specific calculations. Sensitivity, for example, requires the number of true positives and false negatives. These are summed and used in the regular computation of sensitivity:
...
`r r_comp("cls-metrics.html#sec-cls-metrics-wts")`
...
rand_log_loss <-
  purrr::map_dfr(
    1:25,
    ~ mutate(nb_val_pred, class = sample(class)),
    .id = "rep"
  ) |>
  group_by(rep) |>
...
Looking at our mushroom model, the Brier score was estimated as `r signif(original_int$.estimate, 4)` with 90{{< pct >}} bootstrap confidence limits of (`r signif(original_int$.lower, 4)`, `r signif(original_int$.upper, 4)`). The permutation estimate shows that a poor model would have an average score of `r signif(nb_mtr_perm$mean[nb_mtr$.metric == "brier_class"], 4)`. While not perfect, the model can achieve a fairly small average squared error. 
...
Let's demonstrate with a binary outcome ($C = 2$). Suppose out of $n$ data points there are only $n_p$ unique values of $\hat{p}_{i1}$. We'll denote these unique values as $\hat{p}_{j}$ ($j=1, \ldots, n_p$) each occurring with frequency $n_j$. 
...
Refinement is interesting because it indirectly uses the model probability estimates (by contributing the $j$ data groupings). The pattern $\bar{y}_j (1 - \bar{y}_j)$ also drives the binomial variance and the Gini Index^[There is also a direct connection between refinement and the area under the ROC curve. See @hand2001simple.]. This pattern measures _impurity_. The best-case scenario is that the true outcome values for every unique model probability estimate _are as similar as possible_. We want to minimize this term too. @murphy1973new shows how to further divide refinement, and @yates1982external compares the two decompositions.
...
#| fig-alt: "Two-panel scatter plot. Left panel shows predicted probability versus observed rate for the reliability component; points mostly follow the diagonal with a smoothed line showing slight deviation in the 0.1 to 0.5 range. Right panel shows the refinement component versus predicted probability; values are near zero at the extremes and spike around 0.2 and 0.85 where impurity is highest. Point size encodes the number of observations per unique probability."
p_cal <-
  brier_decomp_data |>
...
If we can construct them, these diagnostic plots can help us determine where the model may lack correctness. We'll see related plots in @sec-cls-calibration.
...
### Receiver Operating Characteristic Curves {#sec-roc}
...
If our model's application was more focused on specificity, we could raise the threshold above 50%. In this case, we increase specificity but do harm to sensitivity (by designating fewer samples to be events).
...
#| fig-alt: "ROC curve plotting sensitivity versus one minus specificity for the naive Bayes mushroom model. The curve rises steeply from the origin toward the upper-left corner, indicating strong discrimination. Three highlighted points mark the 10, 50, and 90 percent probability thresholds. A dashed diagonal line represents the no-information baseline."
autoplot(nb_val_roc) +
  geom_point(
    data = roc_points,
    aes(
...
#| fig-alt: "Precision-recall curve for the naive Bayes mushroom model. The curve starts at high recall and low precision on the right and rises to high precision at lower recall on the left. Three highlighted points mark the 10, 50, and 90 percent thresholds. A horizontal dotted line marks the prevalence baseline. The area under the curve is high, indicating good retrieval performance."
autoplot(nb_val_pr) +
  geom_point(
    data = pr_points,
    aes(
...
Note that neither the precision nor recall (Equations [-@eq-precision] and [-@eq-recall]) directly use the true negative component of the confusion matrix. This reflects the focus of these two statistics on finding positives. 

While the ROC curve has its optimal point in the top left, the optimal setting for PR curves is in the plot area's top right. We can also summarize the PR curve using the area under the curve, a value of one being the best. Based on @fig-pr-curve, the PR AUC was `r round(nb_mtr$mean[nb_mtr$.metric == "pr_auc"], 3)`.
...
In this scenario, the quality of the model should be assessed using multiple metrics: the ROC and PR curves, as well as the Brier score. We'll show a technique for blending metrics in @sec-cls-multi-objectives. 
...
When choosing between these two curves, we should consider how aligned our goals are with the goal of the modeling problem. This is discussed further in @sec-imbalance-goals.
...
#| fig-width: 5
#| fig-height: 5
#| fig-cap: "A gain curve based on the validation set. The dotted lines indicate the percentage of samples to query/acquire in order to capture 50{{< pct >}} of the relevant samples."
#| fig-alt: "Gain curve plotting the percentage of relevant samples found versus the percentage of the dataset queried. The curve rises steeply and stays close to the optimal boundary until about 25 percent of samples are retrieved. Dotted reference lines show that approximately 30 percent of the data must be queried to capture half of the truly relevant samples. A shaded region indicates the feasible area for possible curve values."
nb_val_gain |>
  autoplot() +
  geom_segment(
    data = gain_point,
...
`r r_comp("cls-metrics.html#sec-cls-metrics-soft")`
...
#| fig-alt: "Two-panel calibration plot using binned probabilities. Panel (a) for the naive Bayes model shows points falling close to the diagonal, indicating good calibration. Panel (b) for the sabotaged boosted tree shows points deviating substantially from the diagonal, with overestimated probabilities in the lower range and underestimated probabilities in the upper range. Rug marks along the top and bottom edges show the distribution of actual observations."
break_plot_1 <-
  nb_val_pred |>
  cal_plot_breaks(class, .pred_poisonous, include_ribbon = FALSE) +
  ggtitle("(a) Naive Bayes")
...
break_plot_2 <-
  ...
  ggtitle("(b) Impeded Boosted Tree")
...
#| fig-alt: "Two-panel calibration plot for the naive Bayes model. Panel (a) uses a moving window approach with overlapping 20 percent bins, producing a smooth curve that closely tracks the diagonal with minor deviations. Panel (b) uses a logistic regression fit with a smoothing spline, showing a similar pattern with slight miscalibration in the mid-range of predicted probabilities."
cal_plot_1 <-
  nb_val_pred |>
  cal_plot_windowed(
    class,
...
three_cal_val <-
  ...
  full_join(
    iso_boot_cal_val |> dplyr::select(.row, isoreg = .pred_poisonous),
    by = ".row"
  ) |>
  full_join(
    lr_cal_val |> dplyr::select(.row, logistic = .pred_poisonous),
    by = ".row"
  )

compute_brier_int <- function(split) {
  # arrange here for reproducibility
  predictions <- analysis(split) |> arrange(.row)
  brier_logistic <- predictions |>
    brier_class(class, logistic) |>
    dplyr::select(estimate = .estimate) |>
    mutate(term = "Logistic Calibration")
  brier_isoreg <- predictions |>
    brier_class(class, isoreg) |>
    dplyr::select(estimate = .estimate) |>
    mutate(term = "Isotonic Boot. Calibration")
  brier_raw <- predictions |>
    brier_class(class, original) |>
    dplyr::select(estimate = .estimate) |>
    mutate(term = "Uncalibrated")

  res <- bind_rows(brier_raw, brier_logistic, brier_isoreg)
  diffs <- tibble(
    estimate = c(
      brier_raw$estimate - brier_logistic$estimate,
      brier_raw$estimate - brier_isoreg$estimate
    ),
    term = c("Logistic Diff", "Isotonic Boot. Diff")
  )
  bind_rows(res, diffs)
}
...
diff_cal_int <-
  three_cal_val |>
  bootstraps(times = 5000) |>
  mutate(brier = map(splits, compute_brier_int)) |>
  int_pctl(brier, alpha = .10)

single_tbl <- diff_cal_int |>
  filter(!grepl("Diff", term)) |>
  dplyr::select(-.alpha, -.method) |>
  mutate(
    .lower = signif(.lower, digits = 4),
    .estimate = signif(.estimate, digits = 4),
    .upper = signif(.upper, digits = 4)
  )

diff_tbl <- diff_cal_int |>
  filter(grepl("Diff", term)) |>
  mutate(term = gsub("Diff", "Calibration", term)) |>
  mutate(
    .lower = signif(.lower, digits = 3),
    .estimate = signif(.lower, digits = 3),
...
int_f <- function(l, u) {
  ...
  cli::format_inline("({l}, {u})")
}

int_tbl <- full_join(single_tbl, diff_tbl, by = "term") |>
  arrange(desc(term)) |>
  mutate(
    .int = map2_chr(.lower, .upper, int_f),
    .diff_int = map2_chr(.lower_diff, .upper_diff, int_f)
  ) |>
  dplyr::select(term, .estimate, .diff, .diff_int)
```
...
#| fig-alt: "Two-panel calibration plot showing the naive Bayes model after logistic recalibration. Panel (a) uses moving 20 percent windows and shows the recalibrated curve tracking the diagonal more closely than the original model. Panel (b) uses a logistic regression diagnostic and also shows improved alignment with the diagonal, indicating that recalibration reduced the mid-range miscalibration."
cal_plot_3 <-
  lr_cal_val |>
  cal_plot_windowed(
    class,
...
Using this approach to calibration, the new probability estimates have a Brier score of `r diff_cal_int$.estimate[diff_cal_int$term == "Logistic Calibration"]`. As shown in @tbl-calibration-results, this is slightly less than the uncalibrated score. However, since the uncalibrated and calibrated predictions are so highly correlated, the 90{{< pct >}} intervals do not overlap with zero; the difference is real but not large enough to be seen as important. 
...
Validation set Brier scores for the original model predictions and calibrated versions. The differences are the uncalibrated values minus the calibrated values. 
...
We can improve the isotonic regression results using _bagging_. As discussed later in @sec-cls-bag, bagging creates an ensemble of models by taking $B$ bootstrap samples to build $B$ distinct models. The final model prediction is the average of the $B$ model predictions. For isotonic regression, this reduces the variability in the predicted (i.e., recalibrated) probabilities and also increases the number of unique probability values. Using 1,000 bootstrap samples, we have `r num_isotonic_boot_vals` unique probability estimates. 
...
#| fig-alt: "Two-panel calibration plot using moving windows after isotonic regression recalibration. Panel (a) shows a single isotonic regression result with a curve that tracks the diagonal but has some wiggle due to the step-function nature of the method. Panel (b) shows the bootstrapped ensemble result, which is slightly smoother and closer to the diagonal with reduced variability compared to the single model."
cal_plot_5 <-
  iso_cal_val |>
  cal_plot_windowed(
    class,
...
@fig-recal-iso-curves(b) shows the ensemble results and @tbl-calibration-results contains the Brier scores. The curve is slightly closer to the line, and there is far less "wiggle" around the values. The bootstrapping method reduces the Brier score from `r signif(nb_mtr$mean[nb_mtr$.metric == "brier_class"], 3)` to `r diff_cal_int$.estimate[diff_cal_int$term == "Isotonic Boot. Calibration"]`. Like logistic calibration, the difference is statistically significant but trivial. 
...
`r r_comp("cls-metrics.html#sec-cls-calibration")`
...
wordle_errors <-
  ...
  mutate(
    n2 = case_when(
      ...
      abs(as.integer(Prediction) - as.integer(Truth)) == 1 ~ cell_spec(
        n,
        format = "html",
        color = "indianred1",
        bold = TRUE
      ),
      abs(as.integer(Prediction) - as.integer(Truth)) == 2 ~ cell_spec(
        n,
        format = "html",
        color = "indianred2",
        bold = TRUE
      ),
      abs(as.integer(Prediction) - as.integer(Truth)) == 3 ~ cell_spec(
        n,
        format = "html",
        color = "indianred3",
        bold = TRUE
...
bind_cols(wordle_conf_mat, wordle_errors) |>
  ...
  add_header_above(c(
    ...
    "Truth" = length(lvls),
    " " = 1,
    "Linear Weight 'Errors'" = length(lvls)
  )) |>
  kable_styling(full_width = FALSE)
```
...
About `r round(mean(rule_te_pred$tries == rule_te_pred$.pred_class) * 100, 0)`{{< pct >}} of the class predictions fall along the diagonal line of the confusion matrix. The classes corresponding to three, four, and five tries had appreciable errors. 
...
The score is normalized by dividing by $C-1$. The minus one compensates for the fact that all predictions are perfect for the largest class level (since they are cumulative). 
...
`r r_comp("cls-metrics.html#sec-ordered-categories")`
...
#| fig-alt: "Faceted line plot showing four desirability function mappings. The Brier Score panel shows a linear decrease from desirability of 1 at zero to 0 at 0.1. The Sensitivity panel shows a linear increase from 0 at 0.5 to 1 at 1.0. The Specificity panel shows a box constraint where any value between 0.8 and 1.0 has full desirability and values outside have zero. The Active Features panel shows a linear decrease from 1 at one feature to 0 at 20 features."
len <- 500
...
where $A$ and $B$ are the low and high values, respectively. The $\gamma$ parameter is a scaling factor that modulates how easy (or difficult) it should be to satisfy the goal. The curves in @fig-desirability use the default value of $\gamma = 1$, producing straight lines between the limits. Values smaller than one make desirability more easily satisfied, and larger values will result in individual desirability having more impact on the overall optimization. 
...
`r r_comp("cls-metrics.html#sec-cls-multi-objectives")`
