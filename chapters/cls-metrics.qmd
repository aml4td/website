...
Much like correlation statistics, the range of Kappa is $\pm$ 1, with values of one corresponding to perfect agreement and zero meaning negative agreement. While negative values are possible, they are uncommon. Interpreting Kappa can be difficult since no well-defined values indicate that the results are good enough. This is compounded by the fact that the values can change with different prevalences of the classes [@thompson1988reappraisal;@guggenmoos1996meaning]. For this reason, it isn’t easy to compare values between data sets or to have a single standard. There are also differences in range when the outcome data has different numbers of classes; a Kappa value of 2.5 has a different meaning when $C = 2$ than with some other number of possible classes. 

The Kappa statistic can also incorporate different weights for different types of errors [@cohen1968weighted]. This is discussed in more detail in @sec-ordered-categories.
