...
#| fig-alt: "Precision-recall curve for the naive Bayes mushroom model. The curve starts at high recall and low precision on the left and rises to high precision at lower recall on the right. Three highlighted points mark the 10, 50, and 90 percent thresholds. A horizontal dotted line marks the prevalence baseline. The area under the curve is high, indicating good retrieval performance."
autoplot(nb_val_pr) +
  geom_point(
    data = pr_points,
    aes(
...
#| fig-alt: "Two-panel calibration plot using moving windows after isotonic regression recalibration. Panel (a) shows a single isotonic regression result with a curve that tracks the diagonal but has some wiggle due to the step-function nature of the method. Panel (b) shows the bootstrapped ensemble result, which is slightlu smoother and closer to the diagonal with reduced variability compared to the single model."
cal_plot_5 <-
  iso_cal_val |>
  cal_plot_windowed(
    class,
