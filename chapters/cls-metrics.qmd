...
This chapter discussed important characteristics of qualitative outcomes and appropriate metrics for quantifying how well the model functions. There are an abundance of metrics; we describe many of them here. Additionally, we’ll highlight different aspects of "performance" and when might be more important than others. 
...
First, accuracy is very sensitive to class imbalances (where one or more classes do not occur very often). Suppose that we have two classes, and one only occurs 1{{< pct >}} of the time (called the "minority class" in this context). When accuracy is used for these data, an inferior model can easily achieve 99{{< pct >}} accuracy simply by always predicting the majority class. If accuracy is all that we care about, then we can satisfy the literal goal, but our model will probably fail to fulfill the intention of the user^[Class imbalances can be difficult to overcome. @sec-imbalances will discuss different appraoches to alleviating their effects.]. 
...
- A classification model is well-calibrated when the estimated probability predictions are close to their real theoretical counterparts.  In other words, if we estimate a probability to be 55{{< pct >}}, the event should occur at this same rate if realized multiple times (on average).   
...
To demonstrate classification metrics, we'll primarily use a data set where mushroom characteristics are used to predict whether specific mushrooms are edible or poisonous [@wagner2021mushroom]. These data were obtained from the  the [UCI Machine Learning Archive](https://archive.ics.uci.edu/dataset/848/secondary+mushroom+dataset)) and we'll use these data in conjunction with a naive Bayes classification model^[This data set is very easy. Other models, even simple decision trees, can produce nearly perfect results. We’ll use it here with a less powerful model for illustration purposes. The gravity of incorrectly predicting a poisonous mushroom to be edible makes the discussion of certain errors more compelling.] (see @sec-naive-bayes). This model uses a simplified version of Bayes' Rule. 
...
These distributions demonstrate a few attractive characteristics. First, the probability estimates span the entire range of possible values and each distribution peaks at appropriately extreme values (i.e., zero or one). Conversely, the model rarely produces "confidently incorrect" predictions (i.e., erroneously predicting with a large probability estimate). We'll see the calibration and separation properties for this model in later sections.   
...
Like Kappa, the statistic mirrors the correlation coefficient for numeric data in that it ranges from -1 to +1 and a similar interpretation to ordinary correlation coefficients. For the mushroom data, the MCC estimate, `r signif(nb_mtr$mean[nb_mtr$.metric == "mcc"], 3)`, is very close to the Kappa estimate shown above. We find that this is often the case and the similarity was investigated more in @delgado2019cohen. 

Accuracy, Kappa, and MCC can be computed regardless of the number of classes. There are numerous specialized metrics for $C = 2$, i.e. "binary classification.". The next section reviews these and @sec-cls-metrics-wts describes a few methods for using these metrics when $C > 2$.
...
Specificity is focused on accuracy but we often refer to the false positive rate which is simply one minus the specificity. 

Sensitivity and specificity are bedrock metrics for two classes since they represent the two possible ways models can go wrong. In the statistics literature, they correspond to "Type I" and "Type II" errors, respectively. We’ll examine their relationship to one another below when receiver operating characteristic (ROC) curves are discussed.

With the mushroom data, the sensitivity  was `r round(nb_mtr$mean[nb_mtr$.metric == "sensitivity"] * 100, 1)`{{< pct >}} and the specificity was nearly the same: `r round(nb_mtr$mean[nb_mtr$.metric == "specificity"] * 100, 1)`{{< pct >}}. Since these are simple ratios, confidence intervals can be computed using the same technique mentioned for overall accuracy. 

These two metrics are commonly reported when a two-class model is evaluated. However, they is a subtle issue in their usage. If we want to know how well the model works, they don’t really answer a question that anyone would ask. For example, sensitivity is the answer to: 
...
To explain, we have to return to Bayes’ Rule (@eq-bayes-rule). In this context, the sensitivity is the likelihood term $Pr[x = poison | y = poison]$ (where $x$ represents the predicted class). To answer the question equation, what we need is: 

$$
Pr[y = poison| x = poison] = \frac{Pr[y = poison] Pr[x = poison | y = poison]}{Pr[x = poison]}
$$ {#eq-bayes-ppv}

This equation reflects the idea that we need to know how often the event really occurs (independent of our model). This is the Bayesian prior probability. If the event rarely happens anyway, that should affect the probability of us predicting it to happen. 
...
Without knowing anything about how often the average mushroom is poisonous, we could choose a prevalence of 50{{< pct >}} as an uninformative prior. Our estimate from the training set (`r obs_prev`{{< pct >}}) is close to uninformative, so we will use that in our computations. 
...
Using the prevalence from our traning set and the metrics computed on the validation set, the NPV estimate is `r round(nb_mtr$mean[nb_mtr$.metric == "npv"] * 100, 1)`{{< pct >}}. 

The PPV and NPV are functions of three quantities, and it might be challenging to understand how the prevalence can affect the statistics. @fig-prevalence can help us understand these relationships. For example, if the prevalence is near zero, it may be difficult to achieve an acceptable PPV value unless the specificity is perfect (in which case it is quite easy to attain a good NPV). 
...
The recall is the rate that identified documents are actually relavant: 
...
We've described that, by maximizing the likelihood, we find values(s) of the parameter(s) that explain the data best. To improve numerical stabiklity, we typically try to minimize the negative log-likelihood. Logging $\ell(y_{ik}; \pi)$ and subtituting our estimate $\hat{p}_{i1}$ for $\pi$, we also arrive at: 
...
The attractive aspect of this statistic is that can produce models that fit the data well. On the theoretical side, it also has the characteristic of being a _proper scoring rule_; it is optimized at the true probability distribution. We’ll see a little more about this below when Brier scores are discussed. 

One issue is that the scale of the value is not interpretable. For example, knowing that the cross-entropy value for our naive Bayes model is `r signif(nb_mtr$mean[nb_mtr$.metric == "mn_log_loss"], 3)` does indicate whether that value is good or bad. We can determine what a poor value of cross-entropy is for this data set by repeatedly permuting the true class column of the validation set, recomputing the metrics, and taking the average. This gives us an estimate of the metrics when there is no connection between the observed classes and their predictions. Following this approach with 100 permutations, the baseline cross-entropy of a poor model was `r signif(nb_mtr_perm$mean[nb_mtr$.metric == "mn_log_loss"], 3)`.

Also, there can be numerical issues when some probability estimates are at or near zero; $\log(\hat{p}_{ik})$ can diverge quickly to a large number and is undefined as zero. The common correction for these issues is to cap the probability estimates to a very small number. This helps avoid $\log(0)$ but there is still the issue that extremes of the probability scale have more dramatic emphasis (and influence) on the metric. 
...
See @brier1950verification and, more recently, @bella2013effect. A perfect model would estimate probabilities to be either 1.0 or 0.0, depending on the class, and would minimize this statistic. What would an ineffective model produce? While there is only one way to be perfect, there are many was a model could produce poor estimates. Let’s say that the model is completely equivocal and every prediction for this binary outcome is 0.5. In this case, we are summing repeated values of 0.5<sup>2</sup>  so that the worst possible result in this situation is 0.25. This serves as a good rule of thumb but a better (but more complex) approach is via the permutation method previously described in @sec-permutations.

Looking at our mushroom model, the Brier score was estimated as `r signif(nb_mtr$mean[nb_mtr$.metric == "brier_class"], 3)` with 90{{< pct >}} confidence limits of (`r signif(orig_brier$.lower, 3)`, `r signif(orig_brier$.upper, 3)`). The permutation estimate shows that a poor model would have an average score of `r signif(nb_mtr_perm$mean[nb_mtr$.metric == "brier_class"], 3)`. While not perfect, the model is able to acheive a fairly small average squared error. 
...
Based on this equation, a more general version of our rule of thumb would be $(1 - (1/C))^2$ for the worst-case Brier score.  For example, with ten class levels, an unsatisfactory model would have a score around `r (1 - (1/10))^2`. 

This metric has some interesting quailities that make is attrctive. As previously mentioned, a perfect model has a score of zero. If a performance metric achieves its smallest value when the probability estimate is perfect, it is called a *proper scoring rule*. There are theoretical benefits to using such a metric which @kruppa2014probability describe succinctly: 
...
Additionally, the Brier score can be partitioned into smaller components that measure different aspects of the model fit (similar to the variance-bias decomposition from @sec-variance-bias). @sanders1963subjective was able to split the Brier score into two components: "reliability" and "resolution." This requires the probability estimates to have replicate values, i.e., the estimated probability values are not unique.
...
Refinement is interesting because it uses the model probability estimates indirectly (by contributing the $j$ data groupings). The values $\bar{y}_j (1 - \bar{y}_j)$ echo the terms in the binomial variance and the Gini Index^[There is also a direct connection between refinement and the area under the ROC curve. See @hand2001simple.]: ($\pi$(1-$\pi$)). This pattern measures _impurity_. The best-case scenario is that the true outcome values for every unique model probability estimate _are the same_. We want to minimize this term too. @murphy1973new shows how to further divide refinement and @yates1982external compares the two approaches.

This partition could be very helpful except for the problem that most models produce nearly unique probability estimates. For example, there are `r format(nrow(nb_val_pred), big.mark = ",")` mushrooms in the validation set and `r format(vctrs::vec_unique_count(nb_val_pred$.pred_poisonous), big.mark = ",")` unique predictions. In order to show how the decomposition could be useful, we’ll artificially round our probabilities to the third decimal place, yielding `r format(vctrs::vec_unique_count(brier_decomp_data $.pred_poisonous), big.mark = ",")` unique predictions. @fig-brier-decomp shows the $n_p$ = `r format(vctrs::vec_unique_count(brier_decomp_data $.pred_poisonous), big.mark = ",")` values for the reliability and refinement terms. On the left, we can see that there is a strong linear relationship between the estimates and the $\bar{y}_j$. The calibration is more variable in the middle of the probability range and there is some deviation from the diagonal line when the estimates are in the range of [0.1, 0.5] (a smoothed line is shown in blue). 
...
The right-hand panel shows the refinement data. We see that the extreme ranges of the probability estimates are very pure. Just outside of the extremes, the values spike around 0.20 and 0.85. 

If we can construct them, these diagnostic plots can be helpful in determining where the model may lack correctness. We'll see similar plots in @sec-calibration.

Finally, in comparison to cross-entropy, the Brier score has a slight advantage in how extreme probabilities influence the overall statistic. Recall that the logarithm of the probabilities might be problematic when estimates are close to zero. The Brier score is less affected due to its use of squared terms. At most, an extreme probability estimate has a definitive limit to how much it can impact the overall score. 

### Reciever Operating Characteristic Curves {#sec-roc}

Earlier, we found that our naive Bayes model had a sensitivity of `r round(nb_mtr$mean[nb_mtr$.metric == "sensitivity"] * 100, 1)`{{< pct >}} and a specificity of `r round(nb_mtr$mean[nb_mtr$.metric == "specificity"] * 100, 1)`{{< pct >}}. Since a false negative might end up poisoning someone, we might want to try to increase our sensitivity to identify every poisonous mushroom we can. One way to do this is to lower the weight of evidence required to make a class prediction of "poisonous." The sensitivity and specificity quoted above correspond to a rule where a 50{{< pct >}} probability or more indicates poison. What if we were to loosen that threshold and make it 10{{< pct >}}?

In doing this, we will designate many more mushrooms as poisonous and, as a result, the sensitivity increases to `r round(roc_points$sensitivity[roc_points$target == 0.10] * 100, 1)`{{< pct >}}. Now, we’re identifying almost all of the poisonous mushrooms. The consequence is that many of these newly predicted mushrooms are actually edible. The specificity falls to `r round(roc_points$specificity[roc_points$target == 0.10] * 100, 1)`{{< pct >}}. That might be acceptable; we would probably want to discard perfectly edible mushrooms to avoid being poisoned. 
...
This presents some difficulty, though. We probably want to know the sensitivity and specificity so that we can determine if the model is good enough or compare it to other models. Should we try to optimize the probability cutoff for each model as it is being developed? How _would_ we pick an appropriate threshold? 
...
Based on these patterns, we can use the area under the ROC curve as an optimization metric to differentiate or rank different models (or tuning parameter candidates). This single metric value encapsulates all possible thresholds. This allows us to refrain from finding an optimal cutoff for each model during development; we can wait until we have narrowed our choice of the model down to a few and then examine their ROC curves to determine the best sensitivity and specificity for the situation. 

We could show the different ROC curves in the same plot and the curve that moves closer to the upper left corner is better than the others. 

Of course, there are disadvantages to summarizing an entire curve with a single number. Some models might have portions of their curves that show superiority to other models, but not uniformly. It can also be difficult to conceptualize what the area means, how much difference in AUCs is important, and so on. 

If a particular section of the curve is specifically interesting, a partial area under the ROC curve can be computed [@McClish] and might offer a more selective metric. 

The ROC curve is defined for data sets where the outcome has two classes. However, @Hand2001us derived a method to calculate a single AUC when there are three or more classes. @Hanley1982uc had previously recognized that the ROC AUC has a form equivalent to the Wilcoxon statistic. Given this, Hand and Till derived a method to combine the AUC values of all pairwise ROC curves to find an overall metric. While it is possible to derive a standard error (and confidence intervals) of a single ROC curve, the authors suggest using bootstrap methods to estimate these statistics for multiclass ROC AUCs. 
...
As we increase the threshold, we select fewer items, the recall decreases and eventually becomes zero at the left-hand side of the curve since no documents are selected. In contrast, the precision increases since the ratio of the selected true positives increases. 
...
Note that the ROC curve has its optimal point in the top left. For PR curves, the optimal setting is in the top right of the plot area. We can also summarize the PR curve using the under the curve, a value of one being the best. Based on @fig-pr-curve, the PR AUC was `r round(nb_mtr$mean[nb_mtr$.metric == "pr_auc"], 3)`.

However, note that the horizontal baseline value for the curve corresponds to the prevalence. The PR curve baseline, unlike ROC, moves with outcome distribution (as it does with accuracy). This does complicate the understanding of the AUC since the baseline section of the curve moves from data set to data set and may not be obvious just be looking at the AUC value. 

### Comparing ROC and PR Curves {#sec-compare-roc-pr}

There are many resources both in social media, blog posts, and the literature [@saito2015precision;@ozenne2015precision] that make the claim that: 

- PR curves should always be used for models with class imbalances and 
- ROC curves are ineffectual with rare events. 

Many of the arguments made about these effects are not quite correct. One assumption is that when there is a class imbalance, the goal is always to identify events in a population (i.e., information retrieval). Indeed, a curve based on metrics important to information retrieval will always be better at information retrieval than alternatives. However, there are many instances where we are interested in knowing that there is good class separation even though one class occurs infrequently. 

Let’s look at a real example. Pharmaceutical companies use machine learning to derisk drug candidates by predicting the probability that the potential drug has different types of toxicity. For example, @maglich2014more considered models using laboratory tests to predict whether some molecules can cause steroidal toxicity. 
...
Once deployed, a model would be used as follows: a medicinal chemist designs a potential drug _in-silico_ (i.e., via a computer). They pass the chemical structure to an machine learning model that can predict the probability of being toxic. 
...
Arguably, in this scenario, the quality of the model should be assessed by multiple metrics: the ROC and PR curves, as well as the Brier score. 

Let’s also look at a counterexample where a PR curve has more utility than other metrics. Suppose we work for a retailer trying to find customers in their database who will most likely make a large purchase within a year. We can create an machine learning model to predict this event based on their demographics, previous purchases, and other predictors. We can apply this model to new customers. Still, the initial goal is to find the target customers from the existing data so the company might send a promotional offer to incentivize them to purchase. 

Here, we are looking for customers in an existing database to find positives.  There is less emphasis on accuracy on the truly negative customers; here isn’t much downside to offering a promotion to someone unlikely to act on it. Good discrimination isn’t very important, and neither is calibration if we will be choosing the top X{{< pct >}} of most likely customers. In this instance, _the PR curve is clearly more appropriate for the job_. 

There are some technical characteristics that related the two curves to one another, gathered from @davis2006relationship: 
...
We often talk about a _model_ having good calibration but, as noted by @lichtenstein1977calibration, calibration is really a characteristic of a single prediction. A prediction is well-calibrated if the probability estimate is close to the true value. We have no way of knowing or proving this and often think of calibration in the sense that if we could repeatedly collect the data point, the rate at which the event occurs should match our prediction. As seen in the previous section, the only numeric estimates that we have for the true value are the binary indicators for each class ($y_{ik}$). Unfortunately, these values do not facilitate diagnostics to assess how well our model works^[This will not be the case when it comes to assessing calibration for regression models because our outcome is in the same numerical format as the prediction.]. 

For classification models, the primary diagnostic available is the **calibration plot** of the entire data set (a.k.a. the reliability diagram). For two-class outcomes, this plot is usually created by ordering the data by the estimated probability of the event and binning these into groups (usually ten). From these groups, we can compute the event rate from the outcome data and plot that rate against the midpoint of the bins. The points should fall on a 45-degree diagonal line if the model is well-calibrated. 

@fig-cal-break-curves shows this type of curve for two models. Panel (a) shows the validation results for the naive Bayes model. The points fall on a fairly straight line, demonstrating good calibration. Panel (b) has the same visualization for a boosted tree model that was limited to only three trees in the ensemble. The result is a curve that is not close to the diagonal. The points on the lower left indicate that the predicted probabilities are much larger than they should be. The opposite occurs in the upper right; the probabilities are generally underestimated when compared to the rate in the groups. 
...
num_isotonic_boot_vals <- vctrs::vec_unique_count(
  pred_cal_iso_boot$.pred_poisonous
)
...
This brings up questions about which data should be used to fit the calibration model and which to evaluate its efficacy. We’ve previously mentioned that using simple repredictions of the training set is a very bad idea and that is especially true here. If the model was resampled, the out-of-sample predictions can be used to fit the calibration model. In our example, a single validation set was used. Unfortunately, if all of the held-out data were used for recalibration, the only unused data left would be the test set (which should not be compromised).

TODO change below

We were able to set aside `r format(nrow(pred_cal), big.mark = ",")` samples for the calibration set and we will use these to fit the calibration model. @fig-re-cal-curves shows the results where panel (a) uses a sliding window to assess the smaller data set using a window size of 20{{< pct >}} that moves in 5{{< pct >}} increments. From these figures, the class probabilities appear to have been improved. Note that these values now encompass the entire probability range.
...
#| fig-width: 7
#| fig-height: 4
#| fig-cap: Validation set calibration curves for naive Bayes model _after_ recalibration using a logistic regression fit to the calibration set.
cal_plot_3 <-
  pred_cal %>%
  cal_plot_windowed(
    class,
...
The nice feature of this tool is that it is nonparametric and makes almost no distributional assumptions. However, one downside is that there are a small number of unique recalibrated probability estimates. For this reason, it is advised to use isotonic regression for large data sets. For example, for the naive Bayes data used for calibration, there were originally `r format(num_logistic_vals, big.mark = ",")` unique probability estimates while, after isotonic regression is applied, there are only `r num_isotonic_vals` possible values. @fig-recal-iso-curves(a) shows the calibration curve after isotonic regression is applied. 

We can improve the isotonic regression results using _bagging_. As discussed later in @sec-cls-bagging, bagging creates an ensemble of models by taking $B$ bootstrap samples used to build $B$ discinct models. The final model prediction is the average of the $B$ model predictions. For isotonic regression, this has the effect of reducing the variability in the predicted (i.e., recalibrated) probabilities and also increases the number of unique probability values. Using 1,000 bootstrap samples, we end up with `r num_isotonic_boot_vals` unique probability estimates. 
...
#| fig-width: 7
#| fig-height: 4
#| fig-cap: Validation set calibration curves for naive Bayes model _after_ recalibration using isoreg regression from a calibration set. Both panels show moving 20{{< pct >}} windows. 
...
It is important to recompute _all_ metrics using the recalibrated predictions. Also note that, since the class probabilities have been altered, any hard predictions should be recomputed since some values may have moved across the chosen probability threshold. Using the set of `r format(nrow(pred_cal), big.mark = ",")` data points that have been recalibrated, the reestimated sensitivity and specificity were `r round(re_class_stats$.estimate[re_class_stats$.metric == "sensitivity"] * 100, 1)`{{< pct >}} and `r round(re_class_stats$.estimate[re_class_stats$.metric == "specificity"] * 100, 1)`{{< pct >}}, respectively and the area under the ROC curve was `r signif(re_class_stats$.estimate[re_class_stats$.metric == "roc_auc"], 3)`.

::: {.note-box}
The user should not set expectations very high; recalibration methods often have limited ability to correct predictions. In our example, a large amount of data was set aside for calibration, and we were able to fix a minor issue with the original model’s predictions. In most cases, recalibration might be able to fix gross miscalibration trends. 
:::
...
bind_cols(wordle_conf_mat, wordle_errors) %>% 
  ...
  add_header_above(c(" " = 2, "Truth" = length(lvls), 
                     " " = 1, "Linear Weights" = length(lvls))) %>% 
  kable_styling(full_width = FALSE)
```

A six class ordinal outcome with a confusion matrix (left) and a linear set of weights for each potiental error (right).
:::
::::
...
There are metrics that use hard classification predictions from ordinal models. We previously mentioned the Kappa statistic. To penalize misclassification errors based on their distance to the true classification, REF developed the *weighted Kappa* statistic where the errors' weights depend on their positional differences. 

First, let's rewrite the Kappa statistic defintion from @eq-kappa in terms of classification error (i.e., the opposite of accuracy). We can compute the error as the number of off-diagonal counts divided by the total count. For expected errors, can use an approach where the cell counts shown in @eq-exp-agree are multiplied by their corresponding weights. With these two types of error in hand, Kappa is: 

$$
Kappa = 1 - \frac{error}{expected\:error}
$$ {#eq-kappa-error}

Weighted Kappa assigns larger costs (i.e., errors) to cells in the conufsion matrix that are "farther away" from the true category. A linear weighting for the Kappa statistic is shown in @tbl-confusion-matrix-costs. There are bands of diagonal integer weights that are 0, 1, 2, $\ldots$, 5. A quadratic weighting scheme would square each of these weights (e.g., 0, 1, 2<sup>2</sup>, 3<sup>2</sup>, etc.). Weight Kappa statistics enable "more incorrect" errors more than others. 

For this test set, the estimate of the unweighted Kappa statistic was `r signif(rule_mtr$test[rule_mtr$.metric == "kap"], 3)`, which is outstanding. If we increase the penalty for more disparate errors, the performance values for linear and quadratic weights were `r signif(rule_mtr$test[rule_mtr$.metric == "kap_lin"], 3)` and `r signif(rule_mtr$test[rule_mtr$.metric == "kap_quad"], 3)`, respectively. Notice that the Kappa values _increase_ as the weights become more severe. In these data, the "cost" of the errors inceases faster in the exepected agreement than in the observed agreement. This is not often the case. 

@nakov2019semeval suggested ordinal versions of common regression metrics, such as the mean squared error (MSE). These use the positional "errors" between the true and predicted outcomes. For example, if a word requires four tries, but we predict that the puzzle will be solved with two tries, the error is -2. These metrics should be fairly understandable since they mimic common statistics used to evaluate regression models. However, there has been scant evidence that they offer a benefit over existing metrics that have been studied and evaluated for decades. The mean squared error for these data was `r signif(sqrt(mse_ordered(rule_te_pred)), 3)`. How bad is this? If we repeatedly permute the outcome classes and measure the RMSE we find that the worst case-scenario is about `r signif(sqrt(mean(mse_perm$null)), 3)`.
...
The denominator is normalized by dividing by $C-1$. The minus one compensates for the fact that, for the largest class level, all predictions should be perfect (since they are cumulative). Instead of using a binary value $y_{ik}$ corresponding to class $k$ only, the RPS assigns a value of 1 to $\tilde{y}_{ik}$ if the position of the true class is $\le k$. Similarly, the value of $\tilde{p}_{ik}$ is the cumulative sum of the estimated probabilities corresponding to classes $\le k$. For example, with the Wordle data, $k = 3$ corresponds to four tries and $\tilde{y}_{i3}$ would have a value of 1.0 if the i<sup>th</sup> sample was solved in two, three, or four tries (and is zero otherwise). The corresponding cumulative probability is the sum of the individual probabilities of solving the puzzle in two, three, or four tries. The RPS inherits the same benefits as the Brier score since they are both proper scoring rules. See @stael1970family and @gneiting2007strictly.

The ranked probability score measures the correctness (and calibration) of the question: "How well am I predicting that the event occurs in $k$ or less classes?". 

The test set probabilities associated with the test set were used to compute the Brier score estimate and the ranked probability score value. The RPS estimate was `r signif(rule_mtr$test[rule_mtr$.metric == "ranked_prob_score"], 3)` and its corresponding permuted "no information" estimate was `r signif(rule_mtr$permuted[rule_mtr$.metric == "ranked_prob_score"], 3)`. This indicates that the model appears to function effectively when evaluating the cumulative probability estimates. For comparison, the unordered Brier score for these data was `r signif(rule_mtr$test[rule_mtr$.metric == "brier_class"], 3)` (with permuted value `r signif(rule_mtr$permuted[rule_mtr$.metric == "brier_class"], 3)`), indicating reduced effectiveness when the classes are treated as unordered. This does make sense since the RPS is more forgiving of inaccuracies in the individual probability estimates as long as their sum is accurate. See @murphy1970ranked for a comparison of these two flavors of the Brier score. 

## Multi-Objective Assessments {#sec-multi-objectives}

It may be challenging to choose a single metric that captures all of the model’s goals. When hard class predictions are important, the choice of the probability cutoff should be driven by measures such as sensitivity, specificity, or precision. We might want to optimize on a function of multiple metrics (e.g., the ratio of sensitivity and specificity, etc.). Additionally, these metrics don’t consider model calibration and other aspects of the model.  

In some cases, a joint metric like the F-score can be used if it aligns with the project goals. Similarly, the J-index [@Youdencs] might also suffice: 

$$
J = Sensitivity + Specificity - 1
$$

Additionally, we might want to factor in the characteristics of our model. For example, we’ve previously seen that the number of active features in the model can help us make better choices about which candidate to select. As another example, if we are choosing between different tuning parameters in a neural network, we might be interested in seeing how the total number of model parameters affects the primary performance metric. There might be an advantageous trade-off between model complexity and performance. 

However, in general, a more advanced _multiparameter optimization_ may be required. Many different methods for accomplishing this are well summarized in REF. If we have $s$ candidate models, some approaches require the full set of metrics and characteristics to decide (e.g., Pareto optimality). Alternatively, we might be more interested in developing a single objective function that combines metrics in a meaningful way that can measure a single candidate. Iterative optimization (or racing methods) would require a combined performance metric in real-time. 
...
Desirability functions can be defined for any important facet of the model result to optimize ML models. For example, we could articulate that we should *minimize the Brier score* such that
...
where $A$ and $B$ are the low and high values, respectively. The $\gamma$ parameter is a scaling factor that modulates of easy (or difficult) it should be to satisfy the goal. 
...
There are many different types of desirability functions for different goals (box constraints, qualitative values, etc.). 
