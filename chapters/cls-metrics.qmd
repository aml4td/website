...
since $\hat{p}_{i2} = 1 - \hat{p}_{i1}$.

It turns out that if we assume the data points independently arise from a Bernoulli distribution with probability $\pi$, the likelihood statistic is 

$$
\ell(y_{ik}; \pi) = \prod_{i=1}^n \pi^{y_{i1}} (1 - \pi)^{y_{i2}}
$$ {#eq-binomial-lh}

We typically try to optimize the negative log-likelihood. Logging the function and subtituting our estimate $\hat{p}_{i1}$ for $\pi$, we also arrive at: 

$$
-\log \ell(y_{ik}; \hat{p}_{i1}) = -\sum_{i=1}^n \left[y_{i1}\log(\hat{p}_{i1}) + y_{i2}\log(1 - \hat{p}_{i1})\right]
$$ {#eq-binomial-neg-llh}

When there are more than two classes, the same equivalence can be shown to the multinomial log-likelihood. 

Recall that the likelihood is the probability of our data occurring given a set of parameter values (in this case, the $\hat{p}_{ik}$). It measures how well our model explains observed data. If the likelihood is to be maximized, the negative log-likelihood (i.e., cross-entropy) is minimized. Often, with classification models, this statistic will be referred to as "log loss^[This does lead to confusion since the loss function in question, the likelihood, is different for different data types (e.g., real numbers, counts, etc). In linear regression, "log loss" also corresponds to the sum of squared errors.]. 

One issues is that the scale of the value is not interpretable. For example, knowing that the cross-entropy value for our naive Bayes model is `r signif(nb_mtr$mean[nb_mtr$.metric == "mn_log_loss"], 3)` does indicate whether that value is good or bad. We can determine what a poor value of cross-entropy is for this data set by repeatedly permuting the true class column of the validation set, recomputing the metrics, and taking the average. This gives us an estimate of the metrics when there is no connection between the observed classes and their predictions. Following this approach with 100 permutations, the baseline cross-entropy of a poor model was `r signif(nb_mtr_perm$mean[nb_mtr$.metric == "mn_log_loss"], 3)`.

also logging things near zero makes them explode; impossible results when p = 0


like brier, strictly proper scoring rule in the sense that it is optimized at the true probability distribution.
...
brier_decomp_2cls <-
  ...
  summarize(n = n(), r = sum(class == "poisonous"), .groups = "drop") %>%
  mutate(cal = n * (r - .pred_poisonous)^2, refine = n * r * (1 - r)) %>%
...
#| fig-width: 7
#| fig-height: 4
#| fig-cap: Other types of calibration curves. 
