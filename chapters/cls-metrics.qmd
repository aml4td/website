...
We've described that, by maximizing the likelihood, we find values(s) of the parameter(s) that explain the data best. To improve numerical stabiklity, we typically try to minimize the negative log-likelihood. Logging the $\ell(y_{ik}; \pi)$ and subtituting our estimate $\hat{p}_{i1}$ for $\pi$, we also arrive at: 
...
Additionally, the Brier score can be partitioned into smaller components that measure different aspects of the model fit (very similar to the variance-bias decomposition from @sec-variance-bias).  
...
The Brier score also measures how well a model is calibrated (see @sec-calibration). In fact, the Brier score can be decomposed into different components, one of which specifically measures calibration. However, this can only be computed when the estimated probability values are not unique. 

Suppose $C = 2$. Let $n_p$ be the number of unique values of $\hat{p}_{i1}$ denoted as $\hat{p}_{j}$ ($j=1, \ldots, n_p$). We'll also denote $n_j$ to indicate the number of data points with value $\hat{p}_{j}$, and the rate of events in group $j$ as $\bar{y}_j$. From this, we can partition that statistic as: 
...
Brier &= \frac {1}{n_p} \sum_{j=1}^{n_p}n_{j}(\hat{p}_{j} - \bar{y}_j)^2 + \frac {1}{n_p} \sum_{j=1}^{n_p}n_{j} \bar{y}_j (1 - \bar{y}_j) \label{eq-brier-cal} \\
 &= \text{calibration} + \text{refinement} \notag \\
\end{align}


the log-loss puts more dramatic emphasis on the extremes of the probability scale than does the Brier loss. 
...
#| fig-width: 8
#| fig-height: 4
#| fig-cap: The data making up the two-component partition of the Brier score (calibration and refinement). The left panel plots $\hat{p}_{j}$ versus $\bar{y}_j$ and the right panels plots $\hat{p}_{j}$ versus $n_{j} \bar{y}_j (1 - \bar{y}_j)$.

p_cal <-
  brier_decomp_data |>
  ggplot(aes(.pred_poisonous, y_j, size = n_j)) +
  geom_abline(col = "green", lty = 2) +
  geom_point(alpha = 1 / 4, show_legend = FALSE) +
  coord_obs_pred() +
  labs(x = "Predicted Probability", y = "Observed Rate")

p_refine <-
  brier_decomp_data |>
  ggplot(aes(.pred_poisonous, refine, size = n_j)) +
