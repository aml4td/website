...
It is important to recompute performance using the recalibrated predictions. Also note that, since the class probabilities have been altered, any hard predictions should be recomputed since some values may have moved across the chosen probability threshold. Using the set of `r nrow(pred_cal)` data points that have been recalibrated, the reestimated sensitivity and specificity were `r round(re_class_stats$.estimate[re_class_stats$.metric == "sensitivity"] * 100, 1)`{{< pct >}} and `r round(re_class_stats$.estimate[re_class_stats$.metric == "specificity"] * 100, 1)`{{< pct >}}, respectively and the area under the ROC curve was `r signif(re_class_stats$.estimate[re_class_stats$.metric == "roc_auc"], 3)`
...
It may be diffiuclt to choose a single metric that captures all of the goals of the model. When hard class predictions are important, the choice of the probability cutoff should be driven by measures such as sensativity, specificity, or precision. We might want to optimize on a function of multiple metrics (e.g., the ratio of senstivity and specificity, etc.). Additionally, these metrics don’t consider model calibration and other asepcts of the model.  

In some cases, a joint metrics like the F-score can be used if it aligns with the porject goals. Similarly, the J-index might also suffice: 

$$
J = Sensitivity + Specificity - 1
$$

Additionally, we might want to factor in characteristics of our model. For example, we’ve previously seen that the number of active features in the model can help make better choices about which candidate to select. As another example, if we are chooing between different tuning parameters in a neural nbetwork, we might be interesting in seeing how the total number of model parameters and the primary performance metric. There might be an advantageous trade-off between model complexity and performance. 


However, in general, a more advanced _multiparameter optimization_ may be required. There are many different methods for accomplishing this which are well summarized in REF. If we have $s$ cancidate models, some approaches require the full set of metrics and characteristics to make a decision (e.g. Pareto optimality). Alternatively, we might be more interested in coming up with a single objective function that combines metrics in a meaningful way that can measure a single candidate. Iterative optimization (or racing methods) would require a combined performance metric in real time. 

We’ll focus on one method to blend multiple criertia into a single composite value called “desirability functions.” Original conveived by @harrington1965desirability, the versions of desirability functions defined by @derringer1980simultaneous are more commonly used. They are extensively used in statistical experimental design but also in a wide range of applications, including designing cancer combination therapy [@kuhn2000incorporating], medicinal chemistry [@wager2010moving], waste water processing [@corral2019coagulation], and others. These tools are knowledge-based; users must state specific values of their criteria that are desirable or unacceptable. 
