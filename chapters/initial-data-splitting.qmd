...
bind_cols(
  ...
  mutate(Distribution = NA) %>%
  {
    ...
    dat |>
      ...
      text_transform(locations = cells_body(c(Distribution)), fn = function(x) {
...
In general, a decent rule of thumb is that 75% could be used for training.
...
Simple random sampling does not control for any data attributes, such as the percentage of data in the classes. When one class has a disproportionately small frequency compared to the others (discussed in @sec-cls-imbalances), the distribution of the outcomes may be substantially different between the training and test sets.
...
#| fig-alt: "Two-panel density plot. Panel (a) shows the distribution of sale prices with a right-skewed shape and dashed vertical lines marking quintile boundaries, with a rug of individual points along the axis. Panel (b) overlays two density curves for the training and test sets after stratified splitting; the solid and dashed lines are nearly identical, demonstrating that stratification preserved the outcome distribution."
parition_plot <-
...
#| out-width: 70%
#| fig-cap: "Maximum dissimilarity sampling of 25 points in the Ames data. The small black circles are individual properties. Larger, lighter colors indicate earlier selection."
#| fig-alt: "Animated scatter plot of Ames property locations by longitude and latitude. Small circles represent all properties. Sequentially, 25 larger points appear, with earlier selections shown in lighter shades and later ones darker. The first points chosen are near the outskirts and outlying locations; subsequent points fill in the interior, covering the geographic space without redundancy."
#| message: false
#| warning: false
...
As previously discussed, validation sets are a separate partition of the data that function as a precursor for the testing set. They allow us to obtain performance estimates on our model(s) during the development cycle. These are commonly used in deep learning and other domains where the initial data sizes range from very large to massive. This additional partition is often created simultaneously with the training and testing sets. 
...
Without loss of generality, we will treat the validation set as a particular case of resampling where there is a single resample of the training set. This difference is not substantive and allows us to have a common framework for measuring model efficacy (before the testing set).
...
Think of an overhead color satellite image of a forest. Different types of foliage---or lack thereof---have different colors. If we focus on the color green, the distribution of "greenness" is not a random pattern of the x/y axes (random distribution would resemble static). In other words, pixels cannot be assumed to be independent of one another. 
...
#| fig-alt: "Interactive map of Washington State showing sampled locations as small circles. Green circles indicate forested locations and golden circles indicate non-forested locations. The points are distributed unevenly across the state, with denser sampling in some regions."
forested_labs <-
  forested::forested |>
...
#| fig-alt: "Two-panel plot of 65 spatial data points. Panel (a) shows the points grouped into seven hexagonal blocks, each block distinguished by a different shade. Panel (b) adds a translucent circular buffer around one block, illustrating that nearby points in adjacent blocks fall within the buffer zone and would be excluded to reduce spatial autocorrelation."
p_hex <-
  hex_data |>
  ggplot() +
...
#| fig-alt: "Interactive map of Washington State showing the spatial data split. Training set locations appear in magenta, testing set locations in purple, and buffer locations in black. The testing hexagons are distributed across the state to provide broad geographic coverage, with buffer zones separating them from nearby training points."
leaflet() |>
  addProviderTiles(providers$CartoDB.PositronNoLabels) |>
  addCircles(
