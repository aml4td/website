...
dark_strata_plot <- 
  ggplot(ames, aes(x = Sale_Price)) +
  geom_line(stat = "density", col = dark_data, linewidth = 1.2) +
  geom_segment(data = price_bins,
               aes(x = value, xend = value, y = 0, yend = y),
               lty = 2, col = dark_tan) +
  geom_rug(alpha = 1 / 10, length = unit(0.02, "npc"), col = dark_data) +
  labs(x = "Sale Price (USD)", y = NULL, title = "(a) before") +
  thm_dk
```
