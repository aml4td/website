...
We'll use both of these methods below for illustration. @Roberts2017, @Pohjankukka2017, @karasiak2022spatial, and @mahoney2023assessing have good overviews of these techniques.

For our example, we’ll use blocking and buffering to allocate roughly 20% of the data to the testing set. 

TODO details

25 hexagons with a buffer of `r format(80^2, big.mark = ",")` squared map units.


The results are a training set of `r format(forested_n_tr, big.mark = ",")` locations and a test set of `r format(forested_n_te, big.mark = ",")` locations (about `r forested_p_te`%). The buffer contained `r format(forested_n_bf, big.mark = ",")` locations which is about `r forested_p_bf`% of the original pool of data points. The event rates were similar between the data sets: `r round(event_rates$event_rate[event_rates$group == "training"], 1)`% for the training set and `r round(event_rates$event_rate[event_rates$group == "testing"], 1)`% for the testing set.
