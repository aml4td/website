...
The problem with missing data is that many models are not naturally equipped to deal with this lack of information. As a simple example, consider the multiple linear regression model. Deriving the regression coefficients depends on computing the covariance matrix. This calculation cannot be performed if the predictors contain just one missing value. Partial least squares (@sec-pls-cls), support vector machines (@sec-svm-cls), K-nearest neighbors (@sec-knn-cls), and neural networks (@sec-neural-nets-cls), are also unable to tolerate missing data.\
Consequently, it is crucial to address the presence of missing data to build any of these predictive methods. In addition to addressing missing data, the pattern and nature of missingness can sometimes serve as a significant predictor of the response. In this chapter, we will explore the root causes of missing data, examine approaches for resolving the problem, and understand when it should be addressed in the modeling process.

@allison2002missing has an excellent and succinct summary of the issues that arise in data analysis when incomplete data points are present. @emmanuel2021survey and @hasan2021missing provide literature surveys on methods for missing data, while @nijman2022missing describes a survey on just how _badly_ these topics are described/documented in specific studies in the literature. 
...
For example, naive Bayes [@webb2010naive] builds models using each predictor separately. When a predictor contains a missing value, this missing sample’s information is omitted from the probability calculation for only this predictor. If a distribution is affected by missingness, the probabilities computed during prediction will be biased, perhaps systematically, and may result in poor results in new samples. If missing data informs our understanding of the response, omitting the missing samples will be detrimental to the model. 
...
# TODO center and scale these predictors at the end
set.seed(331)
...
Like nearest-neighbor imputation, the bagged tree places most of the imputed values close to the original data distribution, as seen in @fig-imputation-comparison. The distribution of distances (@fig-imputation-distance-comparison) for bagged imputation is similar to that of nearest neighbors.  @fig-imputation-exploration provides a comparison of the imputation techniques discussed here with the nonlinear simulated data and a data set in which the two groups are optimally separated by a linear boundary.  In this comparison, the percentage of missing data can be adjusted to demonstrate how each imputation method performs.
