...
#| fig-alt: "Two side-by-side scatter plots of simulated two-class data. The left panel shows a nonlinear relationship where the optimal class boundary is a parabolic curve. The right panel shows a linear relationship with a straight diagonal boundary. Each class is distinguished by point shape and shade, with moderate overlap near the decision boundaries."
p_nonlin <-
...
$K$-nearest neighbor calculations will be discussed in @sec-mds, where it will be used in some multidimensional scaling methods, and in @sec-cls-knn in the context of a supervised model. In short, when imputing a missing predictor value for a new sample, the $K$ most similar samples from the training set are determined (using all complete predictor values). The $K$ predictor values for the predictor of interest are summarized via the mean (for numeric outcomes) or the mode. This summary statistic is used to fill in the missing value. This is a localized version of the most likely value approach.
