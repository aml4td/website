...
The model is somewhat attractive since it is reasonably fast (if the training data are not huge) and requires very little preprocessing of its own. However, it would require embedding the training set (at the stage of its use) into the model pipeline. This can be infeasible for very large data sets. 
...
#| fig-width: 12
#| fig-height: 7
#| fig-cap: "A decision tree model that can be used to predict missing stalk root values as a function of the population and odor predictors."
...
Imputation is an operation that can be easily misapplied. When developing the primary ML model, we should evaluate the imputation models inside of the primary resampling/validation loop (see  @fig-within-model-process). That will ensure that the good or bad effects of imputation are appropriately measured and propagated into the primary performance metrics for the primary model. One should _not_ develop and estimate an imputation model and fill in the missing data _prior to the initial split_. This can severely bias the metrics for the primary ML model to be overly optimistic. 
