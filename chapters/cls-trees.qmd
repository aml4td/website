...
### Types of Splits  {#sec-split-types}
...
The Gini value for this split is smaller than the corresponding value from the parent node and this indicates a reduction in impurity. Note that a number of tree-based models include a tuning parameter that specifies _how much of a gain_ occurs in order to make the split. 
...
The _information gain_ from applying a split can be calculated as the difference between the information content before and after the split:

$$
\text{gain(split)} = \text{CE(parent)} - \text{CE(split)}
$$
...
Both regularization methods can be used simultaneously. 

During the search for the optimal split, the XGBoost splitting statistic is computed for both sides of the split so that the gain can be measured:

$$
\psi_{gain} = \psi_L + \psi_R - \psi_P
$$

where again, L/R/P signifies the statistic for the left side, the right side, and the parent. 

During training, we can also impose a condition that a split can only be made if the expected gain exceeds a minimum threshold and/or if there are at least $n_{min}$ data points in the leaf. These two conditions, along with the two penalty parameters, can be considered tuning parameters. 

Once the split is made, the probability estimates for the leaves are estimated by: 
...
As we'll see in the next chapter, these models often pre-bin numeric predictor data (a.k.a. quantization) to increase computational efficiency. Those techniques are described in @sec-TODO. 
...
For example, suppose we have a predictor with 26 categories, ranked _A_ to _Z_. If we were to only consider five splits in each pass, we would consider partitions: 

:::: {.columns}

::: {.column width="50%"}
- {_A_} versus {others}
- {_A_, _B_} versus {others}
- {_A_, _B_, _C_} versus {others}
- {_A_, _B_, _C_, _D_} versus {others}
:::

::: {.column width="50%"}
- {_Z_} versus {others}
- {_Z_, _Y_} versus {others}
- {_Z_, _Y_, _X_} versus {others}
- {_Z_, _Y_, _X_, _W_} versus {others}
:::
