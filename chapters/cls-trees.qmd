...
Let's assume that the splits in our trees are _binary_; they produce two partitions of the data. This can be via thresholding a continuous predictor or by assigning levels of categorical predictors to two groups (e.g., `{stevens, yakima}` vs `{okanogan}`). In ether case, a proposed split can be represented by a 2 $\times$ 2 table as seen in @tbl-split-2-x-2.  
...
The Gini value for this split is smaller than the corresponding value from the parent node and this indicates a reduction in impurity. We often formulate the importance of a split in terms of its _gain_ over the current situation. Following the notation from @sec-gradient-opt, our objective function is denoted as $\psi$ and the gain is computed as:
...
Since this statistic measures the difference between our model and an uninformative state, we want to maximize its value. This $\chi^2$ statistic is often used in _conditional inference trees_ [@hothorn2006unbiased]. In @sec-cls-cit we'll describe the strategic advantages to minimizing the associated p-value of this value rather than maximizing the statistic itself. 
...
As with previous statistics, this splitting statistic is computed for both sides of the split so that the gain can be measured. 

CatBoost uses essentially the same statistic with some slight alterations. First, there is no L<sub>1</sub> regularization applied. Second, the L<sub>2</sub> penalty value is scaled to be: 
...
Many of these criteria show similar patterns and recommend splits in the same region (if ot the same value), except for the   Gini statistic. It suggests a larger split value of `r split_best$split[split_best$metric == "Gini Index"]` although any split point larger than 1,400 produces nearly the same result. 
...
The final split is the configuration with the best gain statistics that were described above for these two methods. This approach effectively limits the computational costs of splitting categories for a predictor, even when the number of categories is very large. 
...
For _ordered_ categories, some algorithms restrict splits to maintain the ordering of categories, only allowing splits that group adjacent categories.
...
An example of rectangular and obique splits. The data were simulated using a linear predictor of $\eta = 0.1 + 5.0\, A - 5.0\, B$ and a logit link.
...
For example, the first split in @fig-split-adjacent-tree is based on the maximum vapor predictor.  The first surrogate for this split is `r first_surr`  There are no missing values for the forestation data set.  However, if a sample had a missing value for the maximum vapor predictor, then the model would use the `r first_surr` at this point in the tree to send direct the sample to a terminal node (provided the sample did not have a missing value for this predictor).
...
Rule-based models, which can be derived from classification trees, also need mechanisms for handling missing values. In C5.0Rules, which converts C5.0 trees into rule sets, the handling of missing values is integrated into the rule evaluation process. When evaluating whether a rule applies to a sample with missing values, C5.0Rules considers the rule to be partially satisfied, with the degree of satisfaction depending on the distribution of non-missing values in the training data. This partial satisfaction contributes to the overall confidence in the rule's prediction.
...
Most growing processes begins with all the training data in a single root node. The algorithm then searches for the best predictor and split point according to the chosen splitting criterion, as described in the previous section.  Once the best split is identified, the data is divided into child nodes based on this split. This process is then repeated recursively for each child node until one or more stopping criteria are met.
...
#| fig-width: 8
#| fig-height: 5
#| fig-cap: Heat maps of predicted probabilities of forestation from two different logistic models with main effects and interactions. (a) A model containing maximum vapor and roughness.  (b) A model of with maximum vapor and northness.
...
vapor_max_northness_fig <-
  ...
  labs(title = "(b)", x = "Maximum Vapor", y = "Northness", color = "P(class)") +
  theme_light() +
  theme(
...
Pessimistic pruning, used in the C5.0 algorithm, takes a different approach to pruning than cost-complexity pruning. Instead of using cross-validation, which can be computationally expensive, pessimistic pruning uses a statistical heuristic to estimate the expected error on new data.

The key insight of pessimistic pruning is that the apparent error rate on the training data is an optimistic estimate of the true error rate on new data. To correct for this optimism, C5.0 calculates an upper confidence bound on the error rate, which serves as a pessimistic estimate of the true error rate.
...
After the classification tree is built, each terminal node generates a raw probability for each class based on the training samples that fall into that node. For a binary classification problem, this vector consists of the proportion of samples in each class within the node. For example, in the table shown in @sec-tree-splitting, the left split contains six samples and, based on the split results, the probability estimates would be (5/6, 1/6).
