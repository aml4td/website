...
TODO: 
 ...
 - rule conversion
 - review for CIT
 - compare to previous best models
...
tree_bt_stats |>
  ...
  mutate(
    Model = factor(Model, c("CART", "C5.0", "Conditional Inference", "Oblique (Ridge)")) 
  ) |> 
  arrange(Model) |> 
  gt() |> 
  gt_plt_dist(
...
best_brier <-
  all_sets |>
  map(
    ~ rank_results(.x, rank_metric = "brier_class", select_best = TRUE) |>
      filter(.metric == "brier_class")
  ) |>
  map_dfr( ~ slice_min(.x, mean, n = 1)) |>
  mutate(
    wflow_id = gsub("basic_", "Oblique (", wflow_id),
    wflow_id = ifelse(grepl("Oblique", wflow_id), paste0(wflow_id, ")"), wflow_id),
    wflow_id = ifelse(
      grepl("logistic_mle", wflow_id),
      paste0("Logistic (", wflow_id),
      wflow_id
    ),
    wflow_id = ifelse(
      grepl("logistic_mle", wflow_id),
    ...
    wflow_id = ifelse(
      grepl("logistic_mle", wflow_id),
      gsub("_logistic_mle", "", wflow_id),
      wflow_id
    ),
    wflow_id = ifelse(model == "mlp", paste0("MLP (", wflow_id), wflow_id),
    wflow_id = ifelse(model == "mlp", gsub("_", ", ", wflow_id), wflow_id),
    wflow_id = ifelse(model == "mlp", gsub(" none,", "", wflow_id), wflow_id),
    wflow_id = ifelse(model == "mlp", paste0(wflow_id, ")"), wflow_id),
    wflow_id = ifelse(
      model == "mlp",
      "glmnet (encoded + transformations + splines)",
      wflow_id
    )
  ) |>
  mutate(
    ...
    wflow_id = reorder(wflow_id, mean)
  )

all_brier <-
  all_sets |>
  map_dfr(
    ~ rank_results(.x, rank_metric = "brier_class", select_best = FALSE) |>
      filter(.metric == "brier_class")
  )

brier_rng <- range(c(best_brier$lower, best_brier$upper, all_brier$mean))
```
...
#| fig-width: 7
#| fig-height: 4
#| fig-cap: "Progress for predicting the probability of forestation so far." 

all_p <- 
  all_brier |> 
  ggplot(aes(x = mean)) + 
  geom_histogram(col = "white", binwidth = 0.008) +
...
all_p / best_p + plot_layout(heights = c(2, 5))
```
