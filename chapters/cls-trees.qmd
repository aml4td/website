...
To describe the myriad aspects of tree-based models, we'll start with the fundamental building block:  selecting a split point for the logical statement inside the rule. After this, we'll describe how missing values can be handled algorithmically during split selection, tree growth, and pruning. With these foundational concepts in place, we examine specific single-tree-based models including CART, C5.0, and others.
...
Since this statistic measures the difference between our model and an uninformative state, we want to maximize its value. _Conditional inference trees_ often use the $\chi^2$ statistic [@hothorn2006unbiased]. @sec-cls-cit will demonstrate why minimizing the associated p-value provides strategic advantages over direct maximizing. 

Modern boosting algorithms typically construct splits using log-likelihood based criteria, a choice that we will rationalize in the next chapter.  For simplicity, we'll restrict our attention to binary classification problems ($C = 2$ classes) and focus on the three most popular models: XGBoost [@xgb16], LightGBM [@lgb17], and CatBoost [@cat18].
...
where $n_{+j}$ is the number of events in in category $j$ within all of the $n$ rows under consideration. CatBoost uses $\beta = 1$ and calls $\alpha$ the "prior" parameter, which can be used for tuning. Once all of the $j$ values of $\widehat{p}_j$ are computed, the split determination treats the predictor as if it were quantitative from the start. CatBoost's algorithm, which uses the CTR values to determine the best predictor (and value) to split on, is fairly complex and is detailed in the next chapter. 
...
Some advanced tree algorithms use linear combinations of predictors to create oblique splits [@murthy1994system;@wickramarachchi2016hhcart].  Unlike the tree algorithms discussed thus far which are limited to axis-parallel splits, oblique trees can capture more complex decision boundaries--for example "if $2 \times \text{Predictor A} + 3 \times \text{Predictor B} > 5$ then...".  The added complexity of oblique splits can lead to more accurate models, but often sacrifices interpretability.
...
An oblique tree required just a single split to produce the class boundary shown in the right-hand panel. Using a training set of $n_{tr} = 500$ data points, the diagonal split does a remarkable job approximating the underlying pattern; its split uses the rule $\widehat{\eta} = `r obl_eqn`$.
