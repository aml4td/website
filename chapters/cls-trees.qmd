...
As we'll see in the next chapter, modern boosting models often use splitting criteria based on the log-likelihood. The rationales for the methods discussed in this chapter are described in that chapter. Also, for simplicity, we'll assume that the outcome has $C = 2$ classes. We'll focus on the three most popular models: XGBoost [@xgb16], LightGBM [@lgb17], and CatBoost [@cat18].
...
Differences between the two are largely related to the addition of a boosting algorithm that will be described in SECTION. One change between the two versions is that C5.0 has an option to conduct a _global_ pruning phase at the end of tree construction. This uses a form of the one standard error rule from CART. Pessimistic pruning occurs locally, seeking to trim the tree at a specific node. The global process considers the entire tree at once. 
