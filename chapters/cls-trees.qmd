...
Example notation for a binary split of an outcome with two class levels. 
...
The Gini value for this split is smaller than the corresponding value from the parent node and this indicates a reduction in impurity.

When there are $C > 2$ classes, the more general form of the statistic is: 

$$
Gini = \sum _{c=1}^{C}\left(p_{c}\sum _{c'\neq c}^Cp_{c'}\right)
$$
...
$\chi^2$ test by conditional inference trees

$$
\chi^2=\sum _{i=1}^{C}\sum _{j=1}^{C}{\frac {{\left(n_{ij}-e_{ij}\right)}^{2}}{e_{ij}}}
$$

where $e_{ij} = n p_{i+} p_{+j}$.
...
Boosting/likelihood based methods
