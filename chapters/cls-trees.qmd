...
How well do these models perform relative to previously discussed techniques? Currently, the `r tree_best` model has the smallest Brier score. @fig-cls-tree-compare visualizes the Brier score values for the best candidate in each evaluated model along with 90% Bayesian credible intervals.  
...
#| fig-width: 7
#| fig-height: 4
#| fig-cap: "Comparisons of current forestation models. The Bayesian methods in @sec-compare-resample-bayes were used to estimate the probability distributions for each of the best candidates for each model. The intervals show the 5%, 50%, and 90% quantiles. The bars on the right show the probability of practical equivalence to the current best model, assuming that a difference in Brier scores of 0.01 is practical."
