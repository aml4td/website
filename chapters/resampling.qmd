...
knitr::include_graphics("../premade/resampling-dk.svg")
```

:::

A general data usage scheme that includes resampling. The colors denote the data used to train the model (in tan) and the separate data sets for evaluating the model (colored periwinkle).
...
knitr::include_graphics("../premade/validation-dk.svg")
```

:::

An initial data splitting scheme that incorporates a validation set.
...
knitr::include_graphics("../premade/mc-cv-dk.svg")
```

:::

A schematic of three Monte Carlo cross-validation resamples created from an initial pool of 30 data points.
...
Variance and bias statistics for simulated data using different configurations for Monte Carlo cross-validation. The range of the y-axes are common across similar plots below for different resampling techniques.
...
knitr::include_graphics("../premade/10-fold.svg")

knitr::include_graphics("../premade/10-fold-dk.svg")
```
...
Variance and bias statistics for simulated data using different configurations for $V$-fold cross-validation.
...
knitr::include_graphics("../premade/bootstraps-dk.svg")
```

:::

A schematic of three bootstraps resamples created from an initial pool of 30 data points.
...
Variance and bias statistics for simulated data using different configurations for the bootstrap.
...
boot_tbl <- 
  table_boot_632_sub %>%
  dplyr::select(statistic, brier_class_logistic, brier_class_knn) %>%
  gt() %>%
  fmt_number(
    columns = c(brier_class_logistic, brier_class_knn),
    decimals = 3
  ) %>%
  cols_label(
    statistic = " ",
  ...
  tab_row_group(
    label = "Estimates",
    rows = ind_est[1]:ind_est[2]
  ) %>%
  tab_row_group(
    label = "Intermediates",
    rows = int_est[1]:int_est[2]
  ) %>%
  tab_row_group(
    label = "Final Estimates",
    rows = fin_est[1]:fin_est[2]
  )
...
knitr::include_graphics("../premade/rolling-dk.svg")
```
:::

A schematic of rolling origin resampling for time series data. Each box corresponds to a unit of time (e.g., day, month, year, etc.).
