...
Time series data [@Hyndmanforecast] are sequential observations ordered over time. The most common example is daily stock prices. These data are a special case due to _autocorrelation_: rows of the data set are not independent. This can occur for many different reasons (e.g. underlying seasonal trends, etc.), but it complicates the resampling process in different ways: 
 ...
 - We want to avoid analysis and assessment partitions where older data are in the assessment set (to avoid predicting past data with future data). 
...
We also have the choice to cumulatively expand the assessment data from the start of the training set to the end of the current slice.
...
Like time-series data, the resampling scheme for this type of data emulates the initial splitting process. We could create groupings for the training set using: 

 - Clustering methods to derive partitions of the data where the spatial coordinates are closer to one another than they are to points in other clusters. 
 - A grid of equally sized blocks, rectangular or hexagonal, are created to encompass the training set points. 

From these groupings, procedures that are analogous to the previous variations of cross-validation could be used: 

- Leave-one-group out resampling would have as many resamples as there are groups. The data points from one group are used as the assessment set and the remaining groups are used to train the model. The process repeats for each group. 
- _V_-fold cross-validation allocates each of the groups to one of _V_ meta-groups (e.g., folds). One fold is left out as the assessment set during each of the _V_ iterations. 

Repeated _V_ fold cross-validation can also be used by restarting the grouping process with a different random number seed. 
...
Note that resampling the specific set of top predictors determined from the resamples is not appropriate. The rule to determine the top set should be resampled, not the specific output of that rule.
