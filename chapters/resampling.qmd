...
- The **analysis set** (of size $n_{mod}$) estimates quantities associated with preprocessing, model training, and postprocessing. 
- The **assessment set** ($n_{perf}$)is only used to compute measures of model effectiveness (e.g., RMSE, classification accuracy, etc).  
...
\begin{algorithm}
\begin{algorithmic}
\State $D^{tr}$: training set of predictors $X$ and outcome $y$
\State $B$: number of resamples
\State $M(D^{tr}, B)$: a mapping function to split the data for the $B$ iterations.
\State $f()$: model pipeline 
\Procedure{Resample}{$D, B, f(), M()$}
  \For{$b =1$ \To $B$}
    \State Partition $D^{tr}$ into $D_b^{model}$ and $D_b^{perf}$ using $M_b(D, B)$.
    \State Train model $f$ on the analysis set to produce $\hat{f}_{b}(D_b^{model})$.
    \State Generate assessment set predictions $\hat{y}_b$ by applying model $\hat{f}_{b}$ to $D_b^{perf}$.
