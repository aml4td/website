...
There have been some attempts to de-bias the bootstrap estimates. The "632 rule" uses the standard bootstrap estimate ($\hat{Q}_{bt}$) and the resubstitution estimate ($\hat{Q}_{rsub}$) together: 

$$\hat{Q}_{632} = e^{-1}\; \hat{Q}_{rsub} + (1 - e^{-1})\; \hat{Q}_{bt} = 0.368\:\hat{Q}_{rsub} + 0.632\:\hat{Q}_{bt}$$
@fig-boot-stats shows that there is a significant drop in the bias when using this correction. The 632 estimator combines two different statistical estimates, and we only know the standard error of A. Therefore, the right-hand panel does not show a standard error curve.  

Let's look at an example from one of the simulated data sets. @tbl-bootstrap-bias has the estimator values and their intermediate values. Let's first focus on the values for the column labeld "Logistic." The standard bootstrap estimate was $\hat{Q}_{bt}$ = `r signif(Q_bt, 3)` and repredicting the training set produced $\hat{Q}_{rsub}$ = `r signif(Q_resub, 3)`. The 632 estimate shifts the Brier score downward to $\hat{Q}_{632}$ = `r signif(Q_632, 3)`. This reduces the pessimistic bias; our large sample estimate of the true Brier score is `r signif(Q_true, 3)` so we are closer to that value.

Another technique, the 632+ estimator, factors in a model’s “no-information rate”: the metric value if the predicted and true outcome values were independent. REF gives a formula for this, but it can also be estimated using a permutation approach where we repeatedly shuffle the outcome values and compute the metric.  We will denote this value as $\hat{Q}_{nir}$. For our simulated data set, the Brier score's corresponding estimate is $\hat{Q}_{nir}$ = `r signif(Q_nir, 3)`. For this data set, that is the worst case value for our metric. 
