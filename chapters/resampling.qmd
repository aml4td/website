...
The important idea is that they are measuring performance on training sets similar to ours (and the same size). 
...
Another technique, the 632+ estimator [@Efron1997tc], factors in a model’s “no-information rate”: the metric value if the predicted and true outcome values were independent. The author gives a formula for this, but it can also be estimated using a permutation approach where we repeatedly shuffle the outcome values and compute the metric.  We will denote this value as $\hat{Q}_{nir}$. For our simulated data set, the Brier score's corresponding estimate is $\hat{Q}_{nir}$ = `r signif(Q_nir, 3)`. For this data set, that is the worst case value for our metric. 
...
The simulations studies shown here in @fig-boot-stats replicated that @Molinaro2005p47 found; the 632+ rule has bias properties about the same as 10-fold cross-validation. 
