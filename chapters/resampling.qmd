...
\Procedure{Resample}{$\mathfrak{D}^{tr}, f, M(\mathfrak{D}^{tr}, B)$}
  ...
  \EndFor 
  \State Compute resampling estimate $\hat{Q} = \sum_{b=1}^B \hat{Q}_{b}$.
  \Return $\hat{Q}$.
\Endprocedure
\end{algorithmic}
\end{algorithm}
...
Our training set contains `r format(nrow(forested_train), big.mark = ",")` locations. Across each of the ten folds, every location appears once in the analysis and assessment sets. 
