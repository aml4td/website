...
## The Resampling Process and Nomenclature {#sec-resampling-basics}
...
#| fig-alt: "Flowchart showing a data usage scheme. The original data is split into a training set and test set. The training set is further divided into B resamples, each containing an analysis set and an assessment set. The analysis sets are used to fit models and the assessment sets produce performance estimates that are averaged into an overall resampling estimate."
...
\Procedure{Resample}{$\mathfrak{D}^{tr}, f, M(\mathfrak{D}^{tr}, B)$}
  ...
  \EndFor 
  \State Compute resampling estimate $\hat{Q} = \frac{1}{B}\sum_{b=1}^B \hat{Q}_{b}$.
  \Return $\hat{Q}$.
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
Up until @sec-time-series-resampling, we will assume that each row of the data is independent. We'll see examples of non-independent data in the later sections.

## What is Resampling Trying to Do? {#sec-resampling-goals}
...
#| fig-alt: "Diagram of a three-way data split. The original data is partitioned into three segments: a training set, a validation set, and a test set, shown as adjacent horizontal bars."
...
#| fig-alt: "Schematic showing three Monte Carlo cross-validation resamples from 30 data points. Each resample randomly assigns about 80 percent of samples to the analysis set and the remainder to the assessment set. Some data points appear in multiple assessment sets across resamples."
...
#| fig-alt: "Two-panel line plot showing MCCV simulation results. Left panel shows percent bias versus number of resamples for different analysis set retention proportions; bias decreases as the retention proportion approaches one and is unaffected by the number of resamples. Right panel shows standard error versus number of resamples; precision improves nonlinearly as resamples increase, with diminishing returns beyond about 50 resamples."
mc_bias <-
  stats_mc_cv |>
...
The results of this simulation indicate that, in terms of precision, there is little benefit in including more than 70% of the training set in the analysis set. Also, while more resamples always help, there is little incremental benefit in using more than 50 or 60 resamples (for this size training set). Regarding bias, the decrease somewhat slows down near 80% of the training set retained. Retention proportions of around 75% - 80% might be a reasonable rule of thumb (these are, of course, subjective and based on this simulation).

`r r_comp("resampling.html#sec-cv-mc")`
...
![Schematic of 3-fold cross-validation showing 30 data points assigned to three equally sized folds. Each fold is highlighted in turn as the assessment set while the other two folds form the analysis set.](../premade/three-CV.svg){width=45% fig-align="center"}

![Diagram showing three iterations of 3-fold cross-validation. In each iteration a different fold is held out as the assessment set and the remaining two folds are combined into the analysis set, so every data point is assessed exactly once.](../premade/three-CV-iter.svg){width=60% fig-align="center"}
  ...
  median(
    stats_v_fold_cv$bias[
      stats_v_fold_cv$model == "logistic" &
        ...
        stats_v_fold_cv$.metric == "brier_class"
    ]
  ) *
  100
cv_10_bias <-
...
#| fig-alt: "Two-panel line plot showing V-fold cross-validation simulation results. Left panel shows percent bias versus total resamples for different values of V; bias decreases as V increases, with V equals 5 showing the most bias. Right panel shows standard error versus total resamples; smaller values of V have better precision, and 10-fold CV needs more resamples to match the precision of 5-fold CV."
vfold_bias <-
  stats_v_fold_cv |>
  filter(model == sel_model & .metric == sel_metric) |>
...
#| fig-alt: "Schematic showing three bootstrap resamples from 30 data points. Each analysis set is the same size as the original training set but contains duplicate rows due to sampling with replacement. Unselected data points form the assessment set, which varies in size across resamples."
...
#| fig-alt: "Two-panel line plot showing bootstrap simulation results. Left panel shows percent bias versus number of resamples for the standard, 632, and 632-plus estimators; the standard estimator has substantial positive bias that remains flat regardless of the number of resamples, while the 632 and 632-plus corrections reduce bias considerably. Right panel shows standard error for the standard estimator, which is very low even with few resamples."
...
table_boot_632 <-
  ...
  mutate(
    ...
    type = ifelse(
      statistic %in%
        c("relative overfitting rate", "weights", "no information rate"),
      "intermediates",
      type
    ),
    blank = "   "
...
Q_632p <- table_boot_632$brier_class_logistic[
  table_boot_632$statistic == "632+"
]

denom <- Q_nir - Q_resub
numer <- Q_bt - Q_resub
ror <- table_boot_632$brier_class_logistic[
  table_boot_632$statistic == "relative overfitting rate"
]
wt_632p <- table_boot_632$brier_class_logistic[
  table_boot_632$statistic == "weights"
]
...
ror_knn <- table_boot_632$brier_class_knn[
  table_boot_632$statistic == "relative overfitting rate"
]
wt_632p_knn <- table_boot_632$brier_class_knn[
  table_boot_632$statistic == "weights"
]
...
table_boot_632_sub <-
  ...
  mutate(
    type = factor(
      type,
      levels = c("estimates", "intermediates", "final estimates")
    )
  ) |>
...
The simulation studies shown here in @fig-boot-stats replicated what @Molinaro2005p47 found; the 632+ estimator has bias properties about the same as 10-fold cross-validation. 
...
#| fig-alt: "Schematic of rolling forecasting origin resampling. Each row represents a resample where a fixed-width analysis window slides forward in time, followed by an assessment window of fixed size. Successive resamples shift both windows forward by one time unit, preserving the temporal order of the data."
knitr::include_graphics("../premade/rolling.svg")
```

We also have the choice to cumulatively expand the analysis data from the start of the training set to the end of the current slice.  For example, in Resample 2 of @fig-rolling-scheme, the model would be fit using samples 1-9; in Resample 3, the model would be fit using samples 1-10; and so on.  This approach may be useful when the number of periods of time is smaller and we would like to include as much of the historical data as possible in each resample.
...
`r r_comp("resampling.html#sec-time-series-resampling")`

## Spatial Data {#sec-spatial-resampling}

@sec-spatial-splitting described the problem of spatial correlation between locations and a method for creating an initial split for the forestation data. 
...
- _V_-fold cross-validation: This method assigns each group to one of _V_ meta-groups (e.g., folds). In each of the _V_ iterations, one fold is left out as the assessment set, while the remaining folds are used to train the model.
- Repeated _V_-fold cross-validation: This method involves restarting the grouping process with a different random number seed, allowing for multiple rounds of V-fold cross-validation.

Each of these approaches can be augmented using a buffer to exclude very close neighbors from being used. 

Going back to our forestry data, we previously used hexagonal blocks to split the data (see @fig-forested-split). The same blocking/buffering scheme was re-applied to the training set in order to create a 10-fold cross-validation. Recall that a 25 x 25 grid was used. For the training set, some of these hexagons are missing all of their data (since they were previously added to the test set). Of the remaining grid points, we systematically assign them to the ten folds. @fig-forested-blockcv shows this process for the first fold. The figure shows the blocks of data, along with the corresponding buffer for this iteration of cross-validation. The purple blocks are, for the most part, not adjacent to one another. These are combined into the first assessment set. The other locations that are not in the buffers are pooled into the first analysis set. 

```{r}
#| label: fig-forested-blockcv
#| echo: false
#| fig-cap: An example of one split produced by block cross-validation on the forestation data. The analysis (magenta) and assessment (purple) sets are shown with locations in the buffer in black. The empty spaces represent the data reserved for the test set (see @fig-forested-split).
#| fig-alt: "Interactive map of the Pacific Northwest showing one fold of spatial block cross-validation on the forestation data. Scattered point clusters represent analysis set locations, assessment set locations, and buffer zone locations, each in a distinct shade. The assessment blocks are spatially separated from each other. Empty regions correspond to test set data."
leaflet() |>
...
Our training set contains `r format(nrow(forested_train), big.mark = ",")` locations. Across each of the ten folds, every location appears in one of the assessment sets. 

Just as with standard _V_-fold cross-validation, the number of folds can be chosen based on bias and variance concerns. Unlike standard resampling, any investigation into this topic would depend on a specific spatial autocorrelation structure. As such, we have decided to follow the recommendations for non-spatial data and use _V_=10. If we find that the variation in our statistics estimated from ten resamples is too large, we can employ the same process of repeating the block resampling process using different random number seeds. For our data, the average number of locations in the assessment sets is `r floor(mean(map_int(forested_rs$splits, ~ length(.x$out_id))))` and this appears large enough to produce sufficiently precise performance statistics. 

`r r_comp("resampling.html#sec-spatial-resampling")`
...
`r r_comp("resampling.html#sec-multilevel-resampling")`
...
#### Can/Should I Use the Individual Resampling Results to Change the Model Pipeline? {#sec-resampling-faq-indiv .unnumbered}
...
One important thing to consider is that _you now have a different model pipeline than what was just resampled_. To understand the performance of your newly informed model, you need to resample a model pipeline that _algorithmically_ selects the _X_% of predictors.
...
For the original pipeline, the final predictor set is determined when you fit that pipeline on the training set. The resampling results tell you about what _could_ have happened; the training set results show what _did happen_.

#### What Happens to the *b* Trained Models? Which One Should I Keep? {#sec-resampling-faq-keep .unnumbered}
...
#### Is this some Sort of Ensemble? {#sec-resampling-faq-ensemble .unnumbered}
...
#### Can I Accidentally Resample Incorrectly? {#sec-resampling-faq-error .unnumbered}

Unfortunately, yes. As previously mentioned, improper data usage is one of the most frequent ways that machine learning models silently fail; that is, you won't know that there is a problem until the next set of labeled data. A good example discussed in @sec-selection-overfitting is @Ambroise2002p1493. 
...
#### What is Nested Resampling? {#sec-resampling-faq-nested .unnumbered}

This is a version where an additional layer of resampling occurs. For example, suppose you are using 10-fold cross-validation. Within each of the 10 iterations, you might add 20 bootstrapping iterations (that resample the analysis set). That ends up training 200 distinct models^[We will refer to the first resample (10-fold CV) as the _outer_ resample and the bootstrap as the _inner_ resample.]. 
...
#### Is Resampling Related to Permutation Tests? {#sec-resampling-faq-perm .unnumbered}

Not really. Permutation methods (@sec-permutations) are similar to resampling only in that they perform multiple calculations on different versions of the training set. 
...
#### Are Sub-Sampling or Down-Sampling the Same as Resampling? {#sec-resampling-faq-subsample .unnumbered}

No. These are techniques to modify your training set (or analysis set) to rebalance the data when a classification problem has rare events. They'll be discussed in @sec-cls-imbalances.

#### Why Do I Mostly Hear About Validation Sets? {#sec-resampling-faq-val .unnumbered}

As discussed in @sec-deep-learning, stereotypical deep learning models are trained on _very_ large sets of data. There is very little reason to use multiple resamples in these instances, and deep learning takes up a lot of space in the media and social media. 
