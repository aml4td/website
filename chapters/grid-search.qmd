...
This extra correlation means that a simple ANOVA model cannot be used. Instead, our interim analysis can use a hierarchical random effects model. This is the same methodology used in @sec-effect-encodings for effect encodings. We’ll treat our set of resamples as a random sample of possible resampling indices and then estimate the within-resample variation. The model itself is: 
...
Racing can be an effective tool for screening a large number of models while using comprehensive grids. 
...
For the racing analysis, nested resampling was `r round(knn_sfd_nest_time[3]/knn_sfd_grid_time[3], 1)`-fold slower for the 15-point grid than using a single 10-fold cross-validation. For the larger gird, the slow-down was `r round(large_nest_race_time[3]/large_nest_grid_time[3], 1)`-fold. 
...
bind_rows(
  ...
  mutate(bias = (Brier_Nested_CV - Brier_Single_CV) / Brier_Single_CV * 100) %>%
  kable(digits = 3, col.names = c("Algorithm", rep(c("Brier", "Std. Err."), 2), "Bias (%)")) %>% 
  add_header_above(c(" ", "Single CV" = 2, "Nested CV" = 2, " ")) %>% 
...
Brier scores for two tuning algorithms evaluated with regular 10-fold cross-validation and nested 10-fold cross-validation.
...
both_selections %>% 
    ggplot(aes(neighbors, dist_power)) +
    # geom_point(data = unused_points, pch = 1, alpha = 1 / 6, cex = 3/4) +
    geom_point(aes(col = Algorithm, size = `# Selections`), alpha = 3/4) +
    labs(x = neighbors()$label, y = dist_power()$label) +
