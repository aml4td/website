...
#| fig-alt: "Three scatter plots arranged side by side, each showing 15 candidate points in the space of number of trees versus learning rate on a log scale. The regular grid shows a strict factorial layout with uniform spacing. The random grid shows points scattered with some gaps and clustering. The space-filling design shows points distributed more uniformly across the parameter space than the random grid."
bst_grids |>
...
format_best_bst <- function(x, ...) {
  bst <- select_best(x, ...)
  paste0(
    format(bst$trees, big.mark = ","),
    " trees and a learning rate of 10<sup>",
...
bst_sfd_brier <- signif(
  show_best(bst_sfd_res, metric = "brier_class", n = 1)$mean,
  3
)
bst_sfd_brier_se <- signif(
  show_best(bst_sfd_res, metric = "brier_class", n = 1)$std_err,
  3
)
...
#| fig-alt: "Line plot of Brier score versus number of trees, with separate lines for each learning rate value. A single tree performs poorly across all learning rates. The two larger learning rates show optimal performance at a low to mid number of trees, while the smallest learning rate continues to improve as the number of trees increases, revealing an interaction between the two tuning parameters."
...
#| fig-alt: "Four-panel scatter plot arranged in a two-by-two grid. Rows correspond to the random and space-filling grids; columns show the marginal relationship of each tuning parameter with Brier score. Spline smooths are overlaid on the points. Very small learning rates and very few trees are associated with worse Brier scores, while moderate to large values perform similarly well. The interaction seen in the regular grid is not visible in these marginal views."
bst_irr_grid_res |>
...
#| fig-alt: "Three scatter plots side by side, each showing ten points from a Latin hypercube design in a unit square divided into a ten-by-ten grid. Each row and column of the grid contains exactly one point. The three replicates use different random seeds, producing different spatial arrangements; one replicate clusters points near the diagonal while the others spread them more evenly."
...
Many designs assume that all the tuning parameter values are quantitative. That may not always be the case. For example, K-nearest neighbors can adjust its predictions by considering how far the neighbors are from a new point; more distant points should not have the same influence as close neighbors. A weighting function can be used for this purpose. For example, weights based on the inverse of the distance between neighbors may produce better results. Equal weighting is often called a "rectangular weighting" function. The type of algebraic function used for weighting is a qualitative tuning parameter. 

A simple workaround for creating a space-filling design is to repeat the unique parameter values as if they were $s$ distinct values when making the design. This allows them to be used with a Latin hypercube design, but any space-filling design that uses this approach is not technically optimal. Practically speaking, this is an effective approach for tuning models.  However, there are _sliced_ LHD that can accomplish the same goal [@qian2012sliced;@ba2015optimal].
...
if (
  isTRUE(all.equal(
    best_race[, c("trees", "learn_rate")],
    best_grid[, c("trees", "learn_rate")]
  ))
) {
  best_text <- paste0(
    "The numerically best candidate set for racing and basic grid search were the same: ",
    best_large,
    "."
  )
} else {
  best_text <-
    glue::glue(
      "The numerically best candidate set for racing and basic grid \\
               search were different. Racing chose a model with \\
               {best_race$trees} trees and an exponent value of \\
...
#| fig-alt: "Horizontal confidence interval plot showing the first interim analysis in model racing. Each row represents a candidate configuration ranked by estimated performance. One-sided 95 percent confidence intervals extend from each point estimate toward zero. Candidates whose intervals do not overlap zero are marked for removal, while the remaining candidates continue to the next round of resampling."
brier_param |>
  ggplot(aes(y = candidate)) +
  geom_point(aes(x = estimate), cex = 1 / 2) +
  geom_vline(xintercept = 0, lty = 2) +
  geom_errorbar(
    aes(xmin = conf.low, xmax = estimate, col = Decision),
    alpha = 1 / 3
  ) +
...
#| fig-width: 7
#| fig-height: 4
#| fig-alt: "Line plot of Brier score versus learning rate on a log scale for a boosted tree with 500 trees. The curve decreases as the learning rate increases, reaches a minimum, then rises at high learning rates. A dotted vertical line marks the optimal learning rate. Colored rug marks along the bottom axis show the optimal learning rate found within each individual resample fold, illustrating variation across folds."
autoplot(grid_res) +
  labs(y = "Brier Score") +
...
#| fig-alt: "Two-panel figure for nested resampling. Left panel shows ten semi-transparent lines of Brier score versus learning rate on a log scale, one per inner resample. Filled circles mark each inner resample's optimal learning rate, which varies but falls in a consistent region. Right panel shows paired boxplots comparing inner and outer Brier score estimates, demonstrating similar distributions and confirming that optimization bias is small for this example."
p_trend <-
  nest_grid_lines |>
...
The panel on the right shows boxplots of the Brier scores for the inner resamples and the corresponding outer resample estimates. The standard error of the outer resamples is very similar to the level of noise in the conventional estimate.  
...
> We tuned the learning rate of our boosted classification tree using grid search and found a rate of 10<sup>`r signif(log10(grid_brier$learn_rate), 3)`</sup> to be best. We think that this model has a corresponding Brier score, measured without optimization bias, of `r signif(nest_grid_brier$mean, 3)`. 
