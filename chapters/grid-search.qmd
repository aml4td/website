...
If there is a single tuning parameter, as in the learning rate example of @sec-grid-and-sequential, each candidate takes a scalar value (quantitative or qualitative). There are multiple tuning parameters in other cases, such as the K-nearest neighbors example shown below. In this case, a candidate is multi-dimensional. 

The previous chapter demonstrated that using external data to evaluate the model is crucial (be it resampling or a validation set). There is no free lunch in grid search: we cannot simply fit the model and evaluate it by predicting the same data. 

To formalize the process, @alg-grid-search illustrates the process. For a model with $m$ tuning parameters, we let $\Theta$ represent the collection of $s$ candidate values. For each specific value ($\theta_j$), we resample the model to produce some measure of efficacy (e.g., $R^2$, accuracy, etc.)^[This process was illustrated in [Algorithm 9.1](tmp_resampling.html#alg-resampling).]. From there, the best value is chosen or additional work is carried out to find a suitable candidate. 
...
\begin{algorithm}
\caption{Grid search for tuning parameter optimization}
\begin{algorithmic}
...
knn_prm <-
  parameters(
    neighbors(c(1, 50)),
    dist_power(),
    weight_func(c("rectangular", "inv"))
  )

grid_seed <- 5

knn_reg <-
  grid_regular(knn_prm, levels = c(4, 3, 2)) %>%
  mutate(grid = "Regular")

sfd_size <- nrow(knn_reg)

vals <- map(knn_prm$object, ~ rep_len(value_seq(.x, 25), length.out = 25))
...
knn_sfd <- 
  get_design(3, 25, type = "Audze-Eglais") %>% 
  update_values(vals) %>% 
...
knn_grids <- 
  ...
  mutate(
    ...
    weight_func = ifelse(weight_func == "inv", "inverse", weight_func),
    `Distance Weighting Function:` = weight_func,
  )
```

To demonstrate, this chapter will initially focus on grids for a K-nearest neighbors (KNN) model with three tuning parameters^[These are described in more details in @sec-cls-knn]: 
...
We'll illustrate different strategies using this model setup with different configurations of these three tuning parameters. 
...
A regular grid starts with a sequence or set of values for each individual tuning parameter and then creates all combinations. In statistics, this is referred to as a factorial design. The number of values per tuning parameter does not have to be the same. To illustrate a regular grid, we'll use four values of the number of neighbors, all three weight functions, and two values of the distance order. This grid of `r nrow(knn_reg)` candidates is shown in @fig-knn-grids. Each weight function occurs at the same values as the other two parameter values. The grid covers the entire space with significant gaps in-between. 
...
#| fig-width: 8.5
#| fig-height: 5
#| fig-cap: Three types of grids illustrated with tuning parameters from a K-nearest neighbor model. The regular grid shows 24 points; the candidates for the two values of the weight function overlap on the plot. The space-filling design was created using the MaxiMin approach defined in @sec-irregular-grid.
...
knn_grids %>%
  ggplot(
    aes(
      neighbors, 
      dist_power, 
      col = `Distance Weighting Function:`, 
      pch = `Distance Weighting Function:`
    )
  ) +
  geom_point(cex = 3, alpha = 2 / 3, show.legend = FALSE) +
  facet_grid(`Distance Weighting Function:` ~ grid) +
...
format_best_knn <- function(x, ...) {
  ...
  paste0(
      bst$neighbors, " neighbors, ",
      wts, " weights, and a Minkowski distance parameter of ", 
...
The primary downside to regular grids is that, as the number of tuning parameters increases, the number of points required to fill the space becomes extremely large (due to the curve of dimensionality). However, for some models and/or pre-processing methods, regular grids can be very efficient despite the number of tuning parameters (see the next section that describes the "submodel trick"). 
...
#| fig-width: 9
#| fig-height: 3
#| fig-cap: Three replicate Latin hypercube sampling designs of size five for for two parameters. Each design uses different random numbers. The grid lines illustrate the 25 hypercubes used to generate the values. 
...
For example, we could choose points to maximize the minimum pairwise distances between the candidates. These designs are usually referred to as MaxiMin designs [@pronzato2017minimax]. Comparing the two irregular designs in @fig-knn-grids, the random grid has a maximum minimum distance value of `r rnd_maxmin` while the corresponding space-filling design's value (`r mm_maxmin`) is `r round(mm_maxmin/rnd_maxmin, 1)`-fold larger^[Using Euclidean distance.]. The latter design was optimized for this criterion and we can see that there is far less redundancy in the optimized design. 

Another approach, maximum entropy sampling, selects points based on assumptions related to the distributions of the tuning parameters and their covariance matrix [@shewry1987maximum;@joseph2015maximum]. 
...
The lower panel of @fig-knn-irregular-grid-tuning shows the tuning results for the MaxiMin design. The results are somewhat cleaner than the random grid results but would lead to the same conclusions. Here, the best candidate was nearly identical to the random grid: `r knn_sfd_best`. 

The advantage of space-filling designs over random designs is that, for smaller designs, the candidates do a better job covering the space and have a low probability of producing redundant points. Also, it is possible to create a space-filling design so that the candidates for each numerical parameter are equally spaced (as was done in @fig-knn-grids). 

Overall, we recommend space-filling designs since they are more efficient than regular designs. Regular designs have a lot of benefit when using an unfamiliar modeling methodology since you will learn a lot more about the nuances of how the tuning parameters affect one another. 
...
Fortunately, none of these `r format(nrow(sim_folds_rep) * sfd_size, big.mark = ",")` models depend on one another and can be computed separately. Since almost all modern computers have multiple CPUs and GPUs, we can break the computations into different "chunks" of work and execute them simultaneously on different processors (or different computers entirely). The parallel processing of models can lead to a remarkable reduction of the time it takes to tune using grid search.
...
There are some nuances related to parallel processing. It is a good idea to parallelize the "longest loop" (literally or figuratively). 
...
Also, some models can exploit the "submodel trick" where a single training model can make predictions for many tuning parameter candidates. For example, the glmnet model (@sec-penalized-logistic-regression) can compute the model coefficients for _all_ penalty values. If we were to tune the penalty value, a single model fit can be used to evaluate numerous candidate values (assuming all of the other tuning parameters are the same). For a regular grid, this can effectively drop a dimension of the computations. There are other models and pre-processors that can have this quality, including boosted trees (@sec-boosting-cls), partial least squares (@sec-colinear-projections), principal component feature extraction (@sec-linear-feature-extraction), and others. 
