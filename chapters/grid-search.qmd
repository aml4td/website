...
\For{$j=1$ \To $m$}
  \State Generate $\hat{Q}_{j} =$ \Call{Resample}{$D, B, f(\cdot; \theta_j)$} corresponding to candidate $\theta_j$.
\EndFor
...
A regular grid starts with a sequence or set of values for each tuning parameter and then creates all combinations. In statistics, this is referred to as a factorial design. The number of values per tuning parameter does not have to be the same. 
...
\State $\Theta$: Parameter set ($s \times m$) with candidates $\theta_j$
\For{$j=1$ \To $m$}
  \State Generate $\hat{Q}_{j} =$ \Call{Resample}{$D, B_{min}, f(\cdot; \theta_j)$} corresponding to candidate $\theta_j$.
\EndFor
\State Eliminate candidates so that only $s_i$ remain. 
  \For{$k = B_{min} + 1$ \To $B$} 
    \State Generate $\hat{Q}_j^k =$ \Call{Resample}{$D, k, f(\cdot; \theta_j)$} corresponding to candidate $\theta_j$ as of resample $k$.
