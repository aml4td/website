...
For the racing analysis, nested resampling was `r round(large_nest_race_time[3]/large_race_time[3], 1)`-fold slower than using a single 10-fold cross-validation. 


```{r}
#| label: tbl-nested-cv
#| echo: false
#| tbl-cap: Brier scores for two tuning algorithms evaluated with regular 10-fold cross-validation and nested 10-fold cross-validation.
bind_rows(
  knn_sfd_res %>% 
  ...
  kable(digits = 3, col.names = c("Algorithm", rep(c("Brier", "Std. Err."), 2))) %>% 
...
#| fig-width: 5
#| fig-height: 5.2
#| fig-cap: KNN space filling design with the frequency that points selected during nested resampling for a complete grid and with a large grid combined with racing. Empty circles represent points that were never selected.

both_selections %>% 
    ggplot(aes(neighbors, dist_power)) +
    geom_point(data = unused_points, pch = 1, alpha = 1 / 6, cex = 3/4) +
    geom_point(aes(col = Algorithm, size = `# Selections`), alpha = 1) +
    labs(x = neighbors()$label, y = dist_power()$label) +
    coord_fixed(ratio = 50) +
