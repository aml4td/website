...
The results of this process indicates that the smallest Brier score is achieved with a learning rate of 10<sup>`r signif(log10(grid_brier$learn_rate), 3)`</sup>. The Brier score that corresponds to this tuning parameter values is estimated to be `r signif(grid_brier$mean, 3)`.
...
#| fig-width: 7
#| fig-height: 4
#| fig-cap: Basic grid search for learning rate with 100 grid points. 
...
adjusted the Brier score from `r signif(debiased_grid$mean, 3)` to `r signif(debiased_grid$adjusted, 3)`. 

@boulesteix2009optimal and maybe @tibshirani2018excess
...
p_diff <- 
  two_levels %>% 
  ggplot(aes(level, brier, group = id, col = id)) + 
  geom_point(show.legend = FALSE) +
  geom_path(show.legend = FALSE, , linewidth = 1 /2 , alpha = 1 / 3) + 
  ylim(brier_range) +
  labs(y = NULL, x = NULL) + 
...
The nested resampling estimate of the Brier score was estimated as `r signif(nest_grid_brier$mean, 3)`.

The computational costs of nested resampling of this basic grid search was `r sprintf("%2.1f", grid_nest_time[3]/grid_time[3])`-fold.
...
@tbl-nested-cv shows how the performance statistics differs between a single 10-fold cross-validation and two-level nested cross-validation. For this scenario, the Brier score estimates are negligible worse for nested cross-validation. However, notice that the standard errors are very similar since the same external resampling indices are used. The percent bias from single resampling was ` nested_stats$bias[1]`%. This values is very small but lack any sense of uncertainty. The final column of the table shows the raw difference in Brier scores divided by the standard error to form a signal-to-noise ratio (SNR). This shows that the bias is small relative to the experimental noise^[To conceptualize this, many hypothesis testing statistics are signal-to-noise ratios. For example, to assess the simple difference between means, the basic t-test would probably use an SNF cutoff between 1.6 and 2 to be thought of as a “significant” signal.]. The computational costs of nested resampling of basic grid search and racing were ` sprintf("%2.1f", sfd_nest_time[3]/sfd_time[3])`-fold  and ` sprintf("%2.1f", large_nest_race_time[3]/large_race_time[3])`-fold slower, respectfully.
