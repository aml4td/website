...
set.seed(943)
sim_tr <- sim_logistic(5000, sim_f)
...
seq_ord <- format(sort(unique(bst_reg$learn_rate)))
...
To demonstrate, this chapter will initially focus on grids for a K-nearest cost model with two tuning parameters^[These are described in more detail in @sec-cls-boosting]. Nearest cost were initially described in @sec-isomap for multidimensional scaling embeddings. Supervised KNN models predict a new point by finding the _K_ most similar data point in the training set. It then uses the _K_ outcome values to form a prediction for the new data. The number of cost is a tuning parameter. For our example, we’ll consider 2 to 50 cost. Another parameter focuses on how to compute the distance. There are a lot of different choices, and several commonly used distance metrics are special cases of Minkowski distance:
...
To illustrate a regular grid, we'll use `r xfun::numbers_to_words(length(seq_tree))` values of the number of cost (K = `r txt_tree`) and `r xfun::numbers_to_words(length(seq_ord))` values of the distance order ($\phi$ = `r txt_ord`). This grid of `r nrow(bst_reg)` candidates is shown in @fig-bst-grids. The grid covers the entire space with significant gaps in between.  
...
format_best_bst <- function(x, ...) {
  bst <- select_best(x, ...)
  paste0(
      bst$trees, " trees and and a learning rate of 10<sup>",
...
We’ll use the simulated training set in @sec-complexity-overfitting with the same 10-fold cross-validation scheme described there. Once again, Brier scores were used to measure how well each of the `r nrow(bst_reg)`  configurations of KNN predicted the data. @fig-bst-regular-grid-tuning shows the results: a small number of nearest cost is a poor choice (due to overfitting), and the Minkowski distance parameter matters little for these data. Even though this is not a diverse set of candidate values, we can probably pick out a reasonable candidate with a small Brier score, such as `r bst_reg_best` (although any of the values of $\phi$ would be good choices). 
...
#| fig-width: 4
#| fig-height: 3.5
#| fig-cap: "The tuning results for the regular grid shown in @fig-bst-grids."
...
For this example, the patterns for the number of cost do not change across values of the Minkowski distance parameter (i.e., there is no interaction). If this were not the case, the relationship between these parameters and performance would be more difficult to understand in marginal effect plots. Examples of this will be seen in later chapters, such as in TODO and TODO.
...
For example, @fig-bst-grids shows a `r sfd_size` point “Audze-Eglais“ space-filling design. The space is covered more compactly than the regular design of the same size and is much more uniform than the `r sfd_size` point random design. The lower panels of @fig-bst-irregular-grid-tuning show the tuning results. The results are somewhat cleaner than the random grid results but would lead to the same conclusions. Here, the best candidate was similar to those found by the random grid: `r bst_sfd_best`. 
...
rnd_maxmin <-
  encode_set(bst_random %>% select(-grid), pset = bst_prm, as_matrix = TRUE) %>% 
  DiceDesign::mindist() %>% 
  round(2)

sfd_maxmin <-
  encode_set(bst_sfd %>% select(-grid), pset = bst_prm, as_matrix = TRUE) %>% 
  DiceDesign::mindist() %>% 
  round(2)

reg_maxmin <-
    encode_set(bst_reg %>% select(-grid), pset = bst_prm, as_matrix = TRUE) %>% 
    DiceDesign::mindist() %>% 
    round(2)
```
...
large grid covering the same space with the same number of grid points had a `r round(large_grid_time[3]/large_grid_reg_time[3], 1)`-fold speed-up. 
...
if (isTRUE(all.equal(best_race[, c("trees", "learn_rate")], best_grid[, c("trees", "learn_rate")]))) {
  best_text <- "The numerically best candidate set for racing and basic grid search were the same."
} else {
  best_text <- 
...
To demonstrate the racing procedure, a space-filling design with `r format(nrow(large_grid), big.mark = ",")` candidates was created and evaluated with racing. If we were starting from scratch and, since racing can be more efficient, we might expand the hyperparameter space to be $K \in \{1, 100\}$ and $\phi \in \{0.5, 2.5\}$. The folds were randomized, and after `r control_race()$burn_in` resamples, the ANOVA method was used to analyze the results. @fig-racing-initial shows the one-sided 95% confidence intervals for the loss of Brier score relative to the current best configuration (`r initial_resamples$trees[1]` trees and a learnig rate of 10<sup>`r log10(initial_resamples$learn_rate[1])`</sup>). `r num_removed` were eliminated at this round. 
...
#| fig-width: 6
#| fig-height: 4.25
#| fig-cap: The first interim analysis in racing using a larger space-filling design.
...
In the end, the racing process eliminated all but `r num_remaining` candidates. The process eliminated candidates at iterations `r loss_text[[1]]`. In all, `r num_fit` models were fit out of the possible `r format(nrow(large_grid) * nrow(sim_rs), big.mark = ",")` (`r round(num_fit / (nrow(large_grid) * nrow(sim_rs)) * 100, 1)`%). The speed-up relative to sequential grid search was `r round(large_seq_time[3]/large_race_time[3], 1)`-fold and `r round(large_grid_time[3]/large_race_time[3], 1)`-fold relative to parallel processing with basic grid search^[Since this same size is small and KNN models are very efficient (for this data size), there is a limited computational efficiency gained by using racing. However, @kuhn2014futility, @tmwr [Section 15.4](https://www.tmwr.org/workflow-sets#racing-example), and other references demonstrate how fast racing can be for screening or optimizing models.]. `r best_text`
...
Looking back at the analysis of the space-filling design analysis shown in @fig-bst-irregular-grid-tuning, we found that the numerically best candidate value was `r bst_sfd_best`. The corresponding resampling estimate of the Brier score was `r bst_sfd_brier` with a standard error of `r bst_sfd_brier_se`. If someone were to ask us how well the KNN model performs, we would probably give them these estimates. 
...
bst_sfd_nest_metrics <- collect_metrics(sfd_nest_res)
bst_sfd_nest_brier <- signif(bst_sfd_nest_metrics$mean, 3)
bst_sfd_nest_brier_se <- signif(bst_sfd_nest_metrics$std_err, 3)

sfd_selected_configs <- 
  sfd_nest_res %>%
  select(.selected) %>%
  unnest(.selected) %>%
  count(trees, learn_rate, name = "# Selections") %>%
  mutate(Algorithm = "Basic Grid Search")

bst_race_nest_metrics <- collect_metrics(bst_race_nest_res)
bst_race_nest_brier <- signif(bst_race_nest_metrics$mean, 3)
bst_race_nest_brier_se <- signif(bst_race_nest_metrics$std_err, 3)

race_selected_configs <- 
  bst_race_nest_res %>%
  select(.selected) %>%
  unnest(.selected) %>%
  count(trees, learn_rate, name = "# Selections") %>%
  mutate(Algorithm = "Racing")

both_selections <- 
  sfd_selected_configs %>% 
  bind_rows(race_selected_configs) 

all_grid_points <- 
  bind_rows(bst_sfd, large_grid) %>% 
  distinct(trees, learn_rate)

unused_points <- 
  anti_join(
    all_grid_points, 
    both_selections %>% select(trees, learn_rate), 
    by = c("trees", "learn_rate"))
...
@fig-nested-cv visualizes the selected candidates for each of the ten outer resamples. Since some of the grid points are selected more than once, the image uses the size of the points to illustrate this. Also, the figure’s axis ranges portrays the entire tuning space for the two parameters (based on the larger grid). The y-axis variation is quite large, futher illustrating the lack of importance of the $\phi$ parameter. For the number of cost, relatively small number of cost appears to be optimal more often.
...
#| fig-width: 5
#| fig-height: 5.2
#| fig-cap: KNN space filling design with the frequency that points selected during nested resampling for a complete grid and with a large grid combined with racing. The range of each axis shows the entire hyperparameter space.

bst_rngs <- map(large_param$object, ~ as.numeric(.x$range))

both_selections %>%
  ggplot(aes(trees, learn_rate)) +
  geom_point(aes(col = Algorithm, pch = Algorithm, size = `# Selections`), alpha = 3/4) +
  labs(x = trees()$label, y = learn_rate()$label) +
  lims(x = bst_rngs[[1]]) +
  # coord_fixed(ratio = diff(bst_rngs[[1]]) / diff(bst_rngs[[2]])) +
  scale_y_log10(limits = 10^bst_rngs[[2]]) +
  scale_size(breaks = 1:max(both_selections$`# Selections`), 
             range = c(1, 5)) +
...
@tbl-nested-cv shows how the performance statistics differs between a single 10-fold cross-validation and two-level nested cross-validation. For basic grid search and racing, the Brier score estimates are worst for nested cross-validation (as expected). However, notice that the standard errors are very similar since the same external resampling indices are used. The percent bias from single resampling was `r nested_stats$bias[1]`%. These values are fairly substantial but lack any sense of uncertainty. The final column of the table shows the raw difference in Brier scores divided by the standard error to form a signal-to-noise ratio (SNR). This shows that the bias is small relative to the experimental noise^[To conceptualize this, many hypothesis testing statistics are signal-to-noise ratios. For example, to assess the simple difference between means, the basic t-test would probably use an SNF cutoff between 1.6 and 2 to be thought of as a “significant” signal.]. 
...
nested_stats %>%
  kable(digits = 3, 
        col.names = 
          c("Resampling", "Mean", "Std. Err.", "Bias (%)", "SNR")) %>% 
...
The computational costs of nested resampling of basic grid search and racing were `r sprintf("%2.1f", sfd_nest_time[3]/sfd_time[3])`-fold  and `r sprintf("%2.1f", large_nest_race_time[3]/large_race_time[3])`-fold slower, respectfully.
