...
bst_grids_p +
  geom_point(cex = 2, alpha = 2 / 3, col = dark_data) +
  thm_dk
```

:::

Three types of grids (in columns) are illustrated with tuning parameters from a boosting model. Each grid contains 15 candidates. The space-filling design was created using the Audze-Eglais method described in @sec-irregular-grid.
...
format_best_bst <- function(x, ...) {
  bst <- select_best(x, ...)
  paste0(
      format(bst$trees, big.mark = ","), " trees and a learning rate of 10<sup>",
...
::: {#fig-bst-regular-grid-tuning}

::: {.figure-content}

```{r}
#| label: bst-regular-grid-tuning
#| renderings: [light, dark]
#| echo: false
#| out-width: 50%
#| fig-width: 4
...
The boosted tree tuning results for the regular grid shown in @fig-bst-grids.
...
::: {#fig-bst-irregular-grid-tuning}

::: {.figure-content}

```{r}
#| label: bst-irregular-grid-tuning
#| renderings: [light, dark]
#| echo: false
#| out-width: 80%
#| fig-width: 7
...
The tuning results for the irregular grids shown in @fig-bst-grids. The lines are spline smooths.
...
lhs_p + 
  geom_point(cex = 1.5, col = dark_data) +   
  thm_dk
```

:::

Three replicate Latin hypercube sampling designs of size ten for two parameters. Each design uses different random numbers. The grid lines illustrate the 100 hypercubes used to generate the values.
...
race_p + 
  geom_point(aes(x = estimate), cex = 1 / 2, col = dark_data) +
  geom_vline(xintercept = 0, lty = 2) +
  geom_errorbar(aes(xmin = conf.low, xmax = estimate, col = Decision), 
                alpha = 1 / 3, col = dark_data) +
  thm_dk
```
:::

The first interim analysis in racing using a larger space-filling design for the boosted tree model.
...
lr_p <- 
  ...
  labs(y = "Brier Score", x = "Learning Rate") +
  scale_x_log10()+
  geom_rug(
...
lr_p +
  geom_vline(xintercept = grid_brier$learn_rate, lty = 3, col = "red") +
  geom_point(col = dark_data) +
  thm_dk
```
...
p_diff_dk <- 
  p_diff + 
  geom_boxplot(col = dark_data, fill = dark_bg) +
  geom_point(alpha = 2 / 5, col = "#F28500", cex = 2) +
  dk_gif_thm

((p_trend + thm_lt) + (p_diff_lt) +  
    plot_layout(widths = c(5, 1)))

((p_trend + thm_dk + (p_diff_dk)) +  
    plot_layout(widths = c(5, 1))) 
```

:::

Each line corresponds to a complete resampled grid search that uses the inner resamples. The points indicate the estimates of the optimal learning rate as determined by the inner results. The panel on the right contrasts the Brier score for the inner assessment set with the outer assessment sets.
...
server <- function(input, output) {
	...
	output$contours <- renderPlot({
	  ...
	  # Not coloring the area around plot
	  if(input$mode == "light") {
	    thm_current <- 
	      thm_lt + 
	      theme(plot.background = element_rect(fill = light_bg, color = light_bg))
	  } else {
	    thm_current <- 
	      thm_dk + 
	      theme(plot.background = element_rect(fill = dark_bg, color = dark_bg))
	  }
		...
		p <- ggplot(grid, aes(A, B)) +
			coord_equal() +
			labs(x = "Predictor 1", y = input$mode) +
			thm_current
