...
How can we estimate the potential bias in our performance metric by "picking the winner" and citing its resampled estimator?  @tibshirani2009bias  describes a simple analytical estimator applicable when multiple resamples are used to estimate performance. First, we obtain the performance estimates corresponding to the optimal tuning parameter candidate _within each fold_. Let's call that $\hat{Q}_j^*$ (where $j=1\ldots B$) and denote $\hat{Q}_j^{opt}$ as the performance value associated with the overall best candidate value (determined in the usual way). The difference between these values is an estimate of the bias in optimization.  If different resamples have very different optimal performance metrics compared to the optimal performance determined using the averages of the resamples (as shown in @fig-1d-boost), bias can increase. The estimate of the bias is then: 

$$\widehat{bias} = \frac{1}{B}\sum_{j=1}^B\left[\hat{Q}_j^{opt} - \hat{Q}_j^*\right]$$
For this particular example, the bias is fairly small (`r format(debiased_grid$mean_bias, digits = 3, scientific = FALSE)` with standard error `r format(debiased_grid$std_err_bias, digits = 3, scientific = FALSE)`). To correct the conventional estimator, we add the bias; the Brier score is adjusted from `r signif(debiased_grid$mean, 3)` to `r signif(debiased_grid$adjusted, 3)`. A more complex analytical estimate can be found in @tibshirani2018excess.

We can also estimate optimization bias with more complex resampling schemes. Recall that the issue is that we are overextending resampling by estimating performance for individual candidates as well as the optimal performance for that candidate. Nested resampling [@varma2006bias;@boulesteix2009optimal] prevents overuse by using two layers of resampling: the "inner resamples" are used to estimate the optimal candidate values, and the "outer resamples" estimate performance at those values. 
...
The process starts with the inner resamples. The same model tuning procedure is used (basic grid search in our example), and each of the inner resamples estimates its own optimal candidate $\hat{\theta}_k$. The outer resample takes this candidate value and estimates its performance using the assessment sets from the outer resampling loop. These outer estimates are averaged to produce the nested resampling estimate of performance. 
