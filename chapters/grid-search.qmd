...
For example, take the KNN space-filling design with `r nrow(knn_sfd)` candidates evaluated across `r nrow(sim_rs)` resamples. We spread these models  `r nrow(knn_sfd) * nrow(sim_rs)` model fits across ten worker processes (on the same computer). Despite the meager computation costs of training each KNN model, there was still a `r knn_speedup`-fold speedup (`r round(max(knn_times$time), 1)`s versus `r round(min(knn_times$time), 1)`s) when run in parallel. 
...
num_remaining <- knn_large_race_res %>% collect_metrics() %>% nrow()
num_fit <- nrow(map_dfr(knn_large_race_res$.metrics, I))

loss_text <- knn_large_race_res %>% 
...
best_race <- show_best(knn_large_race_res, metric = "brier_class", n = 1) 
best_grid <- show_best(knn_large_par_res, metric = "brier_class", n = 1) 

if (isTRUE(all.equal(best_race[, c("neighbors", "dist_power")], best_grid[, c("neighbors", "dist_power")]))) {
  ...
  best_text <- 
    glue::glue("The numerically best candidate set for racing and basic grid \\
               ...
               {best_race$neighbors} neighbors and a exponent value of \\
               {round(best_race$dist_power, 3)}. Full grid search chose \\
               {best_grid$neighbors} neighbors and an exponent of \\
               {round(best_grid$dist_power, 3)}. The difference in Brier \\
...
initial_resamples <- 
  ...
  unnest(.metrics) %>% 
  select(id, .config, .estimate, neighbors, dist_power)
...
knn_race_nest_metrics <- collect_metrics(knn_race_nest_res)
knn_race_nest_brier <- signif(knn_race_nest_metrics$mean, 3)
knn_race_nest_brier_se <- signif(knn_race_nest_metrics$std_err, 3)
...
all_grid_points <- 
  bind_rows(knn_sfd, large_grid) %>% 
  distinct(neighbors, dist_power)

unused_points <- 
  anti_join(
    all_grid_points, 
    both_selections %>% select(neighbors, dist_power), 
    by = c("neighbors", "dist_power"))

nested_stats <- 
  bind_rows(
  knn_sfd_res %>% 
    show_best(metric = "brier_class", n = 1) %>% 
    mutate(Algorithm = "Basic Grid Search", `Estimation Method` = "Single_CV"),
  knn_sfd_nest_metrics %>% 
    mutate(Algorithm = "Basic Grid Search", `Estimation Method` = "Nested_CV"),
  knn_large_race_res %>% 
    show_best(metric = "brier_class", n = 1) %>% 
    mutate(Algorithm = "Racing", `Estimation Method` = "Single_CV"),
  knn_race_nest_metrics %>% 
    mutate(Algorithm = "Racing", `Estimation Method` = "Nested_CV")
) %>% 
  select(Brier = mean, std_err, Algorithm, `Estimation Method`) %>% 
  pivot_wider(
    id_cols = c(Algorithm),
    names_from = c(`Estimation Method`),
    values_from = c(Brier, std_err)
  ) %>% 
  select(Algorithm, Brier_Single_CV, std_err_Single_CV, 
         Brier_Nested_CV, std_err_Nested_CV) %>% 
  mutate(
    bias = (Brier_Nested_CV - Brier_Single_CV) / Brier_Single_CV * 100,
    bias = round(bias, 2),
    SNR = signif((Brier_Nested_CV - Brier_Single_CV) / std_err_Single_CV, 3)
  )
```

Let’s use two of the previous examples from this chapter to demonstrate nested rsampling: the space-filling design with `r sfd_size` candidates and the larger grid used with racing. Each used the same 10-fold cross-validation indices. We’ll use this as the outer resampling method and, within each, we’ll create 10-fold inner cross-validations based on each of the 90% analysis sets of the outer reamples. 

@fig-nested-cv visualizes the selected candidates for each of the ten outer resamples. Since some of the grid points are selected more than once, the image uses the size of the points to illustrate this. Also, the figure’s axis ranges portrays the entire tuning space for the two parameters (based on the larger grid). The y-axis variation is quite large, futher illustrating the lack of importance of the $\phi$ parameter. For the number of neighbors, relatively small number of neighbors appears to be optimal more often.
...
both_selections %>% 
  ...
  labs(x = neighbors()$label, y = dist_power()$label) +
  lims(x = c(1, 100), y = c(0.5, 2.5)) +
  coord_fixed(ratio = 50) +
  scale_size(breaks = 1:3, range = c(1, 5)) +
  theme(legend.direction='horizontal', legend.box = "vertical") +
  scale_color_brewer(palette = "Set1") + 
  scale_shape_manual(values = c(0, 2))
```

@tbl-nested-cv shows how the performance statistics differs between a single 10-fold cross-validation and two-level nested cross-validation. For basic grid search and racing, the Brier score estimates are worst for nested cross-validation (as expected). However, notice that the standard errors are very similar since the same external resampling indices are used. The percent bias from single resampling ranges between `r floor(min(nested_stats$bias))`% and `r ceiling(max(nested_stats$bias))`%. These values are fairly substantial but lack any sense of uncertainty. The final column of the table shows the raw difference in Brier scores divided by the standard error to form a signal-to-noise ratio (SNR). This shows that the bias is relatively small relative to the experimental noise^[To conceptualize this, many hypothesis testing statistics are signal-to-noise ratios. For example, to assess the simple difference between means, the basic t-test would probably use an SNF cutoff between 1.6 and 2 to be thought of as a “significant” signal.]. 
...
nested_stats %>%
  kable(digits = 3, 
        col.names = 
          c("Algorithm", rep(c("Brier", "Std. Err."), 2), "Bias (%)", "SNR")) %>% 
  add_header_above(c(" ", "Single CV" = 2, "Nested CV" = 2, " " = 2)) %>% 
...
The computational costs of nested resampling of basic grid search and racing were `r sprintf("%2.1f", knn_sfd_nest_time[3]/knn_sfd_grid_time[3])`-fold  and `r sprintf("%2.1f", large_nest_race_time[3]/large_nest_grid_time[3])`-fold slower, respectfully.
