...
where $A$ is a matrix that translates or embeds the original features into a potentially lower dimensional space without losing much information. It is easy to see why this matrix operation is linear when the elements of the $X$ and $A$ matrices are expanded.  For example, the first score for the $i^{th}$ sample would be:

$$ 
x^*_{i1} = a_{11} x_{i1} + a_{21}x_{i2} + \ldots +  a_{p1}x_{ip} 
$$ {#eq-pca-linear-combo}

The $a_{ij}$ values are often referred to as the _loadings_ for each predictor. 
...
#### What to know before PCA {.unnumbered}
...
#### How does it do it? {.unnumbered}

deflation description; only works with correlation
...
#### How to incorporate it {.unnumbered}

tuning; interpretability, re-standardize/decending size of components; useful for EDA

scaling plots of components and their general scale (re-normalize)? 

num comp/scree; onl works with correlation in predictors
