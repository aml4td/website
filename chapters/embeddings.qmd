...
barley_base <- recipe(barley ~ ., data = barley_train) %>%
...
(barley_p + theme_transparent()) / (spect_p + theme_transparent())

(barley_p + dk_thm) / (spect_p + dk_thm)
```

:::

(a) The distribution of the outcome for the entire data set. The bar colors reflect the percent barley distribution and are used in subsequent sections. (b) Selected training set spectra for four barley samples. Each line represents the set of 550 predictors in the data.
...
barley_rec <- recipe(barley ~ ., data = barley_train) %>%
  step_zv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  prep()
...
barley_linear_trans_base <- recipe(barley ~ ., data = barley_preproc)

barley_pca <- barley_rec %>%
...
barley_cumulative_variance <- tidy(
  barley_pca_estimates,
  id = "PCA",
  type = "variance"
) %>% # OR number=3.  Do the same for ICA
...
barley_norm_rec <- recipe(barley ~ ., data = barley_train) %>%
  step_zv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
  prep()
...
barley_ica <- recipe(barley ~ ., data = barley_train) %>%
  step_zv(all_predictors()) %>%
...
#| out-width: 100%
#| dev: png
barley_ord_orig_p <- barley_norm_train %>%
...
light_barley_ord_orig_p <- barley_ord_orig_p +
  scale_color_viridis(option = "viridis") +
  theme_transparent()

dark_barley_ord_orig_p <- barley_ord_orig_p +
  scale_color_viridis(option = "viridis") +
  dk_thm
```
...
knitr::include_graphics("../premade/anime_barley_pca_dark.gif")
```
...
A scree plot of the PCA component number versus the percent of variance explained for the first 10 components of the barley data.
...
source(
  "https://raw.githubusercontent.com/aml4td/website/main/R/shiny-linear-scores.R"
)
...
#| cache: true
#| warning: false
barley_ica <- barley_linear_trans_base %>%
...
barley_ica_data_for_pairs_plot <- barley_ica_data %>%
  ...
  mutate(
    barley_bin = factor(barley_bin, levels = c("Q1", "Q2", "Q3", "Q4"))
  ) %>%
  dplyr::select(-barley)

barley_linear_trans_base <- recipe(barley ~ ., data = barley_preproc)

barley_pls <- barley_linear_trans_base %>%
...
barley_pca_data_long <- barley_pca_data %>%
...
barley_ica_data_long <- barley_ica_data %>%
...
#| cache: true
#| warning: false
barley_pls_data_long <- barley_pls_data %>%
...
for_comparison_figure <- bind_rows(
...
component1_correlations <- tibble(
...
barley_pca_data_2_comp <- barley_pca_data %>%
  dplyr::select(barley, PC01, PC02) %>%
  rename(Comp1 = PC01, Comp2 = PC02) %>%
  mutate(Method = "PCA")

barley_ica_data_2_comp <- barley_ica_data %>%
  dplyr::select(barley, IC01, IC02) %>%
  rename(Comp1 = IC01, Comp2 = IC02) %>%
  mutate(Method = "ICA")

barley_pls_data_2_comp <- barley_pls_data %>%
  rename(Comp1 = PLS1, Comp2 = PLS2) %>%
  mutate(Method = "PLS")

two_component_comparison <- bind_rows(
...
# fmt: skip
comp1_wavelength_comparison <- bind_rows(
  pca_wavelength,
  ica_wavelength,
  pls_wavelength
) %>%
  rename(`1300` = wvlgth_001, `1900` = wvlgth_250, `2398` = wvlgth_550) %>%
  pivot_longer(
    cols = c(`1300`, `1900`, `2398`),
...
barley_pca_tune <- recipe(barley ~ ., data = barley_train) %>%
  step_zv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
...
barley_ica_tune <- recipe(barley ~ ., data = barley_train) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
...
barley_pls_tune <- recipe(barley ~ ., data = barley_train) %>%
  step_zv(all_predictors()) %>%
  step_orderNorm(all_numeric_predictors()) %>%
...
three_emb <- bind_rows(pca_int, ica_int, pls_int) %>%
  mutate(Method = factor(method, levels = c("PCA", "ICA", "PLS"))) %>%
...
Results for linear regressions using three different linear embeddings.
...
iso_data <- recipe(class ~ length + depth, data = pen_subset) %>%
...
dark_orig_p <- dark_p +
  ...
  geom_text(aes(label = sample), col = "#222", show.legend = FALSE) +
  labs(title = "(a) Original Data", x = "Predictor A", y = "Predictor B")
...
((light_orig_p + knn_light_p) / one_d_light_p) +
  plot_layout(design = layout)

((dark_orig_p + knn_dark_p) / one_d_dark_p) +
  plot_layout(design = layout)
```

:::

(a) A collection of samples associated with three outcome classes. (b) A diagram of the two nearest neighbors for each data point. (c) A one-dimensional projection using two different methods: non-metric MDS and Isomap.
...
for (i in neighbor_grid) {
  ...
  tmp_iso <- bake(tmp_rec, new_data = barley_val) %>%
    mutate(Neighbors = i)
...
Isomap for the barley data for different numbers of nearest neighbors. The training set was used to fit the model and these results show the projections on the validation set. Lighter colors indicate larger values of the outcome.
...
cent_class <- as_tibble(t(res$centroids)) %>%
  mutate(class = paste("class", 1:3))
...
three_class_points <- three_class_dat %>%
  ggplot(aes(x_1, x_2)) +
  geom_point(
...
light_three_class_points <- three_class_points +
  scale_color_brewer(palette = "Dark2") +
  theme_transparent()

dark_three_class_points <- three_class_points +
  scale_color_brewer(palette = "Set2") +
  dk_thm

three_class_path <- three_class_dat %>%
  ggplot(aes(x_1, x_2)) +
...
three_class_path_light <- three_class_path +
  geom_point(
    data = cent_overall,
    col = "black",
    cex = 8,
...
three_class_path_dark <- three_class_path +
  geom_point(
    data = cent_overall,
    col = "#CCDEEC",
    cex = 8,
...
light_three_class_points +
  three_class_path_light +
  plot_layout(guides = 'collect') +
  theme(legend.direction = "horizontal")

dark_three_class_points +
  three_class_path_dark +
  plot_layout(guides = 'collect') +
  theme(legend.direction = "horizontal")
```
...
An example of shrunken, class-specific centroids. Panel (a) shows data where there are three classes. Panel (b) demonstrates how, with increasing regularization, the centorids converge towards the global centroid (the black open circle).
