...
When there are a multitude of predictors, it might be advantageous to condense them into a smaller number of artificial features. To be useful, this smaller set should represent what is essential in the original data. This process is often called _feature extraction_, _dimension reduction_, or _manifold learning_. We'll use a more general term currently en vogue: **embeddings**. In an excellent overview,  @Boykis_What_are_embeddings_2023 defines embeddings as: 

> "intermediate elements that live within machine learning services to refine models."

While this chapter focuses on feature extraction, embeddings  can be used for other purposes, such as converting non-numeric data (e.g., text) into a more usable numeric format. In @sec-cls-neural-nets, we'll use this term again and again, often when transforming features (or an existing embedding) into a different representation. 
...
#| fig-alt: "Two-panel figure stacked vertically. Panel (a) is a bar chart of the barley oil percentage distribution, showing that most samples have very low barley content with a long right tail. Panel (b) shows four spectral line plots across 550 wavelengths; each line represents one sample and darker lines correspond to lower barley content. The spectra are highly correlated, following similar shapes with vertical shifts."
key <- tibble(
...
`r r_comp("embeddings.html#sec-barley")`
...
For the problem of highly correlated predictors, some embedding methods can additionally _decorrelate_ the predictors by deriving embedded features with minimal correlation. 
...
if (!file.exists("../RData/barley_linear_embeddings.RData")) {
  save(
    all_scores_val,
    all_loadings,
    file = "../RData/barley_linear_embeddings.RData"
  )
}
```

Karl Pearson introduced PCA over a century ago, yet it remains a fundamental dimension reduction method [@jolliffe2016principal].  Its relevance stems from the underlying objective of the technique: to find linear combinations of the original predictors that maximize the amount of variation in the new features^[As is, this is far too general. We could maximize the variance by making every value of $Q$ equal to $\infty$. PCA, and other methods, impose an implicit constraint to limit the elements of $Q$. In this case, PCA constrains the loading vectors to have unit length.]. 
...
In a way, PCA components have an order or hierarchy. The first principal component has a higher variance than any other component. The second component has a higher variance than subsequent components, and so on. We can think of this process as one of deflation; the first component extracts the largest sources of information/variation in the predictor set. The second component extracts as much as possible for whatever is left behind by the first, and so on. For this reason, we can track how much variation in the original data each component accounts for. We often use the phrase that "PCA chases variation" to achieve its goals.
...
::: {.mathy-box}
The remainder of this subsection discusses a mathematical operation called the singular value decomposition (SVD). PCA and many other computations rely on the SVD. It is a fundamental linear algebra calculation. 
...
The SVD process is intended to find a way to _rotate_ the original data in a way that the resulting columns are orthogonal to one another. It takes a square, symmetric matrix (such as a covariance matrix) as an input, where it is equivalent to the _eigendecomposition_, and can decompose it into several pieces:
...
q_11 <-
  barley_pca_coefs |>
  filter(terms == "wvlgth_001" & component == "PC1") |>
  pluck("value") |>
  round(2)
q_12 <-
  barley_pca_coefs |>
  filter(terms == "wvlgth_050" & component == "PC1") |>
  pluck("value") |>
  round(2)
q_21 <-
  barley_pca_coefs |>
  filter(terms == "wvlgth_001" & component == "PC2") |>
  pluck("value") |>
  round(2)
q_22 <-
  barley_pca_coefs |>
  filter(terms == "wvlgth_050" & component == "PC2") |>
  pluck("value") |>
  round(2)
```
...
$$
\begin{align}
x^*_{i1} &= `r q_11` x_{i1} + `r q_12` x_{i2} \notag \\
...
#| out-width: 100%
#| dev: png
#| fig-alt: "Scatter plot of two standardized barley predictors (wavelengths 1300 and 1398) showing a strong positive correlation. Points are shaded by barley percentage, with lighter points indicating higher barley content."
barley_norm_train |>
...
#| warning: false
#| out-width: 100%
#| fig-alt: "Animated scatter plot showing the same two barley predictors being continuously rotated around their center. The animation pauses at two angles where the horizontal axis captures maximum variance, illustrating that PCA is simply a rotation of the data to align with the directions of greatest variability."
...
#| fig-alt: "Line chart of cumulative percent total variance on the y-axis versus PCA component number (1 through 10) on the x-axis. The first component alone captures the vast majority of variance. The curve rises steeply at the first component and flattens quickly, indicating that a small number of components summarize most of the variability in the barley data."
ggplot(
  barley_cumulative_variance,
...
Various methods exist for estimating the PCA loadings. Some are well-suited to different dimensions of the predictor's data. For example, "small $n$, large $p$" data sizes can make PCA computations difficult. @wu1997kernel and others describe a _kernel_ method more appropriate for very wide data sets. @scholkopf1998nonlinear extended this even further by using nonlinear kernel methods^[We will see similar methods in @sec-cls-svm.]. Still, others use modified objective functions to compensate for some potential deficiencies of the canonical technique.

For example, the standard loading matrix (i.e., the eigenvectors) is _dense_. It is possible that the loadings for some predictors shouldn't affect specific components. The SVD might estimate these to be close to zero but not exactly zero. Sparse PCA techniques can, through various means, estimate some loading values to be exactly zero, indicating that the predictor has no functional effect on the embedding. 

For example, @shen2008sparse take a penalization approach that puts a high price for loadings to have very large values. This regularization method can shrink the values towards zero and make some absolute zero. This is similar to the regularization approach seen in @sec-effect-encodings and is directly related to techniques discussed in @sec-logistic-reg, @sec-cls-nnet, and @sec-linear-reg. This option can coerce values across the loadings to be zero. It is unlikely to force a particular predictor's values to be zero across all components. In other words, it may not completely erase the effect of a predictor from the embedding altogether. 
...
barley_ica_data_for_pairs_plot <-
  ...
  mutate(
    barley_bin = ifelse(
      barley < as.numeric(quantile(barley, 0.25)),
      "Q1",
      ifelse(
        barley >= as.numeric(quantile(barley, 0.25)) &
          barley < as.numeric(quantile(barley, 0.5)),
        "Q2",
        ifelse(
          barley >= as.numeric(quantile(barley, 0.5)) &
            barley < as.numeric(quantile(barley, 0.75)),
          "Q3",
          "Q4"
        )
      )
...
for_comparison_figure <-
  ...
  mutate(
    Component = factor(Component, levels = c("Comp1", "Comp2")),
    Method = factor(Method, levels = c("PCA", "ICA", "PLS"))
  )

component1_correlations <-
  tibble(
    Method = c("PCA", "ICA", "PLS"),
    R = c(
      round(cor(barley_pca_data$barley, barley_pca_data$PC01), 2),
...
pca_wavelength <-
  bind_cols(three_wavelengths, barley_pca_data |> dplyr::select(PC01)) |>
  rename(Comp1 = PC01) |>
  mutate(Method = "PCA")

ica_wavelength <-
  bind_cols(three_wavelengths, barley_ica_data |> dplyr::select(IC01)) |>
  rename(Comp1 = IC01) |>
  mutate(Method = "ICA")

pls_wavelength <-
  bind_cols(three_wavelengths, barley_pls_data |> dplyr::select(PLS1)) |>
  rename(Comp1 = PLS1) |>
  mutate(Method = "PLS")
...
`
  ...
  mutate(
    Wavelength = factor(Wavelength, levels = c("1300", "1900", "2398")),
    Method = factor(Method, levels = c("PCA", "ICA", "PLS"))
  )
```

`r r_comp("embeddings.html#sec-pca")`
...
In practice, satisfying the statistical independence requirement of ICA can be done several ways.  One approach maximizes the "non-Gaussianity" of the resulting components. As shown by @hyvarinen2000independent, 
...
marginal_txt <- cli::pluralize(
  "{num_marginals} components that appear to differentiate levels of barley by themselves (components {comp_marginals})"
)
```
...
Like principal component analysis, there are many modified versions of partial least squares. For example, @le2008sparse and @le2009sparse describe a sparse estimation routine that can set some loadings equal to absolute zero. Also, @barker2003partial and @Liu2007b adapt the algorithm for classification problems (i.e., qualitative outcomes). 

`r r_comp("embeddings.html#sec-pls")`

### Overall Comparisons {#sec-linear-embed-compare}
...
#| fig-alt: "Line chart of validation RMSE versus number of components for PCA, ICA, and PLS, with 90% confidence bands around each curve. All three methods decrease in RMSE as components are added and converge by 25 components. PLS achieves lower RMSE with fewer components than PCA or ICA, reflecting its supervised nature."
...
pen_subset <-
  pens |>
  sample_n(n) |>
  select(length = bill_length_mm, depth = bill_depth_mm, class = species) |>
...
#| fig-alt: "Three-panel figure. Panel (a) is a scatter plot of ten labeled points in two dimensions, styled by three outcome classes. Panel (b) shows the same points with lines connecting each point to its two nearest neighbors, forming a graph. Panel (c) displays two one-dimensional projections of the data: non-metric MDS and Isomap. Both largely preserve the relative ordering of points, though some nearby pairs differ between methods."
base_p <-
  pen_subset |>
...
for (i in 1:n) {
  ...
  knn_p <-
    ...
    geom_segment(
      data = dat,
      aes(xend = length_end, yend = depth_end),
      col = "grey"
    )
}
knn_p <-
...
one_d_p <-
  ...
  geom_text(aes(label = sample), col = "white", show.legend = FALSE) +
  labs(
    title = "(c) 1 Dimensional MDS (two ways)",
    y = NULL,
    x = "Embedded Value"
  ) +
  scale_color_brewer(palette = "Dark2")
...
#| fig-alt: "Faceted scatter plot with three panels showing two-dimensional Isomap embeddings of the barley validation data for 5, 20, and 50 nearest neighbors. Points are shaded by barley percentage, with lighter points indicating higher values. Higher barley samples separate clearly from low-barley clusters across all configurations. The embeddings become more densely packed as the number of neighbors increases."
isomap_grid |>
...
`r r_comp("embeddings.html#sec-isomap")` 
...
Numerical optimization methods^[Specifically gradient descent with a user-defined learning rate.] are used to estimate the $n \times m$ values $x^*_{ij}$. The process is initialized using a very sparse Laplacian eigenmap, the first few PCA components, or random uniform numbers. The objective function is based on cross-entropy and attempts to make the graphs in the input and embedded dimensions as similar as possible by minimizing:   
...
`r r_comp("embeddings.html#sec-umap")`
...
#| fig-width: 6.25
#| fig-height: 4
#| fig-cap: An example of shrunken, class-specific centroids. Panel (a) shows data where there are three classes. Panel (b) demonstrates how, with increasing regularization, the centroids converge towards the global centroid (the black open circle).
#| fig-alt: "Two-panel figure. Panel (a) is a scatter plot of simulated data with three classes in two dimensions, each class forming a distinct cluster. Panel (b) shows class-specific centroids at four levels of increasing regularization. As regularization grows, the three centroids converge toward the global centroid, illustrating how shrinkage reduces class separation."
three_class_points <-
  three_class_dat |>
  ggplot(aes(x_1, x_2)) +
  geom_point(
...
`r r_comp("embeddings.html#sec-centroids")`

## Other Methods {#sec-other-embeddings}

There are numerous other methods to create embeddings such as autoencoders [@michelucci2022introduction;@borisov2022deep]. Again, @Boykis_What_are_embeddings_2023 is an excellent survey and discussion of modern embedding methods, highlighting methods for text and deep learning.
