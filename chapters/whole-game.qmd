...
We'll avoid many details in this chapter by copiously referencing upcoming chapters. Also, in this and upcoming chapters, there are icons (e.g., {{< fa brands r-project >}}) that link to separate pages that illustrate how to perform the calculations.
...
#| fig-align: "center"
#| out-width: "80%"
#| fig-cap: "Histograms of the training set outcome data with and without a transformation."
#| fig-alt: "Two side-by-side histograms of delivery time. Panel (a) on the original scale shows a right-skewed distribution with a peak near 25 minutes and a long tail of slower deliveries. Panel (b) on a log scale shows a more symmetric distribution, suggesting a log transformation may be useful. Rug plots along the horizontal axis show individual observations."
delivery_hist <-
  deliveries |>
...
`r r_comp("whole-game.html#sec-delivery-times")`
...
The other option is to initially split the data into three subsets: the training set, the testing set, and the **validation set**. Validation sets are a small subset used for evaluating the model repeatedly during development.
...
`r r_comp("whole-game.html#sec-data-spending-whole-game")`

## Exploratory Data Analysis {#sec-eda-whole-game}

Our goal now is to determine if there are aspects of predictors that would affect how we provide them to the model. We should look at individual variables as well as combinations of multiple variables.
...
#| fig-alt: "A four-panel figure. Panel (a) is a scatter plot of distance versus delivery time with a smooth trend showing a roughly linear increase that flattens at large distances. Panel (b) is a scatter plot of order hour versus delivery time with a smooth trend peaking around 17:30 then declining. Panel (c) shows box plots of delivery time by day of week, with increasing medians and spread from Monday through Saturday and a decrease on Sunday. Panel (d) overlays smooth trends of delivery time versus order hour for each day, revealing an interaction where weekday curves are flat and low while Friday and Saturday curves peak sharply in the evening."
day_cols <- c(
  "#000000FF",
  "#24FF24FF",
  "#009292FF",
...
Panel (b) has a similar visualization for the order time. In this case, there is another nonlinear trend where the delivery time increases with the hour, then peaks around 17:30, then begins to decrease. There is also an increase in delivery time variation as the order time approaches the peak. 
...
#| fig-alt: "A dot plot with horizontal error bars showing the percent increase in delivery time for each menu item. Items are ranked vertically by effect size. A vertical reference line at zero separates items that increase delivery time from those that decrease it. A few items at the top show a clear positive increase with confidence intervals not crossing zero, many items in the middle have intervals overlapping zero, and one item at the bottom shows a decrease."
resampled_ratios |>
...
`r r_comp("whole-game.html#sec-eda-whole-game")`
...
`r r_comp("whole-game.html#sec-model-development-whole-game")`
...
#| fig-alt: "Scatter plot of observed versus predicted delivery times for the linear regression model on the validation set. A dashed diagonal reference line marks perfect predictions. A smooth trend line shows the model tracks the diagonal well for short deliveries but increasingly under-predicts for observed times above about 40 minutes, curving below the reference line."
lin_reg_res |>
  collect_predictions() |>
...
`r r_comp("whole-game.html#linear-regression")`
...
#| fig-alt: "A diagram of a regression tree for delivery time. The root node splits on order hour, then subsequent branches split on day of week and distance, producing six terminal nodes. Each node shows the predicted value and sample size. The tree illustrates how hierarchical binary splits partition the data into regions with different predicted delivery times."
...
all_rules <- get_rule_info(100, x = cb_fit)
num_rules <- nrow(all_rules)
mean_rule_vars <- round(mean(all_rules$rule_mem), 1)
mean_reg_vars <- round(mean(all_rules$reg_mem), 1)

row_1 <- delivery_val |> slice(1)

all_rules <-
  all_rules |>
  mutate(is_active = map_lgl(rule_expr, ~ rlang::eval_tidy(.x, data = row_1)))

val_1_rules <- sum(all_rules$is_active)
val_1_pct <- round(100 * val_1_rules / num_rules)

cb_active <- unique(unlist(map(all_rules$rule_expr, all.vars)))
cb_unused <- ncol(delivery_train) - 1 - length(cb_active)
...
When fit to the training set, the Cubist model created an ensemble with `r format(num_rules, big.mark = ",")` rules. On average, the rules consisted of `r mean_rule_vars` variables and the regression models contained `r mean_reg_vars` predictors. Two example rules, and their regression models, were:
...
These two examples are reasonable and understandable. However, Cubist blends many rules together to the point where it becomes a black-box model. For example, the first data point in the validation set is affected by `r format(val_1_rules, big.mark = ",")` rules (about `r val_1_pct`% of the total ensemble). 
...
#| fig-alt: "Scatter plot of observed versus predicted delivery times for the Cubist model on the validation set. Points cluster more tightly along the diagonal reference line compared to the linear regression, though a few large residuals remain at long delivery times. The smooth trend line stays closer to the diagonal across the range of predictions."
cb_res |>
  collect_predictions() |>
  ggplot(aes(time_to_delivery, .pred)) +
  geom_abline(col = "green", lty = 2) +
...
`r r_comp("whole-game.html#rule-based-ensemble")`
...
nnet_mae <-
  nnet_res |>
  collect_metrics() |>
  filter(.metric == "mae" & hidden_units == best_nnet$hidden_units) |>
  pluck("mean")
...
nnet_mae_median <-
  ...
  filter(
    .metric == "mae" &
      hidden_units >= best_nnet$hidden_units - 5 &
      hidden_units <= best_nnet$hidden_units + 5
  ) |>
  pluck("mean") |>
  median()
...
nnet_fit <-
  nnet_wflow |>
  finalize_workflow(tibble(hidden_units = best_nnet$hidden_units)) |>
...
#| fig-alt: "Scatter plot of the number of hidden units on the horizontal axis versus validation set MAE on the vertical axis. MAE is high for very few hidden units, drops steeply as units increase, then slightly decreases with a very noisy trend. A smooth trend line highlights the overall decreasing-then-flat pattern."
nnet_mae_vals |>
...
#| fig-alt: "Scatter plot of observed versus predicted delivery times for the neural network model on the validation set. Points follow the diagonal reference line well for most deliveries. The smooth trend line departs below the diagonal for longer delivery times, indicating systematic under-prediction for slow deliveries, though there are fewer extreme outliers than in the Cubist results."
nnet_res |>
  collect_predictions(
    parameters = tibble(hidden_units = best_nnet$hidden_units)
  ) |>
  ggplot(aes(time_to_delivery, .pred)) +
  geom_abline(col = "green", lty = 2) +
  geom_point(alpha = 0.4, cex = 1) +
...
These results suggest that the Cubist and neural network models have roughly equivalent performance. Like Cubist, the finalized model is very complex; it is a highly nonlinear regression with `r format(nnet_param, big.mark = ",")` model parameters (i.e., slopes and intercepts). 

`r r_comp("whole-game.html#neural-network")`

## Choosing Between Models {#sec-model-selection-whole-game}
...
`r r_comp("whole-game.html#calibration")`
...
test_mae_uncal <- test_metrics_uncal$.estimate[
  test_metrics_uncal$.metric == "mae"
]
...
is_same <- ifelse(
  dec_to_time(lin_reg_mae) == dec_to_time(test_mae_uncal),
  "_also_",
  ""
)
```
...
#| fig-alt: "Two side-by-side scatter plots of observed versus predicted test set delivery times for the final linear regression model. Panel (a) shows uncalibrated predictions that under-predict long delivery times, with the smooth trend curving below the diagonal. Panel (b) shows calibrated predictions where points at long delivery times shift closer to the diagonal, indicating a modest improvement from post-hoc calibration."
p_uncal <-
  delivery_test_res |>
  collect_predictions() |>
  ggplot(aes(time_to_delivery, .pred)) +
  ...
  coord_obs_pred() +
  labs(
    x = "Observed Delivery Time",
    y = "Predicted Delivery Time",
    title = "(a) uncalibrated"
  )
...
`r r_comp("whole-game.html#sec-test-results-whole-game")`
...
#| fig-alt: "A flow diagram showing the high-level model building process. The initial data pool is split into training, validation, and test sets. The training set feeds into model development, whose outputs are evaluated on the validation set. After model selection and finalization, the chosen model is assessed on the held-out test set for a final unbiased performance estimate."
if (knitr::is_html_output()) {
...
For individual models, @fig-within-model-process summarizes the cyclic process of speculation, configuration, and validation of model parameters and features. For these data, we discovered most of the important trends prior to our first model fit. As previously mentioned, we would cycle through a few rounds where we try to make improvements for each of the models. 

```{r}
#| label: fig-within-model-process
#| echo: false
#| out-width: 90%
#| fig-cap: The cycle of developing a specific type of model. EDA stands for exploratory data analysis.
#| fig-alt: "A circular flow diagram illustrating the iterative cycle within model development. Steps include exploratory data analysis, feature engineering, model fitting, and evaluation on the validation set. Arrows connect the steps in a loop, indicating that the modeler repeatedly refines features and re-evaluates until satisfied with performance."
if (knitr::is_html_output()) {
...
save(
  ...
  cb_unused,
  all_rules,
  file = "../RData/deliveries_cubist.RData",
  compress = TRUE
)
```
