...
dark_delivery_hist <- 
  deliveries %>% 
  ggplot(aes(x = time_to_delivery)) +
  geom_histogram(bins = 30, col = dark_bg) +
  geom_rug(alpha = 1 / 4, col = dark_data) +
  labs(x = "Time Until Delivery (min)", title = "(a)") +
  thm_dk
...
Histograms of the training set outcome data with and without a transformation.
...
light_day_cols <-  c("#000000FF", "#24FF24FF", "#009292FF",  "#B66DFFFF", 
                     "#6DB6FFFF",  "#920000FF",  "#FFB6DBFF")
dark_day_cols <-  c(dark_tan, "#24FF24FF", "#009292FF",  "#B66DFFFF", 
                    "#6DB6FFFF",  "#920000FF",  "#FFB6DBFF")
...
light_delivery_day <- 
  ...
  scale_color_manual(values = light_day_cols) +
  thm_lt
...
dark_delivery_dist <- 
  delivery_train %>% 
  ggplot(aes(x = distance, time_to_delivery)) +
  geom_point(alpha = 1 / 10, cex = 1, col = dark_data) +
  labs(y = "Time Until Delivery (min)", x = "Distance (miles)", title = "(a)") +
  geom_smooth(se = FALSE, col = "red") +
  thm_dk
...
dark_delivery_day <- 
  delivery_train %>% 
  ggplot(aes(x = day, time_to_delivery, col = day)) +
  geom_boxplot(show.legend = FALSE, fill = dark_bg)  +
  labs(y = "Time Until Delivery (min)", x = NULL, title = "(c)") +
  scale_color_manual(values = dark_day_cols) +
  thm_dk
...
( light_delivery_dist + light_delivery_time) /
  ( light_delivery_day + light_delivery_time_day ) +
  plot_layout(guides = 'collect') &
  theme(legend.title = element_blank(), legend.position = "bottom")

( dark_delivery_dist + dark_delivery_time) /
  ( dark_delivery_day + dark_delivery_time_day ) +
  plot_layout(guides = 'collect') 
```

:::

Visualizations of relationship between the outcome and several predictors using the training set.
...
::: {#fig-delivery-increases}

::: {.figure-content}

```{r}
#| label: delivery-increases
#| renderings: [light, dark]
#| message: false
#| fig-width: 5
#| fig-height: 5
...
dark_ratios <-
  ...
  geom_errorbar(aes(xmin = .lower - 1, xmax = .upper - 1), width = .5, col = dark_data) +
  scale_x_continuous(labels = scales::percent) +
  labs(y = NULL, x = "Increase in Delivery Time When Ordered") +
  thm_dk +
  theme(axis.text.y = element_text(hjust = 0))
...
Ranking the items by the increase in delivery time when ordered at least once.
...
::: {#fig-delivery-lm-obs-pred}

::: {.figure-content}

```{r}
#| label: delivery-lm-obs-pred
#| renderings: [light, dark]
#| message: false
#| dev: "ragg_png"
#| fig-width: 4
...
A scatter plot of the observed and predicted delivery times, via a linear regression model, for the validation set. The red line is a smooth trend estimate.
...
knitr::include_graphics("../premade/delivery-tree-dark.svg")
```

:::

An example of a regression tree used to create a set of six rules.
...
::: {#fig-delivery-cubist-obs-pred}

::: {.figure-content}

```{r}
#| label: delivery-cubist-obs-pred
#| renderings: [light, dark]
#| message: false
#| dev: "ragg_png"
#| fig-width: 4
...
A scatter plot of the observed and predicted delivery times from the Cubist model, for the validation set.
...
::: {#fig-delivery-nnet-tune}

::: {.figure-content}

```{r}
#| label: delivery-nnet-tune
#| renderings: [light, dark]
#| message: false
#| fig-width: 6.25
#| fig-height: 4
...
The relationship between the number of hidden units and the mean absolute deviation from the validation set data.
...
::: {#fig-delivery-nnet-obs-pred}

::: {.figure-content}

```{r}
#| label: delivery-nnet-obs-pred
#| renderings: [light, dark]
#| message: false
#| dev: "ragg_png"
#| fig-width: 4
...
::: {#fig-delivery-test-obs-pred}

::: {.figure-content}

```{r}
#| label: delivery-test-obs-pred
#| renderings: [light, dark]
#| message: false
#| dev: "ragg_png"
#| fig-width: 8
...
p_light_uncal + p_light_cal

p_dark_uncal + p_dark_cal
```

:::

Observed versus predicted test set delivery times for the final model. Results are shown for pre-calibrated and post-calibration. 
...
A general high level view of the process of creating a predictive model. The diagram reflects which steps use the test set, validation set, or the entire training set. @fig-within-model-process describes the operations labeled "model development."
...
::: {#fig-within-model-process}

::: {.figure-content}

```{r}
#| label: within-model-process
#| echo: false
#| renderings: [light, dark]
#| out-width: 90%
...
knitr::include_graphics("../premade/diagram_within_model_cycle_dark.svg")
```

:::

The cycle of developing a specific type of model. EDA standard for exploratory data analysis. 
...
save(cb_fit, cb_active, cb_unused, all_rules, 
     file = "../RData/deliveries_cubist.RData", compress = TRUE)
