...
Cubist is a rule-based ensemble method. It creates a rule set by first generating a regression tree that is composed of a hierarchical set of if/then statements^[The details for regression trees and Cubist are in @sec-reg-trees]. @fig-reg-tree shows a simple tree for our data. A set of splits on different predictors results in six terminal nodes (at the bottom of the tree). A rule is a distinct path through the tree to a terminal node. For example, the right-most node in the figure has the corresponding rule: `hour >= 14.77 & day in {Fri, Sat} & distance >= 4.045`. For each rule, Cubist fits a linear regression to the data that the rule covers ($n = 261$ for this node). It builds new rule sets sequentially and uses many rules/regression equations to predict new samples. 
...
knitr::include_graphics("../premade/delivery-tree-light.svg")
```
...
```{r}
#| label: fig-model-building-process-light
#| echo: false
#| fig-align: center
#| out-width: 60%

knitr::include_graphics("../premade/overall-process-validation-light.svg")
```
...
```{r}
#| label: fig-model-building-process-dark
#| echo: false
#| fig-align: center
#| out-width: 60%

knitr::include_graphics("../premade/overall-process-validation-dark.svg")
```
...
```{r}
#| label: model-devel-light
#| echo: false
#| fig-align: center
#| out-width: 90%

knitr::include_graphics("../premade/model-devel-light.svg")
```
...
```{r}
#| label: model-devel-dark
#| echo: false
#| fig-align: center
#| out-width: 90%

knitr::include_graphics("../premade/model-devel-dark.svg")
```
