...
no_int <- 
  ...
  geom_point() + 
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 1 / 20) + 
  geom_line() +
...
pie_n <-
  delivery_train %>%
  mutate(
    hour = cut(hour, breaks = 2 * (6:11), include.lowest = TRUE),
    delivery_time = cut(
      time_to_delivery,
  ...
  count(hour, day, delivery_time, .drop = FALSE)
...
pie_prop <-
  full_join(pie_n, pie_by_group, by = c("hour", "day")) %>%
...
ames_train %>% 
  ...
  geom_line(data = super_pred,
            aes(Latitude, .pred),
            col = bin_cols["supervised"],
            lwd = 3 / 4) +
  geom_line(data = unsuper_pred,
            aes(Latitude, .pred),
