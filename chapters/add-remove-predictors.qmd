...
no_int <- 
  delivery_train %>% 
  mutate(
    item_02 = factor(ifelse(item_02 > 0, "yes", "no")),
    `Contains Item 1` = factor(ifelse(item_01 > 0, "yes", "no"))
  ) %>% 
  nest(.by = c("Contains Item 1", "item_02")) %>% 
  mutate(
    stats = map(data, ~ t.test(.x$time_to_delivery, conf.level = 0.9) %>% tidy())
  ) %>% 
  select(`Contains Item 1`, item_02, stats) %>% 
  unnest(stats) %>% 
  ggplot(aes(item_02, estimate, col = `Contains Item 1`, group = `Contains Item 1`)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 1 / 20, alpha = 2 / 3) + 
  geom_line() +
  labs(x = "Contains Item 2", y = "Mean ADR", title = "(a) no interaction") +
  theme(legend.position = "bottom")

has_int <- 
  delivery_train %>% 
  mutate(
    item_10 = factor(ifelse(item_10 > 0, "yes", "no")),
    `Contains Item 1` = factor(ifelse(item_01 > 0, "yes", "no"))
  ) %>% 
  nest(.by = c("Contains Item 1", "item_10")) %>% 
  mutate(
    stats = map(data, ~ t.test(.x$time_to_delivery, conf.level = 0.9) %>% tidy())
  ) %>% 
  select(`Contains Item 1`, item_10, stats) %>% 
  unnest(stats) %>% 
  ggplot(aes(item_10, estimate, col = `Contains Item 1`, group = `Contains Item 1`)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 1 / 20, alpha = 2 / 3) + 
  geom_line() +
  labs(x = "Contains Item 10", y = " ", title = "(b) interaction") +
  theme(legend.position = "bottom")
...
cv_hstats <-
  hstats(cb_fit,
         X = delivery_train %>% select(-time_to_delivery),
         pairwise_m = 10,
         verbose = FALSE)
cv_two_way_int_obj <- h2_pairwise(cv_hstats, zero = TRUE)
int_vars <-
  strsplit(rownames(cv_two_way_int_obj), ":") %>%
  map(sort) %>%
  map(~gsub("_0", " ", .x)) %>%
  map(~gsub("_", " ", .x)) %>%
  map_chr( ~ paste(.x[1], "x", .x[2]))
cv_two_way_int <-
  tibble(terms = int_vars, effect = cv_two_way_int_obj$M[, 1]) %>%
...
three_class_points <- 
  three_class_dat %>% 
  ggplot(aes(x_1, x_2)) + 
  geom_point(aes(col = class, pch = class), alpha = 0.65, show.legend = FALSE) +
...
three_class_path <- 
  three_class_dat %>% 
  ggplot(aes(x_1, x_2)) + 
  geom_point(aes(col = class, pch = class), alpha = 0.15, pch = 1) +
  geom_point(data = cent_class, aes(col = class, pch = class), cex = 4) +
  geom_path(data = cent_shrunk, aes(col = class), show.legend = FALSE) +
  geom_point(data = cent_overall, col = "black", cex = 4, pch = 16, show.legend = FALSE)  +
  labs(x = expression(x[1]), y = expression(x[2])) +
  scale_color_brewer(palette = "Dark2") + 
  scale_shape_manual(values = 0:2) +
  ggtitle("(b)")
