...
TODO:
 ...
 - Rename this chapter. "Feature Engineering by Adding Features"? "Data Focused Transformations"? "Features for Nonlinearity" ¯\_(ツ)_/¯
 - Add background for this chapter and why it is important for preparation
 - Decide whether to split this chapter into two (adding predictors and removing predictors)
...
Previous chapters have mostly focused on transforming one predictor "in place." For example, embeddings take groups of predictors and represent them in a smaller dimension. Other transformations have described different ways to encode data. Examples of this are converting qualitative predictors to numeric encodings via indicator variables, hashing variables, etc. In either case, these methods are often motivated by _what the model needs_. If a model is sensitive to colinearity, then embeddings are a way to compensate for that weakness. If a model cannot consumer qualitative predictors as non-numeric data, then we have to find numerical encodings for them. 

In this chapter, we’ll discuss techniques that address _what the data require_. For example, if the relationship between a predictor is nonlinear, how can we represent that predictor so that the relationship can be modeled? This is a _dataset-specific issue_ and not directly motivated by the model. This is one of the primary applications of **feature engineering**: what can we do to make it easier for the model to understand and predict the outcome? 
...
It is essential to understand that the _data_ requires this transformation. We can put the original predictors into a model as-is. The model won’t produce an error but won’t have good predictive performance. We can induce a separation of the classes when a joint feature is used. 
...
#| out-width: "85%"
#| warning: false
#| fig-cap: "Delivery times versues the time of the order, colored by the day of the week. (a) A duplicate of a panel from @fig-delivery-predictors. (b) The same data with trends lines where the underlying data did _not_ contain an interaction between the delivery day and hour."
