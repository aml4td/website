...
$\mathbb{a}(\boldsymbol{q}, \boldsymbol{k}_i)$ bounds the attention weights to be on [0, 1]. Let's denote a function $\mathbb{S}(\boldsymbol{X})$ that performs row-wise softmax on $\boldsymbol{X}$, which is $(n \times p)$, and produces a matrix of the same size whose rows sum to one.
...
$\boldsymbol{Q}$, $\boldsymbol{K}$ and $\boldsymbol{V}$ are all $(n \times p)(p \times m) = n \times m$ where $m$ is the outgoing embedding dimension. The $\boldsymbol{W}^*$ matrices are supervised weights that can be optimized for performance during gradient descent. 
...
If the incoming embeddings are $n \times p$, the new embedding matrix is $(n \times m)(m \times n)(n \times m) = n \times m$. Note that, for models that are optimized with stochastic gradient descent, $n$ in this scenario is the batch size, not the size of the training set. 
...
There are some differences in how column attention works. Let's start by describing how attention operates during model training. 

We will be attending feature-to-feature, meaning each feature has its own attention computation. To illustrate, let's describe the process for feature $j$ ($j = 1,\ldots, p$). For feature $j$, the dimensions of $\boldsymbol{Q}^{(j)}$ and $\boldsymbol{K}^{(j)}$ are ($n \times m$). 

The attention weights for how feature $j$ attends feature $j^'$ are contained in the ($p\times p$) matrix $\boldsymbol{S}$. To compute its elements^[Both $j$ and $j^'$ go from $1\ldots p$.], we can perform a row-wise dot product for each sample:

$$
s^{(j, j')}_i = \frac{1}{\sqrt{m}} \sum_{\ell=1}^{m} \boldsymbol{Q}^{(j)}_{i,\ell} \cdot \boldsymbol{K}^{(j')}_{i,\ell}
$$

As a reminder, the $\boldsymbol{Q}$ and $\boldsymbol{K}$ matrices are ($n \times m$), and we are summing via the sum over the embedding dimension. Our score is then a vector that is ($n \times 1$). This gives us the scalar values that are the weight of how feature $j'$ is used to blend feature $j$. There are $n$ of these values since the blend is tailored to each sample. 

TODO the dimensions and.or notation are still off
...
Let's demonstrate with $p = 4$ incoming embedding variables. For row $i$ of $\boldsymbol{A}^{(j)}$, four columns show how each of the four $H_j$ values is blended to make $Z_j$ (the outgoing embedding). 
