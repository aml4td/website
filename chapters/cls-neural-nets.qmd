...
To be more formal, let's describe the computation for a _single sample_. After the embedding layer, each of the $p$ predictors is represented by an $m$-dimensional embedding. Collecting these as rows, one sample's incoming data is a ($p \times m$) matrix $\boldsymbol{X}$: $p$ feature embeddings, each of length $m$. The attention block works on one sample at a time, and the $n$ samples are each processed the same way, so every matrix below describes a single sample^[In code, these per-sample ($p \times m$) matrices are stacked together and computed in one shot for speed, but the algebra is identical to handling them one at a time.]. The output of the attention block is a re-mixed set of embeddings $\boldsymbol{Z}$ of the same shape, ($p \times m$).
...
The attention block estimates three sets of weight matrices, all of which are $m \times m$, that project each feature embedding into a query, key, and value: 
...
$\boldsymbol{Q}$, $\boldsymbol{K}$ and $\boldsymbol{V}$ are all $(p \times m)(m \times m) = p \times m$. The $\boldsymbol{W}^*$ matrices are _shared_ across the $p$ features (the same $\boldsymbol{W}^q$ is applied to every feature embedding) and are the supervised weights optimized during gradient descent. 
...
The score matrix $\boldsymbol{Q}\boldsymbol{K}'$ is $(p \times m)(m \times p) = p \times p$: one similarity for every ordered pair of features. After the row-wise softmax and the product with $\boldsymbol{V}$, the attended embeddings are $(p \times p)(p \times m) = p \times m$. The number of samples $n$ never enters these dimensions — there is simply one ($p \times p$) attention matrix per sample. This is exactly what lets attention adapt to each sample: every sample gets its own matrix of feature-to-feature weights. 
...
Because the attention matrix is built _per sample_, let's describe the computation for one sample $i$; every other sample in the batch is handled the same way, independently. 

For sample $i$, write its feature-embedding matrix as $\boldsymbol{E}_i$ ($p \times m$), where row $j$ is the embedding of predictor $j$. Projecting with the shared weights gives the query, key, and value matrices:
...
We attend feature-to-feature. The score for how feature $j$ attends feature $j'$ in sample $i$ is the scaled dot product of feature $j$'s query and feature $j'$'s key^[Both $j$ and $j^'$ go from $1\ldots p$.]:
...
Let's demonstrate with $p = 4$ features. For sample $i$, row $j$ of $\boldsymbol{A}_i$ has four weights showing how the four value embeddings $\boldsymbol{v}_i^{(j')}$ ($j' = 1\ldots 4$) are blended to form feature $j$'s attended embedding: 
...
3. Column-wise self-attention with an embedding dimension of $m = 2$. The adjusted hidden units are denoted as $Z_1$ and $Z_2$.
...
#| dev: "ragg_png"
#| warning: false
#| fig-cap: "The same data from @fig-nnet-ex using column attention. A visualization of the attention weights across the entire predictor space. The colors show how each of the four embeddings is mixed into the context-aware versions produced by the attention head. The black line is the class boundary from @fig-attention-fit."
...
Now that we've seen how the attention weights have influenced the blending of the embeddings, let's also remember that the attention block produced $m = 2$ outputs. This number is abnormally small but was chosen to simplify the results and make them easier to understand. There is a numerical consequence for using a two-dimensional embedding: $Z_1$ and $Z_2$ are redundant^[Across the predictor space, the second coordinate is, to numerical precision, a fixed rescaling of the first: ($z_{j,2} \approx 1.42 z_{j,1}$ for every feature $j$, with perfect correlation. The model has learned a linear dependency. This is acceptable since the final linear layer is going to combine two elements that are effectively the same data.]. For this reason, we can visualize one of the embedding values to understand how they both work. 

Let's examine how the updated hidden units $H_j^u$ affect one of the two outgoing embeddings. @fig-attended has heatmaps that show the relationships. The first updated embedding has a boundary (with zero values) that separates somewhat diagonal regions of the predictor space. $H_2^u$ has strong negative values in the lower right side of the predictor space, but its boundary substantially breaks that pattern when predictor $A \ge 1$. The other two updated embeddings exhibit similar patterns: mostly positive values in the regions where $B \ge 0$, with a slight downward dip at the center of the predictor space. Also, the boundary for $Z_3^u$ curves diagonally for $A \le -1$. 
...
Generally, after the attention head, the network can include additional layers of hidden units or feed directly into the final linear layer. We'll describe a specific model soon and see how well it works with the forestation data. 
