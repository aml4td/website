...
saint_brier <-
  ...
  filter(.metric == "brier_class") |>
  select(all_of(saint_prm_nms), mean) |>
  mutate(
  ...
  select(-learn_rate, -batch_size) |> 
  pivot_longer(
    cols = c(-mean, -`Attention Type`, -`Learning Rate Scheduler`),
    names_to = "parameter"
  )
...
#| fig-width: 8
#| fig-height: 7
#| fig-cap: Saint tuning results for the forestation data.
saint_brier |>
  ggplot(aes(value, mean, col = `Learning Rate Scheduler`, pch = `Attention Type`)) +
  geom_point() +
