...
Data-generating mechanisms can be created very quickly and do not require storage beyond the time they are used for training. The deep learning model takes these data sets as data points. Let's say that our stochastic gradient descent optimizer uses a batch size of 16. That translated into 16 randomly generated data sets of different types and sizes. Foundational models are trained on large numbers of these "data points". For @hollmann2025accurate reports that 100 million synthetic data sets were used for training, @grinsztajn2026tabpfn states that "8 trillion tokens" were used for verison 3 of TabPFN, and @qu2026tabiclv2 used 82 million datasets to train the first version of TabICL. 
...
bind_rows(pfn_icl_metrics, icl_icl_metrics) |>
  ...
  geom_quantile(quantiles = 0.5, linewidth = 1,
                formula = y ~ splines::ns(x, df = 4), col = "#009F3FFF") +
  geom_quantile(quantiles = c(0.05, 0.95), linewidth = 3/4,
                formula = y ~ splines::ns(x, df = 4), col = "#8FDA04FF") +
...
icl_predictions |> 
  ...
  facet_wrap(~ id)  +
  scale_x_continuous(transform = log2_trans()) + 
  labs(col = NULL, pch = NULL, x = "Training Set Size", 
       y = "Pr[Forested]") + 
