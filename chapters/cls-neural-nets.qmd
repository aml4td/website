...
1. **Preprocessing** (no parameters): each of the 16 columns is mean-imputed and standardized, and a completeness indicator column is computed alongside each feature. Output: two (16 × 2)-wide tables (values and indicators).
2. **Feature grouping** (no parameters): each cell is replaced by a circular-shifted group of $g = 3$ sibling columns, concatenated with their 3 indicators, giving 6 values per cell.
...
5. **Column aggregator** (cross-feature interaction): 4 learnable target tokens are prepended to each row's 16 column tokens, giving a sequence of 20 tokens; 3 transformer blocks with 8 heads and rotary positional embeddings, the last of which reads out only the 4 target tokens. The output shape is (4 × 128) per row.
...
6. **Flatten to the ICL dimension** (no parameters): the 4 CLS vectors are concatenated into a per-row embedding of dimension $D = 4 \times 128 = 512$.
