...
The final block uses **columnar cross-attention** where the queries are the 4 aggregation-token positions of the sequence as it stands after the two self-attention blocks. By this point the aggregation tokens have already absorbed information from the feature columns, so these are updated vectors rather than the original pretrained $\boldsymbol{t}_1, \dots, \boldsymbol{t}4$. The keys and values are the entire processed sequence (all $p + p_{agg}$ token positions).
