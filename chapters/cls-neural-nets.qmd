...
#| echo: false
#| results: "asis"
prm_1 <- cbind(coefs$model.0.bias, coefs$model.0.weight) 
colnames(prm_1) <- c("", "\\! \\!\\!\\!A", "\\! \\!\\!\\!B")
rownames(prm_1) <-  paste0("H_{", 1:3, "} =")

aligned(prm_1, digits = 2)
```


The intercept estimate for the first hidden unit is significantly larger than those of the other two. That unit also has a large positive slope for predictor A. The second hidden unit emphasizes predictor B, while $H_{3}$ has strong positive effects for both predictors. 
...
The figure's bottom panels illustrate the effect of the ReLU function, where values less than zero are set to zero. At this point, $H_{1}$ and $H_{3}$ have the most substantial effect on the model, but in somewhat overlapping regions.
...
#| echo: false
#| results: "asis"
prm_2 <- cbind(coefs$model.2.bias, coefs$model.2.weight) 
colnames(prm_2) <- c("", "\\! \\!\\!\\!H_{1}", "\\! \\!\\!\\!H_{2}", "\\! \\!\\!\\!H_{3}")
rownames(prm_2) <-  paste0("Y_{", 1:2, "} =")

aligned(prm_2, digits = 2)
```
...
How do these results compare to the previous KNN model? The Brier scores are very close: ` signif(mlp_brier, 3)` for the neural network and ` signif(knn_brier, 3)` for KNN The difference is very small, and a 90% confidence interval for the difference is (` signif(mlp_mtr_diffs$.lower[mlp_mtr_diffs$.metric == "brier_class"], 3)`, ` signif(mlp_mtr_diffs$.upper[mlp_mtr_diffs$.metric == "brier_class"], 3)`), suggesting the FDA model has statistically better performance although the difference is very small. The difference in the area under the ROC curve is ` signif(mlp_roc - knn_roc, 3)` with interval (` signif(mlp_mtr_diffs$.lower[mlp_mtr_diffs$.metric == "roc_auc"], 3)`, ` format(signif(mlp_mtr_diffs$.upper[mlp_mtr_diffs$.metric == "roc_auc"], 3), scientific = FALSE)`), leading us to a similar conclusion. Given that the KNN model is simpler, we might favor that model over our neural network. 
...
talk about batchNorm here

`r r_comp("cls-neural-nets.html#sec-cls-resnet")`

### Attention Mechanisms {#sec-cls-attention}

Basic description of attention; maybe use the usual NLP examples

row- and column-attention

in-context learning

AutoInt and maybe SAINT
