...
For v3; this can change from version to version. Much of this descritption is from the source code; the intent is not always written down. Key constants: $p_{emb} = 128$, $C_{max} = 10$, $p_{agg} = 512$

What is the structure of TabPFN's network? Keeping to the same convention used above, where is a summary for the forestation data: 
...
3. **Cell embedding layer** maps each cell's 6 values to a 128-dimensional vector:
   * Linear operation from 6 to 128: 6 × 128 + 128 = 896 parameters (includes 128 bias terms)
...
4. **Feature-distribution embedder**: 3 identical induced-self-attention blocks (each time 2 stages). The output dimension is 128 for each column.
   * Each block, using 289,872 parameters:
    * 128 learnable inducing vectors (128 dimensions each) using 16,384 parameters
    * Cross-attention stage 1 (inducing points attend to training rows) using 142,032 parameters
        * Query/key/value/output projections: 4 × (128 × 128) = 65,536 parameters
        * Quaery multiplier via MLI using 10,576 parameters
        * GELU MLP with layer sizes [128, 256, 128] using 65,536 parameters
        * 3 layer normalizations (128 dimensions) using 384 parameters
    * Cross-attention stage 2 (all rows attend to the inducing points) using 131,456 parameters
   * Feature-distribution embedder parameters: 3 × 289,872 = 869,616
5. **Column aggregator** (cross-feature interaction): 4 learnable pooling tokens are prepended to each row's 16 column tokens, giving a sequence of 20 tokens; 3 transformer blocks with 8 heads and rotary positional embeddings, the last of which reads out only the 4 pooling tokens. The output shape is (4 × 128) per row.
   * 4 pooling tokens (128 dimensions each) using 512 parameters
   ...
   * Each block, using 131,328 parameters:
      * Self-attention (query/key/value/output projections): 4 × (128 × 128) = 65,536 parameters
      * GELU MLP with layer sizes [128, 256, 128] using 65,536 parameters
      * 2 layer normalizations (128 dimensions) using 256 parameters
   * Column aggregator parameters: 512 + 8 + 128 + 3 × 131,328 = 394,632
   * Flatten to the ICL dimension (no parameters): the 4 CLS vectors are concatenated into a per-row embedding of dimension $D = 4 \times 128 = 512$.
...
8. **ICL transformer**: 24 identical blocks with 8 attention heads (head dimension 64), where test rows attend to training rows only. The output dimension is 512 for each row.
   * Each block, using 2,139,904 parameters:
      * Attention (query/key/value/output projections): 4 × (512 × 512) = 1,048,576 parameters
   ...
   * ICL transformer parameters: 24 × 2,139,904 = 51,357,696
   * Followed by an output layer normalization (512 dimensions) using 512 parameters
9. **Output head** output shape is $C = 2$
   * Many-class retrieval decoder with head dimension 64 and 6 heads:
      * Query projection (test embeddings, 512 to 384): 512 × 384 + 384 = 196,992 parameters
      * Key projection (train embeddings, 512 to 384): 512 × 384 + 384 = 196,992 parameters
      * Attention retrieves a score-weighted average of the one-hot training labels (the values are the labels themselves, not learned), then takes the log to produce class logits.
   * Output head parameters: 393,984

In total, there are 53,023,736 learnable parameters that have already been estimated. This count is the same regardless of the number of predictors, the number of categorical levels, or the number of classes (as long as they are within the model's supported range).
...
- $+4$ fpr negative infinity

In other words, we begin with $n$ rows and $p$ columns, and each value is represented as a two-dimensional vector.
...
Finally, a centering-and-scaling step is applied using statistics from the imputed training set. Because the imputation mean equals the complete-case column mean, it is subtracted back out during centering, so every value that was originally non-finite ends up encoded as (approximately) zero.

#### Feature Groupings {#sec-cls-pfn-feature-grouping}

TabPFN reorganizes the cells into groups whose values are reused across neighbors. Our data stay $n \times p$, but each cell — previously two numbers (value + completeness indicator) — now packs together $g = 3$ of the original cells, for a total of six numbers. Here $g$ is the number of features per group and defaults to 3; there are $p$ groups, one anchored at each column.

To define each group, a circular shift gathers neighbors: for a column, it collects the values sitting a fixed number of positions to the right, wrapping past the last column back to the first. The offsets are $2^i$ for $i = 0, 1, 2$, i.e., 1, 2, and 4 positions. Illustrating the shift on predictors $A, B, C, D, E$:

- shift 1: $B, C, D, E, A$
- shift 2: $C, D, E, A, B$
- shift 4: $E, A, B, C, D$

These three shifted copies are stacked to form the groups. Take predictor $C$ in column 3: its group becomes $(D, E, B)$. _Positionally_, column 3 is still $C$'s cell and it now carries $C$'s neighbors. However, literally, it does not value $C$'s values^[However, for some values of $p$, the shifting might put a predictor back into its original position.]. $C$'s own value is reused in the groups of three other columns (1, 2, and 4).

With many columns, the grouped neighbors are genuinely nearby columns. This can be very effective when the columns have a natural sequential or spatial ordering. For example, recall that the barley predictors in @sec-barley represented consecutive wavelengths in a spectra. For this type of data, this additional column-neighborhood data can be very effective. 
...
where $\boldsymbol{W}_{emb}$ is (128 x 6) and there are 128 bias terms. Note that this process is a linear projection from 6 columns to $p_{emb} = 128$. Both the predictor's _values_ and completness _indicators_ are blended in this projection; there is no separate indicator channel form this point on.

The influence of the target classes are added to the cell embedding, but only for the training data. 

Each class is represented by a pre-learned 128-dimensional vector. These vectors are initialized to be orthonormal^[They have a length of one and have pairwise dot products of zero.], which keeps the classes maximally distinguishable at the start of training; they are then learned during pretraining. TabPFN has a hard limit on the number of classes, $C_{max}$ and creates a static table $\boldsymbol{Y}$ of size $C_{max} \times 128$. When the data have $C < C_{max}$ classes, only the $C$ corresponding rows are ever looked up.

For the training data, the appropriate row of $\boldsymbol{Y}$  is added to every one of that row's cell embeddings (the same vector across all 128 columns), nudging them in the "direction" of the class. The test data are kept as-is. 

#### Feature-Distribution Embedding {#sec-cls-pfn-distirbution}

Before proceeding, recall that each cell in our $n \times p$ array is now a 128-dimensional vector, and that here a "feature" is one of the $p$ columns.
...
Second, this process will use a version of row attention. This produces a learned co-representation for a single feature at a time. Each row's embedding is re-expressed as a learned function of the other rows in its column. The model will have to compare rows from the training set to rows from the test set. This sets up an apples-to-apples comparison between the two that will occur during the in-context learning portion of the model.

The attention process used here is a little different from the one described in @sec-cls-attention. We previously focused on _self_-attention.  In our current situation, _cross-attention_ uses one data source for the queries and another for the keys and values. 

The cross-attention block has many moving parts. Before we get deep into the minutiae, let's illustrate the primary computations using an extremely simplified scenario. For example, the real process has many supervised components and a high dimensionality (128). The example shown below is unsupervised and in two raw dimensions. This will give us a sense of what this part of the network is doing without getting buried in details. This illustration will be inaccurate, but hopefully helpful. We'll follow this with a description of the actual process. 

The current TabPFN model uses three attention blocks in this section. Each has its own pretrained set of learned query inputs, denoted as $\boldsymbol{P}$, that is 128 x 128. These 128 rows are called the _inducing_ points, and we can think of them as representatives or prototype vectors for the data distributions^[That the number of inducing points ($n_{ind}=128$) equals the embedding dimension ($p_{emb}=128$) is coincidental.]. They are used as the query data for attention, while our embeddings serve as the keys and values (as we'll see shortly). 

This stage uses row attention. Within a single feature (column), the rows attend to one another. This is done indirectly here by having the inducing points $\boldsymbol{P}$ attend to the training rows' embeddings for that feature. Our first action is to use cross-attention with the $n_{tr}$ training points. This produces an array of size $n_{ind} \times p_{emb}$ (128 x 128) containing the attended training rows influenced by the inducers. We also add a residual connection back to the original inducer data. We'll label the results as $\boldsymbol{H}_{ind}$, and they can be thought of as distributional reference values. 

@fig-cross-attention shows a simplified low-dimensional example. The left panel shows the training set from @fig-nnet-ex, along with 16 inducer points^[We chose these inducers to be a regular grid not because they are realistic but to show how the resulting values change during this process.] in red and a single test point in green. These computations are simplified by omitting the initial embedding process, setting the attention matrices to the identity matrix (such as $\boldsymbol{W}^{q} = \boldsymbol{I}$, etc.), and omits a few ancillary steps.

```{r}
#| label: cross-attention
#| include: false
point_types <- c("Training", "Inducer", "Test")
type_cols <- c("#088BBEFF", "#FF3F38FF", "#3CBC38FF")
...
inducers <-
  ...
  scale() |>
  as_tibble() |>
  mutate(type = factor("Inducer", levels = point_types))

num_ind <- nrow(inducers)

# ------------------------------------------------------------------------------
# cross attention calcs

# convert to matrices
p_mat <- inducers |> select(1:2) |> as.matrix()
...
# queries = the 6 inducing points; keys/values = the n training rows
...
crs_att_x <- extendrange(c(inducers$A, test_wts$A))
crs_att_y <- extendrange(c(inducers$B, test_wts$B))
...
#| fig-width: 10
#| fig-height: 3.5
#| fig-cap: An simplified illustration of cross-attention. The data are scaled versions of the values in @fig-nnet-ex.
...
The middle panel shows how cross-attention uses the training-set points to shift the 16 inducer points to $\boldsymbol{H}_{ind}$. The result is that the points are emulating the correlation structure in the original data, albeit with larger variances. The induced values occupy a space larger than the training data mainstream. 

The second stage of cross-attention uses all data (train and test) as queries, with $\boldsymbol{H}_{ind}$ as keys and values. The resulting attended values are also combined with their original values via a residual connection. 

The right panel shows that the single test sample was shifted toward the upper-right of the distribution. The variable-sized red points indicate the attended inducer points, and their sizes reflect the extent to which the test sample attended to each reference point^[Again, keep in mind that this has been an unsupervised example of how cross-attention works. The effect shown here isn't necessarily indicative of how cross-attention functions in the wild.]. 

The literature often describes operations like these as "distributionally aware." Beyond the anthropomorphizing, this term doesn't really describe what is occurring. The inducing points are learned during pretraining and function as prototype vectors. Prototypes are learned vectors that act as "stand-ins" for a larger set of data. Since $\boldsymbol{P}$ is optimized by the objective function, it also serves as a prototype that carries some supervised tilt. In other words that are the best prototypes that encourage predictive accuracy. 

The first stage of cross-attention pulled our example prototypes into the mainstream of the data. It recontextualized the inducing points to _our_ data. 

The second stage does the same thing but in reverse, and with our observed data. It pulls our data to the more realistic prototypes. The blended values are added to the row's own vector (the residual connection). This keeps the original information and gains information describing which prototypes it resembled and how strongly.

In reality, the process starts by normalizing the inducing points and our observed features. TabPFN uses RMSnorm to do this. Later on, we'll use some of the original values for a residual connection. 

After normalization, our previously described first stage of cross-attention occurs. The inducing points and the attention parameter matrices have all been estimated during pretraining. Operationally, the attention process is split into 8 attention heads, each taking 16 features from the full collection of 128, and coupling them with their corresponding columns of the attention $\boldsymbol{W}$ matrices. From this process, we have estimated the attention weights for the inducing points. 

However, the model takes an extra step to alter the attention weights. The concern is that the row-wise softmax operation, which coerces the attention weights to sum to 1, can lose focus when the training set is large. A multiplier for the queries is computed from two different MLP submodels adn then softmax is used. After this, the row-wise weights are coerced to sum to 1, and the attended inducing points are computed. These are denoted as $\boldsymbol{P}_a$

The newly adjusted inducing points $\boldsymbol{P}_a$ are then combined with a residual connection to their values (pre-normalization), producing $\boldsymbol{P}_r$ . These values are then used in an MLP that first re-normalizes them, then uses layer sizes of (256, 128) and GELU activation (see @fig-relu-activations). The resulting features are then added back to $\boldsymbol{P}_r$ to produce our new prototypes previously labeled as $\boldsymbol{H}_{ind}$. Thus concludes the actual first stage of cross-attention. 

The second stage is the same as the first except that: 

- The combined training and test data serve as queries.
- $\boldsymbol{H}_{ind}$ plas the roles of keys and values. 
...
- Regardless of the original dimensions of the data, pool/flatten the data to a smaller but standardized size.

The second point is interesting. In our data, the number of columns $p$ will vary from dataset to dataset. However, the foundational model has been trained with specific fixed dimensions. This part of the network is here to rectify this discontinuity. 
...
$$
\boldsymbol{E}_i = \bigl[\boldsymbol{e}_{i,1}, \dots, \boldsymbol{e}_{i,p} \bigr] \quad (p \times p_{emb})
$$

where each $\boldsymbol{e}$ is a vector of size $p_{emb} = 128$. 

The column aggregator will not augment that data structure with some pretrained values that we'll call pooling tokens^[Historically, these would be called "CLS tokens" where CLS was an abbreviation for the target classes. To avoid the connotation of being cupervised, we'll call these pooling tokens.] and denote them as $\boldsymbol{t}$. For TabPFN version 3, there are $p_{agg} = 4$ tokens, each with a pretrained vector of $p_{emb} = 128$ learned parameter estimates. Now, for a given row, we have 

$$
\boldsymbol{E}_i^+ = \bigl[\, \underbrace{\boldsymbol{t}_1, \dots, \boldsymbol{t}_4}_{k \text{ CLS tokens}},\ \underbrace{\boldsymbol{h}_{i1}, \dots, \boldsymbol{h}_{ip}}_{p \text{ embedding tokens}} \,\bigr]  \quad ((p + p_{agg}) \times p_{emb}).
$$

The same values are added to each row. These columns will come into play during the attention process in this block. They are designed to help summarize features within a row of data in order to reduce and standardize the feature dimension. They are not unlike the induced values in the previous block. The pooling tokens are designed to help aggregate the current data more effectively.  
...
The final block uses columnar cross-attention where the pooling tokens $\boldsymbol{t}_1, \dots, \boldsymbol{t}_4$ are the queries and the augmented data $\boldsymbol{E}_i^+$ are the keys and values. Since there are $p_{agg}=4$ queries, the output of this process is also reducing the embeddings down to $p_{agg}$ vectors. This is where the utility of the pooling tokens comes into play. If the pretraining worked well, this cross-attention block will have designed the $\boldsymbol{t}$ parameters very effective at projecting the set of feature columns to a potentially much smaller space. 
...
This block is the core of how this foundational model operates. Its process is very similar to past blocks, but the scale changes dramatically. It will apply _24 cross-attention blocks in sequence_. These attention blocks will use the combined training and test data as queries, with the training rows serving as keys and values. This identifies the training rows that the test rows most resemble and reweights them accordingly. 

Recall that, in @sec-pfn-initial-embeddings, the training rows received additional terms that encoded the target information (labeled as $\boldsymbol{Y}$). During these attention steps,  these additional terms project the test samples in a supervised direction. 

The details of this cross-attention process most resemble the one shown in @sec-cls-pfn-distirbution: 
...
Now that we have reduced the embedding dimension to a static value of 512, the 8 attention heads use a subset size of 64. 
...
At this point, we are ready to translate our embeddings to class probability estimates for the test samples. For a more conventional model, we would cast the embeddings down to $C$ columns and then apply softmax to obtain probability-like outputs. 
...
- The test rows serve as the queries
- The training rows are the keys.
