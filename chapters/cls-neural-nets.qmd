...
Again, if the incoming embeddings are $n \times p$, the new embedding matrix is $(n \times m)(m \times n)(n \times m) = n \times m$. Note that, for models that are optimized with stochastic gradient descent, $n$ in this scenario is the batch size, not the size of the training set. 
...
The part of the network that encompasses these calculations is called _an attention head_.  Attention for tabular data can occur in two different ways: column-attention and row-attention. Let's talk about each in turn. 
...
Our scaled attention weights for embedding $j$ are then
...
3. Column-wise self-attention with an embedding dimension of $m = 2$. The adjusted hidden units are denoted as $Z_j$.
4. Linear activation to produce $C = 2$ estimates for each class.
5. Softmax normalization. 

We'll be most concerned with the details of the third operation, where the normalized embeddings enter the attention process. The chosen number of embeddings in the attention head (2) is very small for illustrative reasons. 
...
However, we are using column attention and the final estimates of the three $\boldsymbol{W}^*$ matrices. From these, we can determine how the values of the four hidden units are modified (i.e., "attended"). @tbl-attention shows how each of the four hidden units is blended. The first location $Z_1$ is equally attended by the second and third hidden units, whereas $Z_2$ and $Z_3$ are functions of $H_1$ only. The last attended embedding is a nearly even mixture of all four of the original embeddings. The second location's $Z_2$ and $Z_3$ embeddings are mixtures of   $H_2$ and $H_3$.  $Z_1$ and $Z_4$ only use $H_4$. The third location's first three embeddings are just $H_4$, and the last only uses $H_2$.  
...
location_df |>
  ...
  mutate(
    ...
    Attended = map_chr(split_up, ~ paste0("$$Z_", .x[2], "$$"))
  ) |>
  select(location = label, weights, embedding_number, Attended) |>
  pivot_wider(
    id_cols = c(Attended),
  ...
  fmt_markdown(columns = Attended) |>
  fmt_number(columns = c(-Attended), decimals = 2) |>
  cols_label(
    contains("H_1") ~ md("$H_1$"),
    contains("H_2") ~ md("$H_2$"),
    contains("H_3") ~ md("$H_3$"),
...
When reading this visualization, the values in the contour plots along each row will sum to 1.0 at each location. A few of the patterns show that the hidden units have little impact on the attended versions, including cells $(H_1, Z_1)$, $(H_2, Z_2)$, and $(H_4, Z_4)$. Other hidden units have a broad influence on the new embeddings in large regions of the predictor space (e.g., $(H_4, Z_4)$). Embedding $H_1$ has a similar effect on $Z_2$, $Z_3$, and $Z_4$, and so on. 
...
#| dev: "ragg_png"
#| warning: false
#| fig-cap: "The same data from @fig-nnet-ex using column attention. A visualization of the attention weights across the entire predictor space. The colors show how each of the four embeddings is mixed into the context-aware versions produced by the attention head and the black line is the class boundary from @fig-attention-fit."
...
Now that we've seen the attention weights, how are the four attended embeddings different from the global values that preceded the attention? @fig-attended shows them before ($H_j$) and after attention ($Z_j$). In each case, the attended versions are similar to their original counterparts with small to medium changes. For example, attention places greater weight on the first embedding due to a negative space in the central east of the region. The second attended embedding also adds a similar negative component in the same area. The third and fourth attended embeddings also have more negative values and are vertically flipped versions of one another.  
...
#| dev: "ragg_png"
#| warning: false
#| fig-cap: "A visualization of how column attention affects the embeddings just prior to the final layer of linear combinations. The top panel shows the relationship produced by @eq-unattended. The bottom panel illustrates how these variables are changed. The black line shows the contour where the values are zero and the grey line is the class boundary from @fig-attention-fit."
col_att_grid |>
  ...
  labs(fill = NULL) +
  facet_wrap(~ attended_number, nrow = 1) +
  scale_fill_gradient2(high = "#5D74A5FF") +
  coord_fixed() +
  theme_bw()
```

After the attention head, the network can include additional layers of hidden units or feed directly into the final linear layer. We'll describe a specific example soon and see how well it works with the forestation data. 

The next section expands more on how multiple attention heads can be added to the model. 
...
load("../RData/forest_tabnet.RData")
