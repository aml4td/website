...
Column attention might sound complex but, operationally, its results are simple: a new feature that is a linear combination of the original embedding set. For example,  for some sample $i$, one of its attended values $A_{i1}$ is a combination of three previous embeddings: 

$$
A_{i1} = 0.15\, X_{i1} + 0.09\, X_{i2} + 0.76\, X_{i3}.
$$

For column attention, these coefficients always sum to one and will be different from sample to sample.

The set of attenede values are used to produce the new embeddings (denoted as $Z$'s).
