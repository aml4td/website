...
Ensembls thrive on propagating variation via prediction replicates. This figure indicates that the multiple estimates do not increase prediction diversity as new replicates are added. Therefore, adding multiple estimators does not help much for this data set. We'll also see in @sec-cls-pfn-forestation that there is no benefit in tuning the number of estimators for TabPFN and TabICL. However, this may not be the case for other data sets with more class levels and more qualitative predictors.
