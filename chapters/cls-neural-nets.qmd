...
ranks_p <-
  resnet_brier |>
  ggplot(aes(rank)) +
  geom_point(aes(y = mean, col = wflow_id)) +
  geom_errorbar(
    aes(
...
saint_epochs <-
  saint_res |>
  collect_extracts() |>
  mutate(epochs = map_int(.extracts, ~ .x$epoch_actual)) |>
  summarize(
    `Mean Actual Epochs` = mean(epochs),
    .by = c(.config)
  )
...
saint_brier <-
  ...
  select(all_of(saint_prm_nms), mean, `Mean Actual Epochs`) |>
  mutate(
...
saint_brier <-
  saint_brier |>
  mutate(parameter = factor(parameter, levels = prm_order))
