...
#| fig-height: 5
#| warning: false
#| fig-cap: "Histograms of a distribution of a hidden unit with ReLU activation across batches and epochs. The red bar indicates the frequency that absolute zero values occur. The model and data are from @fig-nnet-ex but with a larger batch size (225)."
...
A residual network, shown on the right-hand side, takes the current features prior to the hidden layer and adds them back into the embeddings (post-activation). In other words, the skip comes *before* $f_i(\cdot)$ and is added to the result *after* $f_i(\cdot)$ applies its activation. We'll use the term _residual block_ to describe the operations shown in each of the top three rows of the right-hand side.

The idea is that ordinary feed-forward networks are tasked with learning, at each layer, a better representation of the outcome via $f_i(\cdot)$. This parameterization may be difficult to optimize since the changes between layers might be very small, leading to potential issues with the magnitude of the gradients. By adding the previous embeddings to this function, we've reframed the problem so that we are really learning $f_i(\boldsymbol{H}_{i-1}) = \boldsymbol{H}_i - \boldsymbol{H}_{i-1}$; a residual. 
...
For tabular data, the same residual strategy applies, but there are some complications: we need to align the dimensions. Suppose we begin with $p=2$ predictors within $\boldsymbol{x}$. Suppose we use a "bottleneck" architecture in which the hidden unit layers have output dimensions (10, 3, 10) across the three layers. If $\boldsymbol{H}_1$ is ($10 \times 1$) and $\boldsymbol{x}$ is ($2 \times 1$), how can we add those to one another?
...
This bounds the attention weights to be on [0, 1]. Let's denote a function $\mathbb{S}(\boldsymbol{X})$ that performs row-wise softmax on $\boldsymbol{X}$, which is $(n \times p)$, and produces a matrix of the same size whose rows sum to one.
...
In essence, we are updating the query data on a per-query basis.  A different query would generate different weights. As we'll see shortly, attention weights focus the computations to be relevant for each individual query that is made. 
...
The part of the network that encompasses these calculations is called _an attention head_.  Attention for tabular data primarily occurs in two different ways: column-attention and row-attention. Let's talk about each in turn. 
...
There are some differences in how column attention works. For a moment, let's view how attention operates during model training. 
...
As a reminder, these matrices are ($n \times m$), and we are summing via the sum over the embedding dimension. Our score is then a vector that is ($n \times 1$). This gives us the scalar values that are the weight of how feature $j'$ is used to blend feature $j$. There are $n$ of these values since the blend is tailored to each sample. 
...
For row $i$ of $\boldsymbol{A}^{(j)}$, the $p = 4$ columns show how each of the four $H_j$ values is blended to make $Z_j$ (the outgoing embedding). 
