...
Column attention might sound complex but, operationally, its results are simple: a new feature that is a linear combination of the original embedding set. For example,  for some sample $i$, the new feature $Z_1$ might be a combination of three previous embeddings: 

$$
Z_{i1} = 0.15\, X_{i1} + 0.09\, X_{i2} + 0.76\, X_{i3}
$$

These coefficients always sum to one. 

We've seen previous preprocessing techniques that created a new feature as a function of one or more others. For example, a $d$ degree spline term for predictor $x$ is a localized version of the original predictor. Taken together, the set of spline features can be used to capture nonlinear relationships between $x$ and the outcome. PCA is another good example. Leveraging the correlation structure of the predictors, the principal components are linear combinations of the original predictors that have desirable properties. 
...
autoint_brier <-
  ...
  select(all_of(autoint_prm_nms), mean, `Mean Actual Epochs`, `# Parameters (log-10)`) |>
  mutate(
...
common dimesnion for intial embedding and attention blocks
...
Let's look at the details of a Saint model using 
