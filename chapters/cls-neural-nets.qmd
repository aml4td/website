...
$\boldsymbol{Q}$, $\boldsymbol{K}$ and $\boldsymbol{V}$ are all $(p \times m)(m \times m) = p \times m$. The $\boldsymbol{W}_*$ matrices are _shared_ across the $p$ columns (the same $\boldsymbol{W}_q$ is applied to every column's embedding) and are the supervised weights optimized during gradient descent. 
...
Again, the three $\boldsymbol{W}_*$ matrices are _learned_ during training and, because they are estimated, attention embeddings are indirectly supervised. The target data do _not_ affect the computation's attention during a forward pass; it isn't part of any of the equations above. The target influences the parameters during the backpropagation phase of training (i.e., when the gradients are updated). 
