...
To be more formal, let's describe the computation for a _single sample_. It enters the attention block as $p$ columns, that is, $p$ scalar values (across the batch, an $n \times p$ matrix). These data might be the original predictors, or the hidden units from an earlier layer, depending on context.
...
Because the attention matrix is built _per sample_, let's again describe the computation for one sample $i$; every other sample in the batch is handled the same way, independently. 

For sample $i$, write its feature-embedding matrix as $\boldsymbol{E}_i$ ($p \times m$), where row $j$ is the embedding of predictor $j$. This is the per-sample version of $\boldsymbol{X}$.
