...
A fair amount of research has been motivated by the empirical observation that tree ensembles often outperform tabular neural network models and often do so with less tuning and computational effort.  This observation has motivated two general strategies for improving tabular neural network models. The first embeds explicit analogs of tree operations within the network architecture. The second adapts to the tabular domain the innovations that have driven the success of neural networks in generative AI, specifically attention mechanisms and in-context learning. This section will examine potential upgrades to neural networks to improve their performance on tabular data.
...
To illustrate, let's consider an MLP in which each of the three hidden layers has the same number of units as there are predictors ($p$), as shown on the left-hand side of @eq-resid-net:
...
For the MLP architecture, feed-forward network is tasked with learning, via $f_i(\cdot)$, a better representation of the data for predicting the outcome. This parameterization can be difficult to optimize because the changes from one layer to the next may be very small, which can lead to problems with the gradient magnitudes. 

A residual network, shown on the right-hand side of @eq-resid-net, takes the features entering each hidden layer and adds them back into the embeddings (post-activation). In other words, the skip originates *before* $f_i(\cdot)$ and is added to the result *after* $f_i(\cdot)$ applies its activation. We will use the term _residual block_ to describe the operations shown in each of the top three rows of the right-hand side.

 By adding the previous embeddings to  the block's output reframes the learning problem: rather than specifying the next representation, each block only needs to estimate how much the new representation should differ from the previous one, $f_i(\boldsymbol{H}_{i-1}) = \boldsymbol{H}_i - \boldsymbol{H}_{i-1}$, the _residual_ which gives this approach its name.

This reparameterization makes training easier, especially for models with many layers. Early practitioners assumed that performance could be increased simply by adding more layers, but this turned out not to be the case. @he2016deep showed that, paradoxically, large models saw both training and test error increase as the number of layers increased. This degradation isn't believed to be related to overfitting, but from the way gradients flow through the system during optimization.  Residual connections address this by giving gradients a direct path back through the network: for layer $i$, the gradient is $f_i'(\boldsymbol{H}_{i-1}) + I$. If we add too much capacity to the model (e.g., more layers), it can simply estimate zero for the residual. Also, the extra term $I$ in the gradient prevents them from becoming too small across the network, even if $f_i'(\boldsymbol{H}_{i-1})$ has elements that are approximately zero. We'll describe other potential benefits to this model towards the end of this section. 
...
The answer is that we use a parameter matrix to form a linear combination of the inputs, thereby achieving the correct dimensionality (i.e., projecting the inputs to the correct dimensions). For the first hidden layer, the skip path adds $\boldsymbol{\Theta}_1\boldsymbol{x}$ where $\boldsymbol{\Theta}_1$ is a (10 × 2) _projection matrix_. This generates a version of the original predictors that has the correct size. 

We also need to project the inputs again for the main path, where activation is applied. We'll use a different matrix of parameters ($\boldsymbol{W}_1$) that is the same size as $\boldsymbol{\Theta}_1$. Both matrices coerce the input to the correct dimension, but serve different purposes:

- $\boldsymbol{\Theta}_1\boldsymbol{x}$ should represent the original input as plainly as possible in order to produce an appropriate residual effect. 
...
$$
\boldsymbol{H}_1 = \boldsymbol{W}_1^{(2)}f_1\left(\boldsymbol{W}_1^{(1)}\boldsymbol{x}\right) + \boldsymbol{\Theta}_1\boldsymbol{x}
$$
...
$$
\boldsymbol{H}_j = \boldsymbol{W}_j^{(2)}f_j\left(\boldsymbol{W}_j^{(1)}\boldsymbol{H}_{j-1}\right) + \boldsymbol{\Theta}_j\boldsymbol{H}_{j-1} 
$$
...
$$
\boldsymbol{H}_1 = \boldsymbol{W}_1^{(2)}f_1\left(b(\boldsymbol{W}_1^{(1)}\boldsymbol{x})\right) + \boldsymbol{\Theta}_1\boldsymbol{x}
$$
...
The mathematics of attention mechanisms differ, but all pursue the same strategy as the database example above. Attention computes quantitative similarity scores between the elements of the data, usually from dot products, and maps these scores to the interval $[0, 1]$. The result is a form of fuzzy matching rather than the exact matching produced by a SQL `FILTER`. 
...
$$
\begin{align}
\boldsymbol{Q} &= \boldsymbol{X}\boldsymbol{W}_{q} \notag \\
\boldsymbol{K} &= \boldsymbol{X}\boldsymbol{W}_{k} \notag \\
\boldsymbol{V} &= \boldsymbol{X}\boldsymbol{W}_{v} \notag
\end{align}
$$ {#eq-self-attention}

$\boldsymbol{Q}$, $\boldsymbol{K}$ and $\boldsymbol{V}$ are all $(p \times m)(m \times m) = p \times m$. The $\boldsymbol{W}$ matrices are _shared_ across the $p$ columns (the same $\boldsymbol{W}_q$ is applied to every column's embedding) and are the supervised weights optimized during gradient descent. 
...
$$
\begin{align}
\boldsymbol{Q}_i = \boldsymbol{E}_i\boldsymbol{W}_q \notag \\
\boldsymbol{K}_i = \boldsymbol{E}_i\boldsymbol{W}_k \notag \\
\boldsymbol{V}_i = \boldsymbol{E}_i\boldsymbol{W}_v \notag
\end{align}
...
To make this more concrete, we return to the simple binary classification example shown in @fig-nnet-ex.  We now fit a modified neural network that begins with our two numeric predictors ($X$'s), then: 

1. Adds a fully connected layer with 4 hidden units ($H$'s) and ReLU activation (as before). 
2. Batch normalizes the 4 hidden units.
3. Computes column-wise self-attention with an embedding dimension of $m = 2$; the outgoing embeddings are denoted as $Z$'s.
4. Produces $C = 2$ outputs, one per class, using a linear activation function.
5. Converts the outputs into class probabilities using Softmax normalization. 
...
#| fig-height: 4.4
#| warning: false
#| fig-cap: "An alternate model is used to classify the data from @fig-nnet-ex. The model uses column attention after the four-unit hidden layer. Three points in the predictor space are shown and will serve as landmarks for the discussion. The black line shows the class boundary when using column attention, and the grey line is the original MLP class boundary."
...
- The third location has two patterns. $A_1$ and $A_3$ are mostly equal blends of $H_1$ and $H_2$. Also, $A_2$ and $A_4$ are equal blends across all four antecedents. 
...
Let's examine how the updated hidden units $A_j$ affect $Z_1$. @fig-attended has heatmaps showing relationships across the space of the original predictors. The first updated embedding has a boundary (with zero values) that separates somewhat diagonal regions of the predictor space. $A_2$ has strong negative values in the lower right side of the predictor space, but its boundary substantially breaks that pattern when predictor $A \le -1$. The other two updated embeddings exhibit similar patterns to one another: mostly positive values in the regions where predictor $B \ge 0$, with a slight downward dip at the center of the predictor space. Also, the boundary for $A_3$ curves diagonally when predictor $A \le -1$. 
...
Suppose that, for the forestation data, we specified 7 initial embedding dimensions per predictor and used three attention blocks, each with two attention heads, each with an embedding size of 13. A summary of the AutoInt model, tracking the output feature dimensions and the number of learned parameters at each stage, would be:

1. **Embedding layer** that outputs 16 sets of 7-dimensional embeddings:
...
3. **Output head** output shape is $C = 2$
...
autoint_info <-
  ...
  collect_extracts() |>
  mutate(
    epochs = map_int(.extracts, ~ .x$epoch_actual),
...
The best results corresponded to a model using `r autoint_best$num_embedding` dimensions for its initial embedding with a dropout rate of `r round(autoint_best$dropout_embedding, 2)`. For attention, `r autoint_best$num_attn_blocks` blocks, each with `r autoint_best$num_attn_heads` and a dimension of `r autoint_best$num_attn_heads`, were best and had an associated dropout rate of `r round(autoint_best$dropout_attn, 2)`. The best learning rate was 10<sup>`r round(log10(autoint_best$learn_rate), 2)`</sup> with a `r fmt_sched(autoint_best$rate_schedule)`. The batch size was `r autoint_best$batch_size` samples and the best momentum value was  `r round(autoint_best$momentum, 2)`. The estimated Brier score for this configuration was `r signif(autoint_best$mean[1], 3)`, which is competitive but does not beat the current best model.
...
where layer normalization is a variant of batch normalization described in @ba2016layer. 

After this block, an analogous row attention block takes the embeddings and applies the same process, but swaps row attention for column attention in the first step. 
...
1. **Embedding layer** starts with:
   - Categorical predictor: 29 country levels mapped to 10 embedding vectors using 290 parameters
   - Numeric predictors: each of the 15 is embedded via an MLP block with layer sizes [1, 100, 10], using 18,150 parameters. 
   - The target token also uses 10 parameters. 
2. **Transformer**: 2 identical blocks with 8 attention heads (column and row). The output dimension is 10 for each.  
...
3. **Hidden Layer** ingests 10 and outputs 20 embeddings
   - Linear operation followed by ReLU activation, with 220 parameters. 
4. **Output head** output shape is $C = 2$
...
A Saint model was tuned over 25 candidates. The parameter ranges were the same as in the previous AutoInt model, as was the type of attention: column-only, row-only, or both. The tuning results in @fig-saint-cls-tune where, similar to the AutoInt results, most potential patterns are drowned out by the noise. There were several candidates whose training ended quickly on average, with very poor Brier scores^[Using AdamW as the optimizer did not improve this. Many of these used an exponential decay learning rate scheduler.].  These results detrimentally affected the visualization and were removed from the figure. 

The use of a target token didn't appear to improve performance appreciably, and there is no real trend in the number of attention heads and blocks. Fewer embedding dimensions are preferred (but not too few), and low dropout rates appear favorable. Also, batch sizes with 25 or fewer samples did not perform well.

Across the candidates, the best conditions were `r saint_best$num_embedding` dimensions for its initial embedding. For attention, `r saint_best$attention_type` attention was optimal with `r saint_best$num_attn_blocks` blocks, each with `r saint_best$num_attn_heads` and a dimension of and had an associated dropout rate of `r round(saint_best$dropout_attn, 2)`. The optimization favored a learning rate of 10<sup>`r round(log10(saint_best$learn_rate), 2)`</sup> with a `r fmt_sched(saint_best$rate_schedule)`, a batch size of `r saint_best$batch_size` samples and momentum of `r round(saint_best$momentum, 2)`. This resulted in a Brier score of  `r signif(autoint_best$mean[1], 3)` which, for the moment, has a slight edge over the other models for these data.
...
::: {.important-box}
The models discussed in this section are pretrained. When we use them, no additional training or parameter estimation is required. We are applying a forward pass on a finalized model. The notion of the _training set_ is different. Training has already occurred and, for these models, there were millions of training sets. However, the training data for **our** specific data set is crucial to _set the context_ for the foundational model. 
:::

We'll focus on two implementations of this type of foundational model: TabPFN [@grinsztajn2026tabpfn] and TabICL [@qu2026tabiclv2]. There is substantial engineering effort for both models, and TabPFN has been on a rapid development trajectory. TabPFN is also a proprietary model, and we don't have all of the implementation details. For these reasons, only some of the nuanced aspects of these models will be discussed here. We'll focus on the general process of how those models are trained and how they perform. These results will also lead us to describe certain parts of the architectures to gain additional intuition.

Let's start by discussing the general process for training a prior fitted network. 

### Developing a Tabular Foundational Model {#sec-cls-pfn-development}
...
### Data Generation {#sec-cls-pfn-training-data}

Our previous discussions on Bayesian estimation discussed that a prior distribution reflects our belief about what is possible for our _model parameters_ before the data are observed. For example, in @sec-logistic-bayes, we illustrated Bayesian inference for two parameters in a logistic regression model, using two Gaussian priors.  Once the data are observed, the posterior distribution combines our prior belief with the observed data. 

For prior-fitted networks, the priors also define the characteristics of the data that the model can observe, not strictly the model parameters. This can include data dimensions (e.g., number of features, classes, samples), distributional characteristics (e.g., Gaussian, uniform), relationships among predictors (e.g., multicollinearity), and relationships with the outcome. For example, we might have a small probability that a predictor has extreme outliers or missing values. 

One way that all of this can come together is to use a structural causal model (SCM). This is a directed graph that uses randomness to generate the data and relationships. We start with the general notation of a _cause_; these are nodes on the graph that are latent factors that influence the joint distributions of what will be our observed data (e.g., predictors and outcome). The causes are represented as nodes on a directed graph that starts with a set of random numbers and ends with out observed columns. The causal node are not often univariate. They will modify the joint distribution of intermediate variables. 

Since TabICL is a completely open-source model, let's walk through an example in which it generated a classification data set containing 1,024 data points using a specific random seed. For simplicity, we picked an example that, effectively, uses a fairly direct causal pathway. 
...
TabICL generated a random causal graph that contains nodes and directed edges. @fig-scm-example shows an overview of the quantities being generated. The prior sampled a five-node graph but only node $N_2$ directly affects the observed data. This leaves three of the five (Nodes $N_3$, $N_4$, and $N_5$) with nothing downstream. Those were pruned and never evaluated. What remained were two nodes, $N_1$ feeding $N_2$. 

```{r}
#| label: fig-scm-example
#| echo: false
#| out-width: 70%
#| fig-width: 6
#| fig-height: 6
#| fig-cap: An overview of the process that the structural causal model used to generate a random data set for classification.

knitr::include_graphics("../premade/dag_combined.png")
```

The process begins when 1,024 random uniform values were generated (the "u" node in the figure), which were then transformed in Node $N_1$ by a randomly chosen function. The eight possible transformations were: multilayer perceptrons, ensembles of decision trees, nearest-neighbor discretization, linear maps, quadratic forms, Gaussian processes, soft cluster assignment, and products of two simpler functions. For this example, $N_1$ used a randomly constructed quadratic form: 

$$
\tilde{u}_i = 0.999  -0.801\,u_i -0.595\,u_i^2
$$ 

As we'll see, all subsequent simulated relationships were born from these rowwise values. 

Node $N_2$ did the majority of the work for this realization. First, it selected another randomly chosen function^[From the previous list of eight functional families.] to project $\tilde{u}_i$ onto 13 latent variables. Here, a Gaussian process (with randomly generated coefficients) was chosen to create the projection. Being functions of a single random variable, the thirteen variables are _not independent_ of each other, and none of them carries any information that the first number did not already have. 
...
For the numeric predictor, it was transformed using the method of @kumaraswamy1980generalized using: 
...
For example, $x_2$ selects random points for each category in seven-dimensional space while $x_3$ randomly selects prototypes from a single latent variable ($Z_9$).
...
- $n_{tr} \sim U[L,  2048]$ for some unknown lower limit $L$
- $n_{val} = 128$
...
We'll discuss how these models can handle non-informative or very weakly informative predictors in @sec-cls-pfn-features.  

### Model Structure and Training {#sec-cls-pfn-model}

What is the architecture of the pretrained model? This depends on the implementation. In this section, the details are driven by version 3 of TabPFN. While we probably have the least information about how this model was trained, most of the details below are based on inspecting the source code^[Keep in mind that the rate of change for these models is high; the details are likely to change over time, but the core operations should be stable.]. This means that some of our discussions about why the architecture is the way it is reflect our supposition about the authors' intent. In some cases, there is very little written on why things are done as they are. For the remainder of this section, our use of "TabPFN" should be assumed to be version 3, unless otherwise noted. As we will see in @sec-cls-icl-rate, TabPFN and TabICL have remarkably similar results. 

For a broader perspective on these models, see  @bilovs2026mechanistic. 

TabPFN has a few constants for parameters that could potentially change in the future. These are parameterized as: 
...
- The number of distribution inducing points: $p_{proto} = 128$

These numbers will be used in the details below, but may change in future versions. 

What is the structure of TabPFN's network? Keeping to the same convention used previously, here is a summary of the forestation data, the details of which are discussed in the subsequent sections: 
...
1. **Preprocessing** (no parameters):
   ...
   - Output: two (16 × 2)-wide tables (values and indicators) per data point.
...
4. **Feature-distribution embedder**: 3 identical self-attention blocks (each time 2 stages). The output dimension is 128 per column.
   - Each block, using 289,872 parameters:
     - 128 learned inducing points (128 dimensions each) using 16,384 parameters
...
5. **Column aggregator** (cross-feature interaction): 4 learned aggregation tokens are prepended to each row's 16 column tokens, giving a sequence of 20 tokens; 3 transformer blocks with 8 heads. The output shape is (4 × 128) per row.
   ...
   - Flatten to the ICL dimension (no parameters): the 4 pooling vectors are concatenated into a per-row embedding of dimension 4 × 128 = 512.
...
8. **ICL transformer**: 24 identical blocks with 8 attention heads (head dimension 64), where unknown rows attend to training rows only. The output dimension is 512 for each row.
...
9. **Output head** output shape is $n_{unk} \times C$
   - Many-class retrieval decoder with head dimension 64 and 6 heads:
     - Query projection (unknown embeddings, 512 to 384): 196,992 parameters
...
In this summary, "unknown samples" refer to the data being predicted. 

In total, there are 53,023,736 learned parameters that have already been estimated. This count is the same regardless of the number of predictors, the number of categorical levels, or the number of classes (as long as $C \le C_{max}$). The vast majority of the parameters (`r round(51357696 / 53023736 * 100, 1)`%) come from the 24 iterations of in-context learning. 

The next set of subsections go into more detail for each phase of the model. 

#### Initial Internal Preprocessing {#sec-cls-pfn-preprocessing}

There are a variety of ways that TabPFN preprocesses data. The specific depends on how many estimators are used (see @sec-cls-pfn-ensembles). 

For a single model estimator, the preprocessing _can_ include: 
...
When two or more estimators are used, the preprocessing is deterministic but vary. This is designed to induce variation in the data to maximize the ensemble's effect. 

For example, instead of using a robust centering/scaling approach, the numeric predictors can be coerced to a uniform distribution (not unlike the ordered quantile normalization from @sec-skewness).

Subsequently, each entry of the predictor table is conceptualized as a _cell_. Every value is transformed into a two-element vector containing the predictor value and an integer indicating its completeness. Possible indicator values are: 
...
To define each group, a circular shift gathers neighboring columns: for a given column position. It collects the values a fixed number of positions to the right, wrapping around the last column(s) back to the first. The offsets are $2^i$ for $i = 0, 1, 2$, i.e., positions 1, 2, and 4. Illustrating the shift on predictors `A`, `B`, `C`, `D`, `E`:

- shift 1 position: `B`, `C`, `D`, `E`, `A`
- shift 2 positions: `C`, `D`, `E`, `A`, `B`
- shift 4 positions: `E`, `A`, `B`, `C`, `D`
...
Either way, the point is the same: each cell is pre-loaded with a small amount of local cross-feature context, so when attention and later operations run, every element already carries some information about its potential neighbors.
...
where $\boldsymbol{W}_{emb}$ is (128 × 6) and there are 128 bias terms. The 6 columns in the weight matrix correspond to the values and indicators for the $g = 3$ groups. Note that this process is a linear projection from 6 columns to $p_{emb} = 128$. Both the predictor's _values_ and completeness _indicators_ are blended in this projection; there is no separate indicator channel from this point on.

The influence of the target classes is added to the cell embedding, but only for the training data. 

Also, each outcome class is represented by a pre-learned 128-dimensional vector. These vectors are initialized to be orthonormal^[They have a length of one and have pairwise dot products of zero.], which keeps the classes maximally distinguishable at the start of training; they are then learned during pretraining. TabPFN has a hard limit on the number of classes, $C_{max}$, and creates a table $\boldsymbol{Y}_{emb}$ of size $C_{max} \times 128$. 

For the training data, the appropriate row of $\boldsymbol{Y}_{emb}$ is added to each cell embedding in that row (the same vector across all $p$ columns), nudging them toward the class associated with that row. The unknown data are kept as-is since their outcome values are unknown.
...
- Stage 1, where inducing points attend to training rows.
- Stage 2, where all rows attend to the inducing points.

Stage 1 has its own pretrained set of learned query inputs, denoted as $\boldsymbol{\Theta}$, that is (128 × 128). These 128 rows are inducing vectors^[That the number of inducing vectors ($n_{prot}=128$) equals the embedding dimension ($p_{emb}=128$) is coincidental.], similar to _prototype points_.  Prototypes in machine learning are vectors chosen or estimated to represent a distribution [@HastieEtAl2017, chapt. 13]. These can be data points selected from a sample or estimated. The values in $\boldsymbol{\Theta}$ are used as the query data for attention, while our embeddings serve as the keys and values (as we'll see shortly). Since there are three attention blocks, there are distinct $\boldsymbol{\Theta}$ matrices for each. 

Both stages use row attention. Within a single feature (column), the rows attend to one another. This is done indirectly here by having the inducing points $\boldsymbol{\Theta}$ attend to the training rows' embeddings for that feature. Our first action is to use cross-attention with the $n_{tr}$ training points. This produces an array of size $n_{prot} \times p_{emb}$ (128 × 128) containing the attended inducing vectors. There is one vector per inducing, each a weighted average of the training rows' embeddings for that feature, with weights reflecting how strongly each training row matched that inducing. We'll label the results as $\boldsymbol{H}_{prot}$, and they can be thought of as distributional reference values. 

@fig-cross-attention shows a simplified low-dimensional example. The left panel shows the training set from @fig-nnet-ex as blue dots, along with 16 inducing points^[We chose these inducings to be a regular grid, not because they are realistic, but to show how the resulting values change during this process.] in red and a single new sample point in black. These computations are simplified by omitting the initial embedding process, setting the attention matrices to the identity matrix (such as $\boldsymbol{W}_{q} = \boldsymbol{I}$, etc.), and omitting a few ancillary steps.

```{r}
#| label: cross-attention
#| include: false

point_types <- c("Training", "Inducing", "New")
...
embd_test <-
  tibble(A = 1, B = 0) |>
  mutate(type = factor("New", point_types))
...
prototypes <-
  ...
  scale() |>
  as_tibble() |>
  mutate(type = factor("Inducing", levels = point_types))
...
test_tmp <- bind_cols(
  ...
  mutate(type = factor("New", levels = point_types))
...
The middle panel shows how cross-attention uses the training-set points to shift the 16 inducing points to $\boldsymbol{H}_{prot}$. The result is that the points emulate the correlation structure of the original data, albeit with larger variances. The inducing values occupy a space larger than the training data mainstream. 

The second stage of cross-attention uses all data (train and unknown) as queries, with $\boldsymbol{H}_{prot}$ as keys and values. The resulting attended values are also combined with their original values via a residual connection. 

The right panel shows that the single unknown sample was shifted toward the upper-right of the distribution. The variable-sized red points indicate the attended inducing vectors, and their sizes reflect the extent to which the unknown sample attended to each reference point^[Again, keep in mind that this has been an unsupervised example of how cross-attention works. The effect shown here isn't necessarily indicative of how cross-attention functions in the wild.]. While not shown, the training set maintains its shape but with a wider distribution than before. 

The literature often describes operations like these as "distributionally aware." Beyond the anthropomorphizing, this term doesn't really describe what is occurring. The inducing points are learned during pretraining and function as prototype vectors. Prototypes are learned vectors that act as "stand-ins" for a larger dataset. Since $\boldsymbol{\Theta}$ is optimized by the objective function, it also serves as a prototype that carries some supervised information. In other words, these are the best prototypes that encourage predictive accuracy. 

::: {.important-box}
In this simplified version, the first stage of cross-attention coerced our inducing points' distribution to resemble the observed data. It recontextualized the inducing points to _our_ data. 

The second stage does the same thing in reverse, using our observed data. It pulls our data to the more realistic prototypes. The blended values are added to the row's own vector (the residual connection). This preserves the original information and adds more context about which prototypes it resembled and how strongly.
:::

Now we will describe the actual process and all of its operations. The process starts by normalizing the inducing points and our observed features. TabPFN uses RMSnorm to do this. Later on, we'll use some of the original values for a residual connection. 

After normalization, our previously described first stage of cross-attention occurs. The inducing points and the attention parameter matrices have all been estimated during pretraining. Operationally, the attention process is split into 8 attention heads, each taking 16 features from the full collection of 128 and coupling them with their corresponding columns of the attention $\boldsymbol{W}$ matrices. From this process, we have calculated the attention weights for the inducing points. 

However, the model takes an extra step to alter the attention weights. The concern is that the row-wise softmax operation, which coerces the attention weights to sum to 1, can lose focus when the training set is large. A multiplier for the queries is computed from two different MLP submodels, and then softmax is used. After this, the row-wise weights are coerced to sum to 1, and the attended inducing points are computed. These are denoted as $\boldsymbol{\Theta}_a$

The newly adjusted inducing points $\boldsymbol{\Theta}_a$ are then combined with a residual connection to their values (pre-normalization), producing $\boldsymbol{\Theta}_r$. These values are then used in an MLP that first re-normalizes them, then passes them through a hidden layer with 256 units and GELU activation (see @fig-relu-activations). The resulting features are then added back to $\boldsymbol{\Theta}_r$ to produce our new inducing points previously labeled as $\boldsymbol{H}_{prot}$. Thus concludes the first stage of cross-attention. 

The second stage is the same as the first except that: 

- The combined training and unknown data serve as queries.
...
This two-stage process repeats in each of the three consecutive blocks, with each block's output feeding the next.
...
The column aggregator will now augment that data structure with some pretrained values that we'll call aggregation tokens^[Historically, these would be called "CLS tokens" where CLS was an abbreviation for the target classes. To avoid the connotation of being supervised, we'll call these aggregation tokens.] and denote them as $\boldsymbol{t}$. For TabPFN version 3, there are $k = 4$ tokens, each with a pretrained vector of $p_{emb} = 128$ learned parameter estimates. Now, for a given row, we have 
...
1. Normalize the augmented data (RMSNorm, in this case)
...
4. RMSNorm
5. A standard MLP architecture with a layer containing 256 units and GELU activation. 
...
- The combined training and unknown data as queries
- The training rows serve as keys and values. 

This identifies the training rows that the unknown rows most resemble and reweights them accordingly. 

Before the first ICL block is applied, the target information is injected a second time. Recall from Section @sec-cls-pfn-initial-embedding that each training row's class vector (a row of the $C_{max} \times 128$ table $\boldsymbol{Y}_{emb}$) was added to its cell embeddings; that signal has since been pooled into each training row's 512-dimensional summary by the column aggregator. A second, separate table of size $C_{max} \times 512$, denoted as $\boldsymbol{Y}_{icl}$, now re-asserts the outcome at the row level: the appropriate row of $\boldsymbol{Y}_{icl}$ is added to each training row's summary vector. Like $\boldsymbol{Y}_{emb}$, this table was initialized to be orthonormal and then learned during pretraining (it accounts for the 5,120 target-encoder parameters counted earlier). The unknown rows again receive nothing.

These label terms are what make the attention steps below informative. The unknown rows never carry target information themselves. Instead, when an unknown row attends to the training rows it most resembles, the content it retrieves includes those rows' label terms. The label information flows from the training rows to the unknown rows through the attention weights.
...
2. Row-wise cross-attention, with all rows as queries and the training rows as keys and values.
3. Compute the query multiplier.
4. Row-wise softmax to generate the attention weights.
5. Compute the attended values.
6. Use pretrained weights in a linear operation.
7. Add a residual connection from the pre-normalized values.
8. Apply RMSNorm to the result.
9. Apply an MLP with one hidden layer of 1,024 units and GELU activation.
10. Add a second residual connection from the values produced in step 7.
...
The output of this ICL block is the arrays of training and unknown values. These are sent to the next block to produce the class probabilities for our unknown samples. 

#### Output Head {#sec-cls-pfn-output}

At this point, we are ready to translate our embeddings to class probability estimates for the unknown samples. For a more conventional model, we would linearly project the embeddings into $C$ columns and then apply softmax to obtain probability-like outputs. 
...
TabPFN uses separate linear operations with different pretrained weights to transform the $n_{unk}$ rows and the $n_{tr}$ rows from 512 to 384 dimensions. Cross-attention is applied in a new way: 

- The $n_{unk}$ unknown rows serve as the queries 
...
### In Context Learning Rate {#sec-cls-icl-rate}

The most important part of the architecture is the ICL block. Based on our training data, the ICL machinery can relate what it learned from the simulated data during training to the current problem's data. However, how much data do we need to provide to the model for it to learn the correct context? Foundational models can be very computationally expensive, even during prediction, and it makes sense to understand how performance changes as more of our project's training data is used to "prime the pump." 

To better understand, the forestation training set was sampled in various sizes, and these data were used to initialize the model. To measure performance, out-of-sample predictions were generated^[Unfortunately, using the test set.], and the Brier score and the area under the ROC curve were computed. This was repeated 25 times, and the random numbers used to sample the training set were standardized so that both implementations used the same data in each replicate. 

@fig-icl-metrics shows how the metrics changed over data size. The 0.05, 0.50, and 0.95 quantiles of the replicate runs are reflected in the center line and shaded regions. We can see that performance on extremely small samples is at the no-information rate for each metric, with a high degree of replicate-to-replicate variation. As data size increases, performance improves, and variation shrinks considerably. The trajectories of both metrics appear to improve only marginally with larger sample sizes, suggesting that additional data might yield only slight improvements. 
...
#| fig-width: 6
#| fig-height: 4
#| fig-cap: "Performance changes over 'training' set sizes. The line represents the estimated median performance value, and the shaded regions indicate the 5% and 95% quantiles for the 25 replicates per sample size."

library(splines)
qr_brier_pfn <-
  quantreg::rq(
    .estimate ~ ns(log2(samples), df = 3),
    tau = c(0.05, 0.5, 0.95),
    data = pfn_icl_metrics |> filter(.metric == "brier_class")
  )
qr_roc_pfn <-
  quantreg::rq(
    .estimate ~ ns(log2(samples), df = 5),
    tau = c(0.05, 0.5, 0.95),
    data = pfn_icl_metrics |> filter(.metric == "roc_auc")
  )
qr_brier_icl <-
  quantreg::rq(
    .estimate ~ ns(log2(samples), df = 5),
    tau = c(0.05, 0.5, 0.95),
    data = icl_icl_metrics |> filter(.metric == "brier_class")
  )
qr_roc_icl <-
  quantreg::rq(
    .estimate ~ ns(log2(samples), df = 5),
    tau = c(0.05, 0.5, 0.95),
    data = icl_icl_metrics |> filter(.metric == "roc_auc")
  )

log_n_grid <- tibble(samples = 2^seq(2, 12, length.out = 100))
icl_ranges <-
  bind_rows(
    predict(qr_brier_pfn, log_n_grid) |>
      as_tibble() |>
      set_names(c("lower", "median", "upper")) |>
      mutate(.metric = "brier_class", model = "TabPFN") |>
      bind_cols(log_n_grid),
    predict(qr_roc_pfn, log_n_grid) |>
      as_tibble() |>
      set_names(c("lower", "median", "upper")) |>
      mutate(.metric = "roc_auc", model = "TabPFN") |>
      bind_cols(log_n_grid),
    predict(qr_brier_icl, log_n_grid) |>
      as_tibble() |>
      set_names(c("lower", "median", "upper")) |>
      mutate(.metric = "brier_class", model = "TabICL") |>
      bind_cols(log_n_grid),
    predict(qr_roc_icl, log_n_grid) |>
      as_tibble() |>
      set_names(c("lower", "median", "upper")) |>
      mutate(.metric = "roc_auc", model = "TabICL") |>
      bind_cols(log_n_grid)
  ) |>
  mutate(
    .metric = ifelse(.metric == "brier_class", "Brier Score", "ROC AUC")
  )
...
bind_rows(pfn_icl_metrics, icl_icl_metrics) |>
  ...
  ggplot(aes(samples)) +
  geom_point(aes(y = .estimate), alpha = 1 / 20, cex = 1) +
...
### Resistance to Irrelevant Predictors {#sec-cls-pfn-features}

Recall that, for simpler MLP models, @sec-nnet-preprocessing emphasized the need to remove irrelevant predictors from the training. A simulation study was presented in @sec-irrelevant-predictors, in which the performance of MLP models fell by about 40% when multiple noise columns were present in the data. 

Interestingly, two other neural network models are shown in @fig-irrelevant-predictors: TabNet and TabPFN. TabNet [@arik2021tabnet] is a tabular deep learning model that "uses sequential attention to choose which features to reason from at each decision step." This happens separately for each row in the data. Generally speaking, "feature selection" refers to removing _original predictors_ from a model to make it more parsimonious and improve performance by eliminating extraneous predictors. TabNet's procedure is more akin to _embedding selection_, since it occurs midway through the network and cannot make the prediction process functionally independent of the original predictors. We can see from @fig-irrelevant-predictors that TabNet also suffers when noise columns are added to the training data. 
...
One idea is that the algorithm used to generate the simulated training data biases the final model towards a sparser feature set. In @hollmann2025accurate, they note that 

> "Disjoint subgraphs lead to features that are marginally
> independent of the target if they are not connected to the target node, reflecting real-world scenarios with uninformative predictors."
...
While not shown in @fig-irrelevant-predictors, TabICL also has resistance to uninformative predictors. Since this is an open-source model, we have more definitive information regarding how it was trained. The prior for the model's supervised structure assigns 70% probability to an MLP-type equation and 30% to a tree-based structure. It also induces uninformative predictors by initializing weights to zero in blocks, thereby eliminating the effects of one or more predictors. 

### Ensembling {#sec-cls-pfn-ensembles}

These foundational models are sensitive to data ordering. For example, one can generate different results if the order of the predictor columns or the class levels of the data (outcome or predictor) changes from run to run. This causes a slight increase in _model variance_. As we will discuss in @sec-cls-ensembles and @sec-ensembles, one way to improve unstable models is to create multiple realizations of the model and then average their outputs to produce the final prediction. Bagging, boosting, and random forests are examples of this strategy. 
...
How much do the predictions change over different data configurations? To assess this, we used the forestation data with reshuffled row orders and the levels of the sole qualitative predictor (county), trained a TabPFN model, and then predicted the test data. This was performed twelve times. From here, we examined many randomly selected prediction replicates across ensemble sizes of 2 to 12. For each distinct row of the predicted data, we saved the individual probability estimates for each estimator and converted them into a logit scle. @fig-cls-tabpfn-ensemble shows a smoothing spline fit to logit's standard deviation as a function of the mean, per ensemble size. There is no meaningful shift in variation as ensemble members are added; the curves for these data show significant overlap.  
...
for (iter in 1:12) {
  set.seed(iter)
  for (bags in (1:5) * 2) {
    bag_fit <- bagger(class ~ ., data = forested_train, times = bags)
    bag_pred <- 
      predict(bag_fit, forested_train, type = "prob") |> 
      bind_cols(forested_train) |> 
      add_rowindex() |> 
      mutate(estimate_index = iter, num_estimators = bags) |> 
      select(.row, estimate_index, class, .pred_Yes, num_estimators)
    bag_pred_var <- bind_rows(bag_pred_var, bag_pred)
  }
}
...
bag_pred_var <- 
  bag_pred_var |> 
  mutate(
    logit = binomial()$linkfun(.pred_Yes), 
    logit = ifelse(is.infinite(logit), sign(logit) * 10, logit),
    Model = "Bagged Tree")
```
```{r}
#| label: fig-cls-tabpfn-ensemble
#| echo: false
#| warning: false
#| message: false
#| out-width: 90%
#| fig-width: 9
#| fig-height: 3.6
#| fig-cap: Variation in TabPFN and bagged tree predictions for different numbers of estimators in an ensemble.

load("../RData/ensemble_estimates.RData")

bind_rows(
  ensemble_estimates |> 
    mutate(Model = "TabPFN", .row = as.integer(.row)),
  bag_pred_var
)  |>
  mutate(Model = factor(Model, levels = c("TabPFN", "Bagged Tree"))) |> 
  summarise(
    mean = mean(logit),
    sd = sd(logit),
    std_err = sd / sqrt(length(logit)),
    .by = c(Model, .row, num_estimators)
  ) |>
  mutate(
...
As a counterpoint, the figure also shows the same analysis for bagged tree models with the same ensemble sizes. In this panel, we see that the variation in the logits is very high with two estimators, but it drops significantly as more estimators are included. 

Ensembles thrive on propagating model variation via prediction replicates. This figure indicates that the multiple TabPFN estimates do not significantly increase prediction diversity as new replicates are added. Therefore, adding multiple estimators may not help much, at least for this data set. We'll also see in @sec-cls-pfn-forestation that there is a weak signal in a very low-noise setting when adding estimators for both TabPFN and TabICL. There is a very small, measurable improvement, much smaller than we see when ensembling high-variance models (e.g., trees).  However, this may not be the case for other data sets with more class levels and additional qualitative predictors. 

### Application to the Forstation Data {#sec-cls-pfn-forestation}

Both TabPFN and TabICL were used to predict the probability of forestation. Each was tuned over the number of estimators. @fig-foundational-cls-tune shows that TabPFN performs better; it has a slight edge over TabICL, given the tight variation in the results. As we'll see in the next section, they are practically equivalent. These results are also the best seen so far. 
...
pfn_brier <-
  full_join(
    pfn_brier,
    pfn_brier |>
      filter(num_estimators == 1) |>
      select(baseline = mean, Model),
    by = "Model"
  ) |>
  mutate(
    pct_imp = (baseline - mean) / baseline * 100,
    pct_imp = round(pct_imp, 1)
  )

multi_imp <-
  pfn_brier |>
  slice_max(pct_imp, n = 1, by = c(Model), with_ties = FALSE)
```
...
#| fig-width: 7
#| fig-height: 4.25
#| fig-cap: TabPFN and TabICL tuning results for the forestation data.

pfn_brier |>
  ggplot(aes(x = num_estimators, y = mean, col = Model)) +
  geom_point() +
  geom_smooth(se = FALSE, method = "gam", formula = y ~ s(x, bs = "cs")) +
  labs(x = "# Estimators", y = "Brier Score", color = NULL) +
  scale_color_manual(values = c("#D84D16FF", "#132157FF")) +
  theme_bw() +
  theme(legend.position = "top")
```

Also, there is a trend in the number of estimators. TabICL shows limited improvement after two estimators, and its best configuration results in a `r multi_imp$pct_imp[multi_imp$Model == "TabICL"]`% improvement over a single estimator. TabPFN shows a greater difference but also demonstrates that adding more estimators does not monotonically improve the situation. Its best configuration is `r multi_imp$pct_imp[multi_imp$Model == "TabPFN"]`% better than a single value. Ensembling doesn't seem to buy us much here, and that's good news; the model variance is low even when we perturb the data ordering and preprocessing methods. 
...
#| fig-width: 6
#| fig-height: 3.5
#| fig-cap: "Comparisons of current forestation models. The intervals are 90% credible intervals. The bars on the right show the probability of practical equivalence to the current best model, assuming a Brier score difference of ±0.01 is practical. "
...
Practically speaking, the KNN model would have some drawbacks if we had used it as the final model. It has to store the entire training set and conduct a fairly expensive nearest-neighbors search when predicting each new point. MLP models are easier to deploy because their prediction equation is fairly simple and their deployment costs are low (especially when converted to SQL). The foundational models show better performance but also have drawbacks. First, some require a commercial license to use. Second, the model requires local files with the model weights, which require hundreds of MD of storage. Lastly, prediction greatly relies on having a GPU installed and configured.
...
save_if_missing(forest_tabpfn_set_res)
save_if_missing(forest_tabicl_set_res)
save_if_missing(forest_autoint_set_res)
save_if_missing(forest_saint_set_res)
```
