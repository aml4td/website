...
When reading this visualization, the values in the contour plots along each row will sum to 1.0 at each location. A few of the patterns show that the hidden units have little impact on the updated versions, including cells $(H_1, A_1)$, $(H_2, A_2)$, and $(H_4, A_4)$. Other hidden units have a broad influence on the new embeddings in large regions of the predictor space (e.g., $(H_4, A_4)$). 
...
* **Column self-attention**: 3 blocks, 2 heads of dimension 13, block output dimension (2 × 13 = 26)
...
* **Output head** output shape is $C = 2$
  * Linear operation from 416 to 2: 416 × 2 + 2 = 834 parameters (includes 2 bias terms)
  * Softmax
