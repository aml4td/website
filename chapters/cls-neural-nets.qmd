...
For v3; this can change from version to version. Much of this descritption is from the source code; the intent is not always written down. 
...
* **Preprocessing** (no parameters): each of the 16 columns is mean-imputed and standardized, and a completeness indicator column is computed alongside each feature. Output: two 16$-wide tables (values and indicators).

* **Feature grouping** (no parameters): each cell is replaced by a circular-shifted group of $g = 3$ sibling columns, concatenated with their 3 indicators, giving 6 values per cell.
...
* **Feature-distribution embedder**: 3 identical induced-self-attention blocks, applied per column with shared weights, 8 attention heads (head dimension 16) and 128 inducing points. The output dimension is 128 for each column.
  * Each block, using 289,872 parameters:
    * 128 learnable inducing vectors (128 dimensions each) using 16,384 parameters
    * Cross-attention block 1 (inducing points attend to training rows) using 142,032 parameters
        * Query/key/value/output projections: 4 × (128 × 128) = 65,536 parameters
        * Query-scaling MLP using 10,576 parameters
        * GELU MLP with layer sizes [128, 256, 128] using 65,536 parameters
        * 3 layer normalizations (128 dimensions) using 384 parameters
    * Cross-attention block 2 (all rows attend to the inducing points) using 131,456 parameters (same as block 1, without the query-scaling MLP)
  * Feature-distribution embedder parameters: 3 × 289,872 = 869,616

* **Column aggregator** (cross-feature interaction): 4 learnable target tokens are prepended to each row's 16 column tokens, giving a sequence of 20 tokens; 3 transformer blocks with 8 heads and rotary positional embeddings, the last of which reads out only the 4 target tokens. The output shape is (4 × 128) per row.
  * 4 target tokens (128 dimensions each) using 512 parameters
...
#### Initial Internal Preprocessing
...
#### Initial Cell and Target Embeddings
...
#### Feature-Distribution Embedding
...
The second stage of cross-attention uses all data (train and test) as queries, with a scaled $\boldsymbol{H}_{ind}$ as keys and values. The resulting attended values are also combined with their original values via a residual connection. 
...
In reality, the process starts by normalizing the inducing points and our observed features. TabPFN uses RMSnorm to do this. Later on, we'll use some of these normalized values for a residual connection. 
...
However, the model takes an extra step to alter the attention weights. The concern is that the row-wise softmax operation, which coerces the attention weights to sum to 1, can lose focus when the training set is large. A multiplier for the pre-softmax attention scores is computed from two different MLP submodels. After this, the row-wise weights are coerced to sum to 1, and the attended inducing points are computed. These are denoted as $\boldsymbol{P}_a$
