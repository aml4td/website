...
The feature-distribution embedder serves several purposes. Before proceeding, recall that each cell in our $n \times p$ array is now a 128-dimensional vector, and that here a "feature" is one of the $p$ columns.

First, it makes the model robust to arbitrary distributions in the current dataset. Because these foundation models are pretrained on a large number of synthetic datasets with widely varying distributions, the frozen model must represent a value by where it sits in its feature's distribution rather than by its raw magnitude. This achieves a normalization-like invariance. However, unlike the fixed centering/scaling used in preprocessing, it is achieved through a learned, nonlinear mechanism whose parameters were tuned during pretraining precisely because this improved predictions.

Second, this process will use a version of row attention. This produces a learned co-representation for a single feature at a time. Each row's embedding is re-expressed as a learned function of the other rows in its column. The model will have to compare rows from the training set to rows from the test set. 
This sets up an apples-to-apples comparison between the two that will occur during the in-context learning portion of the model.

TODO process starts with ....
...
This stage uses row attention. Within a single feature (column), the rows attend to one another. This is done indirectly here by having the inducing points $\boldsymbol{P}$ attend to the training rows' embeddings for that feature. Our first action is to use cross-attention with the $n_{tr}$ training points. This produces an array of size $n_{ind} \times p_{emb}$ (128 x 128) containing the attended training rows influenced by the inducers. We'll label them $\boldsymbol{H}_{ind}$, and they can be thought of as distributional reference values. 

@fig-cross-attention shows another simplified low-dimensional example. The left panel shows the training set from @fig-nnet-ex, along with 15 inducer points in red and a single test point in green. These computations are simplified by omitting the initial embedding process and setting the attention matrices to the identity matrix (i.e., $\boldsymbol{W}^{q} = \boldsymbol{I}$). 
...
levels = 
num_ind <- 15
seq_a <- seq(-2, 2, length.out = num_ind)
seq_b <- seq(-2, 2.5, length.out = num_ind)

inducers <- 
  sfd::get_design(2, num_ind) |> 
  sfd::update_values(list(seq_a, seq_b)) |> 
...
p_0 <- 
  ...
  geom_point(aes(pch = type, col = type), data = inducers, cex = 2.5, alpha = 3/4) +
...
S1 <- (p_mat %*% t(tr_mat)) / scale_term        # scores  (6 x 1000)
attn_wts_1 <- t(apply(S1, 1, softmax))          # weights (6 x 1000), rows sum to 1
Hd <- attn_wts_1 %*% tr_mat                     # panel   (6 x 1000)
...
test_wts <- 
  tibble(weight = A2[1001,], point = 1:15) |> 
  bind_cols(
...
p_2 <- 
  embd_train |> 
  ggplot(aes(A, B)) + 
  geom_point(aes(pch = type, col = type), alpha = 0.2, cex = 1,
             show.legend = FALSE) + 
  geom_point(data = embd_test, pch = 17, cex = 3, show.legend = FALSE, col = type_cols[3]) +
  geom_point(data = test_wts, aes(A, B, size = weight),
             alpha = 2 / 3, col = type_cols[2],
             show.legend = FALSE) + 
  geom_segment(
    data = test_tmp,
    ...
    linewidth = 1 / 2,
    arrow = arrow(length = unit(0.15, "cm"), type = "closed"),
    show.legend = FALSE, col = type_cols[3]
  ) +
  scale_color_manual(values = type_cols) +
  scale_shape_manual(values = c(16, 16, 17)) +
  coord_fixed(ratio = 1.2) +
...
#| dev: "ragg_png"
#| warning: false
#| fig-cap: "A demonstration of cross-attention"
...
The middle panel shows how cross-attention uses the training-set points to shift the 15 values in $\boldsymbol{H}_{ind}$, some of which were outside the mainstream of the training data, towards the center of the training data.  

The second stage of cross-attention uses all data (train and test) as queries, with $\boldsymbol{H}_{ind}$ as keys and values. The right panel shows that the single test sample was shifted toward the upper-right of the distribution. The variable-sized red points indicate the attended inducer points, and their sizes reflect the extent to which the test sample attended to each reference point. Although not shown in this figure, the entire training set was also adjusted. That trend showed a moderate compression of the distribution towards the center, much like the effect observed in the induced samples. 
