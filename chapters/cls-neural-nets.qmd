...
To be more formal, let's describe the computation for a _single sample_. It enters the attention head as $p$ columns, that is, $p$ scalar values (across the batch, an $n \times p$ matrix). These might be the original $p$ predictors, or the hidden units from an earlier layer, depending on the situation.
...
For column attention, these coefficients always sum to one and will be different from sample to sample.
