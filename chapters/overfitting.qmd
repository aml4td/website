...
For many sophisticated models, the complexity can be modulated via one or more tuning parameters that can cause the model to detect inconsequential patterns in the training data. The problem is that they may go too far and over-optimize the model fit. 
...
#| fig-alt: "Four-panel scatter plot of two predictors A and B with two classes shown as circles and triangles. Panel (a) shows the smoothly curved true class boundary. Panels (b) through (d) show estimated boundaries at increasing model complexity: panel (b) has a simple curve that slightly underfits, panel (c) has a more accurate parabolic boundary, and panel (d) has a highly contorted boundary with small islands and extraneous regions that overfit the training data."
f <- expr(-1 - 4 * A - 2 * B - 0.2 * A^2 + 1 * B^2)

x_seq <- seq(-4, 4, length.out = 100)
grid <- crossing(A = seq(-3, 3, length.out = 100), B = x_seq) |>
  mutate(
    lp = rlang::eval_tidy(f)
  )
...
#| fig-alt: "Scatter plot of test set data for two predictors A and B with the overly complex decision boundary from the high-cost model overlaid. The contorted boundary misclassifies many test points, with its small islands and extraneous regions containing the wrong class. The smooth true boundary is shown faintly for comparison."
sim_new |>
  ggplot(aes(A, B)) +
  geom_point(
    aes(col = class, pch = class),
...
#| fig-alt: "Line chart of Brier score versus minimal node size for two evaluation methods. The resubstitution curve decreases sharply as node size decreases, suggesting small values are best. The test set curve tells the opposite story: error is high for small node sizes due to overfitting, reaches a minimum in the middle range, then slowly rises again as the model underfits."
brier_all |>
...
Contrasting these two curves, data that are _external_ to the training set give a much more accurate picture of performance. However:
...
#| fig-alt: "Line chart of Brier score versus minimal node size for three evaluation methods. The resubstitution curve still declines with smaller node sizes. A third resampling curve is added alongside the test set curve; the resampling results follow a similar U-shaped pattern to the test set results, identifying a similar optimal region, though the resampling estimates are slightly more pessimistic."
brier_all |>
  ggplot(aes(min_n, brier, col = Method, pch = Method)) +
  geom_point(cex = 1.2) +
  geom_line() +
...
One tuning parameter used in tree-based ensembles and neural networks is the _learning rate_. This parameter typically governs how quickly a model adapts during iterations of model fitting (i.e., training). In gradient descent, the learning rate specifies how far we proceed in the optimal direction. For machine learning models, the parameter must be greater than zero and usually has a maximum value around 0.1 to 1.0. When evaluating different learning rates, it is common to think of them in logarithmic units (typically, base 10). This is because the effect of the parameter on model performance is often nonlinear; a per-unit change of 0.05 has different effects depending on where in the total range it occurs. 
...
#| fig-alt: "Line chart of error versus learning rate on a logarithmic scale. A faint line shows the overall trend: high error at very low rates, a trough of low error in the middle range, and rising error at high rates. Five solid points are overlaid at evenly spaced grid locations across the rate range, illustrating how a simple grid search samples from the error curve."
set.seed(1)
...
#| fig-alt: "Animated plot of error versus learning rate on a logarithmic scale showing Bayesian optimization in progress. Three open circles mark the initial random samples. Solid circles appear one at a time as the search iterates, first exploring distant regions of the rate range, then concentrating near the optimal low-error region."
#| message: false
#| warning: false
knitr::include_graphics("../premade/anime_learn_rate.gif")
```

The animation shows that the search evaluated very disparate values, including the lower and upper limits that were defined. However, after a few iterations, the search focuses on points near the optimal value. Again, this method is discussed in more detail in @sec-iterative-search.
...
svm_brier_perm <-
  ...
  mutate(
    brier_perm = map_dbl(
      splits,
      ~ brier_class(analysis(.x), class, .pred_one)$.estimate
    )
  ) |>
  pluck("brier_perm") |>
  mean() |>
...
When a model is overfit, the results can be unduly optimistic. Let's take a minute to consider the opposite: if we compute some performance measure, what's the worst value we can achieve? 
...
As a counter-example, the mean absolute error (MAE) used in @sec-whole-game to measure the effectiveness of our time-to-delivery models does depend on the outcome data. We know a value of zero is best, but what is the worst it can be? 
