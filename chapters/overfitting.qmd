...
mae_perm <- 
  lin_reg_res %>% 
  collect_predictions() %>% 
  permutations(permute = time_to_delivery, times = 50) %>% 
  mutate(
    mae_perm = map_dbl(splits, ~ mae(analysis(.x), time_to_delivery, .pred)$.estimate)
  ) %>% 
  pluck("mae_perm") %>% 
  mean() %>% 
  signif(digits = 3)
...
For example, the linear regression model had a validation set MAE of less than two minutes (`r signif(show_best(lin_reg_res, metric = "mae")$mean, 3)`), and the difference in MAE between models was measured _in seconds_. For perspective, the worst possible model would have a MAE of `r mae_perm` fractional minutes (using 50 permutations). Given how large this value is, any of our three modeling approaches for that data do well. 

Permutated performance values will show up several times in subsequent chapters. 
