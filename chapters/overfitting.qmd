...
smoothed <- function(x, y) {
  supsmu(x, y, span = 0.1)$y
}
...
#| fig-width: 6
#| fig-height: 4.25
#| fig-cap: "An example to show how complexity and model error, measured two ways, interact. The tuning parameter on the x-axis is $n_{min}$ used for moderating decision tree growth."

brier_all %>% 
  filter(Method != "Resampling") %>% 
  ggplot(aes(min_n, smoothed, col = Method, pch = Method)) + 
  geom_point(aes(y = brier), cex = 1.2, alpha = 1 / 4) +
...
grid_points_smoothed %>% 
  ...
  geom_line(aes(y = mean), linewidth = 1, alpha = 1 / 4) + 
  geom_point(
    data = grid_points_smoothed %>% slice(c(1, 25, 50, 75, 100)),
    aes(y = mean),
    cex = 3
  ) +
  scale_x_log10() +
