...
pointers_blowback <-
  tibble(A = c(2.20), B = c(-3.5), C = c(1.25), D = c(-3.5))
...
brier_all <-
  ...
  mutate(
    Method = factor(
      Method,
      levels = c("Resubstituion", "Test Set", "Resampling")
    )
  )

method_cols <- c(
  Resubstituion = "#f6c85f",
  "Test Set" = "#6f4e7c",
  Resampling = "#0b84a5"
)
...
We took the training data and fit models using values $n_{min}$ = 3, 4, ..., 80. To understand performance, the Brier score [@brier1950verification;@steyerberg2009] is used (described in @sec-brier). It is an error metric for classification models (similar to RMSE) and measures how close the predicted probabilities are to their binary (i.e., 0/1) values. A Brier score of zero is a perfect model, and ineffectual models tend to have values $\ge$ 0.25. 
