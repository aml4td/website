...
customer_tbl |> thm_tbl_lt()

customer_tbl |> thm_tbl_dk()
```

Indicator columns produced from a categorical column using a reference cell parameterization.
...
::: {#tbl-ref-cell-effects}

```{r}
#| renderings: [light, dark]

ref_cell_tbl <- format_encoding(ref_cell_mod)

ref_cell_tbl |> thm_tbl_lt()

ref_cell_tbl |> thm_tbl_dk()
```

An example of linear regression parameter estimates corresponding to a reference cell parameterization.
...
```{r}
#| renderings: [light, dark]
hot_tbl <- format_encoding(hot_mod)
...
One-hot encoded indicator variables from a categorical column of data.
...
hotel_rate_p + 
  geom_histogram(bins = 40, col = dark_bg) +
  thm_dk
```

:::

The frequency distribution of agents in the training set.
...
hash_tbl <- 
  ...
  rename(Agent = agent) %>%
  mutate(
    Agent = remake_name(Agent),
    across(-Agent, ~ as.character(.x))
    ) %>% 
  gt() %>% 
...
hash_tbl |> 
  thm_tbl_lt() |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`01`), rows = `01` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`02`), rows = `02` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`03`), rows = `03` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`04`), rows = `04` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`05`), rows = `05` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`06`), rows = `06` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`07`), rows = `07` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`08`), rows = `08` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`09`), rows = `09` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "grey80")),
            locations = cells_body(columns = c(`10`), rows = `10` == " 0")
  ) 

hash_tbl |> 
  thm_tbl_dk() |> 
  tab_style(style = list(cell_text(color = "#494949")),
            locations = cells_body(columns = c(`01`), rows = `01` == " 0")
  ) |> 
  tab_style(style = list(cell_text(color = "#494949")),
            locations = cells_body(columns = c(`02`), rows = `02` == " 0")
...
Signed indicators for agent via feature hashing.
...
$$
y_{ij} = { \underbrace{(\beta_0 + \beta_{0j})}_{\text{intercept(s)}} }  + \epsilon_{ij} = \mu_j + \epsilon_{ij}
...
agent_dark <- 
  agent_means +
  geom_vline(xintercept = overall_mean, lty = 3, col = dark_line) +
  geom_point(aes(size = n), alpha = 3/5, col = dark_data) +
  thm_dk

blup_plot <- 
  blups %>% 
  ggplot(aes(x = centered)) + 
  labs(x = ~ paste(hat(beta)[0][j]), title = "(a) Random Intercept Estimates")

blup_light <- 
  blup_plot  +
  geom_histogram(col = light_bg, bins = 20) +
  geom_rug(alpha = 1 / 2) +
  thm_lt

blup_dark <- 
  blup_plot  +
  geom_histogram(col = dark_bg, bins = 20, fill = dark_data) +
  geom_rug(alpha = 1 / 2, col = dark_data) +
  thm_dk

(blup_light + theme(plot.margin = unit(c(0,20,0,0), "pt"))) + 
  (agent_light + theme(plot.margin = unit(c(0,0,0,20), "pt")))

(blup_dark + theme(plot.margin = unit(c(0,20,0,0), "pt"))) + 
  (agent_dark + theme(plot.margin = unit(c(0,0,0,20), "pt")))
```

:::

The effect encoding results for the hotel rate data. Panel (a) shows the distribution of the estimated per-agent effects. Panel (b) compares the two estimation methods for all unique data patterns in the training set.
...
effect_tbl |> thm_tbl_lt()

effect_tbl |> thm_tbl_dk()
```

Examples of the numeric values that are used in place of each agent's data when effect encodings are used.
...
knitr::include_graphics("../premade/anime_tree_collapse_dark.gif")
```

:::

The progression of using a tree-based method to collapse predictor categories into smaller sets.
...
ordered_p + 
  geom_point(aes(col = quality), cex = 2.5, show.legend = FALSE) + 
  geom_line(aes(x = ordering), alpha = 1 / 3, col = dark_data) + 
  scale_color_brewer(palette = "Set2") +
  thm_dk
```
...
Polynomial contrasts for an ordered categorical predictor with five possible levels.
