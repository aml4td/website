...
format_encoding <- function(x, ...) {
  ...
  table_dat %>%
    rename(`   ` = info ) %>% 
    kbl(digits = 1, escape = FALSE, align = "r") %>% 
    add_header_above(c(" " = 2, "Model Terms" = 4)) %>% 
    row_spec(n, extra_css = "border-top: 2px solid;")
}
...
customer_types %>% 
  ...
  add_header_above(c(" " = 1, "Indicator Columns" = 3))
```
...
hotel_rate_train %>% 
  ...
  ggplot(aes(bookings)) + 
  geom_histogram(bins = 40, col = "#00000000") + 
  scale_x_log10() + 
...
tbl_data <- bind_rows(hash_top, hash_middle, hash_bottom, hash_summary)
n <- nrow(tbl_data)

tbl_data %>%
  rename(Agent = agent) %>%
  mutate(Agent = remake_name(Agent)) %>% 
  kbl(align = "r", row.names = FALSE) %>% 
  add_header_above(c(" " = 1, "Features from Hashing" = 10)) %>% 
  row_spec(n, extra_css = "border-top: 2px solid;border-bottom: 2px solid;")
...
blup_plot <- 
  ...
  geom_histogram(col = "#00000000", bins = 20) + 
  geom_rug(alpha = 1 / 2) + 
...
bind_rows(effect_top, effect_middle, effect_bottom) %>%
  ...
  mutate(Agent = remake_name(Agent)) %>% 
  kbl(align = "r", digits = 3, row.name = FALSE)
```
...
```{r}
#| label: fig-collapse-light
#| echo: false
#| fig-align: center
#| out-width: 60%

knitr::include_graphics("../premade/anime-tree-collapse-light.gif")
```
...
```{r}
#| label: fig-collapse-dark
#| echo: false
#| fig-align: center
#| out-width: 60%

knitr::include_graphics("../premade/anime-tree-collapse-dark.gif")
```
