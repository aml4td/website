...
agent_dark <- 
  agent_means +
  geom_vline(xintercept = overall_mean, lty = 3, col = "#adb5bd") +
  geom_point(aes(size = n), alpha = 3/5, col = "#CCDEEC") +
  dk_thm
...
blup_dark <- 
  blup_plot  +
  geom_histogram(col = dark_bg, bins = 20, fill = "#CCDEEC") +
  geom_rug(alpha = 1 / 2, col = "#CCDEEC") +
  dk_thm
...
ordered_p + 
  geom_point(aes(col = quality), cex = 2.5, show.legend = FALSE) + 
  geom_line(aes(x = ordering), alpha = 1 / 3, col = "#CCDEEC") + 
  scale_color_brewer(palette = "Set2") +
  dk_thm
```
