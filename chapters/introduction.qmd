...
delivery_tbl %>% 
  # mutate(blank = cell_spec(blank, align = "center")) %>% 
  rename(` ` = blank) %>% 
  kbl(digits = 3, escape = FALSE, align = "c") %>%
  # kable_classic("striped", full_width = FALSE) %>% 
  kable_styling(bootstrap_options = "responsive", full_width = FALSE) %>% 
  add_header_above(c(" " = 4, "Item Counts" = 4))
```
...
$$ 
{ \underbrace{ log(time)}_{\text{the outcome}} } = 
{ \overbrace{ \beta_0+\beta_1\, distance}^{\text{an additive function of the predictors}} } +  
{ \underbrace{ \epsilon}_{\text{the errors}} }
$$
...
cell_data %>% 
  kbl(
    ...
    escape = FALSE,
    align = "c",
    col.names = c("ID", "Nucleus",	"Cell", "", "Nucleus",	"Cell", "",
                  "Nucleus",	"Cell")
  ) %>%
  kable_styling(bootstrap_options = "responsive", full_width = FALSE) %>% 
  add_header_above(
    c(" " = 1, "Eccentricity" = 2, " " = 1, "Area" = 2, " " = 1, 
      "Intensity" = 2)
  )
```
...
Cells, such as those found in @fig-cells, translated into a tabular format using three features for the nuclear and non-nuclear regions of the segmented cells.
...
bind_cols(steps, steps_data) %>% 
  kbl(
    ...
    escape = FALSE,
    align = "lcccccc",
    col.names = c("Sentence", "\"always\"", "\"can\"", "...", "\"take\"", 
                  "Characters", "Commas")
  ) %>%
  kable_styling(bootstrap_options = "responsive", full_width = FALSE) 
```
...
An example of converting non-numeric data to a numeric, tabular format.
