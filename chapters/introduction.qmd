...
Machine learning (ML) models are mathematical equations that take inputs, called _predictors_, and estimate some future output value. The output, often called an _outcome_ or _target_, can be numbers, categories, or other types of values. 

For example, in the next chapter, we try to predict how long it takes to deliver food ordered from a restaurant. The outcome is the time from the initial order (in minutes). There are multiple predictors, including: the distance from the restaurant to the delivery location, the date/time of the order, and which items were included in the order. These data are _tabular_; they can be arranged in a table-like way (such as a spreadsheet or database table) where variables are arranged in columns and individual data points (i.e., individual food orders) in rows, as shown in @tbl-deliveries^[Non-tabular data are described later in this chapter.].  
...
How does it do that? A machine learning model has a specific mathematical _prediction equation_ defining exactly how the predictors relate to the outcome. We'll see two very different prediction equations shortly. 
...
To estimate the parameters, we chose a criterion to quantify what it means to be "good" (called an objective function or performance metric). Here we will pick values of the $\beta$ parameters that minimize the squared error, with the error ($\epsilon$) defined as the difference in the observed and predicted times (on the log scale). 
...
With text data, it is common to define the "token": the unit of text that should be analyzed. This could be a paragraph,  sentence, word, etc. Suppose that we used sentences as tokens. In this case, the table for this quote would have four rows. Where do we go from here? The sequence of words (or characters) is likely to be important, and this makes a case for keeping the sentences as strings. However, as will be seen shortly, we might be able to convert these four tokens to numeric columns in a way that preserves information required for prediction. 
...
Our motives for categorizing data as tabular and non-tabular are related to how we choose an appropriate model. Our opinion is that most modeling projects involve tabular data (or data that can be effectively represented as tabular). Models for non-tabular data are very specialized and, while these ML approaches are the ones most discussed in the social media, they tend not to be the best approach for tabular data. 
...
There are many more ways to represent these data. More complex numeric _embeddings_ that are built on separate large databases to reflect different parts of speech, text sequences (called n-grams), and others. @hvitfeldt2021supervised and @Boykis_What_are_embeddings_2023 describe these and other tools for analyzing text data (in either format).
...
The other scenario that we discuss is _classification models_. In this case, the outcome is a qualitative categorical value (e.g., "cat" or "no cat" in an image). One interesting aspect of classification models is that there are two main types of predictions: 
...
It's helpful to discuss different nomenclature and types of data. With tabular data, there are a number of ways to refer to a row. The terms sample, instance, observation, and data point are commonly used. The first, "sample", can be used in a few different contexts; we will also use it to describe a subset of a larger collection of data (as in "we took a random sample of the data".). The notation for the number of rows in a data set is $n$ with subscripts for specificity (e.g., $n_{tr}$). 
...
Dense data describes a collection of values with many possible values, also reflected by the delivery time and distances in  @tbl-deliveries. Sparse data have fewer possible values or when only a few values are contained in the observed data. The product counts in the columns on the right-hand side of the table are good examples. They are integer counts consisting mostly of zeros. 
...
The item columns in @tbl-deliveries are interesting too. If the order could only contain one item, we might configure that data with a qualitative single column. However, these data are multiple-choice and, as such, are shown in multiple binary integer columns. 
...
* Feature selection tools [@fes] often compute measures of predictor importance to remove irrelevant predictors before passing the data to the model. 
* Effect encoding tools, discussed in @sec-effect-encodings, use the effect of the predictor on the outcome as a predictor. 

Since these occur prior to the supervised model, we label them as **preprocessing** operations. 

Also, there can also be post-model adjustments to predictions that require estimation. A good example is when predictions require calibration ([Section -@sec-calibration-whole-game] and [-@sec-cls-calibration]). This is an example of a **postprocessing** operation.
...
While many models can be used for machine learning, _neural networks_ are the elephant in the room. These complex, nonlinear models are frequently used in machine learning to the point where, in some domains, they are synonymous with ML. One class of neural network models is _deep learning (DL) models_ [@goodfellow2016deep;@udl2023]. Here, "deep" means that the models have many structural layers, resulting in extraordinary amounts of complexity. 
