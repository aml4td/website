...
delivery_tbl |>
  ...
  tab_spanner(
    label = "Item Counts",
    columns = c((ncol(delivery_tbl) - 3):ncol(delivery_tbl))
  ) |>
  tab_style(
...
Sadly, @eq-log-linear, while better than a random guess, is not very effective. It does the best it can with the single predictor. It just isn't very accurate. 
...
Non-tabular data does not have the same properties. Consider images, such as the one shown @fig-cells that depicts four cells from a biological experiment [@Yucells]. These cells have been stained with pigments that fluoresce at different wavelengths to "paint" different parts of the cells. In this image, blue reflects the part of the cell called the cytoskeleton, which encompasses the entire cell body. The red color corresponds to a stain that only attaches itself to the contents of the cell nucleus, specifically DNA. This image^[This is a smaller version of a larger image that is 1392  pixels by 1040 pixels.] is 371 pixels by 341 pixels with quantification of two colors. 
...
#| fig-alt: "Fluorescence microscopy image of four cells against a dark background. Dimmer staining highlights the cytoskeleton, showing the full cell body, while brighter staining marks DNA within each cell's nucleus. The cells vary in size and shape, with two larger rounded cells in the upper portion and two smaller elongated cells below."
...
#| fig-alt: "The same four fluorescence-stained cells as the previous figure, now with segmentation boundaries overlaid. Outer outlines trace the edges of each cell body, and inner outlines delineate the nuclei within each cell, demonstrating how image analysis can identify individual cell and nuclear regions from raw pixel data."
...
Dense data describes a collection of numeric values with many possible values, also reflected by the delivery time and distances in  @tbl-deliveries. Sparse data have fewer possible values or when only a few values are contained in the observed data. The product counts in the delivery data are good examples. They are integer counts consisting mostly of zeros. The frequency of non-zero values decreases with the magnitude of the counts.
...
- Post-model adjustments to predictions, such as model calibration ([Section -@sec-calibration-whole-game] and [-@sec-cls-calibration]), are examples of postprocessing operations^[**A note to readers**: these `?sec-*` references are to sections or chapters that are planned but not (entirely) written yet. They will resolve to actual references as we add more content.].
...
After the convolutional operations, a smaller _pooling layer_ is shown in orange. Pooling, usually by finding the maximum feature value across different image regions, compresses/downsamples the larger feature set to a smaller size without diminishing features with large signals. 
...
#| fig-alt: "Architectural diagram of the VGG16 convolutional neural network, read left to right. A color photograph of a dog enters block 1, which contains two convolutional layers and a pooling layer. Four more blocks of similar structure follow, each progressively reducing the spatial dimensions while increasing depth. After block 5, the data are flattened into a single long vector, passed through a dense layer, and finally a softmax output layer for classification."
