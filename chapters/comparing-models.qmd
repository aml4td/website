...
As a refresher, resampling produces multiple variations of the training set. These are used to produce metric estimates, such as accuracy or R<sup>2</sup>, that should mimic the results we would see on data sets similar to the training set. 
...
Regardless of whether we take a Frequentist or Bayesian philosophy to comparing model performance, both methods aim to identify key factors that influence the outcome, such as model-to-model effects. We seek to determine whether the effect of these factors is statistically significant---that is, unlikely to be due to random chance.  
...
#| fig-alt: "Dot plot of accuracy values for three models (Boosting, Logistic, Naive Bayes) when separate cross-validation folds are used. A horizontal dashed line marks the overall mean accuracy. A horizontal bar highlights the boosting model mean, slightly above the overall average. A vertical line segment from the highest boosting point to that mean illustrates the residual error, showing how total variation decomposes into model effect and unexplained variation."
for_fig <-
  bind_rows(
    c50_metrics |>
...
anova_one_way_model_random_intercept <- data.frame(anova(
  one_way_model_random_intercept
))
anova_one_way_model_random_intercept_f <- anova_one_way_model_random_intercept$F.value[
  2
]
anova_one_way_model_random_intercept_ndf <- anova_one_way_model_random_intercept$numDF[
  2
]
anova_one_way_model_random_intercept_ddf <- anova_one_way_model_random_intercept$denDF[
  2
]
anova_one_way_model_random_intercept_pvalue <- anova_one_way_model_random_intercept$p.value[
  2
]
```
...
#| fig-alt: "Dot plot of accuracy for three models using the same cross-validation folds, with lines connecting points from the same fold. The fold-to-fold pattern is clearly visible as roughly parallel lines. Annotations decompose the variation for the highest boosting accuracy point into three parts: the model effect, the fold effect explaining most of the deviation, and a much smaller residual compared to the figure without fold blocking."
Fold03 <-
  for_fig |>
...
`r r_comp("comparing-models.html#sec-nhtm")`
...
When we desire to compare the performance of each pair of models, then Tukey's HSD procedure will provide the appropriate adjustments to the p-values to protect against false positive findings without becoming overly conservative.
...
However, an equivalence test reverses this logic. The null hypothesis is that the models are not equivalent, and the alternative hypothesis is that the models are practically equivalent:
...
#| fig-alt: "Point-and-interval plot showing Tukey-adjusted 90% confidence intervals for three pairwise accuracy differences. Horizontal dashed lines at plus and minus 3% mark the equivalence region. The Boosting minus Logistic interval falls entirely within the equivalence bounds, indicating practical equivalence. The other two comparisons (Boosting minus Naive Bayes and Logistic minus Naive Bayes) extend beyond the bounds, indicating non-equivalence."
...
`r r_comp("comparing-models.html#sec-comparing-equivalence")`
...
#| fig-alt: "Three-panel histogram showing the Bayesian posterior distribution of accuracy for Boosting, Logistic, and Naive Bayes models. Each panel displays a bell-shaped distribution with a horizontal error bar indicating the 90% credible interval and a point at the posterior mean. Boosting and Logistic have similar and overlapping distributions centered near higher accuracy, while Naive Bayes is shifted noticeably lower."
...
Since we are interested in contrasting models, we can get a sample of the posterior and take the difference between the parameters that correspond to the model terms (i.e., $\beta$ parameters). We can create a large number of these samples to form posterior distributions of the pairwise contrasts and compute credible intervals. Bayesian modeling does not use the same null hypothesis testing paradigm as Frequentist methods, so it is unclear whether _post hoc_ adjustments are needed or how that would be computed. See @gelman2012we and @ogle2019should for discussions. @fig-bayesian-contrast-posterior illustrates posteriors for the pairwise contrasts. 
...
#| fig-alt: "Three-panel histogram of Bayesian posterior distributions for pairwise accuracy differences. Each panel shows one contrast with vertical dashed lines marking the 90% credible interval. The Boosting vs. Logistic contrast is centered near zero with the interval spanning both sides, suggesting no clear difference. The other two contrasts (Boosting vs. Naive Bayes and Logistic vs. Naive Bayes) are shifted away from zero, indicating meaningful differences."
ggplot(pairwise_diffs) +
  ...
  labs(x = "Posterior for the Difference in Accuracy", y = "count") +
  scale_x_continuous(label = label_percent())
```
...
#| fig-alt: "Three-panel histogram of posterior pairwise accuracy differences with a region of practical equivalence (ROPE) marked by vertical dashed lines at plus and minus 3%. Bars within the ROPE are shaded differently from those outside it. For Boosting vs. Logistic, nearly all posterior mass falls inside the ROPE. For the other two contrasts, substantial mass falls outside the ROPE, indicating those model pairs are not practically equivalent."
...
`r r_comp("comparing-models.html#sec-compare-resample-bayes")`
...
To adapt this idea, we make predictions on the single holdout set, then repeatedly draw bootstrap samples from it. Each bootstrap sample is the same size as the holdout set and is created via random sampling with replacement (as illustrated in @fig-bootstrap-single-holdout-graphic). We compute our metric(s) of interest on each analysis set, which are just the bootstrap samples. The corresponding assessment sets are not used in this process. 

```{r}
#| label: fig-bootstrap-single-holdout-graphic
#| echo: false
#| out-width: "60%"
#| fig-cap: "A schematic of bootstrap resamples created from a validation set."
#| fig-alt: "Diagram illustrating bootstrap resampling from a validation set. The original validation set is shown at top, with arrows pointing to multiple bootstrap samples of the same size drawn with replacement. Some observations appear more than once in a given sample while others are omitted, and a performance statistic is computed from each bootstrap sample."
...
agreement <- mean(paired_class_pred$logistic == paired_class_pred$boosting) *
  100
...
#| fig-alt: "Two-panel histogram showing bootstrap distributions of test set accuracy for the Boosting and Logistic models. Both distributions are bell-shaped and nearly overlapping, with a slight shift between them. Above each histogram, two horizontal error bars compare the 90% bootstrap percentile interval with the analytical confidence interval; the two interval methods produce nearly identical results for each model."
boot_acc_reps |>
  ggplot() +
...
#| fig-alt: "Line plot showing how the width of a 90% confidence interval for accuracy decreases as data set size increases. The curve drops steeply for small sample sizes and flattens out for larger ones, illustrating that smaller holdout sets produce much wider and less precise intervals."
#|
...
`r r_comp("comparing-models.html#sec-compare-holdout")`

## Conclusion {#sec-compare-conclusion}
