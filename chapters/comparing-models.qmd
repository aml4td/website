...
glmn_best <-
    select_best(glmn_res, metric = "accuracy")

glmn_metrics <-
    glmn_res |>
    filter_parameters(parameters = glmn_best) |>
    collect_metrics(summarize = FALSE)

glmn_final_wflow <- finalize_workflow(glmn_wflow, glmn_best)
```
...
glmn_ns_wflow <- workflow(glmn_ns_rec, glmn_spec)
...
glmn_ns_best <-
    select_best(glmn_ns_res, metric = "accuracy")

glmn_ns_metrics <-
    glmn_ns_res |>
    filter_parameters(parameters = glmn_ns_best) |>
    collect_metrics(summarize = FALSE)

glmn_ns_final_wflow <- finalize_workflow(glmn_ns_wflow, glmn_ns_best)
```
...
Without going into much detail, we'll focus on comparing three modeling strategies that will be described in Part 3: 
...
@fig-variance-partition-one-way illustrates these separate components for the hold out set with the largest accuracy value for the C5.0 model, where the overall intercept is determined by the glmnet+NS model. The horizontal dashed line represents the overall mean of the accuracy estimates, and the blue bar shows the mean of the C5.0 estimates alone. It is slightly higher than the average. For that model, the orange line shows the  residual ($\epsilon_{ij}$ for the largest C5.0 accuracy replicate. The totality of these errors are used to measure the overall error in the model.
...
##### Tukey’s Honestly Significant Difference (HSD) {.unnumbered}
...
##### False Discovery Rate (FDR) {.unnumbered}
...
pairwise_comparison_table <-
  ...
  mutate(
    Unadjusted = map_chr(Unadjusted, pval),
    Tukey = map_chr(Tukey, pval),
    FDR = map_chr(FDR, pval)
  )
...
@fig-equivalence-intervals presents the Tukey-adjusted 90% confidence intervals for each pairwise difference between models. For the forestation data, we would conclude that the C5.0 ensemble and the glmnet model that includes natural splines are equivalent. However, we could not conclude that the glmnet model without splines was equivalent to either the C5.0 model or the glmnet model with splines.  
...
Based on these intervals, the probability of a difference between each pair of models is substantial. Among them, the C5.0 and glmnet model with natural splines exhibit the smallest difference, with a `r round(length(which(pairwise_diffs_c50_glmn_ns$difference > 0))/nrow(pairwise_diffs_c50_glmn_ns) * 100, 1)`% probability that the C5.0 model exhibits a higher accuracy than the glmnet model with natural splines. 
...
After selecting the optimal tuning parameters, the training and testing data are combined to fit the final model. To evaluate this model’s predictive performance, we reserve a portion of the original data—often referred to as a validation set. If the validation set is representative of the training and testing data, we expect the model’s performance metric on the validation set to be consistent with the results observed during cross-validation.
...
set.seed(382)
both_int_res <- 
  both_intervals(glmn_validation_res, times = 2000) |> 
  mutate(Model = "glmnet") |> 
  bind_rows(
    both_intervals(glmn_ns_validation_res, times = 2000) |> 
      mutate(Model = "glmnet+NS") 
  ) |> 
  bind_rows(
    both_intervals(c50_validation_res, times = 2000) |> 
      mutate(Model = "C5.0") 
  ) |> 
  mutate(
    Model = factor(Model),
...
acc_ints <- 
  both_int_res |> 
  mutate(
    ref = if_else(Method == "Analytical", 450, 400)
  )
...
acc_diff_comps <- function(split) {
  ...
  diffs <- 
    bind_rows(
      tibble(
        term = paste(acc$Model[1], acc$Model[2], sep = " vs "),
        estimate = acc$.estimate[1]- acc$.estimate[2]
      ),
      tibble(
        term = paste(acc$Model[1], acc$Model[3], sep = " vs "),
        estimate = acc$.estimate[1]- acc$.estimate[3]
      ),
      tibble(
        term = paste(acc$Model[3], acc$Model[2], sep = " vs "),
        estimate = acc$.estimate[3]- acc$.estimate[2]
      )
    )
...
set.seed(524)
acc_boot_diif <- 
  collect_predictions(glmn_validation_res) |> 
  mutate(Model = "glmnet") |> 
  bind_rows(
    collect_predictions(glmn_ns_validation_res) |> 
      mutate(Model = "glmnet+NS") 
  ) |> 
  bind_rows(
...
#| fig-align: "center"
#| out-width: "65%"
#| fig-cap: For each model, the bootstrap distributions of the accuracy statistics are shown as histograms. The intervals at the top of each panel show the 90% confidence intervals computed using the bootstrap and via the traditional analytical method. 
