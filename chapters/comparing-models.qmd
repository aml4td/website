...
set.seed(382)
both_int_res <- 
  both_intervals(glmnet_validation_res, times = 2000) |> 
  mutate(Model = "glmnet") |> 
  bind_rows(
    both_intervals(glmnet_validation_res_ns, times = 2000) |> 
      mutate(Model = "glmnet+NS") 
  ) |> 
  bind_rows(
...
set.seed(524)
acc_boot_diif <- 
  collect_predictions(glmnet_validation_res) |> 
  mutate(Model = "glmnet") |> 
  bind_rows(
    collect_predictions(glmnet_validation_res_ns) |> 
      mutate(Model = "glmnet+NS") 
  ) |> 
  bind_rows(
