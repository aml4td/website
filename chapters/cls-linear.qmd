...
Note that the parameters are class-specific. The intercepts in these linear predictors represent the differences in the baseline class probabilities. It is possible to optimize the predictor patterns across classes. For example, if there is evidence that the relationships between the predictors and the logits are the same, a common slope estimate can be obtained (leading to a "parallel" model structure).  
