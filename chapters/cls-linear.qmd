...
$$
\hat{\eta} = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \hat{\alpha}_i \left(\sum_{j=1}^p x_{ij} u_j \right) = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \hat{\alpha}_i \boldsymbol{x}_i'\boldsymbol{u}_i
$$ {#eq-svm-linear-model}
...
$$
\hat{\eta} = \hat{\beta}_0 + (\boldsymbol{X}'\hat{\boldsymbol{\alpha}}) \boldsymbol{u}_i = \hat{\beta}_0 + \boldsymbol{u}_i'\hat{\boldsymbol{\beta}}
$$
where neither $\boldsymbol{X}$ nor $\boldsymbol{u}$ contain elements for the intercept. 
