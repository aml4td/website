...
```{r}
#| label: tbl-logistic-colin
#| tbl-cap: "An example of how highly colinear predictors can affect parameter estimates and standard errors in a logistic regression model."
#| echo: false
...
all_terms |> 
  ...
  cols_label(
    contains("estimate_") ~ "Coefficient",
...
All of these techniques make the implicit assumption that the model features have been standardized to be in the same units (including indicator features). 

Probably the first method devised to penalize models was ridge regression (a.k.a. weight decay). The ridge penalty function detracts from the log-likelihood by using the sum of the squared regression coefficients: 
...
adaptive\: lasso &= \lambda\sum_{j=1}^{p} w_j |\beta_j| \notag \\
elastic\: net &= \lambda_1\sum_{j=1}^{p} \beta_j^2 + \lambda_2\sum_{j=1}^{p} |\beta_j| \notag
\end{align}
$$
:::
...
$$
\begin{align}
glmnet &= \lambda \left[(1 - \alpha) \sum_{j=1}^{p} \beta_j^2 + \alpha \sum_{j=1}^{p} |\beta_j|  \right] \notag \\
SCAD &=
  \begin{cases}
