...
The summation is the accumulation of hinge loss values. Again, note that well-predicted data points ($\eta \ge 0$) contribute nothing to this statistic. The constant $\mathcal{C}$ is the _cost value_. This is a tuning parameter that modulates how much effort the model should exert to predict all training set values as well as possible. Tuning this value enables us to modulate model complexity. For linear SVM classifiers, the cost function does not significantly affect the computations. However, we'll see in @sec-cls-svm-nlin that this parameter has a profound effect on the model's quality. Linear classifiers have a much higher bias than their nonlinear counterparts and simply lack the flexibility to overfit at unreasonably large cost values. 

The second term summarizes the overall magnitude of the model parameters, apart from the intercept. This acts as a regularizing term, similar to the ridge penalty^[In fact, some representations of this equation divide both terms by $\gamma = 1/\mathcal{C}$, so that the tuning parameter is a coefficient for the squared norm of the parameters.]. Note that the intercept is not used in this norm. Since this term sums the squared coefficients, SVM models require all numeric predictors to have the same mean and standard deviation, or to be on at least the same scale. 
...
$$
\hat{\eta} = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \tilde{y}_i\hat{\alpha}_i \left(\sum_{j=1}^p x_{ij} u_j \right) = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \tilde{y}_i\hat{\alpha}_i \boldsymbol{x}_i'\boldsymbol{u}_i
$$ {#eq-svm-linear-model}
...
$$
\hat{\eta} = \hat{\beta}_0 + \left[\boldsymbol{X}'(\hat{\boldsymbol{\alpha}} \circ \tilde{y}_i)\right] \boldsymbol{u}_i = \hat{\beta}_0 + \boldsymbol{u}_i'\hat{\boldsymbol{\beta}}
$$
...
One interesting aspect of this result is that training-set points with score predictions greater than 1.0 are assigned $\alpha_i = 0$. This means that the most confidently predicted samples have no direct effect on the prediction equation. The training set points with strictly positive values are called the _support vectors_ since the prediction equation is composed of these values. This result indicates that some portion of the training set must be stored by the model, which can make it unattractive for large training sets.

Again, SVMs only produce predictive score values. To convert them to class probabilities, Platt scaling [@platt1999probabilistic] is used to estimate a slope and intercept for a logistic function that takes $\hat{\eta}$ as input and produces probability-like estimates.  

For multiclass SVM models, a set of distinct $C$ "one-versus-all" binary SVM models is created and trained separately. The hard class predictions are assigned to whichever model predicted the largest value of $\hat{\eta}_{ij}$ (across $j$). Platt scaling is again used to create class probability estimates. An alternative is to use a collection of $C(C-1)/2$ pairwise SVM models. 
...
As before, the most complex feature-engineering strategy yielded the best performance. If we were to fit that pipeline on the training set, we'd find that there were `r num_sv` (out of `r commas(nrow(forested_train))`) support vectors (and non-zero $\alpha_i$ values). For this fit, the equation used by Platt scaling to convert the $\eta$ scores to probability-like values for the forested class level was: 
