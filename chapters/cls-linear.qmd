...
obs_rates |>
  ...
  theme(legend.position = "top") +
  scale_color_viridis_c(option = "mako", begin = 0.2, end = 0.8)
```
...
brier_perm <-
  ...
  mutate(
    lower = permutation - qnorm(0.95) * std_err,
    upper = permutation + qnorm(0.95) * std_err
  )
...
glm_best_int <-
  ...
  int_pctl(times = 1000, metrics = cls_mtr)
```
...
Sections [-@sec-pca] and [-@sec-pls]  described principal component analysis (PCA) or partial least squares (PLS), respectively. Here, we'll focus again on PLS and describe how it can be used as the supervised model (rather than a preprocessor).

Let's summarize our previous points from @sec-pls related to PLS and PCA: 

- Like PCA, PLS creates new features, called components, that are linear combinations of the original predictors. 
- Unlike PCA, PLS is supervised. PLS is intended to use outcome data alongside predictors to derive components. 
- We usually center and scale the predictors before performing PLS. 

PCA is designed to find components that maximize the variances of the data. Its objective function aims to capture as much information as possible (i.e., variance and covariance) from the original predictors. It tries to represent the predictor space well. 
...
p_pca <- ggplot(plot_df, aes(x = x1, y = x2, color = Class, shape = Class)) +
  ...
  scale_color_manual(values = c("A" = "#DF91A3FF", "B" = "#A3AD62FF")) +
  ...
  coord_obs_pred() +
  labs(x = "Predictor 1", y = "Predictor 2", title = "PCA Direction") +
...
p_pls <- ggplot(plot_df, aes(x = x1, y = x2, color = Class, shape = Class)) +
  ...
  coord_obs_pred() +
  labs(x = "Predictor 1", y = "Predictor 2", title = "PLS Direction") +
...
ggplot(scores_long, aes(x = Score, fill = Class)) +
  ...
  scale_color_manual(values = c("A" = "#DF91A3FF", "B" = "#A3AD62FF")) +
  scale_fill_manual(values = c("A" = "#DF91A3FF", "B" = "#A3AD62FF")) +
...
To better understand the algorithm's function, @stone1990continuum linked it to the well-known statistical concepts of covariance and regression. In particular, Stone and Brooks showed that, like PCA, PLS finds linear combinations of the predictors. These linear combinations are commonly called *components* or latent variables. While the PCA linear combinations are chosen to maximally summarize predictor-space variability, the PLS linear combinations are chosen to maximally summarize covariance with the response. Formally, the first PLS direction is the vector of weights $\boldsymbol{w}_1$ that solves

$$
\boldsymbol{w}_1 = \arg\max_{\boldsymbol{w}} \; \operatorname{cov}^2\!\left(\boldsymbol{X}\boldsymbol{w},\, \boldsymbol{Y}\right)
...
Logistic regression is technically constrained to use outcomes with two possible classes. Its multiclass analog is called _multinomial regression_, primarily because the outcome data are assumed to follow a multinomial distribution. The most basic model is parameterized such that each class level has its own linear predictor $\eta_{j} = \boldsymbol{x}'\boldsymbol{\beta}_{j}$. The fundamental complication is that the $C$ probability estimates must sum to 1 and this constraint must be addressed when estimating models.

The primary approach to estimating multinomial regression models is called _multinomial logistic regression_^[Other names are _discrete choice models_ or _polytomous logistic regression_.], which leverages our existing logistic regression framework. 

With $C > 2$ classes, we can estimate a set of $C - 1$ logistic models that contrast two classes from one another. Only $C$ - 1 sets of coefficients are estimated, since including a $C^{th}$ set would make the model unidentifiable.  In this case, the final class, $C$, is fixed as the reference. The most basic contrast is to model each class against the last, leading to $C-1$ linear predictors: 

$$
\log\left(\frac{Pr[Y = j]}{Pr[Y = C]}\right)= \boldsymbol{x}'\boldsymbol{\beta}_j = \eta_j
$$

The prediction for the last class is just one minus the sum of the $C-1$ estimates for the other classes. 

Note that the parameters are class-specific. The intercepts in these linear predictors often represent the differences in the baseline class probabilities. It is possible to optimize the predictor patterns across classes. For example, if there is evidence that the relationships between the predictors and the logits are the same, a common slope estimate can be obtained.  This concept appears in a different form in the proportional-odds model discussed below, where it is applied to cumulative logits rather than the baseline-category logits used here.  

This model _could_ be trained by separately fitting each of the $C-1$ binary logistic regressions in the traditional manner [@becg1984calculation]. However, better estimates are obtained by using the model structure above to compute probabilities and then applying gradient descent to optimize the multinomial log-likelihood which, as previously mentioned, is cross-entropy. See Chapter 6 of @agresti2015foundations. There are also analogous approaches that use regularization or Bayesian estimation. 

A related class of models is for ordered outcome categories [@mccullagh1980regression;@williams2020ordinal]. Ordinal models are a special case of multinomial models that utilize the ordered structure of the categories.  This structure is meaningful only when the number of classes is greater than 2.^[When there are two ordered categories, basic logistic is appropriate because the ordering does not matter.] 

There are a variety of methods to estimate these models [@burkner2019ordinal;@tutz2022ordinal]. Often their logits are cumulative. For example, the proportional odds model is: 

$$
\log\left(\frac{Pr[Y \le j]}{Pr[Y > j]}\right)= \beta_{0j} + \boldsymbol{x}'\boldsymbol{\beta}
$$

for $j < C$ and $\boldsymbol{\beta}$ only contains slope parameters. The slopes are class independent, meaning all of the predictors share the same pattern across classes and only their baseline logit ($\beta_{0j}$) depends on the class. This can be made class-specific, leading to a non-proportional model. 

There are a few other basic complications that occur when there are more than two class levels: 

- Calibration is much more difficult since most methods are specific to a binary problem. The problem can be formulated as a set of "one class versus all other classes" calibration problems, but an additional step is required to ensure that the adjusted probabilities sum to one. 
- It is more difficult to adjust the rule that translates the class probability estimates to a hard class prediction. The default is now $1/C$, or whichever class has the largest probability. To optimize across different error types, a set of $C-1$ thresholds must be estimated (rather than a single threshold). 

Apart from these aspects, almost everything discussed in @sec-logistic-reg applies to ordered or unordered multinomial models. 
...
save_if_missing(forest_logistic_set_res)

forest_glmnet_set_res <- as_workflow_set("glmnet" = glmn_res |> butcher::butcher())
save_if_missing(forest_glmnet_set_res)
```
