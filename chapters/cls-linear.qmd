...
We've seen the use of logistic regression in this chapter, coupled with spline terms, in order to model nonlinear effects of predictors. Two strategies were shown: one in which we tuned the number of spline terms, and another in which we set the number of terms to be large and let regularization help demphasize or eliminate excess complexity in the splines.  Statistically, the downside of these methods is that we never measure the noise in the amount of nonlinearity used; we specify it instead of estimating it directly. For that reason, any inferential estimates that we might want (e.g., confidence intervals, p-values, etc.) are optimistic to some degree

Another avenue is to use _generalized additive models_ [@wood2006generalized]. This specialized class of models offers effective, relatively low-maintenance ways to add nonlinearity. When training the model, we specify which predictors to model nonlinearly and which spline to use. GAMs can directly estimate the degree of nonlinearity in the relationships.

The "additive" in their name is due to the linear predictor having a functional form of: 

$$
log\left(\frac{\pi_i}{1 - \pi_i}\right) = \beta_0 + \sum_{j=1}^{p_{l}} x_{ij} \beta_{j} + \sum_{j'= p_l + 1}^{p_l + p_{s}} f_{j'}\left(x_{ij'}\right)
$$ {#eq-gam}

where there are $p_l$ feature that enter the model linearly and $p_s$ that are nonlinear as defined by the function $f_{j'}(\cdot)$. This isn't very different from our previous logistic models with nonlinear terms. 

What separates GAM models from the previously described techniques is how the parameters are estimated. For GAMs, the most common training method is penalized maximum likelihood estimation, which is very similar to the L<sub>2</sub> penalty shown for logistic regression. However, GAMs also estimate how complex the nonlinear terms should be (rather than assigning a specific value to them or tuning them) by directly estimating the penalty parameter^[@marra2012coverage and @wood2013p also make the case that using the plug-in generalized cross-validation (GCV) estimate can also produce valid inference.].  This is especially valuable when the model needs to be statistically interrogated, such as measuring statistical significance or assigning uncertainty to model parameters or predictions. 

Regarding penalized maximum likelihood estimation, the penalty is slightly more complicated than in the previous logistic case. We want to penalize complexity, which is often called the "wiggliness" of the curve. For splines, the penalty is more than the squared coefficient: 

$$
\mathcal{P}_{gam}(\boldsymbol{S}; \boldsymbol{\beta}, \lambda) =  \frac{1}{2}\lambda\boldsymbol{\beta}'\boldsymbol{S}\boldsymbol{\beta}
$$ {#eq-gam-penalty}

where $\boldsymbol{S}$ is a symmetric matrix of known values that are derived from the integral of squared second derivatives across the range of the basis functions. To illustrate, the (9 × 9) $\boldsymbol{S}$ matrix for the fit shown in @fig-longitude is below in @fig-gam-matrix. 

```{r}
#| label: fig-gam-matrix
#| echo: false
#| out-width: 50%
#| fig-width: 5
#| fig-height: 5
#| dev: "ragg_png"
#| fig-cap: "The smoothing matrix $\\boldsymbol{S}$ for the 9 non-intercept terms corresponding to the fit shown in @fig-longitude. Larger values indicate larger overall curvature."

mgcv_fit <-
  mgcv::gam(class ~ s(longitude), data = forested_train, 
            family = stats::binomial(link = "logit"), method = "REML")

nms <- recipes::names0(mgcv_fit$smooth[[1]]$bs.dim - 1, "spline_")

s_matrix <- mgcv_fit$smooth[[1]]$S[[1]]
colnames(s_matrix) <- nms
s_matrix <- 
  s_matrix |> 
  as_tibble() |> 
  mutate(.row = rev(seq_along(nms))) |> 
  pivot_longer(cols = c(-.row)) |> 
  mutate(
    .col = as.numeric(factor(name))
  )
...
s_matrix |> 
  ...
  coord_fixed(ratio = 1) +
  labs(x = "Basis Index", y = "Basis Index", fill = NULL) +
  scale_x_continuous(
    breaks = seq_along(nms),
    labels = seq_along(nms)
  ) +
  scale_y_continuous(
    breaks = seq_along(nms),
    labels = rev(seq_along(nms))
  )
```

Since $\boldsymbol{S}$ is built from the basis functions' second derivatives, its diagonal entries quantify the curvature energy of each basis function in isolation. In this example, the fifth, sixth, and seventh columns carry the most. The off-diagonal entries represent "co-wiggliness": the overlapping curvature of pairs of basis functions. They can contribute substantially to the total penalty but are near zero when two basis functions' supports don't overlap. $\boldsymbol{S}$ depends only on the basis and knot locations, not on the response or the fitted model.

In penalized maximum likelihood, the objective adds this curvature penalty (@eq-gam-penalty) to the negative log-likelihood. The likelihood pulls the coefficients toward the data and the penalty independently charges them for the wiggliness they create in the fitted function. A set of coefficients is disfavored when the extra fit it buys (in log-likelihood) doesn't outweigh the curvature it adds. The tradeoff is modulated via $\lambda$. In this way, $\beta_5$, $\beta_6$, and/or $\beta_7$ are large only when the data genuinely support that much local structure, rather than when they're chasing noise.

Without penalization, the number of degrees of freedom for a set of splines^[Without an intercept term.] is the same as the number of terms. With penalization, each spline feature has varying amounts of influence. The _effective degrees of freedom_ (edf) reflects the overall nonlinearly of the set, where the maximum is the number of terms. For our fit in @fig-longitude, the optimal penalization was light; out of `r ceiling(sum(mgcv_fit$edf1))` possible degrees of freedom^[While there are `r ceiling(sum(mgcv_fit$edf1))` possible, one is the intercept, which is not penalized.], the model used `r round(sum(mgcv_fit$edf1), 3)`. 

It is also worth noting that the work of @marra2011practical enables the model to shrink to absolute zero, thereby removing model terms (and perhaps entire features) from the model fit. 

The strict additive nature of @eq-gam prohibits interaction terms or other features that involve multiple predictors. However, specialized spline functions can create joint or conditional smooths. For example, @wood2006low describes methods for two or more predictors to create individual smooth basis functions and combine them via a product. Another option is useful when a quantitative and a qualitative predictor interact (as we saw for the food delivery data in @fig-delivery-predictors). A separate smoother can be created for each subset defined by the qualitative predictor so that nonlinear interaction effects are available. 

```{r}
#| label: gam-computations
#| include: false
gam_wflow <- 
  workflow(forest_rec) |> 
  add_model(
    ...
    formula = 
      class ~ s(elevation) + s(roughness) + s(dew_temp) + s(precip_annual) + 
      s(temp_annual_mean) + s(temp_annual_min) + s(temp_annual_max) + 
      s(temp_january_min) + s(vapor_min) + s(vapor_max) + s(longitude) + 
      s(latitude) + county + eastness + northness + year
  )
...
gam_mtr <- collect_metrics(gam_res)
gam_fit <- fit(gam_wflow, forested_train)

# based on print.gam
all_edf <- function(x) {
  edf_ind <- purrr::map(x$smooth, ~ .x$first.para:.x$last.para) |> unlist()
  sum(x$edf[edf_ind])
}
gam_df <- gam_fit |> extract_fit_engine() |> all_edf()
lm_df <- 
  forest_spline_rec |> 
  prep() |> 
  bake(new_data = NULL, contains("_0")) |> 
  ncol()
```

For our forestation data, we used a model similar to the #5 feature engineering scheme, but without the interaction effects.  Instead of adding `r lm_df` explicit spline columns, we added the same predictors to a GAM model as smooth terms. When resampled, the model had a Brier score of `r round(gam_mtr$mean[gam_mtr$.metric == "brier_class"], 3)`, roughly equivalent to its ordinary logistic regression model's performance. The GAM fit's effective degrees of freedom was far smaller (`r round(gam_df, 1)` compared to `r lm_df`). This shows that a more sophisticated technique for including nonlinear trends in logistic regression can produce a smaller model that is capable of valid statistical inference without losing predictive performance. 
...
forest_gam_set_res <- as_workflow_set(
  "GAM" = gam_res |> butcher::butcher()
)
save_if_missing(forest_gam_set_res)
