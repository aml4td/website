...
With $C > 2$ classes, we can estimate a set of $C - 1$ logistic models that contrast two classes from one another.^[Only $C$ - 1 sets of coefficients are estimated, since including a $C^{th}$ set would make the model unidentified.  In this case, the final class, $C$, is fixed as the reference.] The most basic contrast is to model each class against the last, leading to $C-1$ linear predictors: 

$$
\log\left(\frac{Pr[Y = j]}{Pr[Y = C]}\right)= \boldsymbol{x}'\boldsymbol{\beta}_j = \eta_j
$$

The prediction for the last class is just one minus the sum of the $C-1$ estimates for the other classes (or 1 divided by the sum of $exp(\eta_j)$). 
