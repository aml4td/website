...
## Linear Support Vector Machines {#sec-cls-svm-lin}

Support vector machines (SVMs) have been a staple in machine learning for the last few decades.  @cristianini2000introduction is a good resource for beginners.  We'll first introduce them here and constrain ourselves to linear classifiers. This will help concentrate on the model's loss function and a basic classifier. Later, in @sec-cls-svm-nlin, we'll see the well-known version of the model that uses the "kernel trick" to enable nonlinear class boundaries. 

SVMs are driven by an objective function colloquially called **hinge loss** [@boser1992training;@cortes1995support;@wahba1999support]. For now, we'll assume that there are two outcome class levels. We'll also recode the target variable so that these two classes are represented as $\tilde{y} \in \pm 1$ (as opposed to the standard 0/1 encoding). SVMs are _not_ designed to naturally generate class probability estimates. Instead, they generate a predictive score on the real line, with zero as the default threshold for differentiating between the two classes. We'll denote the prediction function for the score as $\eta$, recycling notation from @eq-logit. While SVMs do not automatically create logit values, the linear classifiers are parameterized with a linear predictor function $\eta =\beta_0 + \boldsymbol{x}_{i}'\boldsymbol{\beta}$.

Let's briefly recall a different loss function for a binary classifier. The Brier score estimates the squared error between the probability estimate ($\pi$) and the observed classes (0/1). Let's focus on the first class, say the event of interest, being predicted. When the model is perfect, both the estimate and the observed values are zero. However, as the probability estimates decreases from 1.0 to 0.5, the loss increases quadratically. If we have a probability of 0.75, the Brier score does assign some loss to the data point. Its hard classification is still correct, but the probability estimate is not perfect, and the loss reflects this. 

Hinge loss treats any predicted score that is greater than 1.0 as perfect and assigns zero loss. If the hard class prediction is still correct but not perfect (i.e., $0.0 \le \eta \le 1.0$), a loss is assigned that is increasingly linear as the values become closer to 0.0. This is also the loss for incorrect hard class predictions ($\eta \lt 0.0$). Mathematically, the loss is $max(0, 1 - \tilde{y}\eta)$
...
#| fig-height: 3.5
#| dev: "ragg_png"
#| fig-cap: "Three loss functions shown against a score value on the real line, denoted as $\\eta$. For some models, this could be the logit value or a general linear predictor."
losses <- 
  tibble(probability =  seq(0.001, 0.999, length.out = 100)) |>
  mutate(
    Logit = log(probability / (1 - probability)),
    Brier = (1 - probability)^2,
    `Cross-Entropy` = -log(probability),
    Hinge = pmax(0, 1 - log(probability / (1 - probability)))
  ) |>
  pivot_longer(
    cols = c(-probability, -Logit),
    names_to = "Metric",
    values_to = "Loss"
  )

losses |>
  filter(Logit >= -3 & Logit <= 3) |> 
  ggplot(aes(Logit, Loss, group = Metric, col = Metric)) +
...
SVM models are based on the objective function: 

$$
L(\tilde{y}; \beta_0, \boldsymbol{\beta}) = \frac{\mathcal{C}}{n_{tr}}\sum_{i=1}^{n_{tr}}\Big( max\left[0, 1 - \tilde{y}_i \left(\beta_0 + \boldsymbol{x}_{i}'\boldsymbol{\beta}\right)\right]\Big) + ||\boldsymbol{\beta}||^2 
$$ {#eq-svm-cls-loss}

The summation on the left is the accumulation of hinge loss values. Again, note that well-predicted data points ($\eta \ge 0$) contribute nothing to this statistic. The constant $\mathcal{C}$ is the _cost value_. This is a tuning parameter that modulates how much effort the model should exert to predict all training set values as well as possible. Tuning this value enables us to modulate model complexity. For linear SVM classifiers, the cost function has a negligible impact on performance. However, we'll see in @sec-cls-svm-nlin that this parameter has a profound effect on the model's quality. Linear classifiers have a much higher model bias than their nonlinear counterparts and simply lack the flexibility to overfit at unreasonably large cost values. 

The second term in @eq-svm-cls-loss summarizes the overall magnitude of the model parameters, apart from the intercept. This acts as a regularizing term, similar to the ridge penalty^[In fact, some representations of this equation divide both terms by $\gamma = 1/\mathcal{C}$, so that the tuning parameter is a coefficient for the squared norm of the parameters.]. Since this term sums the squared coefficients, SVM models require all numeric predictors to have the same mean and standard deviation, or to be on at least the same scale. 

The linear SVM prediction equation can be derived from this objective function. When predicting an unknown sample $\boldsymbol{u}$, it is: 

$$
\hat{\eta} = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \tilde{y}_i\hat{\alpha}_i \left(\sum_{j=1}^p x_{ij} u_j \right) = \hat{\beta}_0+\sum_{i=1}^{n_{tr}} \tilde{y}_i\hat{\alpha}_i \boldsymbol{x}_i'\boldsymbol{u}
$$ {#eq-svm-linear-model}

where the $\alpha_i$ values ($i = 1, \ldots, n_{tr}$) that are non-negative. For a linear SVM classifier, we can write this equation as a familiar slope/intercept model:

$$
\hat{\eta} = \hat{\beta}_0 + \left[\boldsymbol{X}'(\hat{\boldsymbol{\alpha}} \circ \tilde{y}_i)\right] \boldsymbol{u} = \hat{\beta}_0 + \boldsymbol{u}'\hat{\boldsymbol{\beta}}
$$

where neither $\boldsymbol{X}$ nor $\boldsymbol{u}$ contain elements for the intercept and $\circ$ denotes element-wise multiplication. 

One interesting aspect of this result is that training-set points with score predictions greater than 1.0 are assigned $\alpha_i = 0$. This means that the most confidently predicted samples have no direct effect on the prediction equation _since they have no impact on @eq-svm-cls-loss._ The training set points with strictly positive values are called the _support vectors_ since the prediction equation is composed of these values. This result indicates that some portion of the training set must be stored by the model, which can make it unattractive for large training sets.

Again, SVMs only produce predictive score values. To convert them to class probabilities, Platt scaling [@platt1999probabilistic] is used to estimate a slope and intercept for a logistic function that takes $\hat{\eta}$ as input and produces probability-like estimates. We'll show an example of this below.

For multiclass SVM models, a set of distinct $C$ one-class-versus-all binary SVM models is created and trained separately. The hard class predictions are assigned to whichever model predicted the largest value of $\hat{\eta}_{ij}$ (across $j$). Platt scaling is again used to create class probability estimates. An alternative is to use a collection of $C(C-1)/2$ pairwise SVM models. 

```{r}
#| label: forestation-svm-linear
#| include: false
#| cache: true
svm_spec <- 
...
forest_svm_linear_set <-
  workflow_set(
    ...
    model = list(`SVM (linear)` = svm_spec)
  )

forest_svm_linear_res <-
  forest_svm_linear_set |>
  workflow_map(
    fn = "fit_resamples",
    resamples = forested_rs,
...
set.seed(83672)
svm_linear_fit <- fit_best(forest_svm_linear_res)
ksvm_fit <- svm_linear_fit |> extract_fit_engine()
num_sv <- commas(ksvm_fit@nSV)
plat_coef <- signif(unlist(ksvm_fit@prob.model), 3)
```

Let's see how this model performed on the forestation data. The same set of preprocessing options used for the logistic model was applied to the SVM model, with a fixed cost of $\mathcal{C} = 1.0$. The results were very similar to those of the unpenalized logistic model, as seen in @fig-forest-logistic-svm. 

```{r}
#| label: fig-forest-logistic-svm
#| echo: false
#| out-width: 70%
#| fig-width: 6
#| fig-height: 3.1
#| fig-cap: Cross-validated Brier scores for several stages of feature engineering of the forestation data using basic logistic regression and linear SVMs. 
bind_rows(
  forest_res |>
    rank_results(rank_metric = "brier_class") |>
    filter(.metric == "brier_class") |>
  ...
  forest_svm_linear_res |>
    ...
    filter(.metric == "brier_class") |>
    mutate(
      pipeline = gsub("_SVM (linear)", "", wflow_id, fixed = TRUE),
      pipeline = factor(pipeline),
      pipeline = reorder(pipeline, -mean),
      ci_pm = qnorm(0.95) * std_err,
      Model = "SVM (Linear)"
  ...
  ggplot(aes(mean, pipeline, col = Model)) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(
    aes(xmin = mean - ci_pm, xmax = mean + ci_pm),
    width = 1 / 3,
...
As before, the most complex feature-engineering strategy yielded the best performance for these data. If we were to fit that pipeline on the training set, we'd find that `r num_sv` training set points (out of `r commas(nrow(forested_train))`) were support vectors (and had non-zero $\alpha_i$ values). For this fit, the equation used by Platt scaling to convert the $\hat{\eta}$ scores to probability-like values for the forested class level was: 

$$
\hat{\pi}_i = \Big[1 + \exp(`r plat_coef[["B"]]` `r plat_coef[["A"]]`\, \hat{\eta}) \Big]^{-1}
$$
We'll delve into more details of these models in the next chapter, including more on preprocessing requirements. 

`r r_comp("cls-linear.html#sec-cls-svm-lin")`
...
forest_svm_linear_set_res <- forest_svm_linear_res |> butcher::butcher()
save(forest_svm_linear_set_res, file = "../RData/forest_svm_linear_set_res.Rdata")
