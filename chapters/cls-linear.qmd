...
One interesting aspect of this result is that training-set points with score predictions greater than 1.0 are assigned $\alpha_i = 0$. This means that the most confidently predicted samples have no direct effect on the prediction equation _since they have no impact on @eq-svm-cls-loss_. The training set points with strictly positive values are called the _support vectors_ since the prediction equation is composed of these values. This result indicates that some portion of the training set must be stored by the model, which can make it unattractive for large training sets.
