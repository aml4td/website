...
For the curious, we might also examine which predictors showed no evidence of being different from zero. The bottom of the table shows five terms with large p-values. These are also spline terms, all for annual precipitation. However, notice that the estimated coefficients and their standard error terms are in the _hundreds_. For these spline terms, the model terms range between zero and `r upper_vals` (on average). Given this, it is difficult to imagine that the coefficients and their values are reasonable. 
...
While this model's predictive performance is good, the model coefficients are extremely unstable. Multicollinearity not only inflates the variance of the parameter estimates but can also reverse the coefficient's sign, leading to an incorrect interpretation. 
