...
- A **simple** model only converts the ZIP code to binary indicators.
- We can add a **normalization** step that transforms the numeric predictors using the ORD transformation. 
- Starting with the normalized pipeline, we can replace the ZIP  code dummy variable operation with an effect **encoding** step. 
- We can add our potential **interactions** to our encoded pipeline. 
