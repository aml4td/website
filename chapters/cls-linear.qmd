...
We've seen the use of logistic regression in this chapter, coupled with spline terms, in order to model nonlinear effects of predictors. Two strategies were shown: one in which we tuned the number of spline terms, and another in which we set the number of terms to be large and let regularization help demphasize or eliminate excess complexity in the splines.  Statistically, the downside of these methods is that we never measure the noise in the amount of nonlinearity used; we specify it instead of directly estimating it. For that reason, any inferential estimates that we might want (e.g., confidence intervals, p-values, etc.) are optimistic to some degree

Another avenue is to use _generalized additive models_ [@wood2006generalized]. GAMs are a specialized class of models that have effective, relatively low-maintenance methods for adding nonlinearity. When training the model, we specify which predictors should be modeled nonlinearly and the type of spline to use. GAMs can be used to directly estimate the degree of nonlinearity in the relationships. 
...
What separates GAM models from the previously described techniques is the manner in which the parameters are estimated. For GAMs, the most common training method is penalized maximum likelihood estimation, which is very similar to the L<sub>2</sub> penalty shown for logistic regression. However, GAMs can also estimate how complex the nonlinear terms should be (rather than assigning a specific value to them or tuning them) by directly estimating the penalty parameter^[@marra2012coverage and @wood2013p also make the case that using the plug-in generalized cross-validation (GCV) estimate can also produce valid inference.].  This has tremendous value when the model needs to be statistically interrogated, such as measuring statistical significance or assigning uncertainty to model parameters or predictions. 

Regarding penalized maximum likelihood estimation, the penalty is slightly more complex than in the previous logistic case. We want to penalize complexity, which is often called the "wiggliness" of the curve. For splines, the penalty is more than the squared coefficient: 

$$
\mathcal{P}_{gam}(\boldsymbol{S}; \boldsymbol{\beta}, \lambda) =  \lambda\boldsymbol{\beta}'\boldsymbol{S}\boldsymbol{\beta}
$$ {#eq-gam-penalty}

where $\boldsymbol{S}$ is a symmetric matrix of known values that are derived from the integral of squared second derivatives across the range of the basis functions. This matrix encapsulates the overall wiggliness of the nonlinear terms. To illustrate, the 9 × 9 $\boldsymbol{S}$ matrix for the fit shown in @fig-longitude is below in @fig-gam-matrix. 
