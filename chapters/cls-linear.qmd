...
# Generalized Linear Classifiers {#sec-cls-linear}
...
This keeps the probability values within [0, 1] no matter how large or small the linear predictor becomes^[Technically, logistic regression uses this nonlinear function, but there are many other functions to choose from. Examples are the probit, complementary log-log, or TODO function. In these cases, we might refer to our model as "binary regression." We TODO for a historical perspective.]. This nonlinear function can be derived using differential equations for growth curves. Although the model has been rediscovered several times, it was first derived in the 1800s and was not commonly known as the "logistic" model until 1925. See @cramer2004early for a history. 
...
linear_percip_fit <- 
  logistic_reg() %>% 
  fit(class ~ log10(annual_precipitation), data = forested_train)
  ...
  layout_columns(
    ...
    column(
      ...
      checkboxGroupInput(
        inputId = "include",
        label = "Include",
        choices = list("Data" = "Data", "Untransformed" = "fit_linear", 
...
server <- function(input, output) {
  ...
  percip <- 
    forested_train %>% 
    mutate(annual_precipitation = log10(annual_precipitation))
...
Logistic regression belongs to a broader class of _generalized linear models_, or GLMs [@glm]. These models can be used with different types of outcomes, such as numeric outcomes or count data. The idea is to have a model regress the linear predictor against a function of the outcome data. In our current case, the _logit_ function is applicable^[ref oragins paper]: 
...
forest_rec <- 
  recipe(class ~ ., data = forested_train) %>% 
  step_orderNorm(all_numeric_predictors()) %>% 
  step_lencode_mixed(zip, outcome = "class")

forest_int_rec <- 
  forest_rec %>% 
  step_interact(
    ~ dew_temperature:elevation + longitude:minimum_vapor + 
      longitude:maximum_vapor + dew_temperature:january_minimum_temperature + 
      dew_temperature:maximum_vapor + latitude:longitude + 
      longitude:roughness + annual_maximum_temperature:dew_temperature + 
      annual_minimum_temperature:dew_temperature + 
      annual_maximum_temperature:maximum_vapor
  )

forest_spline_rec <- 
  forest_int_rec %>% 
  step_spline_natural(
    all_numeric_predictors(), -zip, -eastness, -northness, -year, -contains("_x_"),
    deg_free = 10) %>% 
  step_lincomb(all_predictors())

forest_set <- 
  workflow_set(
    preproc = list(simple = bare_rec, transformations = transformed_rec, 
                   encoded = forest_rec, interactions = forest_int_rec, 
                   splines = forest_spline_rec),
...
- A **simple** preprocessor that only converts the `r length(levels(forested_train$zip))` ZIP codes to binary indicators and removes potential zero-variance predictors. 
- We can add a **normalization** step that transforms the numeric predictors using the ORD transformation. 
- We'll replace the ZIP  code dummy variable operation with an effect **encoding** step (from the previous pipeline).
...
We can see the resampling results in @fig-forest-logistic. It shows that most steps incrementally reduce the Brier score. One of the largest drops occurs when we avoid making many ZIP code indicators and use an effect encoding instead. The normalization step is estimated to help, but the error bars indicate that this improvement might be within the statistical noise of resampling. The end result is a 1/3 reduction in the Brier score by adding better representations of our predictors. 
...
logistic_mtr <- 
  forest_res %>% 
  extract_workflow_set_result("splines_logistic_mle") %>% 
...
logitic_cal <- cal_plot_windowed(logistic_pred, class, .pred_Yes)
logitic_roc <- 
  logistic_pred %>% 
