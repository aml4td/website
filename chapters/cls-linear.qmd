...
With $C > 2$ classes, we can estimate a set of $C - 1$ logistic models that contrast two classes from one another. The most basic contrast is to model each class against the last, leading to $C-1$ linear predictors: 
...
Note that the parameters are class-specific. The intercepts in these linear predictors often represent the differences in the baseline class probabilities. It is possible to optimize the predictor patterns across classes. For example, if there is evidence that the relationships between the predictors and the logits are the same, a common slope estimate can be obtained (leading to a "parallel" model structure as well be seen below).  

This model _could_ be trained by creating a single iteration where each of the $C-1$ logistic regressions is computed in the traditional manner [@becg1984calculation]. However, better estimates are obtained by using the model structure above to compute probabilities and then applying gradient descent to optimize the multinomial log-likelihood which, as previously mentioned, is cross-entropy. See Chapter 6 of @agresti2015foundations. There are also analogous approaches that use regularization or Bayesian estimation. 

A related class of models is for ordered outcome categories [@mccullagh1980regression;@williams2020ordinal]. These models are inherently multinomial; otherwise, basic logistic would be appropriate when there are only two classes. 
