...
That figure also demonstrated that some models can produce simple boundaries while others produce very convoluted partitions. The simplest models produce **linear boundaries**, which are naturally constrained by over-adapting to localized trends in the data and overfitting. Unfortunately, this also increases the model bias so that, for cases where the true boundary is nonlinear, the model may dramatically underperform. 
...
all_terms |>
  pivot_wider(
    id_cols = c(Model),
    names_from = c(text),
    values_from = c(estimate, std_err)
  ) |>
  relocate(
    Model,
    contains("Intercept"),
