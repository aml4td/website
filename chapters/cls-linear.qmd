...
@fig-longitude shows this pattern using the longitude predictor in the forestation data in green. The curve shows a smooth function that increases in $x$ and has an asymptote of 1.0. 

Since the outcome data have only two qualitative values, it is difficult to visualize how well the curve fits the data. However, our training set for this example is fairly large, and we can show the observed event rate over the range of longitude by binning the data into 50 groups. The breakpoints for the bins are quantiles of the training set predictor values. @fig-longitude has an option to overlay these data with the model fit. 
...
server <- function(input, output) {
  ...
  lng_bin <- 
    ...
    mutate(lng_bin = ntile(longitude, 50)) %>% 
    dplyr::summarize(
      rate = mean(class == "Yes"),
...
An example of a logistic regression model with a single predictor (longitude)

:::

Overlaying the estimated event rates shows that the model does a poor job of representing the relationship between the probability of forestation and longitude. The data trend is not monotonically increasing; in the middle of the longitude range, there is a small drop in the binned rates that recovers and begins to increase once more. Our simple linear predictor is not flexible enough to emulate this motif.
...
where $\lambda$ is the total amount of regularization and $\alpha$  controls the mixture of the two methods so that $\alpha = 1$ is a Lasso model and $\alpha = 0$ is ridge regression. The glmnet penalty tends to be favored. 

@fig-two-feature-penalty shows the results for twp features. 
...
The models include an additional tuning parameter, $\gamma$, that provides a “clipping” effect where, once the sum of the absolute values is “large” ($\gamma\lambda$) the penalty becomes constant.


Finally, there are adaptive and/or two-stage methods that combine the unpenalized parameters.  

@zou2006adaptive @meinshausen2007relaxed  @chatterjee2025univariate

group lasso; interactions
...
glmn_pen <- 10^seq(-7, -1, length.out = 50)
...
glmn_best_fit <- fit_best(glmn_res, metric = "brier_class")
...
top_best <- 5
show_pen <- glmn_pen[which.min(abs(glmn_pen - 10^(-6)))]
at_best <- 
  glmn_best_coef |> 
...
```{r}
#| label: fig-logistic-glmnet-tune
#| fig-width: 5.5
#| fig-height: 4.25
#| fig-align: center
#| out-width: 60%
#| fig-cap: Results of tuning the complex logistic model using the glmnet penalty.
#| warning: false
...
```{r}
#| label: fig-logistic-glmnet-path
#| fig-width: 5
#| fig-height: 5
#| fig-align: center
#| out-width: 50%
...
p_glmnet_param <- 
  glmn_best_coef |> 
  ggplot(aes(lambda, estimate, group = term, col = term)) + 
  geom_vline(xintercept = glmn_best_param$penalty, lty = 2) +
  geom_line(show.legend = FALSE, alpha = 1 / 5) + 
  geom_line(data = best_path, show.legend = FALSE, alpha = 1, linewidth = 1.1) + 
  geom_label_repel(data = at_show, aes(label = label), show.legend = FALSE) +
  scale_x_log10()
p_glmnet_param
```
