...
sim_rec <-
  ...
  step_select_forests(
    all_predictors(), outcome = "outcome", 
    top_p = tune(), mtry = num_pred, trees = 1000,
    id = "filter") %>%
...
retained_variables <- 
  ...
  ~ inner_join(.x, mlp_rf_best, 
               by = join_by(top_p, hidden_units, penalty, activation, 
                            learn_rate, .config))
) %>% 
  map(~ .x$.extracts[[1]]) %>% 
...
p_terms <- 
  ...
  geom_bar() + 
  coord_flip() + 
  labs(x = NULL, y = "# Selections in 10 Folds", title = "(b) Selection Frequency") +
...
Similar to the discussion in @sec-resampling-faq, the different predictor sets discovered in the resamples can give us a sense of our results’ consistency. They are “realizations” of what _could_ have occured. Once we fit the final model, we get the _actual_ predictor set. 
