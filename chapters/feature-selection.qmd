...
::: {#fig-irrelevant-predictors}

::: {.figure-content}

```{r}
#| label: irrelevant-predictors
...
For different regression models, the percent increase in RMSE is shown as a function of the number of additional non-informative predictors.
...
selection_sa_dk <-
  ...
  geom_point(cex = 1 / 2, col = dark_data) +
  geom_hline(yintercept = 0.25, col = "green", lty = 2) +
  thm_dk
...
The progress of using simulated annealing to select features for.a neural network. The dashed green line indicators the best RMSE value achievable for these simulated data.
...
::: {#fig-selection-sa-resamples}

::: {.figure-content}

```{r}
#| label: selection-sa-resamples
#| renderings: [light, dark]
#| echo: false
#| out-width: 100%
#| fig-width: 10
...
The individual SA runs that make up the data in @fig-selection-sa.
...
p_terms <- retained_variables %>%
  mutate(terms = gsub("predictor", "x", terms)) %>%
  ggplot(aes(terms)) +
  coord_flip() +
  labs(
    x = NULL,
...
p_terms_dk <- 
  p_terms +
  geom_bar(fill = dark_data) +
  geom_hline(yintercept = 0.25, col = "green", lty = 2) +
  thm_dk

(p_rmse_lt | p_terms_lt) + plot_layout(widths = c(4, 3))

(p_rmse_dk | p_terms_dk) + plot_layout(widths = c(4, 3))
```

:::

Panel (a): The results of tuning a variance importance filter and the parameters from a single-layer neural network for predicting the simulated data from @eq-hooker.  Panel (b): The frequency distribution of predictor selection in the 10 folds for the model with the smallest RMSE.
