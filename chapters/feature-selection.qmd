...
- TabNet, a deep learning model for tabular data. 
- TabPFN, a foundational neural network model
- Support vector machines (radial basis function kernel)

These models are described in detail in subsequent chapters. The colors of the points/lines signify whether the model automatically removes unused predictors from the model equation (see @sec-automatic-selection below). 
...
#| fig-alt: "Faceted line chart with one panel per model. The x-axis is the number of noise features added and the y-axis is the percent increase in RMSE from baseline. Models like KNN, neural networks, and SVMs show steep increases in RMSE as noise features grow. Tree-based ensembles, penalized regression, GAMs, MARS, and Cubist remain close to zero. Point styling distinguishes models that perform automatic feature selection from those that do not."
...
2. The other set of models includes MARS and Cubist. They selectively include predictors in their regression equations, only including those specifically selected as having value. 
...
One outlier is TabPFN. This is a pretrained foundational model that is discussed in @sec-cls-foundational. While it has no traditional or explicit architecture for removing predictors, it shows remarkable resistance to the inclusion of irrelevant predictors.
...
For example, if our model requires all numeric inputs, a categorical predictor is often converted to binary indicators. A date-time column is the original predictor, while the binary indicators for individual days of the week are the derived predictors. Similarly, the 550 measurement predictors in the barley data are the original predictors, and embedded values, such as the PCA components, are the derived predictors. 
...
Recall back in @sec-model-pipeline an argument was made that preprocessing methods _and_ the supervised model encompassed the entire model pipeline. Technically, feature selection (supervised or unsupervised) falls into the definition of preprocessing. It is important that the data used to train the pipeline (i.e., select the predictors) is different from the data used to evaluate how well it worked.  
...
- The _frequency ratio_ is the frequency of the most prevalent value over the second most frequent value. For our example, the ratio was 496/3 = 165.3.
- The _proportion of unique values_, which is 3 / 500 = 0.006. 

We can determine thresholds for both of these criteria to remove the predictor. The default behavior is to remove the column if the frequency ratio is greater than 19 **and** the proportion of unique values is less than 0.1. For the example, the predictor would be eliminated. 
...
#| fig-width: 5
#| fig-height: 6
#| fig-cap: A heatmap of a correlation matrix for fifteen spatial predictors, ordered using a hierarchical clustering algorithm.
#| fig-alt: "Correlation heatmap of fifteen forestry predictors with dendrograms along both axes from hierarchical clustering. Cells range from strong negative to strong positive correlation. Several clusters of highly correlated predictors are visible, particularly among temperature-related variables."
heatmaply(
  cor_mat,
  branches_lwd = .5,
  colors = RdBu(50),
...
- determine how far the search should proceed.
...
However, let's illustrate a wrapper method using an artificial data set from the simulations described in @sec-irrelevant-predictors. We'll use @eq-hooker and supplement the nine truly important predictors with twenty noise columns. For simplicity, we'll use a neural network to predict the outcome with a static set of tuning parameters. The training set consisted of 1,000 data points. 
...
#| fig-width: 5
#| fig-height: 3.25
#| fig-cap: The progress of using simulated annealing to select features for a neural network. The dashed green line indicates the best RMSE value achievable for these simulated data.
#| fig-alt: "Line chart of resampled RMSE averaged across 10 folds versus simulated annealing iteration. RMSE starts high and decreases over roughly 120 iterations before stabilizing near the theoretical minimum shown by a horizontal dashed reference line."
...
#| fig-alt: "Faceted line chart with 10 panels, one per cross-validation fold. Each shows RMSE versus SA iteration for a single search run. The individual traces are far more erratic than the averaged curve, with large spikes and precipitous drops occurring at different iterations across folds. A horizontal dashed reference line marks the theoretical minimum RMSE."
load("../RData/nnet_sa_res.RData")
...
- The Relief algorithm [@kira1992practical;@kononenko1994estimating] and its descendant methods randomly select training set points and use the outcome values of their nearest neighbors to aggregate statistics to measure importance. 
...
#| fig-height: 5
#| fig-align: center
#| fig-cap: "Panel (a): The results of tuning a variable importance filter and the parameters from a single-layer neural network for predicting the simulated data from @eq-hooker.  Panel (b): The frequency distribution of predictor selection in the 10 folds for the model with the smallest RMSE."
#| fig-alt: "Two-panel figure. Panel (a) shows tuning results as scatter plots of resampled RMSE against five tuning parameters; the proportion of retained predictors has the largest effect, with RMSE worsening sharply when too few features are kept. A dashed horizontal reference line marks the theoretical minimum. Panel (b) is a horizontal bar chart showing how often each predictor was selected across the 10 cross-validation folds; informative predictors appear in most or all folds while noise predictors appear in few."
p_rmse <- autoplot(mlp_rf_sfd_res, metric = "rmse") +
  ylim(c(.24, .72)) +
  geom_hline(yintercept = 0.25, col = "darkgreen", lty = 2) +
  labs(title = "(a) Resampled RMSE results", y = "RMSE")
...
if (num_noise == 0) {
  ...
  noise_text <- cli::format_inline(
    "{num_noise} noise predictor{?s} {?was/were}"
  )
} else {
