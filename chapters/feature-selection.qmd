...
Another improper example of data usage that can be found in the literature, as well as in real-life analyses, occurs when the entire data set is used to select predictors _then_ the initial split is used to create the training and test sets. This “bakes in” the false optimism from the start^[Recall out mantra that “Always have a separate piece of data that can **contradict what you believe**.”]. 

However, performing feature selection in each resample can drastically increase the computational cost, especially if the model has many tuning parameters. For this reason, we tend to focus on methods that _atuomatically select features_ during the normal process of training the model (as discussed below). 
...
Unsupervised filters are only needed in specific circumstances. Some models (such as trees) are resistant to the distributions of the predictor’s data, while others are excessively sensitive. 
