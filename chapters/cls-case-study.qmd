...
set.seed(11)
cyp3A4 <- cyp3A4 |> slice_sample(n = 5000) |> select(class, contains("kr_"))
...
mlp_plain_param <- 
  ...
  update(
    ...
    rate_schedule = rate_schedule(c("cyclic", "none"))
  )
...
mlp_cls_wts_grid <- 
  mlp_cls_wts_param |> 
  filter(id != "class_weights") |> 
  grid_space_filling(size = 25) |> 
  crossing(class_weights = 1:5)
...
mlp_cls_wts_best <- select_best(mlp_cls_wts_res, metric = "roc_auc")
...
lgb_cls_wts_param <- 
  ...
  update(
    ...
    scale_pos_weight = class_weights(c(1, 20))
  )
...
lgb_cls_wts_best <- select_best(lgb_cls_wts_res, metric = "j_index")
...
rf_cls_wts_spec |> 
  ...
  update(
    mtry = mtry(c(1, ncol(cyp3A4_train) - 1)),
    class.weights = class_weights(c(1, 10))
  ) |> 
  grid_space_filling(size = 25)
...
rf_cls_wts_best <- 
  ...
  slice_max(mean, n = 1) |> 
  select(mtry, min_n, class.weights, index)
