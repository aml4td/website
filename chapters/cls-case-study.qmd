...
set.seed(11)
cyp3A4 <- cyp3A4 |> select(class, contains("maccs"), contains("pubchem"))
...
cyp3A4_dense_rs <- cyp3A4_rs
cyp3A4_dense_rs$splits[[1]]$data <- as_tibble(cyp3A4_dense_rs$splits[[1]]$data )
...
For this reason, we’ll use this case study to build two sets of models. The first are designed to find events by either distorting the prevalence of the event of interest or by placing more importance on the minority class. 
...
We'll make a training/validation/testing split of the data pool using stratified sampling based on the outcome class. This results in data sets of size `r format(nrow(cyp3A4_train), big.mark = ",")`, `r` format(nrow(cyp3A4_val), big.mark = ",")`, and `r format(nrow(cyp3A4_test), big.mark = ",")`, respectively. 
...
```{r}
#| label: cyp3A4-glmnet-plain
#| include: false
#| cache: true

path <- 10^seq(-5, -1, length.out = 25)

glmn_plain_spec <-
  logistic_reg(penalty = tune(), mixture = 1) |>
  set_engine("glmnet", path_values = !!path)

glmn_plain_wflow <- workflow(class ~ ., glmn_plain_spec)

glmn_plain_grid <- tibble(penalty = path)
...
mlp_plain_wflow <- workflow(bare_rec, mlp_spec)
mlp_plain_param <- 
  mlp_plain_wflow |>
...
mlp_plain_default <- closest_point(mlp_plain_roc_curve)
mlp_plain_sensitive <- closest_point(mlp_plain_roc_curve, "sensitivity", 0.95)
```

```{r}
#| label: cyp3A4-lgb-plain
#| include: false
#| cache: true
...
lgb_plain_wflow <- workflow(bare_rec, lgb_plain_spec)
...
lgb_plain_default <- closest_point(lgb_plain_roc_curve)
lgb_plain_sensitive <- closest_point(lgb_plain_roc_curve, "sensitivity", 0.95)
```

```{r}
#| label: cyp3A4-ranger-plain
#| include: false
#| cache: true
...
rf_plain_wflow <- workflow(bare_rec, rf_plain_spec)
...
xrf_plain_default <- closest_point(xrf_plain_roc_curve)
xrf_plain_sensitive <- closest_point(xrf_plain_roc_curve, "sensitivity", 0.95)
```


```{r}
#| label: cyp3A4-roundup
#| echo: false
...
rank_plain <- 
  rank_results(tt, rank_metric = "roc_auc") |> 
  filter(.metric == "roc_auc")

rank_plain_best <- 
  rank_plain |> 
  slice_max(mean, by = c(wflow_id)) |> 
...
lgb_down_best <-
  ...
  select_best_desirability(
    ...
    constrain(specificity, low = 0.8)
  )

lgb_down_best_mtr <- collect_metrics(lgb_down_res) |>
  inner_join(lgb_down_best |> select(.config), by = ".config")
lgb_down_best_pred <- collect_predictions(
  lgb_down_res,
  parameters = lgb_down_best
)
...
lgb_down_default <- closest_point(lgb_down_roc_curve)
lgb_down_sensitive <- closest_point(lgb_down_roc_curve, "sensitivity", 0.95)
```
...
mlp_cls_wts_wflow <- workflow(bare_rec, mlp_cls_wts_spec)
mlp_cls_wts_param <- 
  mlp_cls_wts_wflow |>
  extract_parameter_set_dials() |> 
  update(
    ...
    penalty = penalty(c(-10, -1)),
    learn_rate = learn_rate(c(-4, -1)),
    class_weights = class_weights(c(1, 15))
  )
...
mlp_cls_wts_res |> 
  ...
  ggplot(aes(class_weights, mean, col = .metric)) + 
  geom_point() + 
...
mlp_cls_wts_best_mtr <- collect_metrics(mlp_cls_wts_res) |>
  inner_join(mlp_cls_wts_best |> select(.config), by = ".config")
mlp_cls_wts_best_pred <- collect_predictions(
  mlp_cls_wts_res,
  parameters = mlp_cls_wts_best
)

mlp_cls_wts_roc_curve <-
  mlp_cls_wts_best_pred |>
  roc_curve(class, .pred_inhibitor)

mlp_cls_wts_default <- closest_point(mlp_cls_wts_roc_curve)
mlp_cls_wts_sensitive <- closest_point(mlp_cls_wts_roc_curve, "sensitivity", 0.95)
```

```{r}
#| label: cyp3A4-lgb-cls-wts
#| include: false
#| cache: true
...
lgb_cls_wts_param <- 
  ...
  extract_parameter_set_dials() |> 
  update(
    mtry = mtry(c(1, ncol(cyp3A4_train) - 1)),
...
lgb_cls_wts_best_mtr <- collect_metrics(lgb_cls_wts_res) |>
  inner_join(lgb_cls_wts_best |> select(.config), by = ".config")
lgb_cls_wts_best_pred <- collect_predictions(
  lgb_cls_wts_res,
  parameters = lgb_cls_wts_best
)

lgb_cls_wts_roc_curve <- 
  lgb_cls_wts_best_pred |>
  roc_curve(class, .pred_inhibitor)

lgb_cls_wts_default <- closest_point(lgb_cls_wts_roc_curve)
lgb_cls_wts_sensitive <- closest_point(lgb_cls_wts_roc_curve, "sensitivity", 0.95)
```



```{r}
#| label: cyp3A4-ranger-cls-wts
#| include: false
#| cache: true
#| eval: false
...
rf_cls_wts_spec <-
  ...
  set_mode("classification") |> 
  set_engine("ranger", class.weights = tune())

rf_cls_wts_wflow <- workflow(class ~ ., rf_cls_wts_spec)
...
rf_cls_wts_grid <- 
  ...
  update(
    mtry = mtry(c(1, ncol(cyp3A4_train) - 1)),
    class.weights = class_weights(c(1, 25))
  ) |> 
  grid_space_filling(size = 50)

rf_cls_wts_grid_split <- 
  rf_cls_wts_grid |> 
  tune:::vec_list_rowwise()

rf_cls_wts_specs <- vector(mode = "list", length = length(rf_cls_wts_grid_split))
names(rf_cls_wts_specs) <- names0(length(rf_cls_wts_grid_split), "rf_")
for (i in seq_along(rf_cls_wts_specs)) {
  wts_vec <- c(rf_cls_wts_grid_split[[i]]$class.weights, 1)
  rf_cls_wts_specs[[i]] <- 
    rf_cls_wts_spec |> 
    finalize_model(parameters = rf_cls_wts_grid_split[[i]] |> select(mtry, min_n))
  rf_cls_wts_specs[[i]]$eng_args$class.weights <- wts_vec
}

rf_cls_wts_set <- 
  workflow_set(
    preproc = list(bare = bare_rec),
    models = rf_cls_wts_specs
  )
...
rf_cls_wts_mtr <- 
  map(rf_cls_wts_res$result, collect_metrics) |> 
  map2_dfr(rf_cls_wts_grid_split, ~ vctrs::vec_cbind(.y, .x)) |> 
  select(-.config) |> 
  full_join(
    rf_cls_wts_grid |> 
      mutate(
        index = row_number(), 
        .config = map_chr(index, ~ paste0("mod", .x))), 
    by = join_by(mtry, min_n, class.weights))

rf_cls_wts_best <-
  rf_cls_wts_mtr |>
  select(-.estimator, -n, -std_err, -index) |>
  pivot_wider(
    id_cols = c(mtry, min_n, class.weights, .config),
    names_from = .metric,
    values_from = mean
  ) |> 
  mutate(
    d_sens = d_max(sensitivity, low = 0.5, high = 1, scale = 2), 
    d_spec = d_box(specificity, low = 0.8, high = 1),
    d_overall = d_overall(d_sens, d_spec)
  ) |> 
  slice_max(d_overall)
  
rf_cls_wts_best_mtr <- rf_cls_wts_mtr |>
  inner_join(rf_cls_wts_best, by = join_by(mtry, min_n, class.weights))

rf_cls_wts_best_pred <- 
  rf_cls_wts_res$result[[rf_cls_wts_best$index]] |> 
  collect_predictions()

rf_cls_wts_roc_curve <- 
  rf_cls_wts_best_pred |>
  roc_curve(class, .pred_inhibitor)

rf_cls_wts_default <- closest_point(rf_cls_wts_roc_curve)
rf_cls_wts_sensitive <- closest_point(rf_cls_wts_roc_curve, "sensitivity", 0.95)
```
