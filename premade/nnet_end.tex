...
\begin{tikzpicture}[x=2.7cm,y=1.6cm]
  ...
  \node[below=0,right=11,mydarkred,scale=0.95] at (NO-1)
    {$\begin{aligned} %\underset{\text{bias}}{\gamma_0}
  ...
  \node[right,scale=0.9] at (1.3,-1.3)
    {$\begin{aligned}
      ...
      \end{pmatrix}
      \\[0.5em]
      {\color{mydarkblue}\boldsymbol{Y}_k} % vector (bold)
      &= \color{mydarkred} \color{black}
           \boldsymbol{\gamma}_1 + \boldsymbol{H}'\boldsymbol{\Gamma}_1 
         \color{mydarkred} \\
      \pi_k &= \frac{\exp(Y_k)}{\sum\limits_{k-1}^C \exp(Y_k)}
    \end{aligned}$};
